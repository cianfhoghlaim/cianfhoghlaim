// convex/geography/getSession — per-subject Convex real-time backend.
//
...
// getSession — fetch a per-subject Geography interactive study session
// by id (or by the most-recent session for a user).
//
// Convex query called from /en/subjects/geography/syllabus and the
// /en/subjects/geography/study-plan page on load.
  ...
  handler: async (ctx, args) => {
    ...
    if (args.userId) {
      const sessions = await ctx.db
        ...
        .filter((q) => q.eq(q.field("subject"), "geography"))
        .order("desc")
        .take(1);
