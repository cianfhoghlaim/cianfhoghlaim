...
def ni_chemistry(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Northern Ireland chemistry syllabus rows from the canonical cache.
    ...
    cache_dir = Path("stedding/site_scrape_samples/biep/ni/chemistry")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "ni_chemistry_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="ni_chemistry")
def ni_chemistry_source(language: str | None = None):
    """DLT source for the Northern Ireland chemistry ingestion."""
    return ni_chemistry(language=language)
