"""Per-subject DLT source for Northern Ireland (mathematics, CCEA).

Per-subject DLT source for the British Isles parity change
(`2026-07-12-british-isles-parity-pipeline-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/site_scrape_samples/biep/ni/mathematics/<lang>/``.
...
def ni_mathematics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Northern Ireland mathematics syllabus rows from the canonical cache.
    ...
    cache_dir = Path("stedding/site_scrape_samples/biep/ni/mathematics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "ni_mathematics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="ni_mathematics")
def ni_mathematics_source(language: str | None = None):
    """DLT source for the Northern Ireland mathematics ingestion."""
    return ni_mathematics(language=language)
