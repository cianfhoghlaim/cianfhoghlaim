...
def sct_chemistry(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Scotland chemistry syllabus rows from the canonical cache.
    ...
    cache_dir = Path("stedding/site_scrape_samples/biep/sct/chemistry")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "sct_chemistry_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="sct_chemistry")
def sct_chemistry_source(language: str | None = None):
    """DLT source for the Scotland chemistry ingestion."""
    return sct_chemistry(language=language)
