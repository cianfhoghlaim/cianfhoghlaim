...
def sct_physics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Scotland physics syllabus rows from the canonical cache.
    ...
    cache_dir = Path("stedding/site_scrape_samples/biep/sct/physics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "sct_physics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="sct_physics")
def sct_physics_source(language: str | None = None):
    """DLT source for the Scotland physics ingestion."""
    return sct_physics(language=language)
