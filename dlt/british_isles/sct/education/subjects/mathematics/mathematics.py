...
def sct_mathematics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Scotland mathematics syllabus rows from the canonical cache.
    ...
    cache_dir = Path("stedding/site_scrape_samples/biep/sct/mathematics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "sct_mathematics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="sct_mathematics")
def sct_mathematics_source(language: str | None = None):
    """DLT source for the Scotland mathematics ingestion."""
    return sct_mathematics(language=language)
