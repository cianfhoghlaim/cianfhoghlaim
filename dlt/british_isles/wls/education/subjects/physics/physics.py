...
def wls_physics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Wales physics syllabus rows from the canonical cache.
    ...
    cache_dir = Path("stedding/site_scrape_samples/biep/wls/physics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "wls_physics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="wls_physics")
def wls_physics_source(language: str | None = None):
    """DLT source for the Wales physics ingestion."""
    return wls_physics(language=language)
