...
def wls_mathematics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Wales mathematics syllabus rows from the canonical cache.
    ...
    cache_dir = Path("stedding/site_scrape_samples/biep/wls/mathematics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "wls_mathematics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="wls_mathematics")
def wls_mathematics_source(language: str | None = None):
    """DLT source for the Wales mathematics ingestion."""
    return wls_mathematics(language=language)
