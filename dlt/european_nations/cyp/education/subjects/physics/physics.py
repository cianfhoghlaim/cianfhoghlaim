"""Per-subject DLT source for Cyprus (Physics, KYSATS).

Per-subject DLT source for the EU nations full-depth expansion
(`2026-07-13-eu-nations-full-depth-expansion-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/cyp/education/subjects/physics/<lang>/``.
...
def cyp_physics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Cyprus Physics syllabus rows from the canonical cache.

    Honours ``USE_LOCAL_SCRAPES=true`` by reading cached Firecrawl
    snapshots under ``stedding/ingest_queue/european_nations/cyp/education/subjects/physics/<lang>/``.
    """
    cache_dir = Path("stedding/ingest_queue/european_nations/cyp/education/subjects/physics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "cyp_physics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="cyp_physics")
def cyp_physics_source(language: str | None = None):
    """DLT source for the Cyprus Physics ingestion."""
    return cyp_physics(language=language)
