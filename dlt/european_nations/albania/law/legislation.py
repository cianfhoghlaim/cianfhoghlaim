"""DLT source for the Legislation of Albania.

Crawls ``https://qbz.gov.al`` and emits one row per ``(legislation_id, language)``
...
``dlt/european_nations/alb/law/legislation.py`` with
``source_id="european_nations.alb.law.legislation"`` and lands in
the DuckLake table ``oideachais.law.european_nations.alb``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/alb/law/<lang>/``.
...
class ALBLegislationSource(NationSource):
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://qbz.gov.al",
                "title": "Legislation of Albania",
            },
        )
...
def legislation(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Albania Legislation rows from the canonical cache."""
...
def legislation_source(language: str | None = None):
    """DLT source for the Legislation of Albania ingestion."""
