"""Per-subject DLT source for the Ukraine {Physics} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/ukr/education/subjects/physics/uk/sample.json``.
"""
...
def ukr_physics(language=None) -> Iterator[dict[str, Any]]:
    """Yield Ukraine Physics rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/ukr/education/subjects/physics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "ukr_physics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="ukr_physics")
def ukr_physics_source(language=None):
    """DLT source for the Ukraine Physics ingestion."""
    return ukr_physics(language=language)
