"""Per-subject DLT source for Bulgaria (Mathematics, MES).

Per-subject DLT source for the EU nations full-depth expansion
(`2026-07-13-eu-nations-full-depth-expansion-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/bgr/education/subjects/mathematics/<lang>/``.
...
def bgr_mathematics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Bulgaria Mathematics syllabus rows from the canonical cache.

    Honours ``USE_LOCAL_SCRAPES=true`` by reading cached Firecrawl
    snapshots under ``stedding/ingest_queue/european_nations/bgr/education/subjects/mathematics/<lang>/``.
    """
    cache_dir = Path("stedding/ingest_queue/european_nations/bgr/education/subjects/mathematics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "bgr_mathematics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="bgr_mathematics")
def bgr_mathematics_source(language: str | None = None):
    """DLT source for the Bulgaria Mathematics ingestion."""
    return bgr_mathematics(language=language)
