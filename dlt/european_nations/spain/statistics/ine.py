...
Crawls ``https://www.ine.es`` and emits one row per ``(ine_id, language)`` for
...
``dlt/european_nations/esp/statistics/ine.py`` with
``source_id="european_nations.esp.statistics.ine"`` and lands in
the DuckLake table ``oideachais.statistics.european_nations.esp``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/esp/statistics/<lang>/``.
...
class SpainStatisticsSource(NationSource):
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://www.ine.es",
                "title": "Instituto Nacional de Estadística",
            },
        )


_NATION_SOURCE = SpainStatisticsSource()
...
def ine(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Spain statistics rows from the canonical cache."""
