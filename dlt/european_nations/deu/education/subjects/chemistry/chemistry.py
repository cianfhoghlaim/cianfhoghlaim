"""Per-subject DLT source for the Germany {Chemistry} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/deu/education/subjects/chemistry/de/sample.json``.
"""
...
def deu_chemistry(language=None) -> Iterator[dict[str, Any]]:
    """Yield Germany Chemistry rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/deu/education/subjects/chemistry")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "deu_chemistry_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
            ...
            yield {{
                ...
                "document_type": "Chemistry_curriculum",
...
@dlt.source(name="deu_chemistry")
def deu_chemistry_source(language=None):
    """DLT source for the Germany Chemistry ingestion."""
    return deu_chemistry(language=language)
