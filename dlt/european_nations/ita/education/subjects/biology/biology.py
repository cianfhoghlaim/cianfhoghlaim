"""Per-subject DLT source for the Italy {Biology} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/ita/education/subjects/biology/it/sample.json``.
"""
...
def ita_biology(language=None) -> Iterator[dict[str, Any]]:
    """Yield Italy Biology rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/ita/education/subjects/biology")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "ita_biology_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="ita_biology")
def ita_biology_source(language=None):
    """DLT source for the Italy Biology ingestion."""
    return ita_biology(language=language)
