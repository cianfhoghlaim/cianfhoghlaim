"""Per-subject DLT source for the France {Language} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/fra/education/subjects/language/fr/sample.json``.
"""
...
def fra_language(language=None) -> Iterator[dict[str, Any]]:
    """Yield France Language rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/fra/education/subjects/language")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "fra_language_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="fra_language")
def fra_language_source(language=None):
    """DLT source for the France Language ingestion."""
    return fra_language(language=language)
