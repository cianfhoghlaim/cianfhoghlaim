"""DLT source for the Quebec Health Authority.
...
``stedding/ingest_queue/commonwealth/can/qc/medicine/<lang>/``.
"""
...
def qc_health_authority(language=None):
    """Yield Quebec Health Authority rows from the canonical cache."""
    import hashlib
    import json
    from datetime import UTC, datetime
    from pathlib import Path

    cache_dir = Path("stedding/ingest_queue/commonwealth/can/qc/medicine")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("qc_health_authority_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="qc_health_authority")
def qc_health_authority_source(language=None):
    """DLT source for the Quebec Health Authority ingestion."""
    return qc_health_authority(language=language)
