"""DLT source for the Nigerian federal National Primary Health Care Development Agency.
...
``stedding/ingest_queue/commonwealth/nga/medicine/nphcda/<lang>/``.
"""
...
FEDERAL_INSTITUTION = "nphcda"
CANONICAL_URL = "https://nphcda.gov.ng"
DEFAULT_LANGUAGE = "en"


@dlt.resource(
    name="nga_federal_nphcda",
...
def nga_federal_nphcda(language=None):
    """Yield Nigerian federal rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/commonwealth/nga/medicine/nphcda")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("nga_federal_nphcda_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="nga_federal_nphcda")
def nga_federal_nphcda_source(language=None):
    """DLT source for the Nigerian federal ingestion."""
    return nga_federal_nphcda(language=language)
