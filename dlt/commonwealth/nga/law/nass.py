"""DLT source for the Nigerian federal National Assembly (routed via nigerialii.org).
...
``stedding/ingest_queue/commonwealth/nga/law/nass/<lang>/``.
"""
...
FEDERAL_INSTITUTION = "nass"
CANONICAL_URL = "https://www.nigerialii.org"
DEFAULT_LANGUAGE = "en"


@dlt.resource(
    name="nga_federal_nass",
...
def nga_federal_nass(language=None):
    """Yield Nigerian federal rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/commonwealth/nga/law/nass")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("nga_federal_nass_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="nga_federal_nass")
def nga_federal_nass_source(language=None):
    """DLT source for the Nigerian federal ingestion."""
    return nga_federal_nass(language=language)
