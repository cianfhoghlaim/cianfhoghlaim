...
def render_storyboard(seq: DemoSequence, output_path: str) -> str:
    ...
    elem_color = {
        "talamh": emerald, "uisce": azure, "tine": amber,
        "aer": indigo, "anam": gold,
    ...
    try:
        ...
        body_font = ImageFont.truetype(
            "/System/Library/Fonts/Supplemental/Georgia.ttf", 32
        )
        small_font = ImageFont.truetype(
            "/System/Library/Fonts/Supplemental/Georgia.ttf", 22
        )
    except OSError:
    ...
    draw.text(
        ...
        fill=elem_color, font=body_font,
    )
    ...
    for i, step in enumerate(summary_steps, 1):
        ...
        draw.text(
            ...
            fill=bone, font=small_font,
        )
        if step.note:
            draw.text(
                (60, y + 70),
                f"  ({step.note})",
                fill=bronze, font=small_font,
            )
        y += 130
...
def export_voiceover_script(seq: DemoSequence, path: str) -> str:
    ...
    for i, step in enumerate(seq.steps, 1):
        lines.append(
            f"[{step.timestamp:5.1f}s] Step {i}: {step.component_id}"
        )
        if step.note:
