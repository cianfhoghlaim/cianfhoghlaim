...
CocoIndex flow. Bilingual coverage (en → ga, cy, gd, gv, kw, br) is reported
per subject.
...
Run:
    cd oideachais && uv run marimo edit notebooks/syllabus_visualizer.py
"""
...
def _(mo, os, pathlib):
    ...
    mo.md(
        ...
        "Set `MOTHERDUCK_ENABLED=true` to switch to a shared MotherDuck database."
    )
    ...
    if pages.empty:
        ...
        tax_ui = mo.vstack([
            ...
            mo.md(
                ...
                "as the `celtic_curriculum_embeddings` table.*"
            ),
        ])
