...
Browse SEC exam papers per subject/year/level and inspect the obfuscated ?fp=
URL health. Provides a runtime-toggleable query engine (MotherDuck Dives or
local DuckDB / DuckLake) so the same notebook can run in development (no
MotherDuck) and in shared / production environments.

Tabs:
    1. Health         — Garage / Lakekeeper / Lance NS / MotherDuck status
...
Run:
    cd oideachais && uv run marimo edit notebooks/exam_papers_explorer.py
"""
...
def _():
    ...
    mo.md(
        ...
        Browse SEC exam materials surfaced by the `exam_materials` DLT pipeline
        and persisted in DuckLake. The query engine is selected at runtime:

        - `MOTHERDUCK_ENABLED=true` → queries a shared MotherDuck database
        - otherwise → local `.dlt/exam_materials/examinations.duckdb`
    ...
    return (
        UTC, alt, boto3, datetime, duckdb, json, lancedb, mo, os, pathlib, pd,
        requests, sys, time, timedelta,
    )
...
def _(mo, os):
    ...
    DUCKDB_PATH = str(pathlib.Path(os.getcwd()) / "oideachais/data_platform/.dlt/exam_materials/examinations.duckdb")
    ...
    mo.md(f"### Query engine: **{engine_label}**\n\nSet `MOTHERDUCK_ENABLED=true` to switch.")
    return (
        AWS_ACCESS_KEY, AWS_ENDPOINT, AWS_SECRET_KEY, DUCKDB_PATH,
        LAKEKEEPER_BASE, LANCE_NS_BASE, MOTHERDUCK_ENABLED, engine_label,
    )
...
def _(mo, os, pathlib):
    query_input = mo.ui.text_area(
        value="SELECT level, subject, material_type, count(*) AS n FROM examinations.all_exam_materials GROUP BY 1,2,3 ORDER BY n DESC LIMIT 50",
...
@app.cell
def _(
    DUCKDB_PATH, MOTHERDUCK_ENABLED, duckdb, mo, os, pd, query_input,
    run_query_btn,
):
    ...
    if run_query_btn.value:
        ...
        try:
            if MOTHERDUCK_ENABLED:
                ...
                _con = duckdb.connect(f"md:?motherduck_token={_token}")
            else:
    ...
    if search_btn.value:
        ...
        if not isinstance(search_results, mo.callout):
            try:
                ...
                if "exam_pages" not in tables and "curriculum_embeddings" not in tables:
                    search_results = mo.md(f"*No vector table found. Available: {tables}*")
                else:
                    table_name = "exam_pages" if "exam_pages" in tables else "curriculum_embeddings"
