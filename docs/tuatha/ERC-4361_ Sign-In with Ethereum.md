...
| --- | --- |
| Created | 2021-10-11 |
| Requires | [EIP-55](https://eips.ethereum.org/EIPS/eip-55), [EIP-137](https://eips.ethereum.org/EIPS/eip-137), [EIP-155](https://eips.ethereum.org/EIPS/eip-155), [EIP-191](https://eips.ethereum.org/EIPS/eip-191), [EIP-1271](https://eips.ethereum.org/EIPS/eip-1271), [EIP-1328](https://eips.ethereum.org/EIPS/eip-1328) |
...
1. The relying party generates a SIWE Message and prefixes the SIWE Message with `\x19Ethereum Signed Message:\n<length of message>` as defined in [ERC-191](https://eips.ethereum.org/EIPS/eip-191).
2. The wallet presents the user with a structured plaintext message or equivalent interface for signing the SIWE Message with the [ERC-191](https://eips.ethereum.org/EIPS/eip-191) signed data format.
3. The signature is then presented to the relying party, which checks the signature’s validity and SIWE Message content.
4. The relying party might further fetch data associated with the Ethereum address, such as from the Ethereum blockchain (e.g., ENS, account balances, [ERC-20](https://eips.ethereum.org/EIPS/eip-20) / [ERC-721](https://eips.ethereum.org/EIPS/eip-721) / [ERC-1155](https://eips.ethereum.org/EIPS/eip-1155) asset ownership), or other data sources that might or might not be permissioned.
...
This specification defines the following SIWE Message fields that can be parsed from a SIWE Message by following the rules in [ABNF Message Format](https://eips.ethereum.org/EIPS/#abnf-message-format):
...
- For Externally Owned Accounts (EOAs), the verification method specified in [ERC-191](https://eips.ethereum.org/EIPS/eip-191) MUST be used.
- For Contract Accounts,
	- The verification method specified in [ERC-1271](https://eips.ethereum.org/EIPS/eip-1271) SHOULD be used, and if it is not, the implementer MUST clearly define the verification method to attain security and interoperability for both wallets and relying parties.
	...
	- Implementers SHOULD take into consideration that [ERC-1271](https://eips.ethereum.org/EIPS/eip-1271) implementations are not required to be pure functions, and can return different results for the same inputs depending on blockchain state. This can affect the security model and session validation rules. For example, a service with [ERC-1271](https://eips.ethereum.org/EIPS/eip-1271) signing enabled could rely on webhooks to receive notifications when state affecting the results is changed. When it receives a notification, it invalidates any matching sessions.
...
- The relying party or wallet MAY additionally perform resolution of ENS data, as this can improve the user experience by displaying human-friendly information that is related to the `address`. Resolvable ENS data include:
	- The [primary ENS name](https://eips.ethereum.org/EIPS/eip-181).
	- The ENS avatar.
...
- The SIWE Message MUST be checked for conformance to the ABNF Message Format in the previous sections, checked against expected values after parsing (e.g., `expiration-time`, `nonce`, `request-uri` etc.), and its signature MUST be checked as defined in [Signing and Verifying Messages with Ethereum Accounts](https://eips.ethereum.org/EIPS/#signing-and-verifying-messages-with-ethereum-accounts).
...
- The full SIWE message MUST be checked for conformance to the ABNF defined in [ABNF Message Format](https://eips.ethereum.org/EIPS/#abnf-message-format).
- Wallet implementers SHOULD warn users if the substring `"wants you to sign in with your Ethereum account"` appears anywhere in an [ERC-191](https://eips.ethereum.org/EIPS/eip-191) message signing request unless the message fully conforms to the format defined [ABNF Message Format](https://eips.ethereum.org/EIPS/#abnf-message-format).
...
- Why [ERC-191](https://eips.ethereum.org/EIPS/eip-191) (Signed Data Standard) over [EIP-712](https://eips.ethereum.org/EIPS/eip-712) (Ethereum typed structured data hashing and signing)
	- [ERC-191](https://eips.ethereum.org/EIPS/eip-191) is already broadly supported across wallets UX, while [EIP-712](https://eips.ethereum.org/EIPS/eip-712) support for friendly user display is pending. **(1, 2, 3, 4)**
	- [ERC-191](https://eips.ethereum.org/EIPS/eip-191) is simple to implement using a pre-set prefix prior to signing, while [EIP-712](https://eips.ethereum.org/EIPS/eip-712) is more complex to implement requiring the further implementations of a bespoke Solidity-inspired type system, RLP-based encoding format, and custom keccak-based hashing scheme. **(2)**
	- [ERC-191](https://eips.ethereum.org/EIPS/eip-191) produces more human-readable messages, while [EIP-712](https://eips.ethereum.org/EIPS/eip-712) creates signing outputs for machine consumption, with most wallets not displaying the payload to be signed in a manner friendly to humans. **(1)** ![](https://eips.ethereum.org/assets/eip-4361/signing.png)
...
- Most wallet implementations already support [ERC-191](https://eips.ethereum.org/EIPS/eip-191), so this is used as a base pattern with additional features.
...
- Certain wallets can use smart contracts and multisigs to provide an enhanced user experience with respect to key usage and key recovery, and these can be supported via [ERC-1271](https://eips.ethereum.org/EIPS/eip-1271) signing.
...
- To prevent phishing attacks Wallets have to implement request origin verification as described in [Verifying the Request Origin](https://eips.ethereum.org/EIPS/#verifying-the-request-origin).
...
- If an [ERC-1271](https://eips.ethereum.org/EIPS/eip-1271) implementation or dependent data changes the signature computation, the server SHOULD invalidate sessions appropriately.
...
Wayne Chang ([@wyc](https://github.com/wyc)), Gregory Rocco ([@obstropolos](https://github.com/obstropolos)), Brantly Millegan ([@brantlymillegan](https://github.com/brantlymillegan)), Nick Johnson ([@Arachnid](https://github.com/Arachnid)), Oliver Terbu ([@awoie](https://github.com/awoie)), "ERC-4361: Sign-In with Ethereum," *Ethereum Improvement Proposals*, no. 4361, October 2021. \[Online serial\]. Available: https://eips.ethereum.org/EIPS/eip-4361.