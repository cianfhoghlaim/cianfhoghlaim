...
Provides the invokeai service for the Cianfhoghlaim platform.
...
- **Ops**: `bonneagar/stacks/invokeai/` (the 6-file GOLD_STANDARD)
- **Code**: `cianfhoghlaim/<code-path>` (if any — see the linked Dagster assets / BAML schemas / DLT sources)
- **IaC**: registered in `bonneagar/iac/komodo/deploy-stacks.ts` with tags `host:bunchloch` + `tier:invoke-utility`
- - **Pangolin**: `https://invokeai.cianfhoghlaim.ie` (if exposed)
