...
LanceDB is an open-source embedded vector database built on the Lance columnar format. It provides serverless, embedded vector search with HNSW indexing, MVCC safety for concurrent reads/writes, and hybrid search combining vector similarity with full-text and metadata filtering.
...
- **Ops**: `bonneagar/stacks/lancedb/` (the 6-file GOLD_STANDARD)
...
- - **Pangolin**: `https://lancedb.cianfhoghlaim.ie` (if exposed)
