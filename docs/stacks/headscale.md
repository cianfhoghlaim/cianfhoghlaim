...
Headscale is an open-source implementation of the Tailscale control server. It lets you create a private mesh VPN using the Tailscale client (or any WireGuard client) with a self-hosted coordination server — no dependency on Tailscale's SaaS platform.
...
- **Ops**: `bonneagar/stacks/headscale/` (the 6-file GOLD_STANDARD)
...
- - **Pangolin**: `https://hs.cianfhoghlaim.ie` (if exposed)
