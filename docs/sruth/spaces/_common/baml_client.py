...
LITELLM_BASE_URL: Final[str] = os.environ.get("LITELLM_BASE_URL", "http://litellm:4000/v1")
...
HF_INFERENCE_BASE_URL: Final[str] = (
    os.environ.get("HF_INFERENCE_URL") or "https://api-inference.huggingface.co"
)
        ...
        for attempt in range(max_model_retries + 1):
            try:
                ...
                if e.code in (429, 500, 502, 503, 504):
                    ...
                    continue
                _log.warning(
                    "HF Inference %d from %s, falling back",
                    e.code,
                    model,
                )
                break
                ...
                last_err = e
                _log.warning(
                    "HF Inference malformed response from %s: %s",
                    model,
                    e,
                )
                break
