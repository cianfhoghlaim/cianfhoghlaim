"""
Unified Curriculum Source - Subject-centric DLT source for English curriculum data.

Merges content from Gov.uk, AQA, OCR, and Edexcel into
a unified, deduplicated data source organized by (key_stage, subject).

Key Features:
- Subject-centric data model (key_stage → subject → source)
- Cross-source deduplication via content hashing
- Source provenance tracking
- English only (no second language)
...
Usage:
    ...
    # Crawl all sources for KS4 Mathematics
    pipeline = dlt.pipeline(pipeline_name="england_curriculum", destination="duckdb")
...
def _crawl_source(
    ...
    """
    Crawl an English curriculum source using Firecrawl.
...
def build_subject_urls(
    ...
    """
    Build all URLs for an English subject (across sources).

    Args:
        key_stage: Education key stage (ks1, ks2, ks3, ks4, ks5)
        subject: Subject slug (mathematics, english, etc.)
        registry: Optional SubjectRegistry for subject name lookups

    Returns:
        List of URL dicts with source and URL
    """
    if registry is None:
    ...
    for source_name, path in subject_config.urls.items():
        ...
        # English URL (England is English only)
        urls.append({
            "source": source_name,
            "language": "en",
...
def parallel_scrape_subject(
    ...
    """
    Scrape all URLs for an English subject in parallel.

    Scrapes URLs concurrently across sources.
...
@dlt.source(name="england_curriculum")
def curriculum_source(
    ...
    """
    Unified DLT source for English curriculum data.

    Subject-centric: crawls all specified sources for a (key_stage, subject)
    combination.
    ...
    logger.info(
        "england_curriculum_source_initialized",
        key_stage=key_stage,
        subject=subject,
        sources=sources,
    ...
    def curriculum_pages() -> Iterator[dict[str, Any]]:
        """Crawled English curriculum pages."""
