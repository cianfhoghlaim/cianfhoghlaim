...
Usage:
    from sruth.oideachais.dlt_sources.celtic.gaois import logainm_source
...
from sruth.shared.http import logainm_client, tearma_client, ainm_client
...
def ainm_source(
    ...
    def professions_resource() -> Iterator[dict]:
        ...
        with factory.create_client() as client:
            ...
            for idx, link in enumerate(soup.find_all("a", href=re.compile(r"profession=|gairm="))):
                href = link.get("href", "")
...
def gaois_combined_source(
    logainm_county: Optional[str] = None,
    ...
    """
    # Yield placenames
    for resource in logainm_source(county=logainm_county, max_results=max_placenames):
        yield resource
