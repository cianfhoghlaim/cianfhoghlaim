...
def _parse_volume_row(row, collection: str) -> Optional[dict]:
...
def _parse_photo_item(item) -> Optional[dict]:
...
def _parse_audio_item(item) -> Optional[dict]:
...
def _fetch_school_xml(client, school_id: str) -> Optional[dict]:
    ...
    try:
        ...
        # Parse pages
        pages = [{"page_id": p.get("id"), "page_number": p.text} for p in root.findall(".//page")]
