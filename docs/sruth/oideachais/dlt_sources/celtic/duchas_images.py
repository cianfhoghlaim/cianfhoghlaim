...
from sruth.shared.http import duchas_client, doras_client
    ...
    with factory.create_client() as client:
        try:
            ...
            for correction in soup.select(".correction-item, .edit-item"):
                corrections.append(
                    {
                        "original_text": correction.select_one(".original").get_text(strip=True)
                        if correction.select_one(".original")
                        else None,
                        "corrected_text": correction.select_one(".corrected").get_text(strip=True)
                        if correction.select_one(".corrected")
                        else None,
                        "contributor": correction.select_one(".contributor").get_text(strip=True)
        ...
        with factory.create_client() as client:
            for list_page in range(1, max_pages + 1):
                ...
                for link in volume_links:
                    ...
                    # Extract page links
                    page_links = vol_soup.select(f'a[href*="/{collection}/{volume_number}/"]')
