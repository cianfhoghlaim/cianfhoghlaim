        ...
        with factory.create_client() as client:
            ...
            for area_id in set(area_ids):
                try:
                    ...
                    for recording_elem in soup.find_all("li", class_="recording"):
                        ...
                        recording_link = recording_elem.find(
                            "a", href=re.compile(r"/[A-Z]+\d+c\d+")
                        )
                        if recording_link:
            ...
            for recording_id in set(recording_ids):
                try:
                    ...
                    logger.warning(
                        "canuint_transcript_fetch_error", recording_id=recording_id, error=str(e)
                    )
                    continue
        ...
        with factory.create_client() as client:
            ...
            for recording_id in set(recording_ids):
                try:
                    ...
                    logger.warning(
                        "canuint_tts_recording_error", recording_id=recording_id, error=str(e)
                    )
                    continue
