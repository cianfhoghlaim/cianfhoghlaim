...
class FalkorDBClient(GraphClient):
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            raise ImportError(
                "falkordb package required for FalkorDB client. Install with: pip install falkordb"
            )
    ...
    def get_schema(self) -> Dict[str, Any]:
        ...
        labels = [list(r.values())[0] for r in labels_result.raw_result if r]

        types_result = self.query("CALL db.relationshipTypes()")
        rel_types = [list(r.values())[0] for r in types_result.raw_result if r]
