...
class MemgraphClient:
    ...
    def _get_driver(self):
        ...
        if self._driver is None:
            try:
                ...
                raise ImportError("neo4j driver is not installed. Run: pip install neo4j")
    ...
    def find_shortest_path(
        ...
        rel_pattern = (
            "|".join(relationship_types) if relationship_types else "PREREQUISITE_OF|RELATED_TO"
        )
        query = f"""
    ...
    def link_pdf_to_outcomes(
        ...
        count = 0
        for code, score in zip(outcome_codes, relevance_scores):
            try:
...
class CurriculumDataLoader:
    ...
    def load_subjects(self) -> int:
        ...
        try:
            ...
            count = 0
            for row in rows:
                code, name_en, name_ga, level, url = (
                    row[:5] if len(row) >= 5 else (row + (None,) * 5)[:5]
                )
                if code:
    ...
    def load_learning_outcomes(self, subject_code: str | None = None) -> int:
        ...
        try:
            ...
            for row in rows:
                ...
                strand_id = (
                    f"{subject}_{level}_{strand}" if strand else f"{subject}_{level}_default"
                )
