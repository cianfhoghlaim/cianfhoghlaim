...
class LanceDBCloudConfig:
    ...
    api_key: str = field(default_factory=lambda: os.getenv("LANCEDB_API_KEY", ""))
    region: str = field(default_factory=lambda: os.getenv("LANCEDB_REGION", "eu-west-1"))
...
class LanceDBCloudClient:
    ...
    async def create_index(
        ...
        logger.info(
            "lancedb_index_created", index_type=index_type, table_name=table_name, column=column
        )
    ...
    async def init_pipeline_tables(self) -> dict[str, bool]:
        ...
        for name, description in self.PIPELINE_TABLES.items():
            if name in existing:
                ...
                results[name] = True
                logger.info(
                    "lancedb_pipeline_table_created", table_name=name, description=description
                )
