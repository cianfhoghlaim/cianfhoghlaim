    ...
    def create_snapshot(
        ...
        try:
            doc_count = conn.execute(
                f"SELECT COUNT(DISTINCT document_id) FROM {results_table}"
            ).fetchone()[0]
            model_count = conn.execute(
                f"SELECT COUNT(DISTINCT model_id) FROM {results_table}"
            ).fetchone()[0]
    ...
    def get_results_at_snapshot(
        ...
        try:
            ...
            snapshot = conn.execute(
                f"SELECT snapshot_timestamp FROM {snapshots_table} WHERE id = ?", [snapshot_id]
            ).fetchone()
            ...
            logger.error(
                "ducklake_results_at_snapshot_failed", snapshot_id=snapshot_id, error=str(e)
            )
            return []
