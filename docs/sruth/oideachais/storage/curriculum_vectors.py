...
class CurriculumVectorSearch:
    ...
    def __init__(
        ...
        self.db_path = db_path or os.getenv(
            "LANCEDB_PATH", os.path.join(os.path.dirname(__file__), "data/curriculum_vectors.lance")
        )
        ...
        for i in range(0, len(outcomes), batch_size):
            ...
            for o, text, vector in zip(batch, texts, vectors):
                data.append(
                    {
                        "outcome_id": o.get(
                            "id", o.get("outcome_id", hashlib.md5(text.encode()).hexdigest())
                        ),
                        "text": text,
                        "vector": vector,
...
async def index_curriculum_from_duckdb(db_path: str | None = None) -> int:
    ...
    db_path = db_path or os.path.join(
        os.path.dirname(__file__), "../storage/data/celtic_education.duckdb"
    )
