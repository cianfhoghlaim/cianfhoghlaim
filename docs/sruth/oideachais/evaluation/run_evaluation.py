...
async def run_evaluation(
    ...
    if save_dataset:
        dataset_path = (
            ...
            / "storage"
            / "evaluation"
            / f"eval_dataset_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    if run_baseline:
        ...
        try:
            ...
            logger.info(
                f"  - Answer Correctness: {baseline_result.get('answer_correctness', 'N/A')}"
            )
    ...
    if run_agentic:
        ...
        try:
            ...
            logger.info(
                f"  - Answer Correctness: {agentic_result.get('answer_correctness', 'N/A')}"
            )
            logger.info(
                f"  - Avg Queries/Question: {agentic_result.get('avg_queries_per_question', 'N/A')}"
            )
            logger.info(
                f"  - Avg Contexts/Question: {agentic_result.get('avg_contexts_per_question', 'N/A')}"
            )
        ...
        try:
            ...
            if "improvement_percentage" in comparison:
                ...
                if comparison["improvement_percentage"] >= target_improvement:
                    logger.info(
                        f"✅ TARGET ACHIEVED: {comparison['improvement_percentage']:.1f}% >= {target_improvement}%"
                    )
                else:
                    logger.info(
                        f"🎯 Target: {target_improvement}% improvement (from 65.2% to 87.9%)"
                    )
