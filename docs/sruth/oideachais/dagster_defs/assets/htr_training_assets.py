...
def duchas_pages(
    ...
    try:
        ...
        with pipeline.sql_client() as client:
            pages_count = client.execute_sql("SELECT COUNT(*) FROM manuscript_pages").fetchone()[0]
            trans_count = client.execute_sql("SELECT COUNT(*) FROM transcriptions").fetchone()[0]
    ...
    try:
        ...
        return MaterializeResult(
            metadata={
                ...
                "county_distribution": MetadataValue.json(stats.get("county_distribution", {})),
                "export_paths": MetadataValue.json(
                    {k: str(v) for k, v in results.items() if isinstance(v, Path)}
                ),
            }
        )
