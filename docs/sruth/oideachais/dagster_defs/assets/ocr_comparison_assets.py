...
def ocr_source_documents(context) -> MaterializeResult:
    ...
    return MaterializeResult(
        metadata={
            ...
            "image_count": len(
                [d for d in documents if d["suffix"] in [".png", ".jpg", ".jpeg", ".tiff", ".tif"]]
            ),
            "total_size_mb": MetadataValue.float(
                sum(d["size_bytes"] for d in documents) / 1024 / 1024
            ),
        }
    )
...
def ocr_model_comparison(context) -> MaterializeResult:
    ...
    - Model provenance
    """
    context.log.info(f"Running OCR model comparison")
    ...
    # Import storage
    try:
        from sruth.oideachais.storage.ducklake_filesystem import (
            DuckLakeOCRStorage,
            OCRResultRecord,
        )
...
def ocr_embeddings(context) -> MaterializeResult:
    ...
    try:
        from sruth.oideachais.cocoindex_flows.ocr_embedding import (
            OCREmbeddingFlow,
            OCREmbeddingConfig,
