...
def create_cycle_asset(cycle: str):
    ...
    def _cycle_asset(context) -> dg.MaterializeResult:
        ...
        from sruth.oideachais.dlt_sources.ireland.curriculum_source import (
            curriculum_source,
        )
        ...
        context.log.info(f"Ingesting: {cycle}/{subject}/{language}")
        ...
        # Get the DLT source for this specific subject
        # The source uses @dlt.resource with proper schema definition
        dlt_src = curriculum_source(
            cycle=cycle,
            subject=subject,
            language=language,
            sources=["curriculumonline", "ncca"],  # Exclude examinations for curriculum pages
            max_pages_per_subject=50,
            deduplicate=True,
        )

        # Extract the curriculum_pages resource (first resource in the source)
        curriculum_pages_resource = dlt_src.resources[0]
        source_name = curriculum_pages_resource.name
        ...
        # Connect the DLT resource to the pipeline
        # This ensures proper schema validation and column mapping
        dlt_pipeline.extract(
            dlt_src,
            write_disposition="merge",
        )

        # Run DLT normalize and load through serial executor for DuckDB safety
        normalize_info = safe_dlt_normalize(dlt_pipeline)
        context.log.info(f"Normalized {page_count} pages for {cycle}/{subject}/{language}")

        load_info = safe_dlt_load(
            dlt_pipeline,
        )
        ...
        return dg.MaterializeResult(
            metadata={
                ...
                "subject": subject,
                "language": language,
                "urls_scraped": 4,  # curriculumonline (en+ga) + ncca (en+ga)
