"""
Isle of Man Curriculum Pipeline - Centralized with MultiPartition.
...
Asset Structure:
    isle_of_man/curriculum/foundation ← MultiPartition(subject, language)
    isle_of_man/curriculum/primary    ← MultiPartition(subject, language)
    isle_of_man/curriculum/secondary  ← MultiPartition(subject, language)
    isle_of_man/curriculum/higher     ← MultiPartition(subject, language)

Partition Keys: "manx|en", "manx|gv", etc.
...
Usage:
    from dagster_defs.assets.isle_of_man import curriculum_dlt_assets
...
KEY_STAGES = ["foundation", "primary", "secondary", "higher"]
...
DLT_PIPELINE_NAME = "isle_of_man_curriculum_unified"
DLT_DATASET_NAME = "isle_of_man_curriculum"
    ...
    return MultiPartitionsDefinition(
        {
            ...
            "language": StaticPartitionsDefinition(["en", "gv"]),
        }
    )
...
def create_key_stage_asset(key_stage: str):
    ...
    Asset key: isle_of_man/curriculum/{key_stage}
    Partition: subject|language (e.g., "manx|en")

    All assets write to the same unified DuckDB for centralized storage.
    """
    display_name = key_stage.replace("_", " ").title()
    asset_key = ["isle_of_man", "curriculum", key_stage]

    @dg.asset(
        key=asset_key,
        group_name="isle_of_man_curriculum",
        compute_kind="dlt",
        description=f"Isle of Man {display_name} Curriculum",
        ...
        tags={
            "key_stage": key_stage,
            "nation": "isle_of_man",
            "pipeline": "isle_of_man_curriculum",
        },
        op_tags={"dagster/concurrency_key": f"isle_of_man_curriculum_{key_stage}"},
    )
    def _key_stage_asset(context) -> dg.MaterializeResult:
        ...
        from sruth.oideachais.dlt_sources.isle_of_man.curriculum_registry import SubjectRegistry
        ...
        context.log.info(f"Ingesting: isle_of_man/{key_stage}/{subject}/{language}")
        ...
        context.log.info(
            f"Loaded {rows_loaded} rows for isle_of_man/{key_stage}/{subject}/{language}"
        )
...
def create_all_curriculum_assets() -> list:
    """Create all Isle of Man curriculum assets (one per key stage)."""
    assets = []
    ...
    _safe_log("info", "created_all_isle_of_man_curriculum_assets", count=len(assets))
    return assets
