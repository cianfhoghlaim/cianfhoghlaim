...
def curriculum_with_terminology(
    ...
    try:
        ...
        terms_count = conn.execute("SELECT COUNT(*) FROM celtic.tearma_terms").fetchone()[0]
    ...
    try:
        ...
        link_count = conn.execute("SELECT COUNT(*) FROM enriched.curriculum_terms").fetchone()[0]
    except Exception as e:
...
def bilingual_aligned_pairs(
    ...
    try:
        ...
        pair_count = conn.execute("SELECT COUNT(*) FROM training.parallel_corpus").fetchone()[0]
    except Exception as e:
...
def folklore_by_curriculum_topic(
    ...
    # Check source data
    try:
        folklore_count = conn.execute("SELECT COUNT(*) FROM celtic.duchas_volumes").fetchone()[0]
    except Exception as e:
...
def dialect_variation_atlas(
    ...
    try:
        ...
        variation_count = conn.execute("SELECT COUNT(*) FROM enriched.dialect_atlas").fetchone()[0]
    except Exception as e:
