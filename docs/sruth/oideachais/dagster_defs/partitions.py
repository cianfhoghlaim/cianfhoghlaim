...
domain_partitions = StaticPartitionsDefinition(["ireland", "uk", "celtic"])
...
dialect_area_partitions = DynamicPartitionsDefinition(name="dialect_areas")
...
def generate_ireland_partition_keys(
    ...
    return [{"source": source, "language": lang} for source in sources for lang in languages]
...
def generate_celtic_partition_keys(
    ...
    return [{"corpus": corpus, "language": lang} for corpus in corpora for lang in languages]
...
def generate_uk_partition_keys(
    ...
    return [{"nation": nation, "source_type": st} for nation in nations for st in source_types]
