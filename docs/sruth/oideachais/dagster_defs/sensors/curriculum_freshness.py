...
Usage:
    from sruth.oideachais.dagster_defs.sensors.curriculum_freshness import (
...
class SourceState:
    ...
    def from_dict(cls, data: dict[str, Any]) -> SourceState:
        ...
        return cls(
            ...
            last_changed=(
                datetime.fromisoformat(data["last_changed"]) if data.get("last_changed") else None
            ),
        )
...
class CurriculumSensorState:
    ...
    def to_cursor(self) -> str:
        ...
        return json.dumps({name: state.to_dict() for name, state in self.sources.items()})
    ...
    def from_cursor(cls, cursor: str | None) -> CurriculumSensorState:
        ...
        try:
            ...
            return cls(
                sources={
                    name: SourceState.from_dict(state_data) for name, state_data in data.items()
                }
            )
...
def unified_curriculum_freshness_sensor(
    ...
    context.log.info(f"Triggering runs for {len(run_requests)} cycles: {list(cycles_to_trigger)}")
...
def curriculum_enrichment_trigger_sensor(
    ...
    try:
        if context.cursor:
            ...
            last_enrichment = datetime(2000, 1, 1, tzinfo=timezone.utc)
