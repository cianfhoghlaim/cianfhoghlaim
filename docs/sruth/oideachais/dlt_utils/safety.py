...
from sruth.oideachais.core import get_executor, run_serial
    ...
    def _run() -> dlt.pipeline.LoadInfo:
        try:
            ...
            if hasattr(result, 'loads_ids') and result.loads_ids:
                logger.info(
                    f"DLT pipeline '{pipeline.pipeline_name}' completed successfully. "
                    f"Load IDs: {result.loads_ids}"
                )
            return result
        except Exception as e:
            logger.error(
                f"DLT pipeline '{pipeline.pipeline_name}' failed: {e}",
                exc_info=True,
                extra={
