...
class CurriculumReranker(BaseReranker):
    ...
    async def rerank(
        ...
        **kwargs,
        ...
        for i, doc in enumerate(documents):
            ...
            combined_score = sum(scores[k] * self.weights.get(k, 0) for k in scores)
    ...
    def _calculate_scores(
        ...
        if subject_filter and doc_subject:
            scores["subject_match"] = 1.0 if doc_subject.lower() == subject_filter.lower() else 0.3
        elif doc_subject:
            # Check if query mentions the subject
            scores["subject_match"] = 0.8 if doc_subject.lower() in query.lower() else 0.4
        else:
        ...
        if level_filter and doc_level:
            scores["level_match"] = 1.0 if doc_level.lower() == level_filter.lower() else 0.3
        else:
