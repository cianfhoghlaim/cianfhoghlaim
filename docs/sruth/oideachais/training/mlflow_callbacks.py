...
class GaelicOCRMLflowCallback(TrainerCallback):
    ...
    def on_evaluate(
        ...
        **kwargs,
        ...
        if state.global_step % self.log_samples_every_n_steps == 0 and self.sample_images:
    ...
    def _log_sample_images(self) -> None:
        ...
        for i, image_path in enumerate(self.sample_images[: self.max_samples_to_log]):
    ...
    def _log_sample_predictions(self, model: Any, tokenizer: Any) -> None:
        ...
        for i, (img_path, gt) in enumerate(
            zip(
                ...
                self.sample_transcriptions[: self.max_samples_to_log],
            )
        ):
            try:
                # Generate prediction
    ...
    def _generate_prediction(
        ...
        try:
            ...
            processor = AutoProcessor.from_pretrained(
                model.config._name_or_path.replace("-bnb-4bit", "").replace("unsloth/", "")
            )
        except Exception:
            # Fallback
            processor = tokenizer
    ...
    def _log_final_predictions(self, model: Any, tokenizer: Any) -> None:
        ...
        results = []
        for img_path, gt in zip(self.sample_images, self.sample_transcriptions):
            try:
                pred = self._generate_prediction(model, tokenizer, img_path)
    ...
    def _save_predictions_artifact(self, predictions: list[dict], filename: str) -> None:
...
class GaelicMetricsCallback(TrainerCallback):
    ...
    def on_step_end(
        ...
        **kwargs,
        ...
        for img_path, gt in self.eval_samples[:5]:  # Limit for speed
            try:
