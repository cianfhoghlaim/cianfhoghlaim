...
def _join_with_duckdb(
    ...
    nation_select = (
        f", b.{nation_column} as matched_nation"
        if nation_column and nation_column in boundaries_df.columns
        else ""
    )
...
def query_area_statistics(
    ...
    if metrics:
        columns = ["area_code", "area_name", "nation", "school_count"] + [
            f"avg_{m}" for m in metrics
        ]
