...
async def compare_vision_models(
    ...
    try:
        if parallel:
            ...
            tasks = [client.run_inference(model_key, image_bytes, prompt) for model_key in models]
...
async def compare_vision_models_batch(
    ...
    """
    tasks = [compare_vision_models(img, models, prompt, prompt_key, litellm_url) for img in images]
...
def compare_vision_models_sync(
    ...
    """
    return asyncio.run(compare_vision_models(image, models, prompt, prompt_key))
