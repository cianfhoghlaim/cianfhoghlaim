...
Usage:
    from ocr.irish_htr_dataset import IrishHTRDatasetGenerator
...
class IrishHTRDatasetGenerator:
    ...
    def add_page(
        ...
        trans_lines = [line.strip() for line in transcription.split("\n") if line.strip()]
    ...
    def _create_sample(
        ...
        if seg_line.image_crop:
            ...
            if self.config.save_images:
                image_path = str(
                    self.output_dir / "images" / f"{sample_id}.{self.config.image_format}"
                )
    ...
    def _generate_augmentations(
        ...
        augmentations.append(
            ("brightness_low", lambda img: ImageEnhance.Brightness(img).enhance(0.9))
        )
        augmentations.append(
            ("brightness_high", lambda img: ImageEnhance.Brightness(img).enhance(1.1))
        )
        ...
        for i in range(min(self.config.augmentation_factor, len(augmentations))):
            ...
            try:
                ...
                if (
                    original.has_tironian_et
                    and random.random() < self.config.tironian_substitution_rate
                ):
    ...
    def export_pylaia(
        self, splits: dict[str, list[DatasetSample]] | None = None
    ) -> dict[str, Path]:
        """
        ...
        try:
            ...
            for split_name, samples in splits.items():
                ...
                with open(output_path, "w", encoding="utf-8") as f:
                    for sample in samples:
                        f.write(
                            json.dumps(sample.to_huggingface_format(), ensure_ascii=False) + "\n"
                        )
                output_files[split_name] = output_path
            return output_files
