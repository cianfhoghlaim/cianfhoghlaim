...
class IrishOCRProcessor:
    ...
    async def process_with_fallback(
        ...
        if analysis.is_irish and analysis.quality_score < self.quality_threshold:
            ...
            fallback_result = await self._run_fallback(image, prompt, primary_model)
            # Compare and return best
            fallback_analysis = self.analyze_irish_quality(fallback_result.get("text", ""))
    ...
    async def _run_fallback(
        ...
        fallback_models = [m for m in self.fallback_models if m != exclude_model]
    ...
    def _detect_dialect(self, text: str) -> DialectAnalysis:
        ...
        mixed = [d for d, s in dialect_scores.items() if s > 0 and d != best_dialect]
    ...
    def _detect_spelling_standard(self, text: str) -> SpellingStandard:
        ...
        pre_reform_count = sum(len(re.findall(p, text_lower)) for p in PRE_REFORM_INDICATORS)
        modern_count = sum(len(re.findall(p, text_lower)) for p in MODERN_INDICATORS)
    ...
    def _generate_recommendations(
        ...
        if fada.missing_fadas:
            ...
            recommendations.append(f"Potentially missing fadas in: {', '.join(words)}")
