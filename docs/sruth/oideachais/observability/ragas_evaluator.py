...
class RagasEvaluator:
    ...
    async def evaluate(
        ...
        try:
            ...
            # Select metrics
            if metrics:
                selected_metrics = [self._metrics[m] for m in metrics if m in self._metrics]
            else:
            ...
            eval_result = EvaluationResult(
                ...
                sample_scores=result.to_pandas().to_dict("records")
    ...
    if log_to_mlflow:
        ...
        with mlflow_run("ragas_evaluation", tags={"type": "rag_quality"}):
            log_evaluation_results(
                ...
                scores={
                    k: v for k, v in scores.items() if isinstance(v, (int, float)) and v is not None
                },
            )
...
async def run_evaluation_suite(
    ...
    if log_to_mlflow:
        from .mlflow_config import log_evaluation_results, mlflow_run, init_mlflow
...
async def evaluate_curriculum_search(
    ...
    Returns:
        Evaluation scores
    """
    contexts = [result.get("content", result.get("text", str(result))) for result in search_results]
