...
async def demo_celtic_languages():
    ...
    for lang in MOCK_CELTIC_LANGUAGES:
        print(
            f"{lang['language']:<20} {lang['native_name']:<15} {lang['speakers']:<12,} {lang['resources']:<10,}"
        )
...
async def demo_api_endpoints():
    ...
    endpoints = [
        ...
        (
            "GET /geospatial/schools",
            "Query schools by location",
            [("region", "string"), ("radius", "float")],
        ),
        ("GET /celtic/languages", "List Celtic languages", None),
        (
            "POST /celtic/translate",
            "Translate text",
            [("text", "string"), ("source", "lang"), ("target", "lang")],
        ),
