...
def tuath_curriculum_context(context: AssetExecutionContext) -> dict:
    ...
    try:
        ...
        if not curriculum_table:
            context.log.warning(
                f"No curriculum embeddings table found in oideachais. Available tables: {tables}"
            )
            return {
                "nation": nation,
                "status": "no_data",
        ...
        try:
            nation_data = (
                table.search().where(f"nation = '{nation}'", prefilter=True).limit(1000).to_pandas()
            )
        except Exception:
...
def tuath_learning_games(context: AssetExecutionContext) -> dict:
    ...
    try:
        ...
        if not OIDEACHAIS_DUCKDB_PATH.exists():
            context.log.warning(f"Oideachais DuckDB not found at {OIDEACHAIS_DUCKDB_PATH}")
