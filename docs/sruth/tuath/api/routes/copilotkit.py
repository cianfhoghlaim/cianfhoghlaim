...
try:
    from ...agents.orchestrator import get_orchestrator, TuathAgentOrchestrator
...
async def generate_agent_response(
    ...
    if agent_type == "celtic_tutor":
        ...
        yield emitter.tool_call_args(args={"query": user_message, "tradition": "irish"}).to_sse()
    ...
    response_content = await generate_response_content(agent_type, user_message, context, player_id)
...
def get_render_components(agent_type: str, context: dict, message: str) -> list[RenderComponent]:
...
def generate_quest_response(
    ...
    current_quest = context.get("current_quest", "tutorial_greetings")

    response = f"""## Quest Guidance
