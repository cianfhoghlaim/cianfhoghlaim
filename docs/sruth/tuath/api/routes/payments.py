...
async def get_usage_status(
    ...
    if x_session_id:
        ...
        if session:
            usage_type = (
                resource_type.replace("_", "s_")
                if "_" not in resource_type
                else resource_type.split("_")[0] + "s"
            )
...
async def request_payment(
    ...
    config = PAYMENT_CONFIG[resource_type]
    token_config = PAYMENT_TOKENS[token]
...
async def verify_payment(proof: PaymentProof) -> PaymentStatus:
    ...
    payment_request["verified_at"] = datetime.now(timezone.utc).isoformat()
...
def create_402_response(
    ...
    if token == "USDC":
        ...
        price_crypto = config["price_usd"] / 2500.0
