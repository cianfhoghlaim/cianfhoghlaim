...
class MythologyService:
    ...
    async def get_deity(
        ...
        try:
            ...
            filtered = df[(df["name"].str.lower() == name.lower()) & (df["entity_type"] == "deity")]
            if tradition:
    ...
    async def get_story(
        ...
        try:
            ...
            filtered = df[
                (df["name"].str.lower() == title.lower()) & (df["entity_type"] == "story")
            ]
            if tradition:
                filtered = filtered[filtered["tradition"] == tradition]
    ...
    async def list_stories_by_cycle(
        ...
        try:
            ...
            filtered = df[(df["entity_type"] == "story") & (df["cycle"] == cycle)]
            if tradition:
                filtered = filtered[filtered["tradition"] == tradition]
