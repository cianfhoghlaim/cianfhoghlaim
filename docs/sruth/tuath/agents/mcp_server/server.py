...
"""
        ...
        output.append(f"Data source: oideachais (via shared LanceDB)")
        return "\n".join(output)
...
"""
        ...
        for i, outcome in enumerate(outcomes, 1):
            ...
            output.append(
                f"   Subject: {outcome.subject} | Level: {outcome.level} | Nation: {outcome.nation}"
            )
        ...
        for lore in lore_list:
            ...
            if lore.celtic_names:
                ...
                for lang, name in lore.celtic_names.items():
                    lang_full = {"ga": "Irish", "gd": "Scottish Gaelic", "cy": "Welsh"}.get(
                        lang, lang
                    )
...
def validate_asset_impl(asset_id: str, concept_title: Optional[str] = None) -> str:
...
async def call_tool(name: str, arguments: dict[str, Any]) -> CallToolResult:
    ...
    try:
        if name == "search_curriculum":
            ...
            result = (
                f"Gaeltacht search for '{arguments.get('query', '')}': Pending GeoJSON integration."
            )
        else:
