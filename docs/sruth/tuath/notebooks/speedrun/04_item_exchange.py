...
def _():
    ...
    for name, pool in pools.items():
        ...
        print(
            f"  Reserves: {stats['reserve_celtic']:,.0f} CELTIC / {stats['reserve_items']:.0f} items"
        )
...
def _(pools):
    ...
    print(
        f"Initial: {apple_pool.reserve_celtic:,.0f} CELTIC / {apple_pool.reserve_items:.0f} Apples"
    )
    ...
    print(
        f"Final State: {apple_pool.reserve_celtic:,.2f} CELTIC / {apple_pool.reserve_items:.0f} Apples"
    )
    return (
        apple_pool,
        celtic_received,
...
def _():
    ...
    class OrderBook:
        ...
        def _match_orders(self, new_order: TradeOrder):
            ...
            opposite_type = (
                OrderType.SELL if new_order.order_type == OrderType.BUY else OrderType.BUY
            )
            ...
            if new_order.order_type == OrderType.BUY:
                ...
                matching_orders = [
                    o for o in matching_orders if o.price_per_item <= new_order.price_per_item
                ]
            else:
            ...
            for match in matching_orders:
                ...
                self.trade_history.append(
                    {
                        "buyer": new_order.player
                        if new_order.order_type == OrderType.BUY
                        else match.player,
                        "seller": match.player
                        if new_order.order_type == OrderType.BUY
                        else new_order.player,
    ...
    if book.trade_history:
        ...
        print(
            f"Trade Executed: {trade['buyer']} bought {trade['quantity']}x from {trade['seller']} @ {trade['price']:.0f} CELTIC"
        )
    return (
        OrderBook,
        OrderStatus,
...
def _():
    ...
    # Demo guild treasury
    guild = Guild(
        guild_id=1, name="Clann na Gaeilge", members=["player1", "player2", "player3", "player4"]
    )
