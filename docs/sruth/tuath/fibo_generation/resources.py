...
class FiboResource(ConfigurableResource):
    ...
    def create_educational_prompt(
        ...
        subject_config = subject_styles.get(
            subject,
            {
    ...
    async def suggest_refinements(
        ...
        for issue in issues:
            if "accuracy" in issue.lower():
                suggestions.append(f"Correct scientific accuracy: {issue}")
            elif "clarity" in issue.lower():
                suggestions.append(f"Improve clarity: {issue}")
            elif "label" in issue.lower():
                suggestions.append(f"Add or fix labels: {issue}")
            else:
