...
class TextureProcessor:
    ...
    def __init__(self, config: Optional[TextureProcessingConfig] = None):
        if not PIL_AVAILABLE:
            raise ImportError(
                "PIL/Pillow is required for texture processing. Install with: pip install Pillow"
            )
    ...
    def generate_mipmaps(self, image_data: bytes) -> list[tuple[bytes, int, int]]:
        ...
        while (
            current.width >= self.config.min_mipmap_size
            and current.height >= self.config.min_mipmap_size
        ):
            ...
            current = current.resize(
                (new_width, new_height), self._get_pil_filter(self.config.mipmap_filter)
            )
    ...
    def create_atlas(
        ...
        sorted_names = sorted(images.keys(), key=lambda n: images[n].height, reverse=True)

        # Calculate atlas size
        total_area = sum(
            (img.width + self.config.atlas_padding) * (img.height + self.config.atlas_padding)
    ...
    def resize_to_size(
        ...
        resized = image.resize((width, height), self._get_pil_filter(self.config.resize_mode))
    ...
    def _resize(self, image: "Image.Image") -> "Image.Image":
        ...
        if (width, height) != image.size:
            image = image.resize((width, height), self._get_pil_filter(self.config.resize_mode))
