...
def audio_features_radar(mo, tracks, alt, pl):
    ...
    chart = (
        alt.Chart(radar_data.to_pandas())
        .mark_bar()
        .encode(
...
def soundcloud_plays_chart(mo, sc_tracks, alt):
    ...
    plays_chart = (
        alt.Chart(top_tracks.to_pandas())
        .mark_bar()
        .encode(
...
def platform_comparison(mo, tracks, sc_tracks, alt, pl):
    ...
    soundcloud_stats = {
        ...
        "total_plays": sc_tracks["playback_count"].sum()
        if "playback_count" in sc_tracks.columns
        else 0,
