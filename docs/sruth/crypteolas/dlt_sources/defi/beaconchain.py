    ...
    with _get_client() as client:
        try:
            ...
            yield {
                ...
                "estimated_entry_wait_days": queue_data.get("entry_waittime", 0) * 6.4 / 60 / 24
                if queue_data.get("entry_waittime")
                else None,
                "estimated_exit_wait_days": queue_data.get("exit_waittime", 0) * 6.4 / 60 / 24
                if queue_data.get("exit_waittime")
                else None,
                "network": "ethereum",
                "data_source": "beaconchain",
                "timestamp": datetime.now(timezone.utc).isoformat(),
