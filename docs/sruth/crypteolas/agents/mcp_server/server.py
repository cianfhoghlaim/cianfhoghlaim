...
async def scrape_url_impl(
    ...
    try:
        from crypteolas.dlt_sources.documentation.protocol_docs import (
            user_urls_source,
        )
        import dlt
...
async def index_local_files_impl(
    ...
    try:
        from crypteolas.dlt_sources.local.file_watcher import (
            workspace_source,
        )
        import dlt
...
async def approve_chunks_impl(
    ...
    try:
        from crypteolas.storage.ducklake_client import (
            get_ducklake_client,
            ChunkApprovalStatus,
        )
...
async def search_unified_impl(
    ...
    try:
        from crypteolas.cocoindex_flows.unified_embedding import unified_search, code_search
