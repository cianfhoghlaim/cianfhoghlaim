...
async def setup_cognee():
    ...
    cognee.config.graph_database_url = os.environ.get("MEMGRAPH_URI", "bolt://localhost:7687")

    # Configure vector store
    cognee.config.vector_db_provider = "lancedb"
    cognee.config.vector_db_url = os.environ.get("LANCEDB_URI", "./storage/lance/cognee")
...
async def compare_protocols(
    ...
    return {
        ...
        "similarities": [],
        "differences": [],
        "insights": [r.get("content") if isinstance(r, dict) else str(r) for r in results],
    }
