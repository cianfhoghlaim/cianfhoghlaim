...
def embed_code_chunk(
    ...
    return text.transform(
        cocoindex.functions.SentenceTransformerEmbed(model="microsoft/codebert-base")
    )
...
def embed_text_chunk(
    ...
    """
    return text.transform(cocoindex.functions.SentenceTransformerEmbed(model="BAAI/bge-m3"))
...
def code_embedding_flow(
    ...
    with data_scope["code_files"].row() as file:
        # Detect language
        file["language"] = file["filename"].transform(lambda f: detect_language_from_extension(f))
