...
def embed_single_text(
    ...
    return text.transform(cocoindex.functions.SentenceTransformerEmbed(model=model))
...
def unified_embedding_flow(
    ...
    with data_scope["documents"].row() as doc:
        ...
        doc["content_type"] = doc["content"].transform(lambda c: classify_content(c, None))
...
async def unified_search(
    ...
    filtered_results = [
        r for r in search_results if (1.0 - r.get("_distance", 1.0)) >= similarity_threshold
    ]
