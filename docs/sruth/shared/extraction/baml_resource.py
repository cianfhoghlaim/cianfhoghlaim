    ...
    async def extract_exam_paper(
        ...
        try:
            if baml:
                ...
                data, tokens = await self._fallback_extract(content, "ExamPaperExtraction")
                return BAMLExtractionResult(
                    data=data,
                    schema_name="ExamPaperExtraction",
                    model_used=self.litellm_model,
    ...
    async def extract_marking_scheme(
        ...
        try:
            if baml:
                ...
                data, tokens = await self._fallback_extract(content, "MarkingSchemeExtraction")
                return BAMLExtractionResult(
                    data=data,
                    schema_name="MarkingSchemeExtraction",
                    model_used=self.litellm_model,
    ...
    async def extract_exam_materials_list(
        ...
        try:
            if baml:
                ...
                data, tokens = await self._fallback_extract(results_page, "ExamMaterialMetadata")
    ...
    async def build_curriculum_graph(
        ...
        try:
            if baml:
                ...
                data, tokens = await self._fallback_extract(syllabi_str, "CurriculumGraph")
                return BAMLExtractionResult(
                    data=data,
                    schema_name="CurriculumGraph",
                    model_used=self.litellm_model,
