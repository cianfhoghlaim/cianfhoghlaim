...
class ExamMetadataSignature(dspy.Signature):
    ...
    exam_content: str = dspy.InputField(desc="Exam paper content including questions and marks")
...
class DocumentClassificationSignature(dspy.Signature):
    ...
    document_content: str = dspy.InputField(desc="Document text content to classify")
...
class LearningOutcomeExtractionSignature(dspy.Signature):
    ...
    strand: str = dspy.OutputField(desc="The strand this section belongs to")
    strand_unit: str = dspy.OutputField(desc="The strand unit this section belongs to")
...
class QuestionExtractionSignature(dspy.Signature):
    ...
    exam_text: str = dspy.InputField(desc="Exam paper text containing questions")
...
class IrishTranslationSignature(dspy.Signature):
    ...
    source_lang: str = dspy.InputField(desc="Source language (en, ga, cy, gd)")
    target_lang: str = dspy.InputField(desc="Target language (en, ga, cy, gd)")
    translated_text: str = dspy.OutputField(
        desc="Translated text maintaining educational terminology"
    )
    confidence: float = dspy.OutputField(desc="Translation confidence score 0-1")
...
class MultiLanguageExtractionSignature(dspy.Signature):
    ...
    detected_language: str = dspy.OutputField(desc="Detected language (ga, cy, gd, gv)")
    structured_data: dict = dspy.OutputField(desc="Extracted structured data from the document")
    english_translation: str = dspy.OutputField(desc="English translation of the document text")
...
class CurriculumAlignmentSignature(dspy.Signature):
    ...
    source_nation: str = dspy.InputField(desc="Source nation (ireland, uk, wales, scotland)")
    target_nation: str = dspy.InputField(desc="Target nation (ireland, uk, wales, scotland)")
