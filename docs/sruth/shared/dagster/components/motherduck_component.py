...
class MotherDuckComponent(dg.Component, dg.Resolvable, dg.Model):
    ...
    def _get_planetscale_config(self) -> dict[str, str] | None:
        ...
        return {
            ...
            or os.getenv("PLANETSCALE_USER", os.getenv("PLANETSCALE_USERNAME", "")),
            "password": self.planetscale_password
            or os.getenv("PLANETSCALE_PASSWORD", os.getenv("PLANETSCALE_PASSWORD_TOKEN", "")),
        }
...
class PlanetScaleComponent(dg.Component, dg.Resolvable, dg.Model):
    ...
    def build_defs(self, context: dg.ComponentLoadContext) -> dg.Definitions:
        ...
        return dg.Definitions(
            resources={self.name: planetscale_resource.configured(self._get_config())}
        )
