    ...
    try:
        ...
        if hasattr(destinations, "NAMESPACE"):
            ...
            if actual_namespace != expected_namespace:
                print(
                    f"  ✗ Namespace mismatch: expected '{expected_namespace}', got '{actual_namespace}'"
                )
                return False
...
def test_oideachais() -> bool:
    ...
    try:
        ...
        print(f"  ✓ create_pipeline() works (DuckDB fallback)")
        print(
            f"  ⚠ Note: oideachais has pre-existing import issues in safety.py (sruth.oideachais.core)"
        )
