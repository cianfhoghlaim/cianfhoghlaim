"""openclaw handler — receive a ContextEnvelope from openchamber or hermes.

The handler unpacks the envelope and returns a `dict` with the unpacked
context_payload, ready for the openclaw channel fanout to consume.
...
"Cross-agent context envelope protocol"):
  ...
  "openclaw_handler.py ... MUST implement a function
...
Usage:
    ...
    from agents.contracts.openclaw_handler import receive_envelope

    envelope = ContextEnvelope(...)
    result = receive_envelope(envelope)
    if result["status"] == "received":
        # feed result["context"] into the channel fanout
        ...
"""
...
EXPECTED_RECIPIENT: AgentSurface = "openclaw"


def receive_envelope(envelope: ContextEnvelope) -> dict[str, Any]:
    """Receive a cross-agent envelope on the openclaw surface.

    Returns a dict with the unpacked context_payload, ready for the
    openclaw channel fanout to consume. The dict has these keys:

    - `status`: "received" | "rejected" | "expired"
    - `recipient`: "openclaw" (always)
    ...
    if envelope.recipient != EXPECTED_RECIPIENT:
        logger.warning(
            "openclaw: envelope addressed to %r, not openclaw; rejecting",
            envelope.recipient,
        )
        return {
            "status": "rejected",
    ...
    if envelope.is_expired(datetime.now(timezone.utc)):
        logger.warning(
            "openclaw: envelope %s expired at %s; rejecting",
    ...
    #    The openclaw fanout expects these keys (channel-specific routing).
    context = dict(envelope.context_payload)
    context.setdefault("source", envelope.sender)
    context.setdefault("trace_id", envelope.parent_trace_id)
    context.setdefault("run_id", envelope.agent_run_id)

    logger.info(
        "openclaw: received envelope run_id=%s from sender=%s",
        envelope.agent_run_id,
        envelope.sender,
    )
