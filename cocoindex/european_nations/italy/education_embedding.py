...
SOURCE_DIR = pathlib.Path("dlt/european_nations/ita/education/subjects")

_splitter = RecursiveSplitter()


@dataclass
class ita_UPPER_EducationChunk:
    chunk_id: str
    jurisdiction: str
    subject: str
    language: str
...
@coco.fn(memo=True)
async def process_ita_education_file(
    file: FileLike,
    table: lancedb.TableTarget,
) -> None:
    """Process one Italy per-subject education file."""
    ...
    language = parts[6] if len(parts) >= 7 else "it"
    for chunk in chunks:
        vec = await coco.use_context(EMBEDDER).embed(chunk.text)
        table.declare_row(
            row=ita_UPPER_EducationChunk(
...
async def app_main() -> None:
    """Wire the Italy education pipeline App."""
    target_table = await lancedb.mount_table_target(
        LANCE_DB,
        table_name=TABLE_NAME,
        table_schema=await lancedb.TableSchema.from_class(
            ita_UPPER_EducationChunk, primary_key=["chunk_id"]
        ),
    )
    ...
    await coco.mount_each(
        process_ita_education_file, files.items(), target_table
    )


ita_education_embedding = coco.App(
    coco.AppConfig(
        name="ita_education_embedding",
...
__all__ = [
    "ita_UPPER_EducationChunk",
    "TABLE_DESCRIPTION",
    "TABLE_NAME",
    "ita_education_embedding",
    "shared_lifespan",
]
