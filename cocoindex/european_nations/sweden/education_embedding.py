...
shared LanceDB table ``cianfhoghlaim.lc.european_nations.swe.education_chunks`` using the canonical
...
TABLE_NAME = "cianfhoghlaim.lc.european_nations.swe.education_chunks"
TABLE_DESCRIPTION = (
    "Multilingual 1024-d BGE-M3 embeddings of every Sweden "
    "per-subject education row (EU full-depth expansion change)."
)
SOURCE_DIR = pathlib.Path("dlt/european_nations/swe/education/subjects")
...
@coco.fn(memo=True)
async def process_swe_education_file(
    file: FileLike,
    table: lancedb.TableTarget,
) -> None:
    """Process one Sweden per-subject education file."""
    ...
    language = parts[6] if len(parts) >= 7 else "sv"
    for chunk in chunks:
        vec = await coco.use_context(EMBEDDER).embed(chunk.text)
        table.declare_row(
            row=SWEEducationChunk(
...
async def app_main() -> None:
    """Wire the Sweden education pipeline App."""
    target_table = await lancedb.mount_table_target(
        LANCE_DB,
        table_name=TABLE_NAME,
        table_schema=await lancedb.TableSchema.from_class(
            SWEEducationChunk, primary_key=["chunk_id"]
        ),
    )
    ...
    await coco.mount_each(
        process_swe_education_file, files.items(), target_table
    )


swe_education_embedding = coco.App(
    coco.AppConfig(
        name="swe_education_embedding",
