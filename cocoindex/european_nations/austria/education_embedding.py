...
shared LanceDB table ``cianfhoghlaim.lc.european_nations.aut.education_chunks`` using the canonical
...
TABLE_NAME = "cianfhoghlaim.lc.european_nations.aut.education_chunks"
TABLE_DESCRIPTION = (
    "Multilingual 1024-d BGE-M3 embeddings of every Austria "
    "per-subject education row (EU full-depth expansion change)."
)
SOURCE_DIR = pathlib.Path("dlt/european_nations/aut/education/subjects")
...
@coco.fn(memo=True)
async def process_aut_education_file(
    file: FileLike,
    table: lancedb.TableTarget,
) -> None:
    """Process one Austria per-subject education file."""
...
async def app_main() -> None:
    """Wire the Austria education pipeline App."""
    target_table = await lancedb.mount_table_target(
        LANCE_DB,
        table_name=TABLE_NAME,
        table_schema=await lancedb.TableSchema.from_class(
            AUTEducationChunk, primary_key=["chunk_id"]
        ),
    )
    ...
    await coco.mount_each(
        process_aut_education_file, files.items(), target_table
    )


aut_education_embedding = coco.App(
    coco.AppConfig(
        name="aut_education_embedding",
