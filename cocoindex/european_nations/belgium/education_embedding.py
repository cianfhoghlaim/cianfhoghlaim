...
SOURCE_DIR = pathlib.Path("dlt/european_nations/bel/education/subjects")
...
@coco.fn(memo=True)
async def process_bel_education_file(
    file: FileLike,
    table: lancedb.TableTarget,
) -> None:
    """Process one Belgium per-subject education file."""
...
async def app_main() -> None:
    """Wire the Belgium education pipeline App."""
    target_table = await lancedb.mount_table_target(
        LANCE_DB,
        table_name=TABLE_NAME,
        table_schema=await lancedb.TableSchema.from_class(
            BELEducationChunk, primary_key=["chunk_id"]
        ),
    )
    ...
    await coco.mount_each(
        process_bel_education_file, files.items(), target_table
    )


bel_education_embedding = coco.App(
    coco.AppConfig(
        name="bel_education_embedding",
