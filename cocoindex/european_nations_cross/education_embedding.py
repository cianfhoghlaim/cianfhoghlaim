...
@coco.fn(memo=True)
async def process_education_file(
    file: FileLike,
    ...
    for chunk in chunks:
        ...
        table.declare_row(
            row=EducationChunk(
                ...
                document_type="eu_nation_education",
            ),
        )
...
async def app_main() -> None:
    ...
    target_table = await lancedb.mount_table_target(
        ...
        table_schema=await lancedb.TableSchema.from_class(
            EducationChunk, primary_key=["chunk_id"]
        ),
    )
    ...
    await coco.mount_each(process_education_file, files.items(), target_table)


european_nations_education_embedding = coco.App(
    coco.AppConfig(
        name="european_nations_education_embedding",
...
__all__ = [
    ...
    "TABLE_DESCRIPTION",
    "TABLE_NAME",
    "european_nations_education_embedding",
    "shared_lifespan",
]
