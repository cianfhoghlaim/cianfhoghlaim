...
SOURCE_DIR = pathlib.Path("dlt/british_isles/ni/education/subjects")

_splitter = RecursiveSplitter()


@dataclass
class NorthernIrelandEducationChunk:
    chunk_id: str
    nation: str
    subject: str
    language: str
...
@coco.fn(memo=True)
async def process_ni_education_file(
    file: FileLike,
    table: lancedb.TableTarget,
) -> None:
    """Process one Northern Ireland per-subject education file."""
    ...
    for chunk in chunks:
        ...
        table.declare_row(
            row=NorthernIrelandEducationChunk(
...
async def app_main() -> None:
    """Wire the Northern Ireland education pipeline App."""
    target_table = await lancedb.mount_table_target(
        LANCE_DB,
        table_name=TABLE_NAME,
        table_schema=await lancedb.TableSchema.from_class(
            NorthernIrelandEducationChunk, primary_key=["chunk_id"]
        ),
    )
    ...
    await coco.mount_each(
        process_ni_education_file, files.items(), target_table
    )


ni_education_embedding = coco.App(
    coco.AppConfig(
        name="ni_education_embedding",
...
__all__ = [
    "NorthernIrelandEducationChunk",
    "TABLE_DESCRIPTION",
    "TABLE_NAME",
    "ni_education_embedding",
    "shared_lifespan",
]
