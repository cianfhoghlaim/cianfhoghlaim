"""
Chemistry v1 CocoIndex Embedding App (BIEP v1 canonical).

Embeds the NCCA Leaving Certificate Chemistry syllabuses, exam
...
LanceDB table: `oideachais.lc.chemistry.<level>_<language>`.
...
DEFAULT_CHEM_ROOT = pathlib.Path(
    os.getenv(
        "CIANFHOGHLAIM_CHEM_ROOT",
        str(
...
if COCOINDEX_AVAILABLE:
    ...
    @dataclass
    class ChemChunk:
        """One chunked + embedded paragraph from a Chemistry PDF."""
    ...
    @coco.fn(memo=True)
    async def process_chemistry_pdf(
        file_path: pathlib.PurePath,
        text: str,
        level: str,
        language: str,
        target_table: lancedb.TableTarget[ChemChunk],  # type: ignore[type-var]
    ) -> None:
    ...
    async def chemistry_app_main(
        ...
        target_table = await lancedb.mount_table_target(
            ...
            table_name=f"oideachais.lc.chemistry.{level}_{language}",
            table_schema=await lancedb.TableSchema.from_class(
                ChemChunk, primary_key=["chunk_id"]
            ),
        )
        ...
        if not sourcedir.exists():
            logger.warning(
                "chemistry_corpus_dir_not_found", path=str(sourcedir)
            )
            return
        ...
        async for record in files.items():
            ...
            try:
                ...
                continue
            await process_chemistry_pdf(
                file_path, text, level, language, target_table
            )

    app = coco.App(
        coco.AppConfig(name="ChemistryEmbedding"),
        chemistry_app_main,
        sourcedir=DEFAULT_CHEM_ROOT,
        level="hl",
        language="en",
    )


async def query_chemistry(
    query: str,
    level: str = "hl",
    language: str = "en",
    top_k: int = 5,
) -> list[dict]:
    """Semantic search over the Chemistry LanceDB table."""
