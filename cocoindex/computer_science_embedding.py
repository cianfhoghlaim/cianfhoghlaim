"""
Computer Science v1 CocoIndex Embedding App (BIEP v1 canonical).

Embeds the NCCA Leaving Certificate Computer Science syllabuses,
exam papers, and marking schemes into LanceDB.
...
LanceDB table: `oideachais.lc.computer_science.<level>_<language>`.
...
DEFAULT_COMP_ROOT = pathlib.Path(
    os.getenv(
        "CIANFHOGHLAIM_COMP_ROOT",
        str(
...
if COCOINDEX_AVAILABLE:
    ...
    @dataclass
    class CompSciChunk:
        """One chunked + embedded paragraph from a Computer Science PDF."""
    ...
    @coco.fn(memo=True)
    async def process_computer_science_pdf(
        file_path: pathlib.PurePath,
        text: str,
        level: str,
        language: str,
        target_table: lancedb.TableTarget[CompSciChunk],  # type: ignore[type-var]
    ) -> None:
    ...
    @coco.fn
    async def computer_science_app_main(
        ...
        target_table = await lancedb.mount_table_target(
            ...
            table_name=f"oideachais.lc.computer_science.{level}_{language}",
            table_schema=await lancedb.TableSchema.from_class(
                CompSciChunk, primary_key=["chunk_id"]
            ),
        )
        ...
        if not sourcedir.exists():
            logger.warning(
                "computer_science_corpus_dir_not_found", path=str(sourcedir)
            )
            return
        ...
        async for record in files.items():
            ...
            try:
                ...
                continue
            await process_computer_science_pdf(
                file_path, text, level, language, target_table
            )

    app = coco.App(
        coco.AppConfig(name="ComputerScienceEmbedding"),
        computer_science_app_main,
        sourcedir=DEFAULT_COMP_ROOT,
        level="hl",
        language="en",
    )


async def query_computer_science(
    query: str,
    level: str = "hl",
    language: str = "en",
    top_k: int = 5,
) -> list[dict]:
    """Semantic search over the Computer Science LanceDB table."""
    ...
    return await semantic_search(
        table=f"oideachais.lc.computer_science.{level}_{language}",
        query=query,
        top_k=top_k,
    )
