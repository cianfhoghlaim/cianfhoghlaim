"""wls_education_embedding — Wave 3 stub.

This is a placeholder CocoIndex v1 App for the
`orchestration/defs/3_model_lifecycle/cocoindex_v1/wls_education_embedding/defs.yaml`
...
`wls_education_embedding` defs.yaml files were a v0-era mistake that we're now stubbing.
"""
...
if COCOINDEX_AVAILABLE:
    ...
    async def wls_education_embedding_flow(  # type: ignore[no-untyped-def,unused-ignore]
        source_id: str,
    ) -> AsyncIterator[dict[str, Any]]:
        """Stub: yield one dummy row for the wls_education_embedding pipeline."""
        yield {"source_id": source_id, "kind": "wls_education_embedding_stub"}

    app = coco.App(
        coco.AppConfig(name="wls_education_embedding"),
        wls_education_embedding_flow,
