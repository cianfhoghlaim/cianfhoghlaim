"""lc_subject_embedding — Wave 3 stub.

This is a placeholder CocoIndex v1 App for the
`orchestration/defs/3_model_lifecycle/cocoindex_v1/lc_subject_embedding/defs.yaml`
...
`lc_subject_embedding` defs.yaml files were a v0-era mistake that we're now stubbing.
"""
...
if COCOINDEX_AVAILABLE:
    ...
    async def lc_subject_embedding_flow(  # type: ignore[no-untyped-def,unused-ignore]
        source_id: str,
    ) -> AsyncIterator[dict[str, Any]]:
        """Stub: yield one dummy row for the lc_subject_embedding pipeline."""
        yield {"source_id": source_id, "kind": "lc_subject_embedding_stub"}

    app = coco.App(
        coco.AppConfig(name="lc_subject_embedding"),
        lc_subject_embedding_flow,
