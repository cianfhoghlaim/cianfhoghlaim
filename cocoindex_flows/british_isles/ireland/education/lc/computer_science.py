"""cocoindex_flows.british_isles.ireland.education.lc.computer_science
-- the BIEP v3 Leaving Certificate Computer Science CocoIndex v1 App.
...
try:
    ...
    logger.warning("cocoindex_v1_not_available_for_computer_science: %s", e)
...
if _BAML_AVAILABLE:
    try:
        ...
        _baml_extract = b.ExtractLCSyllabusComputerScience
    except ImportError:
        _baml_extract = None
else:
    _baml_extract = None
...
def _python_baml_fallback_extract(text: str) -> dict[str, Any]:
    return python_baml_fallback_extract("computer_science", text)


@dataclass
class ComputerScienceChunk:
    chunk_id: str
    subject: str
    language: str
    level: str
...
if COCOINDEX_AVAILABLE:
    @coco.fn(memo=True)
    async def process_computer_science_file(
        ...
        if _baml_extract is not None:
            try:
                ...
                logger.warning("baml_extract_failed_for_computer_science: %s", exc)
    ...
    @coco.fn
    async def computer_science_app_main(sourcedir, language, level):
        target_table = await _coco_lancedb.mount_table_target(
            LANCE_DB,
            table_name=f"cianhoghlaim.ireland.leaving_cycle.computer_science.{level}_{language}_chunks",
            table_schema=await _coco_lancedb.TableSchema.from_class(
                ComputerScienceChunk, primary_key=["chunk_id"]
            ),
        )
        target_table.declare_vector_index(column="embedding")
        if not sourcedir.exists():
            logger.warning("computer_science_corpus_dir_not_found", language=language, level=level, path=str(sourcedir))
            return
        ...
        async for record in files.items():
            ...
            if not (str(file_path).lower().endswith(".pdf") or str(file_path).lower().endswith(".txt") or str(file_path).lower().endswith(".md")):
                continue
            await process_computer_science_file(file=record, subject="computer_science", language=language, level=level, target_table=target_table)


if COCOINDEX_AVAILABLE:
    app = coco.App(
        coco.AppConfig(name="ireland_lc_computer_science_embedding"),
        computer_science_app_main,
        sourcedir=pathlib.Path(os.getenv("CIANFHOGHLAIM_LC_COMPUTER_SCIENCE_ROOT", "leaving_certificate/computer_science")),
        language="en", level="hl",
    )


async def run_computer_science_subject(sourcedir, language="en", level="hl", embed_fn=None):
    if embed_fn is None:
    ...
    if not rows:
        rows = build_subject_fixture("computer_science", num_rows=5)
    out = []
    for row in rows:
        text = row["text"]
        if not text:
        ...
        for i, chunk in enumerate(chunks):
            ...
            out.append({
                ...
                "subject": "computer_science", "language": row["language"], "level": row["level"],
...
__all__ = ["ComputerScienceChunk", "process_computer_science_file", "computer_science_app_main", "app", "run_computer_science_subject", "COCOINDEX_AVAILABLE", "_BAML_AVAILABLE"]
