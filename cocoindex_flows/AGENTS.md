...
# alias: uv run python -m cocoindex_flows._shared.cli conformance

# Update a single App
mise run cocoindex:update -- <module>:<App>
# e.g. mise run cocoindex:update -- cianfhoghlaim.cocoindex_flows.biep_parity.ireland_lc_mathematics_embedding:ireland_lc_mathematics_embedding
...
When you need **N nearly-identical CocoIndex Apps** that differ only by a config row (ISO code, subject, jurisdiction, etc.), **collapse them into one factory-driven module + N 1-line re-export shims**. The canonical example is `cocoindex_flows/european_nations/_factory.py`:

- **Before**: 40 nation CocoIndex Apps at `cocoindex_flows/european_nations/{alb,aut,bel,...}/education_embedding.py` (~5,400 LOC, ~135 LOC each)
- **After**: 1 factory module (`cocoindex_flows/european_nations/_factory.py`, 224 LOC) + 40 1-line re-export shims (`from cocoindex_flows.european_nations._factory import alb_education_embedding  # noqa: F401`, ~400 LOC)
