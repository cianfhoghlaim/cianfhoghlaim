"""CocoIndex v1 App for the Americas education pipeline.

Embeds every Americas per-jurisdiction education row (US-CA / BRA /
MEX / VEN + the 35+ remaining American countries in the much later
follow-on change) into the shared LanceDB table
``cianfhoghlaim.americas.education_chunks`` using the canonical
``BAAI/bge-m3`` 1024-d multilingual embedder.

Conforms to the R1–R4 conformance contract (imports
``shared_lifespan`` + the canonical ContextKeys from ``_lifespan.py``;
the ``coco.App`` is at module scope; the module declares at least
one ``@coco.fn``).
"""
from __future__ import annotations

import pathlib
from dataclasses import dataclass
from datetime import UTC, datetime
from typing import Annotated

import cocoindex as coco
from cocoindex.connectors import lancedb, localfs
from cocoindex.resources.file import FileLike, PatternFilePathMatcher
from cocoindex.resources.id import IdGenerator
from cocoindex.ops.text import RecursiveSplitter
from numpy.typing import NDArray

from ..._shared._lifespan import (
    EMBEDDER,
    LANCE_DB,
    shared_lifespan,
)

TABLE_NAME = "cianfhoghlaim.americas.education_chunks"
TABLE_DESCRIPTION = (
    "Multilingual 1024-d BGE-M3 embeddings of every Americas "
    "per-jurisdiction education row (US-CA / BRA / MEX / VEN + the 35+ "
    "remaining American countries in the much later follow-on change)."
)
SOURCE_DIR = pathlib.Path("dlt/americas")

_splitter = RecursiveSplitter()


@dataclass
class AmericasEducationChunk:
    chunk_id: str
    jurisdiction: str
    language: str
    domain: str
    text: str
    embedding: Annotated[NDArray, EMBEDDER]
    source_url: str
    document_type: str
    extracted_at: str = datetime.now(UTC).isoformat()


@coco.fn(memo=True)
async def process_americas_education_file(
    file: FileLike,
    table: lancedb.TableTarget,
) -> None:
    """Process one Americas per-jurisdiction education file."""
    text = await file.read_text()
    chunks = _splitter.split(
        text, chunk_size=2000, chunk_overlap=500, language="markdown"
    )
    id_gen = IdGenerator()
    jurisdiction = (
        "/".join(file.file_path.parts[2:4])
        if len(file.file_path.parts) >= 4
        else "americas"
    )
    for chunk in chunks:
        vec = await coco.use_context(EMBEDDER).embed(chunk.text)
        table.declare_row(
            row=AmericasEducationChunk(
                chunk_id=await id_gen.next_id(chunk.text),
                jurisdiction=jurisdiction,
                language=file.file_path.parent.name
                if len(file.file_path.parts) >= 2
                else "en",
                domain="education",
                text=chunk.text,
                embedding=vec,
                source_url=str(file.file_path),
                document_type="americas_education",
            ),
        )


@coco.fn
async def app_main() -> None:
    """Wire the Americas education pipeline App."""
    target_table = await lancedb.mount_table_target(
        LANCE_DB,
        table_name=TABLE_NAME,
        table_schema=await lancedb.TableSchema.from_class(
            AmericasEducationChunk, primary_key=["chunk_id"]
        ),
    )
    target_table.declare_vector_index(column="embedding")
    files = localfs.walk_dir(
        SOURCE_DIR,
        recursive=True,
        path_matcher=PatternFilePathMatcher(
            included_patterns=["**/*.md", "**/*.txt", "**/*.json"],
        ),
        live=True,
    )
    await coco.mount_each(
        process_americas_education_file, files.items(), target_table
    )


americas_california_education_embedding = coco.App(
    coco.AppConfig(
        name="americas_california_education_embedding",
        description=TABLE_DESCRIPTION,
    ),
    app_main,
)


__all__ = [
    "AmericasEducationChunk",
    "TABLE_DESCRIPTION",
    "TABLE_NAME",
    "americas_california_education_embedding",
    "shared_lifespan",
]
