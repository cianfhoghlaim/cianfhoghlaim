"""CocoIndex v1 App for the EU nations + Ukraine law pipeline.

Embeds every per-nation law row into the shared
``cianfhoghlaim.eu_nations.law_chunks`` LanceDB table using the
...
TABLE_NAME = "cianfhoghlaim.eu_nations.law_chunks"
TABLE_DESCRIPTION = (
...
async def app_main() -> None:
    ...
    await coco.mount_each(process_law_file, files.items(), target_table)


european_nations_law_embedding = coco.App(
    coco.AppConfig(
        name="european_nations_law_embedding",
    ),
    app_main,
)
...
__all__ = [
    ...
    "TABLE_DESCRIPTION",
    "TABLE_NAME",
    "european_nations_law_embedding",
    "shared_lifespan",
]
