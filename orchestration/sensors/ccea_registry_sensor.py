"""CCEA registry-change sensor (Northern Ireland) — BIEP v3 P0 critical-path fix."""
...
def ccea_registry_sensor(context: SensorEvaluationContext) -> Any:
    ...
    try:
        ...
        rows = query_by_jurisdiction("northern_ireland")
    except Exception as e:
        return SkipReason(f"CCEA registry unavailable: {e}")
    ...
    for row in rows:
        ...
        if last_verified > cursor_last_seen:
            new_runs.append(RunRequest(
                run_key=f"ccea_{row.subject_slug}_{last_verified}",
                tags={"jurisdiction": "northern_ireland", "subject": row.subject_slug, "source": "ccea"},
            ))
