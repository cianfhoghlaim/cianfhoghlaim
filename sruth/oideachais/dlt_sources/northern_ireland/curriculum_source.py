"""
Unified Curriculum Source - Subject-centric DLT source for Northern Irish curriculum data.

Merges content from CCEA, Education NI, and Gaeloideachas into
...
- Bilingual support (Irish/Gaeilge + English)
...
Usage:
    ...
    # Crawl all sources for KS4 Mathematics in English
    pipeline = dlt.pipeline(pipeline_name="northern_ireland_curriculum", destination="duckdb")
...
def _crawl_source(
    ...
    """
    Crawl a Northern Irish curriculum source using Firecrawl.
...
def build_subject_urls(
    ...
    """
    Build all URLs for a Northern Irish subject (sources × languages).

    Args:
        key_stage: Education key stage (ks1, ks2, ks3, ks4)
        subject: Subject slug (mathematics, irish, etc.)
    ...
    for source_name, path in subject_config.urls.items():
        ...
        # Irish URL (if source supports it)
        if "ga" in source_config.language_prefixes:
            urls.append({
                "source": source_name,
                "language": "ga",
                "url": source_config.get_full_url(path, "ga"),
            })
...
def parallel_scrape_subject(
    ...
    """
    Scrape all URLs for a Northern Irish subject in parallel.
        ...
        for source_name in sources:
            ...
            for raw_page in _crawl_source(
                ...
                if raw_page.get("status") in ("error", "no_api_key", "firecrawl_not_installed"):
                    ...
                    raw_page["nation"] = "northern_ireland"
                    yield raw_page
                    continue
...
def curriculum_source(
    ...
    """
    Unified DLT source for Northern Irish curriculum data.

    Subject-centric: crawls all specified sources for a (key_stage, subject, language)
    combination.

    Args:
        key_stage: Education key stage (foundation, ks1, ks2, ks3, ks4)
    ...
    logger.info(
        "northern_ireland_curriculum_source_initialized",
        key_stage=key_stage,
        subject=subject,
        language=language,
        sources=sources,
    ...
    def curriculum_pages() -> Iterator[dict[str, Any]]:
        """Crawled Northern Irish curriculum pages."""
...
def _classify_pdf(url: str) -> str:
    ...
    if "specification" in url_lower or "sócnh" in url_lower:
