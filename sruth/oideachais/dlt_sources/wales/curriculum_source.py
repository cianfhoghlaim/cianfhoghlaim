"""
Unified Curriculum Source - Subject-centric DLT source for Welsh curriculum data.

Merges content from Hwb, WJEC, and CBAC into
...
- Bilingual support (Welsh/Cymraeg + English)
...
Usage:
    ...
    pipeline = dlt.pipeline(pipeline_name="wales_curriculum", destination="duckdb")
...
def _crawl_source(
    ...
    """
    Crawl a Welsh curriculum source using Firecrawl.
...
def build_subject_urls(
    ...
    """
    Build all URLs for a Welsh subject (sources × languages).

    Args:
        key_stage: Education key stage (ks3, ks4, etc.)
        subject: Subject slug (mathematics, welsh, etc.)
    ...
    for source_name, path in subject_config.urls.items():
        ...
        # Welsh URL (if source supports it)
        if "cy" in source_config.language_prefixes or base_source in ("cbac", "hwb"):
            # For CBAC, Welsh is the default (no prefix)
            welsh_prefix = "" if base_source == "cbac" else "/cy"
            urls.append({
                "source": source_name,
                "language": "cy",
                "url": f"{source_config.base_url}{welsh_prefix}{path}",
            })
...
def parallel_scrape_subject(
    ...
    """
    Scrape all URLs for a Welsh subject in parallel.
...
def curriculum_source(
    ...
    """
    Unified DLT source for Welsh curriculum data.

    Subject-centric: crawls all specified sources for a (key_stage, subject, language)
    combination.

    Args:
        key_stage: Education key stage (foundation, ks2, ks3, ks4)
    ...
    logger.info(
        "wales_curriculum_source_initialized",
        key_stage=key_stage,
        subject=subject,
        language=language,
        sources=sources,
    ...
    def curriculum_pages() -> Iterator[dict[str, Any]]:
        """Crawled Welsh curriculum pages."""
...
def _classify_pdf(url: str) -> str:
    ...
    if "specification" in url_lower or "manyleb" in url_lower:
