"""
Curriculum Registry - Subject taxonomy and URL resolution for England.

Provides a centralized registry for:
- Subject configuration and metadata (English curriculum)
- Key stage-based subject filtering
- Source-specific URL resolution (Gov.uk, AQA, OCR, Edexcel)
- Crawl configuration generation

Adapted from the Wales curriculum registry pattern.

Usage:
    from sruth.oideachais.dlt_sources.england.curriculum_registry import (
        SubjectRegistry,
        URLResolver,
    )
...
# Default path to england_curriculum_index.json
DEFAULT_INDEX_PATH = (
    Path(__file__).parent.parent.parent /
    "firecrawl_configs" / "curriculum" / "england_curriculum_index.json"
)


@dataclass
class SourceConfig:
    """Configuration for an English curriculum source (Gov.uk, AQA, OCR, Edexcel)."""

    name: str
    base_url: str
    language_prefixes: dict[str, str]  # {"en": ""} - English only
...
@dataclass
class SubjectConfig:
    """Configuration for an English curriculum subject."""

    slug: str
    name: str  # English only
    key_stages: list[str]  # ["ks1", "ks2", "ks3", "ks4", "ks5"]
    qualification_type: list[str] | None = None  # ["gcse", "a_level"]
...
class KeyStageConfig:
    """Configuration for an English key stage."""

    slug: str
    name: str
    age_range: str
    years: int | None = None
    year_groups: list[str] | None = None
...
class QualificationConfig:
    """Configuration for an English qualification."""

    slug: str
    name: str
    age: str
    levels: list[str] | None = None
    exam_boards: list[str] | None = None
    ...
    def from_dict(cls, slug: str, data: dict[str, Any]) -> QualificationConfig:
        ...
        return cls(
            ...
            exam_boards=data.get("exam_boards"),
...
class SubjectRegistry:
    """
    Registry for English curriculum subjects loaded from england_curriculum_index.json.
    ...
    def __init__(
        ...
        logger.info(
            "england_subject_registry_loaded",
    ...
    def from_json(cls, index_path: Path) -> SubjectRegistry:
        """Load registry from england_curriculum_index.json file."""
        ...
        for name, source_data in data.get("sources", {}).items():
            sources[name] = SourceConfig(
                ...
                language_prefixes=source_data.get("languages", {"en": ""}),
            )
    ...
    def from_default(cls) -> SubjectRegistry:
        """Load registry from the default england_curriculum_index.json location."""
        if not DEFAULT_INDEX_PATH.exists():
            logger.warning(
                "england_curriculum_index_not_found",
...
class URLResolver:
    """
    Resolves subjects to source-specific crawl configurations for England.

    Uses the SubjectRegistry to generate Firecrawl-compatible configurations
    for crawling English curriculum content.
    """

    # Key stage path mappings (for URL generation)
    KEY_STAGE_PATH_MAPPING: dict[str, str] = {
        "ks1": "key-stage-1",
        "ks2": "key-stage-2",
        "ks3": "key-stage-3",
        "ks4": "key-stage-4",
    ...
    def resolve_urls(
        ...
        Args:
            ...
            language: Language code (en only for England)
        ...
        for source_name, path in subject_config.urls.items():
            # Handle compound source names like "aqa_gcse"
            base_source = source_name.split("_")[0]
            source = self._registry.get_source(base_source)
            if not source:
                continue

            include_paths = self._build_include_paths(base_source, key_stage, path)
    ...
    def resolve_key_stage_urls(
        ...
        """
        if sources is None:
            sources = ["gov_uk", "aqa", "ocr", "edexcel"]
        ...
        for source_name in sources:
            ...
            include_paths = self._build_key_stage_include_paths(source_name, key_stage)
    ...
    def _build_include_paths(
        ...
        if source == "gov_uk":
            # Gov.uk National Curriculum paths
            return [f"{subject_path}*"]

        elif source in ("aqa", "ocr", "edexcel"):
            # Exam board qualification paths
            return [f"{subject_path}*"]
    ...
    def _build_key_stage_include_paths(
        ...
        if source == "gov_uk":
            ...
            return ["/national-curriculum/*", "/government/organisations/department-for-education/*"]
