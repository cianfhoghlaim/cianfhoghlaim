...
class HttpClientFactory:
    ...
    _rate_limiter: RateLimiter | None = field(init=False, default=None)
    ...
    async def _pre_request_async(self) -> None:
        ...
        if self._circuit_breaker.is_open(service):
            ...
            retry_after = (
                state.last_failure + self._circuit_breaker.recovery_time
    ...
    def _track_response(self, response: httpx.Response) -> None:
        ...
        if response.is_success:
            ...
            logger.warning(
                f"HTTP {response.status_code} from {service}: {response.text[:200]}"
            )
...
def coingecko_client(api_key: str | None = None) -> HttpClientFactory:
...
def github_client(token: str | None = None) -> HttpClientFactory:
...
def binance_spot_client(api_key: str | None = None) -> HttpClientFactory:
...
def binance_futures_client(api_key: str | None = None) -> HttpClientFactory:
...
def okx_client(api_key: str | None = None) -> HttpClientFactory:
...
def bybit_client(api_key: str | None = None) -> HttpClientFactory:
...
def beaconchain_client(api_key: str | None = None) -> HttpClientFactory:
...
def met_office_uk_client() -> HttpClientFactory:
    ...
    auth = ApiKeyAuth(api_key=api_key, as_query_param=True, query_param_name="key") if api_key else None
    return HttpClientFactory(
...
def edcolearning_client() -> HttpClientFactory:
    """Pre-configured client for EdcoLearning.ie exam audio resources."""
    return HttpClientFactory(
        base_url="https://edcolearning.ie",
