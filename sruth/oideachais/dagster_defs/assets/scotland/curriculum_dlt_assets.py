...
LEVEL_PARTITIONS = {
    level: create_level_partition(level)
...
def create_level_asset(level: str):
    ...
    def _level_asset(context) -> dg.MaterializeResult:
        ...
        context.log.info(
            f"Ingesting: scotland/{level}/{subject}/{language}"
        )
...
def create_all_curriculum_assets() -> list:
    ...
    for level in CURRICULUM_LEVELS:
        try:
            ...
            _safe_log("warning", "failed_to_create_level_asset", curriculum_level=level, error=str(e))
