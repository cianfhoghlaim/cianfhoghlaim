...
class KafkaSourceConfig:
    ...
    schema_registry_secret: str = field(default_factory=lambda: os.getenv("CONFLUENT_SR_API_SECRET", ""))
    ...
    initial_cursor: str = field(
        default_factory=lambda: datetime(2024, 1, 1).isoformat()
    )
...
def run_curriculum_pipeline(
    ...
    # Run pipeline
    info = pipeline.run(
        kafka_source(topics=topics, config=config, max_messages=max_messages)
    )
