"""
Environment-aware DuckLake destination factory for crypteolas.
...
def _get_production_config() -> DuckLakeConfig:
    """Build production DuckLake config using R2 + PlanetScale."""
    r2_bucket = os.environ.get("R2_DUCKLAKE_BUCKET", "ducklake")
    planetscale_uri = os.environ.get(
        "PLANETSCALE_CONNECTION_STRING",
        f"mysql://user:pass@host:3306/ducklake_{NAMESPACE}",
    )

    return DuckLakeConfig(
        data_dir=f"s3://{r2_bucket}/{NAMESPACE}/",
        catalog_uri=planetscale_uri,
    )
...
def get_dlt_destination(
    ...
    """
    Get DLT destination for crypteolas pipelines.
    ...
    Local:
        - Data: Garage S3 at s3://ducklake/crypteolas/
        - Metadata: PostgreSQL at localhost:5432

    Production:
        - Data: Cloudflare R2
        - Metadata: PlanetScale MySQL
