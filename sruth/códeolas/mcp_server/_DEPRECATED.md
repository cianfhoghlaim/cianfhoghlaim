...
`tools.py`) using the pre-v7 `sruth_códeolas` namespace that was
renamed by the
...
functionality (code-search MCP). The canonical code-search surface
is now **ccc** (CocoIndex Code) MCP, which provides the same
semantic-code-search capability with a much richer index.
...
for the full MCP revival context.
...
- Replacement: ccc MCP (already wired)