...
def skill_tree_badge_id(
    ...
    return (
        ...
        f"{date_earned.astimezone(timezone.utc).isoformat(timespec='seconds').replace(':', '-')}"
    )
...
class SkillTreeBadge:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "date_earned": self.date_earned.astimezone(timezone.utc).isoformat(),
...
class SkillTreeMastery:
    ...
    def __post_init__(self) -> None:
        if not self.mastery_id:
            ts = self.date_earned.astimezone(timezone.utc).isoformat(
                timespec="seconds",
            ).replace(":", "-")
