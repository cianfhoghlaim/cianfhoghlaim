...
email_triage_agent = LlmAgent(
    ...
    instruction=(
        ...
        "(port 7778). You help a user triage their personal + professional "
        "inbox across 4 accounts (DKIT.ie M365, 2 Gmail, Hotmail).\n\n"
        "TOOLS:\n"
        ...
        "WORKFLOW:\n"
        "1. For 'what should I reply to?' questions, call find_loose_threads "
        "first to surface prioritised loose threads.\n"
        "2. For 'summarise thread X', call summarise_thread.\n"
        "3. For 'is this legal / medical?' questions, call classify_email_thread.\n"
        "4. For 'link to research' on legal / medical threads, call "
        "link_thread_to_research with k=5 default.\n"
        ...
        "All 4 tools are read-only — the marimo notebook + the openclaw "
        "WebChat sub-UI are the write surfaces. Langfuse auto-traces every "
        "tool call (the oideachais stack's LANGFUSE_* env vars).\n"
    ),
    tools=[
