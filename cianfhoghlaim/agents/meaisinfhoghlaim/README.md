...
| 8 | `sruth/meaisinfhoghlaim/AGENTS.md:77` forward-referenced `sruth/oideachais/scéimre/` (a deferred-rename target per `lateralise-british-isles-domains`). Updated to `sruth/oideachais/baml_src/` + a one-line note pointing at the deferral. | `AGENTS.md:77` | RESOLVED (round 11 audit) |
| 9 | 3 dead stub files in `services/` (`celery_worker.py` + `pipeline_fastapi.py` + `agent_fastapi.py`, 42 lines total). Phase-8-aspirational prototypes never imported anywhere in `sruth/` (excluding `.venv/` + 3rd-party). Deleted; `services/` retains only `__init__.py`. | `services/` | RESOLVED (round 11 audit) |
...
| 13 | `sruth/meaisinfhoghlaim/agents/tools/` (10 files, 3009 lines) was a byte-for-byte duplicate of the canonical `sruth/oideachais/tools/` (10/10 SHA-256 match). The duplicate was a stale copy from before the tools migration landed in `oideachais/tools/`. Deleted; canonical home is now the single source of truth. | `agents/tools/` | RESOLVED (round 11 audit) |
| 14 | 4 meaisínfhoghlaim agent files (`agui_curriculum_agent.py:25` + `curriculum_comparison_agent.py:14` + `geospatial_agent.py:15` + `statistics_agent.py:15`) imported via `from ..tools.X import ...` — a path that resolves to `sruth/meaisinfhoghlaim/tools/X` and does not exist. All 4 would raise `ModuleNotFoundError` at module load time. Fixed: rewired to the absolute canonical home `from sruth.oideachais.tools.X import ...`. | 4 `agents/*_agent.py` files | RESOLVED (round 11 audit) |
| 15 | 2 pre-split multi-source DLT files in `language/gaeilge/` (`canuint.py` 1041 lines bundling 5 canuint sources + `duchas_images.py` 787 lines bundling `duchas_images_source` + `hidden_heritages_source`; 1,828 lines total). All 7 source functions exist at canonical split homes in `sruth/oideachais/dlt_sources/ie/culture/` (5 canuint files + 2 duchas images files, 1,540 lines total). Byte-level near-identity (0.6% diff, decorator-only). Zero active importers. Deleted; `language/gaeilge/` now contains only `__init__.py` + `irish_samples.yaml`. | `language/gaeilge/{canuint,duchas_images}.py` | RESOLVED (round 11 audit) |
