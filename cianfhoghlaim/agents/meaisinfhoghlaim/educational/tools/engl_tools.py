...
async def lookup_engl_lo(topic: str, level: str = "lc_hl", language: str = "en", limit: int = 10) -> list[dict[str, Any]]:
    try:
        ...
        results = await semantic_search(table=f"oideachais.lc.english.{level}_{language}", query=topic, embed_model="BAAI/bge-m3", top_k=limit)
...
async def generate_engl_item(lo_code: str, difficulty: int, level: str = "lc_hl", topic: str = "") -> dict[str, Any]:
    try:
        from cianfhoghlaim.baml_client import b
        import uuid
        item = b.GenerateEnglFormativeItem(lo_code=lo_code, difficulty=difficulty, level=level, topic=topic)
