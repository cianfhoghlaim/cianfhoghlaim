...
def _get_protocol_slugs() -> list[str]:
    ...
    # Fallback defaults
    return [
        "uniswap", "curve-dex", "sushiswap", "aave", "compound", "morpho",
        "lido", "rocket-pool", "gmx", "synthetix", "stargate", "across"
    ]
...
def defi_aggregate_stats(context: AssetExecutionContext) -> dict:
    ...
    try:
        ...
        # TVL stats
        try:
            total_tvl = conn.execute(
                "SELECT SUM(tvl_usd) FROM defi.protocol_tvl"
            ).fetchone()
        ...
        try:
            ...
            stats["exchange_funding_records"] = {
                row[0]: row[1] for row in exchange_count
            }
        except Exception:
            stats["exchange_funding_records"] = {}
