...
Beszel is a lightweight, self-hosted server monitoring solution consisting of a central Hub (dashboard) and per-host Agents. It collects CPU, memory, disk, network, and Docker container metrics with a clean web UI.
...
- **Ops**: `bonneagar/stacks/beszel/` (the 6-file GOLD_STANDARD)
...
- - **Pangolin**: `https://beszel.cianfhoghlaim.ie` (if exposed)
