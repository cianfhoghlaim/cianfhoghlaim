...
SearXNG is an open-source, self-hosted metasearch engine that aggregates results from multiple search engines without tracking users. It queries Google, Bing, DuckDuckGo, Wikipedia, and dozens of other engines in parallel, strips tracking parameters, and returns clean, privacy-respecting results.
...
- **Ops**: `bonneagar/stacks/searxng/` (the 6-file GOLD_STANDARD)
...
- - **Pangolin**: `https://search.cianfhoghlaim.ie` (if exposed)
