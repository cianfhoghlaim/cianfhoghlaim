"""
Geography v1 CocoIndex Embedding App (BIEP v1 canonical).

Embeds the NCCA Leaving Certificate Geography syllabuses, exam
...
LanceDB table: `oideachais.lc.geography.<level>_<language>`.
...
DEFAULT_GEOG_ROOT = pathlib.Path(
    os.getenv(
        "CIANFHOGHLAIM_GEOG_ROOT",
        str(
...
if COCOINDEX_AVAILABLE:
    ...
    @dataclass
    class GeogChunk:
        """One chunked + embedded paragraph from a Geography PDF."""
    ...
    @coco.fn(memo=True)
    async def process_geography_pdf(
        file_path: pathlib.PurePath,
        text: str,
        level: str,
        language: str,
        target_table: lancedb.TableTarget[GeogChunk],  # type: ignore[type-var]
    ) -> None:
    ...
    async def geography_app_main(
        ...
        target_table = await lancedb.mount_table_target(
            ...
            table_name=f"oideachais.lc.geography.{level}_{language}",
            table_schema=await lancedb.TableSchema.from_class(
                GeogChunk, primary_key=["chunk_id"]
            ),
        )
        ...
        if not sourcedir.exists():
            logger.warning(
                "geography_corpus_dir_not_found", path=str(sourcedir)
            )
            return
        ...
        async for record in files.items():
            ...
            try:
                ...
                continue
            await process_geography_pdf(
                file_path, text, level, language, target_table
            )

    app = coco.App(
        coco.AppConfig(name="GeographyEmbedding"),
        geography_app_main,
        sourcedir=DEFAULT_GEOG_ROOT,
        level="hl",
        language="en",
    )
...
async def query_geography(
    ...
    """Semantic search over the Geography LanceDB table."""
    ...
    return await semantic_search(
        table=f"oideachais.lc.geography.{level}_{language}",
        query=query,
        top_k=top_k,
    )
