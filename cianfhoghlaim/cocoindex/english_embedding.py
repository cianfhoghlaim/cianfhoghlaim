"""
English v1 CocoIndex Embedding App (BIEP v1 canonical).

Embeds the NCCA Leaving Certificate English syllabuses, exam
papers, and marking schemes into LanceDB.
...
LanceDB table: `oideachais.lc.english.<level>_<language>`.
...
DEFAULT_ENG_ROOT = pathlib.Path(
    os.getenv(
        "CIANFHOGHLAIM_ENG_ROOT",
        str(
...
if COCOINDEX_AVAILABLE:
    ...
    @dataclass
    class EngChunk:
        """One chunked + embedded paragraph from an English PDF."""
    ...
    @coco.fn(memo=True)
    async def process_english_pdf(
        file_path: pathlib.PurePath,
        text: str,
        level: str,
        language: str,
        target_table: lancedb.TableTarget[EngChunk],  # type: ignore[type-var]
    ) -> None:
    ...
    async def english_app_main(
        ...
        target_table = await lancedb.mount_table_target(
            ...
            table_name=f"oideachais.lc.english.{level}_{language}",
            table_schema=await lancedb.TableSchema.from_class(
                EngChunk, primary_key=["chunk_id"]
            ),
        )
        ...
        if not sourcedir.exists():
            logger.warning(
                "english_corpus_dir_not_found", path=str(sourcedir)
            )
            return
        ...
        async for record in files.items():
            ...
            try:
                ...
                continue
            await process_english_pdf(
                file_path, text, level, language, target_table
            )

    app = coco.App(
        coco.AppConfig(name="EnglishEmbedding"),
        english_app_main,
        sourcedir=DEFAULT_ENG_ROOT,
        level="hl",
        language="en",
    )


async def query_english(
    query: str,
    level: str = "hl",
    language: str = "en",
    top_k: int = 5,
) -> list[dict]:
    """Semantic search over the English LanceDB table."""
    ...
    return await semantic_search(
        table=f"oideachais.lc.english.{level}_{language}",
        query=query,
        top_k=top_k,
    )
