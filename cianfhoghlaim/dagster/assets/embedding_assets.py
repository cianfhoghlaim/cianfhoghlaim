...
def ireland_curriculum_embeddings(context) -> MaterializeResult:
    ...
    try:
        from cianfhoghlaim.observability import langfuse_trace, log_agent_metrics, mlflow_run
    ...
    # Publish to Kafka
    try:
        from kafka import CURRICULUM_PAGES, get_producer
...
def curriculum_rag_evaluation(context) -> MaterializeResult:
    ...
    # Publish to Kafka
    try:
        from kafka import EVALUATION_RAG, get_producer
        producer = get_producer()
        producer.produce(
            EVALUATION_RAG.name,
