"""DLT source for the Katsina state.
...
``stedding/ingest_queue/commonwealth/nga/states/Katsina State/education/ministry_of_education/<lang>/``.
"""
...
@dlt.resource(
    name="nga_kat_ministry_of_education",
...
def nga_kat_ministry_of_education(language=None):
    """Yield state rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/commonwealth/nga/states/Katsina State/education/ministry_of_education")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("nga_kat_ministry_of_education_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="nga_kat_ministry_of_education")
def nga_kat_ministry_of_education_source(language=None):
    """DLT source for the state ingestion."""
    return nga_kat_ministry_of_education(language=language)
...
__all__ = [
    ...
    "STATE_CODE",
    "STATE_NAME",
    "nga_kat_ministry_of_education",
    "nga_kat_ministry_of_education_source",
]
