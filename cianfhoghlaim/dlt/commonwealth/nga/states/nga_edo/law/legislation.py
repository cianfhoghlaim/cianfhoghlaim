...
``stedding/ingest_queue/commonwealth/nga/states/Edo State/law/legislation/<lang>/``.
"""
...
def nga_edo_legislation(language=None):
    """Yield state rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/commonwealth/nga/states/Edo State/law/legislation")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("nga_edo_legislation_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="nga_edo_legislation")
def nga_edo_legislation_source(language=None):
    """DLT source for the state ingestion."""
    return nga_edo_legislation(language=language)
