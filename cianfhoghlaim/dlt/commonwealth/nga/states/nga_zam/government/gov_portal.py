...
``stedding/ingest_queue/commonwealth/nga/states/Zamfara State/government/gov_portal/<lang>/``.
"""
...
def nga_zam_gov_portal(language=None):
    """Yield state rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/commonwealth/nga/states/Zamfara State/government/gov_portal")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("nga_zam_gov_portal_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="nga_zam_gov_portal")
def nga_zam_gov_portal_source(language=None):
    """DLT source for the state ingestion."""
    return nga_zam_gov_portal(language=language)
