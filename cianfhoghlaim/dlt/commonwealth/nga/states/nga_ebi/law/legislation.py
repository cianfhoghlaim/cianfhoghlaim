...
``stedding/ingest_queue/commonwealth/nga/states/nga_ebi/law/legislation/<lang>/``.
"""
...
def nga_ebi_legislation(language=None):
    """Yield state rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/commonwealth/nga/states/nga_ebi/law/legislation")
...
@dlt.source(name="nga_ebi_legislation")
def nga_ebi_legislation_source(language=None):
    """DLT source for the state ingestion."""
    return nga_ebi_legislation(language=language)
