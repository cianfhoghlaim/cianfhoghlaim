...
``stedding/ingest_queue/commonwealth/nga/states/Enugu State/statistics/stats_office/<lang>/``.
"""
...
def nga_enu_stats_office(language=None):
    """Yield state rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/commonwealth/nga/states/Enugu State/statistics/stats_office")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("nga_enu_stats_office_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="nga_enu_stats_office")
def nga_enu_stats_office_source(language=None):
    """DLT source for the state ingestion."""
    return nga_enu_stats_office(language=language)
...
__all__ = [
    ...
    "STATE_CODE",
    "STATE_NAME",
    "nga_enu_stats_office",
    "nga_enu_stats_office_source",
]
