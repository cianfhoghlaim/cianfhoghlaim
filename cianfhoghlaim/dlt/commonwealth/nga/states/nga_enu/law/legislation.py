    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("nga_enu_legislation_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="nga_enu_legislation")
def nga_enu_legislation_source(language=None):
    """DLT source for the state ingestion."""
    return nga_enu_legislation(language=language)
