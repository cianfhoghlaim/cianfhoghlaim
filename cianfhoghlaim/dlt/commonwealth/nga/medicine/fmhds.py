"""DLT source for the Nigerian federal Federal Ministry of Health & Development Services.
...
``stedding/ingest_queue/commonwealth/nga/medicine/fmhds/<lang>/``.
"""
...
FEDERAL_INSTITUTION = "fmhds"
CANONICAL_URL = "https://www.fmhds.gov.ng"
DEFAULT_LANGUAGE = "en"


@dlt.resource(
    name="nga_federal_fmhds",
...
def nga_federal_fmhds(language=None):
    """Yield Nigerian federal rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/commonwealth/nga/medicine/fmhds")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("nga_federal_fmhds_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="nga_federal_fmhds")
def nga_federal_fmhds_source(language=None):
    """DLT source for the Nigerian federal ingestion."""
    return nga_federal_fmhds(language=language)
