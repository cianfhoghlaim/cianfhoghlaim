"""DLT source for the Quebec Statistics Office.
...
DOMAIN = "statistics"
SLUG = "stats_office"
CANONICAL_URL = "https://statistique.quebec.ca/"
DEFAULT_LANGUAGE = "fr"


@dlt.resource(
    name="qc_stats_office",
...
def qc_stats_office(language=None):
    """Yield Quebec Statistics Office rows from the canonical cache."""
    import hashlib
    import json
    from datetime import UTC, datetime
    from pathlib import Path

    cache_dir = Path("stedding/ingest_queue/commonwealth/can/qc/statistics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("qc_stats_office_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="qc_stats_office")
def qc_stats_office_source(language=None):
    """DLT source for the Quebec Statistics Office ingestion."""
    return qc_stats_office(language=language)
