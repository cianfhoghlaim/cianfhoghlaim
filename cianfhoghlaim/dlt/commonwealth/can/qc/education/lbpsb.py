"""DLT source for the Quebec Lester B. Pearson School Board (English Montreal).

The Quebec deep education cluster source. Honours
``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/commonwealth/can/qc/education/lbpsb/<lang>/``.
...
SLUG = "lbpsb"
CANONICAL_URL = "https://www.lbpsb.qc.ca"
...
def qc_lbpsb(language=None):
    """Yield Quebec Lester B. Pearson School Board (English Montreal) rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/commonwealth/can/qc/education/lbpsb")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("qc_lbpsb_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="qc_lbpsb")
def qc_lbpsb_source(language=None):
    """DLT source for the Quebec Lester B. Pearson School Board (English Montreal) ingestion."""
    return qc_lbpsb(language=language)
