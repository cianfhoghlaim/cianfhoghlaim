"""DLT source for the Quebec English Montreal School Board.

The Quebec deep education cluster source. Honours
``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/commonwealth/can/qc/education/emsb/<lang>/``.
...
SLUG = "emsb"
CANONICAL_URL = "https://www.emsb.qc.ca"
...
def qc_emsb(language=None):
    """Yield Quebec English Montreal School Board rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/commonwealth/can/qc/education/emsb")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("qc_emsb_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="qc_emsb")
def qc_emsb_source(language=None):
    """DLT source for the Quebec English Montreal School Board ingestion."""
    return qc_emsb(language=language)
