"""DLT source for the Quebec Legislation.
...
``stedding/ingest_queue/commonwealth/can/qc/law/<lang>/``.
"""
...
DOMAIN = "law"
SLUG = "legislation"
CANONICAL_URL = "https://www.quebec.ca/en/justice"
DEFAULT_LANGUAGE = "fr"
...
def qc_legislation(language=None):
    """Yield Quebec Legislation rows from the canonical cache."""
    import hashlib
    import json
    from datetime import UTC, datetime
    from pathlib import Path

    cache_dir = Path("stedding/ingest_queue/commonwealth/can/qc/law")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("qc_legislation_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="qc_legislation")
def qc_legislation_source(language=None):
    """DLT source for the Quebec Legislation ingestion."""
    return qc_legislation(language=language)
