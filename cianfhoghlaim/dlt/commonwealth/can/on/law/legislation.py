"""DLT source for the Ontario Legislation.
...
``stedding/ingest_queue/commonwealth/can/on/law/<lang>/``.
"""
...
DOMAIN = "law"
SLUG = "legislation"
CANONICAL_URL = "https://www.ontario.ca/laws"
DEFAULT_LANGUAGE = "en"
...
def on_legislation(language=None):
    """Yield Ontario Legislation rows from the canonical cache."""
    import hashlib
    import json
    from datetime import UTC, datetime
    from pathlib import Path

    cache_dir = Path("stedding/ingest_queue/commonwealth/can/on/law")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("on_legislation_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="on_legislation")
def on_legislation_source(language=None):
    """DLT source for the Ontario Legislation ingestion."""
    return on_legislation(language=language)
