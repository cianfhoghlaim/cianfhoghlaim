...
def wls_biology(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Wales biology syllabus rows from the canonical cache.
    ...
    cache_dir = Path("stedding/site_scrape_samples/biep/wls/biology")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "wls_biology_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="wls_biology")
def wls_biology_source(language: str | None = None):
    """DLT source for the Wales biology ingestion."""
    return wls_biology(language=language)
