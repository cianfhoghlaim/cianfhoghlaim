...
def wls_english(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Wales english syllabus rows from the canonical cache.
    ...
    cache_dir = Path("stedding/site_scrape_samples/biep/wls/english")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "wls_english_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="wls_english")
def wls_english_source(language: str | None = None):
    """DLT source for the Wales english ingestion."""
    return wls_english(language=language)
