"""Per-subject DLT source for Wales (computing_science, WJEC).

Per-subject DLT source for the British Isles parity change
(`2026-07-12-british-isles-parity-pipeline-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/site_scrape_samples/biep/wls/computing_science/<lang>/``.
...
def wls_computing_science(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Wales computing_science syllabus rows from the canonical cache.
    ...
    cache_dir = Path("stedding/site_scrape_samples/biep/wls/computing_science")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "wls_computing_science_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="wls_computing_science")
def wls_computing_science_source(language: str | None = None):
    """DLT source for the Wales computing_science ingestion."""
    return wls_computing_science(language=language)
...
__all__ = [
    ...
    "SUBJECT",
    "DEFAULT_LEVEL",
    "wls_computing_science",
    "wls_computing_science_source",
]
