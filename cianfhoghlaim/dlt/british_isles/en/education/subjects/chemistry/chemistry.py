...
def en_chemistry(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield England chemistry syllabus rows from the canonical cache.
    ...
    cache_dir = Path("stedding/site_scrape_samples/biep/en/chemistry")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "en_chemistry_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="en_chemistry")
def en_chemistry_source(language: str | None = None):
    """DLT source for the England chemistry ingestion."""
    return en_chemistry(language=language)
