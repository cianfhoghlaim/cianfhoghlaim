...
def sct_biology(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Scotland biology syllabus rows from the canonical cache.
    ...
    cache_dir = Path("stedding/site_scrape_samples/biep/sct/biology")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "sct_biology_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="sct_biology")
def sct_biology_source(language: str | None = None):
    """DLT source for the Scotland biology ingestion."""
    return sct_biology(language=language)
