"""Per-subject DLT source for Greece (Physics, YPETh).

Per-subject DLT source for the EU nations full-depth expansion
(`2026-07-13-eu-nations-full-depth-expansion-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/grc/education/subjects/physics/<lang>/``.
...
def grc_physics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Greece Physics syllabus rows from the canonical cache.

    Honours ``USE_LOCAL_SCRAPES=true`` by reading cached Firecrawl
    snapshots under ``stedding/ingest_queue/european_nations/grc/education/subjects/physics/<lang>/``.
    """
    cache_dir = Path("stedding/ingest_queue/european_nations/grc/education/subjects/physics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "grc_physics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="grc_physics")
def grc_physics_source(language: str | None = None):
    """DLT source for the Greece Physics ingestion."""
    return grc_physics(language=language)
