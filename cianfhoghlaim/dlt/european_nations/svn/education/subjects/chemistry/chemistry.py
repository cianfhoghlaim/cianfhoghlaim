"""Per-subject DLT source for Slovenia (Chemistry, RIC).

Per-subject DLT source for the EU nations full-depth expansion
(`2026-07-13-eu-nations-full-depth-expansion-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/svn/education/subjects/chemistry/<lang>/``.
...
def svn_chemistry(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Slovenia Chemistry syllabus rows from the canonical cache.

    Honours ``USE_LOCAL_SCRAPES=true`` by reading cached Firecrawl
    snapshots under ``stedding/ingest_queue/european_nations/svn/education/subjects/chemistry/<lang>/``.
    """
    cache_dir = Path("stedding/ingest_queue/european_nations/svn/education/subjects/chemistry")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "svn_chemistry_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="svn_chemistry")
def svn_chemistry_source(language: str | None = None):
    """DLT source for the Slovenia Chemistry ingestion."""
    return svn_chemistry(language=language)
