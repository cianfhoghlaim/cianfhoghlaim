"""Per-subject DLT source for Romania (Chemistry, MEN).

Per-subject DLT source for the EU nations full-depth expansion
(`2026-07-13-eu-nations-full-depth-expansion-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/rou/education/subjects/chemistry/<lang>/``.
...
def rou_chemistry(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Romania Chemistry syllabus rows from the canonical cache.

    Honours ``USE_LOCAL_SCRAPES=true`` by reading cached Firecrawl
    snapshots under ``stedding/ingest_queue/european_nations/rou/education/subjects/chemistry/<lang>/``.
    """
    cache_dir = Path("stedding/ingest_queue/european_nations/rou/education/subjects/chemistry")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "rou_chemistry_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="rou_chemistry")
def rou_chemistry_source(language: str | None = None):
    """DLT source for the Romania Chemistry ingestion."""
    return rou_chemistry(language=language)
