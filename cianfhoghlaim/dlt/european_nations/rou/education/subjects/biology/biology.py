"""Per-subject DLT source for Romania (Biology, MEN).

Per-subject DLT source for the EU nations full-depth expansion
(`2026-07-13-eu-nations-full-depth-expansion-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/rou/education/subjects/biology/<lang>/``.
...
def rou_biology(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Romania Biology syllabus rows from the canonical cache.

    Honours ``USE_LOCAL_SCRAPES=true`` by reading cached Firecrawl
    snapshots under ``stedding/ingest_queue/european_nations/rou/education/subjects/biology/<lang>/``.
    """
    cache_dir = Path("stedding/ingest_queue/european_nations/rou/education/subjects/biology")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "rou_biology_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="rou_biology")
def rou_biology_source(language: str | None = None):
    """DLT source for the Romania Biology ingestion."""
    return rou_biology(language=language)
