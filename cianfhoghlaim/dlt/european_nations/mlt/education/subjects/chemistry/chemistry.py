"""Per-subject DLT source for Malta (Chemistry, MATSEC).

Per-subject DLT source for the EU nations full-depth expansion
(`2026-07-13-eu-nations-full-depth-expansion-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/mlt/education/subjects/chemistry/<lang>/``.
...
def mlt_chemistry(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Malta Chemistry syllabus rows from the canonical cache.

    Honours ``USE_LOCAL_SCRAPES=true`` by reading cached Firecrawl
    snapshots under ``stedding/ingest_queue/european_nations/mlt/education/subjects/chemistry/<lang>/``.
    """
    cache_dir = Path("stedding/ingest_queue/european_nations/mlt/education/subjects/chemistry")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "mlt_chemistry_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="mlt_chemistry")
def mlt_chemistry_source(language: str | None = None):
    """DLT source for the Malta Chemistry ingestion."""
    return mlt_chemistry(language=language)
