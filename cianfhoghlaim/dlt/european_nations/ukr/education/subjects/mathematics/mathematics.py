"""Per-subject DLT source for the Ukraine {Mathematics} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/ukr/education/subjects/mathematics/uk/sample.json``.
"""
...
def ukr_mathematics(language=None) -> Iterator[dict[str, Any]]:
    """Yield Ukraine Mathematics rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/ukr/education/subjects/mathematics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "ukr_mathematics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="ukr_mathematics")
def ukr_mathematics_source(language=None):
    """DLT source for the Ukraine Mathematics ingestion."""
    return ukr_mathematics(language=language)
