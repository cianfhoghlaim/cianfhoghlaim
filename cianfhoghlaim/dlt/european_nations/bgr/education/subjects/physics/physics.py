"""Per-subject DLT source for Bulgaria (Physics, MES).

Per-subject DLT source for the EU nations full-depth expansion
(`2026-07-13-eu-nations-full-depth-expansion-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/bgr/education/subjects/physics/<lang>/``.
...
def bgr_physics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Bulgaria Physics syllabus rows from the canonical cache.

    Honours ``USE_LOCAL_SCRAPES=true`` by reading cached Firecrawl
    snapshots under ``stedding/ingest_queue/european_nations/bgr/education/subjects/physics/<lang>/``.
    """
    cache_dir = Path("stedding/ingest_queue/european_nations/bgr/education/subjects/physics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "bgr_physics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="bgr_physics")
def bgr_physics_source(language: str | None = None):
    """DLT source for the Bulgaria Physics ingestion."""
    return bgr_physics(language=language)
