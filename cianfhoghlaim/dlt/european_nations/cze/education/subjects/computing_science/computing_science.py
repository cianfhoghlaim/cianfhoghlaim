"""Per-subject DLT source for Czechia (Computing / Computer Science, MŠMT).

Per-subject DLT source for the EU nations full-depth expansion
(`2026-07-13-eu-nations-full-depth-expansion-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/cze/education/subjects/computing_science/<lang>/``.
...
def cze_computing_science(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Czechia Computing / Computer Science syllabus rows from the canonical cache.

    Honours ``USE_LOCAL_SCRAPES=true`` by reading cached Firecrawl
    snapshots under ``stedding/ingest_queue/european_nations/cze/education/subjects/computing_science/<lang>/``.
    """
    cache_dir = Path("stedding/ingest_queue/european_nations/cze/education/subjects/computing_science")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "cze_computing_science_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="cze_computing_science")
def cze_computing_science_source(language: str | None = None):
    """DLT source for the Czechia Computing / Computer Science ingestion."""
    return cze_computing_science(language=language)
...
__all__ = [
    ...
    "SUBJECT",
    "SUPPORTED_LANGUAGES",
    "cze_computing_science",
    "cze_computing_science_source",
]
