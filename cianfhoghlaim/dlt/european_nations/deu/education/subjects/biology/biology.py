"""Per-subject DLT source for the Germany {Biology} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/deu/education/subjects/biology/de/sample.json``.
"""
...
def deu_biology(language=None) -> Iterator[dict[str, Any]]:
    """Yield Germany Biology rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/deu/education/subjects/biology")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "deu_biology_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="deu_biology")
def deu_biology_source(language=None):
    """DLT source for the Germany Biology ingestion."""
    return deu_biology(language=language)
