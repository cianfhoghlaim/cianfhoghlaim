"""Per-subject DLT source for the Poland {Mathematics} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/pol/education/subjects/mathematics/pl/sample.json``.
"""
...
def pol_mathematics(language=None) -> Iterator[dict[str, Any]]:
    """Yield Poland Mathematics rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/pol/education/subjects/mathematics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "pol_mathematics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="pol_mathematics")
def pol_mathematics_source(language=None):
    """DLT source for the Poland Mathematics ingestion."""
    return pol_mathematics(language=language)
