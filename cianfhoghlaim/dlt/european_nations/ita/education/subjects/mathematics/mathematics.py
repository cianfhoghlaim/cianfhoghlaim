"""Per-subject DLT source for the Italy {Mathematics} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/ita/education/subjects/mathematics/it/sample.json``.
"""
...
def ita_mathematics(language=None) -> Iterator[dict[str, Any]]:
    """Yield Italy Mathematics rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/ita/education/subjects/mathematics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "ita_mathematics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="ita_mathematics")
def ita_mathematics_source(language=None):
    """DLT source for the Italy Mathematics ingestion."""
    return ita_mathematics(language=language)
