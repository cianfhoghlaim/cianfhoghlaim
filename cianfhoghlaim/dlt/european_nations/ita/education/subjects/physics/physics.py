"""Per-subject DLT source for the Italy {Physics} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/ita/education/subjects/physics/it/sample.json``.
"""
...
def ita_physics(language=None) -> Iterator[dict[str, Any]]:
    """Yield Italy Physics rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/ita/education/subjects/physics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "ita_physics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="ita_physics")
def ita_physics_source(language=None):
    """DLT source for the Italy Physics ingestion."""
    return ita_physics(language=language)
