...
class ContentQualityAssessor:
    ...
    IRISH_WORDS = {
        "an", "na", "le", "ar", "ag", "i", "do", "de", "go", "sa", "is", "a",
        "ní", "sé", "sí", "iad", "mé", "tú", "sinn", "sibh", "atá", "bhí",
        "agus", "ach", "nó", "mar", "nuair", "má", "cé", "cad", "conas",
        "fhoghlaim", "oideachas", "scoil", "rang", "múinteoir", "dalta",
    }
    ...
    def assess(self, content: str, metadata: dict | None = None) -> QualityMetrics:
        """
    ...
    def _analyze_curriculum(self, content: str, metrics: QualityMetrics) -> None:
        ...
        metrics.has_strand_structure = any(
            kw in content_lower for kw in strand_keywords
        )
        ...
        metrics.has_assessment_criteria = any(
            kw in content_lower for kw in assessment_keywords
        )
    ...
    def _analyze_extraction_quality(
        self, content: str, metrics: QualityMetrics
    ) -> None:
    ...
    def batch_assess(
        ...
        return [
            self.assess(content, metadata)
            for content, metadata in zip(contents, metadata_list, strict=False)
        ]
    ...
    def get_summary(
        ...
        return {
            ...
            "avg_overall_score": sum(m.overall_score for m in metrics_list)
            / len(metrics_list),
            "contains_irish_count": sum(1 for m in metrics_list if m.contains_irish),
            "has_garbled_text_count": sum(
                1 for m in metrics_list if m.has_garbled_text
            ),
            "avg_text_quality": sum(m.text_quality_score for m in metrics_list)
            / len(metrics_list),
            "avg_structure_score": sum(m.structure_score for m in metrics_list)
            / len(metrics_list),
