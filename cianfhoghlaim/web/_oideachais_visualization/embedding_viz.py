...
class EmbeddingVisualizer:
    ...
    async def compute_projection(
        ...
        if self.method == "tsne":
            return await self._tsne_projection(
                embeddings, adjusted_perplexity, n_iter
            )
        else:
            return await self._umap_projection(
                embeddings, adjusted_n_neighbors, min_dist
            )
    ...
    async def generate_visualization(
        ...
        if actual_n_clusters:
            ...
            for c in range(actual_n_clusters):
                ...
                if cluster_docs:
                    subjects = [
                        self._get_doc_field(d, "subject") for d in cluster_docs
                    ]
    ...
    def _get_doc_field(self, doc: Any, field: str) -> str | None:
    ...
    async def export_for_plotly(
        ...
        subjects = list({p.subject for p in viz_data.points if p.subject})
        color_map = {
            ...
            "data": [
                {
                    ...
                    "marker": {
                        "color": [
                            color_map.get(p.subject, 0) for p in viz_data.points
                        ],
                        "colorscale": "Viridis",
                        "size": 8,
                    },
