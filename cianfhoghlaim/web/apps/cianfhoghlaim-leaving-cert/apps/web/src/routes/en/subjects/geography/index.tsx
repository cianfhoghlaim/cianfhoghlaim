// /en/subjects/geography — Geography per-subject interactive landing page.
//
...
// This is the production landing page for the Geography subject,
// rendered alongside (and superseding for interactive use) the flat
// `routes/en/subjects/geography.tsx` file.
//
...
export const Route = createFileRoute("/en/subjects/geography/")({
  component: GeographyPerSubjectLanding,
});

function GeographyPerSubjectLanding() {
  if (!isBIEPSubject("geography")) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <nav
        aria-label="Geography interactive sub-routes"
        className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 px-6"
      >
        <SubRouteCard
          to="/en/subjects/geography/syllabus"
          title="Syllabus"
          blurb="NCCA syllabus topics + learning outcomes"
        />
        <SubRouteCard
          to="/en/subjects/geography/exam-papers"
          title="Past exam papers"
          blurb="All Geography past exam papers tagged by topic + paper + year"
        />
        <SubRouteCard
          to="/en/subjects/geography/marking-schemes"
          title="Marking schemes"
          blurb="PCLM (Partial Credit, Logical Marking) patterns"
        />
        <SubRouteCard
          to="/en/subjects/geography/study-plan"
          title="Study plan"
          blurb="Generate a per-subject study plan via the BAML backend"
        />
      </nav>
      <div className="px-6 text-sm text-slate-500">
        <Link to="/ga/subjects/tireolaiocht">Gaeilge: Tíreolaíocht →</Link>
      </div>
    </div>
  );
}
