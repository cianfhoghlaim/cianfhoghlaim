...
`pdf_downloader` DLT source) plus an S3 inspector for the Garage bucket that
backs DuckLake. Use this to spot stuck downloads, error storms, or
oversized files.
...
Run:
    cd oideachais && uv run marimo edit notebooks/pdf_download_dashboard.py
"""
...
def _():
    ...
    mo.md(
        ...
        Live view of the `pdf_downloads` and `pdf_extracted_text` tables.

        The DLT source at `dlt_sources/ireland/pdf_downloader.py` queries
        `curriculum.curriculum_pdfs` and `examinations.all_exam_materials` for
        pending URLs, HEAD-requests each one, and writes the result back with
        a status (`downloaded`, `skipped_too_large`, `skipped_not_pdf`,
        `error_timeout`, `error_http_XXX`, `error_unknown`).
        """
    )
