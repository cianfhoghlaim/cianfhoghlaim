...
Postgres endpoint) for the underlying queries. When the MotherDuck
endpoint is unreachable, the notebook falls back to the local DuckDB
...
def _():
    ...
    # Try MotherDuck first, fall back to local DuckDB. The marimo
    # `mo.sql` engine accepts any DBAPI 2.0 connection.
    try:
        import duckdb
        ...
        if os.path.exists(db_path):
            ...
            # MotherDuck is the prod target; we don't have a token
            # in CI, so render an empty data context.
            ENGINE_LABEL = "MotherDuck (no token)"
            engine = None
    except ImportError:
        engine = None
...
def _(engine, mo, pd):
    ...
    if engine is None:
        return mo.md("*No data backend available.*"), mo.ui.text(placeholder="search…"), pd.DataFrame()
    try:
        df = engine.execute(
            """
            SELECT
        ...
        return mo.md(f"*Query failed: {exc}*"), mo.ui.text(placeholder="search…"), pd.DataFrame()
...
def _(engine, mo, pd):
    ...
    if engine is None:
        return mo.md("*No data backend available.*"), mo.ui.radio(options=["module", "isbn"]), pd.DataFrame()
    ...
    try:
        # The reading list lives in the `recommended_reading` JSON
        # column of the `university_modules` table. We unnest it
        # via DuckDB's `json_extract` + `unnest`.
        df = engine.execute(
            """
            SELECT
                m.module_code,
    ...
    # Parse the JSON array; in a real notebook this would be done
    # via `unnest(json_extract(...))` on DuckDB. The stub here shows
    # the structure: 1 row per (module_code, book_title) pair.
    ...
    if group_by.value == "isbn":
        # In a full implementation, group by `isbn_13` and count
        # modules per book. Here we just re-label the column.
...
def _(engine, mo, pd):
    ...
    try:
        # We can't query FalkorDB directly from the notebook (no
        # FalkorDB client here), so we surface the local UoG
        # artefacts and the scraped course descriptors in two
        # adjacent tables, and let the user visually confirm the
        # joins.
        uog = engine.execute(
            """
