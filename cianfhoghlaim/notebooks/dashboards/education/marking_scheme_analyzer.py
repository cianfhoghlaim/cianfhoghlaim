...
Aggregate SEC marking scheme patterns by subject. Surfaces per-subject rubrics
(PCLM for English, SRPs for Geography/History, equation steps for Mathematics)
and exposes the same query-engine toggle as the rest of the dashboard suite.
...
Run:
    cd oideachais && uv run marimo edit notebooks/marking_scheme_analyzer.py
"""
...
def _():
    ...
    mo.md(
        ...
        The notebook is dual-engine: `MOTHERDUCK_ENABLED=true` swaps the DuckDB
        fallback for a shared MotherDuck database. Marking-scheme text is sourced
        from `curriculum.pdf_extracted_text` (populated by the `pdf_processing`
    ...
    return alt, duckdb, mo, os, pathlib, pd, sys


@app.cell
def _(mo, os, pathlib):
    MOTHERDUCK_ENABLED = os.getenv("MOTHERDUCK_ENABLED", "false").lower() == "true"
    DUCKDB_PATH = str(pathlib.Path(os.getcwd()) / "oideachais/data_platform/.dlt/curriculum_unified/curriculum.duckdb")
