...
IRISH_DIALECTS = {
    ...
    "ulster": {
        "counties": ["donegal", "derry", "antrim", "armagh", "tyrone", "fermanagh", "down", "monaghan", "cavan"],
        "features": ["scottish_influence", "palatalization"],
    },
    "standard": {
        "counties": ["dublin", "meath", "kildare", "wicklow", "louth", "westmeath", "offaly", "laois", "carlow", "kilkenny", "wexford", "longford"],
...
class IrishOCRFederatedServer:
    ...
    def _run_simulation(self):
        ...
        for round_num in range(1, self.config.num_rounds + 1):
            ...
            # Aggregate
            self.state.global_parameters = self.strategy.aggregate_fit(
                self.state, client_updates
            )
    ...
    def _create_flower_client(self):
        ...
        class IrishOCRClient(fl.client.NumPyClient):
            ...
            def fit(self, parameters, config):
                ...
                return self.parent._get_model_parameters(), len(self.parent._get_local_data()), metrics
    ...
    def _train_local(self) -> dict[str, float]:
        ...
        if self.local_model:
            for i, param in enumerate(self.local_model):
                self.local_model[i] = param + np.random.randn(*param.shape).astype(np.float32) * 0.001
...
def create_dialect_specialized_clients(
    ...
    for dialect, _info in IRISH_DIALECTS.items():
