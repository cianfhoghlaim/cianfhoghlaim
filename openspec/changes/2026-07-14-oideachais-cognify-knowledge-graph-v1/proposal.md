...
- The cross-archive graph query API route lives at
  `cianfhoghlaim/agents/api/_oideachais_api/routes/cross_archive_graph.py`.
...
|:--|:--|--:|
| `cianfhoghlaim/storage/cognify/cognee_integration/aistear_cognify.py` | NEW (Stage 1) | +~150 |
| `cianfhoghlaim/storage/cognify/cognee_integration/primary_cognify.py` | NEW (Stage 2) | +~166 |
| `cianfhoghlaim/storage/cognify/cognee_integration/junior_cycle_cognify.py` | NEW (Stage 3) | +~160 |
| `cianfhoghlaim/storage/cognify/cognee_integration/senior_cycle_cognify.py` | NEW (Stage 4) | +~162 |
| `cianfhoghlaim/storage/cognify/cognee_integration/university_cognify.py` | NEW (Stage 5) | +~176 |
| `cianfhoghlaim/storage/cognify/rules/leabharlann_official_media.py` | NEW (leabharlann-1) | +~171 |
| `cianfhoghlaim/storage/cognify/rules/leabharlann_authors_archive.py` | NEW (leabharlann-2) | +~163 |
| `cianfhoghlaim/storage/cognify/rules/leabharlann_culture_heritage.py` | NEW (leabharlann-3) | +~168 |
| `cianfhoghlaim/storage/cognify/rules/cross_archive_biep_edges.py` | NEW (3 BIEP cross-archive edges) | +~496 |
| `cianfhoghlaim/notebooks/10_cognify/01_knowledge_graph.py` | NEW (cognify visualisation) | +~429 |
| `cianfhoghlaim/notebooks/cli.py` | MODIFIED (add `10_cognify` to GROUPS) | +~7 |
...
- The 7 existing cognify adapters at
  `cianfhoghlaim/storage/cognify/cognee_integration/` are NOT
...
- The 4 existing cross-archive rules at
  `cianfhoghlaim/storage/cognify/rules/` are NOT modified.
...
- `cianfhoghlaim/storage/cognify/cognee_integration/*_cognify.py` (5 NEW files)
- `cianfhoghlaim/storage/cognify/rules/leabharlann_*.py` (3 NEW files)
- `cianfhoghlaim/storage/cognify/rules/cross_archive_biep_edges.py` (1 NEW file)
- `cianfhoghlaim/notebooks/10_cognify/01_knowledge_graph.py` (1 NEW file)
- `cianfhoghlaim/notebooks/cli.py` (MODIFIED, +7 LOC)
