...
### Requirement: 5 per-stage cognify adapters SHALL be wired into Dagster

Each of the 5 stage adapters (`aistear`, `primary`, `junior_cycle`, `senior_cycle`, `university`) SHALL have a `defs.yaml` registration at `orchestration/defs/3_model_lifecycle/cognify/<stage>_cognify/defs.yaml` that materializes a Dagster asset invoking the adapter's `cognee.add()` + `cognee.cognify()` calls.

**WHEN** the daily cron `0 3 * * *` fires
**THEN** all 5 stage adapters SHALL materialize in parallel
**AND** each adapter SHALL emit a `cognee_<stage>_edges_check` asset check requiring `edges_count >= 1`

#### Scenario: Aistear cognify materializes 12 guiding principles + 4 themes

- **WHEN** the daily cron fires and `cognee.add()` is called with the 12 Aistear guiding principles
- **THEN** the `cognee.cognify()` call creates 4 Theme + 12 Principle + 12 `BELONGS_TO` edges + 12 `ALIGNS_WITH` edges + 23 `SUPPORTED_BY` LearningExperience + 23 `TARGETS` ChildOutcome edges
- **AND** the `cognee_aistear_edges_check` passes (edges_count >= 1)

#### Scenario: Primary cognify materializes 2,400 LOs across 8 class stages

- **WHEN** the daily cron fires with the 300 Primary LOs × 8 class stages
- **THEN** the cognify creates 8 Strands × 6 CurricularAreas + 300 LOs + 2400 stage-edge + 300 `DEVELOPS` KeyCompetency edges
- **AND** the `cognee_primary_edges_check` passes
...
- **THEN** it queries the Cognee `british_isles_equivalences` dataset for the matching `(lc, ireland, chemistry) ↔ (gcse, england, chemistry)` edge
