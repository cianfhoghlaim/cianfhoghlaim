...
- [ ] 0.1 Archive `2026-08-01-biep-v3-dlt-jurisdiction-pipeline-bugfix-v1` by adding `LAKEHOUSE_DUCKDB = "md:cianfhoghlaim"` to `dlt_sources/common/destinations_cianfhoghlaim.py`
- [ ] 0.2 Fix the broken `from ..storage.ducklake_client import DuckLakeClient` import at `orchestration/resources.py:300` — relocate `DuckLakeClient` from `storage_pkg_temp/ducklake_client.py` into a new `storage/ducklake_client.py` and re-export
- [ ] 0.3 Add `IcebergCatalogResource` (PyIceberg 0.11.1 `load_catalog("kcg", type="rest", uri="http://lakehouse-lakekeeper:8181")`) to `orchestration/resources.py:303`
- [ ] 0.4 Add `LanceNamespaceResource` (Lance REST namespace `rest://lakehouse-lance-namespace:8182`) to `orchestration/resources.py`
- [ ] 0.5 Fix `LanceDBResource.embedding_model = "BAAI/bge-m3"` (1024-d) and `embedding_dim = 1024` — currently wrong (MiniLM 384-d)
- [ ] 0.6 Add canonical 3 BAML clients to `baml_src/clients_biep_v3.py`: `BIEPV3Extract` (minimax-m3), `BIEPV3ExtractStrong` (qwen3-vl-8b-it), `BIEPV3Vision` (qwen3-vl-8b-it-via-llama-swap)
- [ ] 0.7 Add the missing `ExtractUKQualSpec(board: ExamBoard, ...)` generic BAML function to `baml_src/british_isles/england/education/curriculum_syllabus.baml`
- [ ] 0.8 Add the missing `ExtractSyllabusDiagram(pdf_text) -> SyllabusDiagram` BAML function to `baml_src/british_isles/ireland/education/lc_extraction/syllabus_diagram.baml`
- [ ] 0.9 Add the missing `ExtractCrossLinguisticConcept(pdf_text) -> CrossLinguisticConcept` BAML function to `baml_src/british_isles/ireland/education/lc_extraction/cross_linguistic.baml`
- [ ] 0.10 Bring the lakehouse stack up: `docker compose -f bonneagar/stacks/lakehouse/compose.yaml up -d` (13 services)
- [ ] 0.11 Seed the British Isles Subject Registry to ≥210 cohorts via `seed_registry()` (extend `load_ireland_subjects()` to 152 rows + `load_england_subjects()` to 276 rows)
- [ ] 0.12 Standardise the snake_case file naming contract in `dlt_sources/common/snake_case_contract.py` (new)
- [ ] 0.13 Add the `cianhoghlaim` Lance namespace to the Lakekeeper catalog via `scripts/create_lance_namespace.py` (new)
- [ ] 0.14 Add the 6 new `mise run biep:v3:*` task aliases to `mise.toml` under `[tasks.biep.v3]`
- [ ] 0.15 Add `scripts/seed_registry.py` (new) — invoked by `mise run registry:seed`
- [ ] 0.16 Add `scripts/smoke_test_lakehouse.sh` (new) — invoked by `mise run lakehouse:smoke-test`
- [ ] 0.17 Add `scripts/create_lance_namespace.py` (new)
- [ ] 0.18 Add `scripts/m0_foundation_unblock.ts` (new) — the M0 entrypoint
- [ ] 0.19 Add per-milestone acceptance-check scripts `scripts/m{1,2,3,4}_<name>.ts` (new)
- [ ] 0.20 Add `scripts/milestone_gate.ts` (new) — invoked by `mise run biep:v3:gate`

### Dagster assets

- [ ] 0.A1 Add `lakehouse_smoke_test` asset (Layer 5 Agent Ops) — checks 13 services 200 OK
- [ ] 0.A2 Add `baml_codegen_gate` asset (Layer 5 Agent Ops) — `mise run baml:generate` exit 0
- [ ] 0.A3 Add `registry_seed_count` asset (Layer 5 Agent Ops) — ≥210 rows in `cianfhoghlaim.education._registry.subjects`
- [ ] 0.A4 Add `lance_namespace_ready` asset (Layer 5 Agent Ops) — Lance namespace `cianhoghlaim` exists in Lakekeeper

### Asset checks

- [ ] 0.C1 Add `lakehouse_smoke_test_check` — 13 services respond 200 OK
- [ ] 0.C2 Add `baml_codegen_check` — `baml-cli generate` exits 0
- [ ] 0.C3 Add `registry_seed_check` — `SELECT COUNT(*) FROM cianfhoghlaim.education._registry.subjects` returns ≥210
- [ ] 0.C4 Add `lance_namespace_check` — namespace `cianhoghlaim` exists in Lakekeeper
...
- [ ] 1.1 Replace stub `ireland_documents_ingested` with real Firecrawl + filesystem code in `orchestration/defs/2_materials/ireland_education/generic_ireland_assets.py:71-110`
- [ ] 1.2 Replace stub `ireland_extractions` with `EnsembledExtractor.extract()` calls in `orchestration/defs/2_materials/ireland_education/generic_ireland_assets.py:113-160`
- [ ] 1.3 Replace stub `ireland_embeddings` with `cocoindex.subjects.education_subject_embedding.app_main()` in `orchestration/defs/2_materials/ireland_education/generic_ireland_assets.py:163-180`
- [ ] 1.4 Add per-subject backfill jobs (`ireland_lc_<subject>_backfill_job`) in `orchestration/defs/2_materials/ireland_education/backfill_jobs/` (new)
- [ ] 1.5 Add daily automation `AutomationCondition.cron("@daily")` on `ireland_lc_documents_ingested` in `orchestration/automation/biiep_daily_automation.py`
- [ ] 1.6 Add 6 per-subject CocoIndex v1 Apps (`ireland_lc_<subject>_<level>_<lang>_embedding`) in `cocoindex/biep_parity/` (new)
- [ ] 1.7 Add `ga_education_embedding.py` CocoIndex app (currently missing) in `cocoindex/biep_parity/ga_education_embedding.py` (new)
- [ ] 1.8 Add `ireland_education_embedding.py` CocoIndex app (currently missing) in `cocoindex/biep_parity/ireland_education_embedding.py` (new)
- [ ] 1.9 Materialise marimo notebook `notebooks/19_ireland_pipeline_dashboard.py` with per-cohort matrix
- [ ] 1.10 Add MotherDuck Dive `ireland_lc_syllabus_topics` in `motherduck/dives/ireland_lc_syllabus_topics.sql` (new)
- [ ] 1.11 Add MotherDuck Flight `ireland_lc_daily_sync_flight` (cron 02:00 UTC) in `motherduck/flights/ireland_lc_daily_sync_flight.sql` (new)
- [ ] 1.12 Add `ireland_lc_documents_ingested` asset check (cohort count >= 12)
- [ ] 1.13 Add `ireland_lc_extractions_ragas` asset check (score >= 0.70)
- [ ] 1.14 Add `ireland_lc_lance_chunks` asset check (chunk count >= 12_000)
- [ ] 1.15 Add `ireland_lc_audit` DuckLake table (1 audit row per cohort)
- [ ] 1.16 Add `scripts/m1_ireland_lc.ts` (new) — the M1 entrypoint
- [ ] 1.17 Replace stub `jc_pdf_sync_flight` reference (line 122) with `md:cianfhoghlaim` (per `2026-08-12-biep-v3-motherduck-flights-v1` fix)
- [ ] 1.18 Add `ireland_lc` group to `orchestration/automation/biiep_daily_automation.py` (cron 02:00 UTC)
