...
#### Scenario: Lakehouse-to-meaisinfhoghlaim ingest routes through `cianfhoghlaim.orchestration`
...
- **THEN** the consumer MUST import via `from cianfhoghlaim.orchestration.definitions import defs` (the v4 path)
- **AND** the legacy `from cianfhoghlaim.dagster_defs.definitions import defs` path MUST raise `ModuleNotFoundError`
...
1. **Canonical Python import paths** in scenarios SHALL use the v4 form: `from <quadrant>.<module> import <symbol>` (e.g. `from cianfhoghlaim.core.utils import CircuitBreaker`, `from cianfhoghlaim.tools.curriculum_search import compare_curricula`) — NOT `from sruth.cianfhoghlaim.<module> import <symbol>`. The `sruth.cianfhoghlaim.*` and `sruth.meaisinfhoghlaim.*` namespaces no longer exist post-v4.
...
4. **Stale-subpath refs** SHALL be renamed to their v4 equivalents: `cianfhoghlaim.dagster_defs.X` → `cianfhoghlaim.orchestration.defs.X`.
...
- **AND** the contributor SHALL use `cianfhoghlaim.orchestration.X` for Dagster references (NOT `cianfhoghlaim.dagster_defs.X`)
...
- **AND** `cianfhoghlaim.dagster_defs` count is 0 (all renamed to `cianfhoghlaim.orchestration` per line 111 + the ADDED Requirement)
