...
The third small change bundled here is **wire `b.ExtractZoteroMetadata` into `oideachais/dlt_sources/author_archive/zotero.py`** (item #11 in `oideachais/REFACTORING.md`) so the new BAML function is actually invoked from a dlt source.

## What Changes

### 1. Ireland Primary dlt source

`oideachais/dlt_sources/ireland/primary.py` — new `@dlt.source name="ireland_primary"`. Yields 4 resources (per the BAML schema classes):
...
`oideachais/dlt_sources/ireland/junior_cycle.py` — new `@dlt.source name="ireland_junior_cycle"`. Yields 3 resources:
...
`oideachais/dagster_defs/assets/ireland_curriculum_assets.py` (extend existing) — 2 new `@asset`s:
...
Both register in `oideachais/dagster_defs/definitions.py`.

### 4. Wire `b.ExtractZoteroMetadata` into `zotero.py`

`oideachais/dlt_sources/author_archive/zotero.py` — add a 4th resource `arxiv_papers_baml` that invokes `b.ExtractZoteroMetadata(pdf_text, file_name, arxiv_id)` for each arXiv paper (and falls back to a stub when the BAML client is not generated). The `leabharlann_paper_metadata` Dagster asset then invokes this resource and writes the structured `ZoteroPaper` rows.

This closes `oideachais/REFACTORING.md` item #11.

### 5. Leabharlann full-stack demo asset + Marimo notebook

`oideachais/dagster_defs/assets/leabharlann_demo_assets.py` — 1 new `@asset`:
...
`oideachais/notebooks/leabharlann_full_stack_demo.py` (new Marimo notebook) — interactive UI rendering the 5-step pipeline: text extraction preview, BAML extracted fields, top-5 similar chunks from LanceDB, status panel.

Mounted at the existing `/dashboards/leabharlann` route via the new Marimo app definition in `oideachais/marimo/`.

## Impact

| Surface | Before | After |
|:--|:--|:--|
| `oideachais/dlt_sources/ireland/primary.py` | (absent) | New — 4 dlt resources |
| `oideachais/dlt_sources/ireland/junior_cycle.py` | (absent) | New — 3 dlt resources |
| `oideachais/dlt_sources/author_archive/zotero.py` | 3 resources, no BAML | 4 resources, BAML invoked (when client generated) |
| `oideachais/dagster_defs/assets/ireland_curriculum_assets.py` | (existing senior_cycle assets) | + 2 new (primary + JC) |
| `oideachais/dagster_defs/assets/leabharlann_demo_assets.py` | (absent) | New — 1 asset + 4 asset checks |
| `oideachais/notebooks/leabharlann_full_stack_demo.py` | (absent) | New Marimo notebook |
...
- Features 2, 3, 5 from `oideachais/REFACTORING.md` (Cognee+FalkorDB, LanceDB blob storage, public web exposure) — separate openspec changes.
...
- `oideachais/REFACTORING.md` — refactor backlog (Features 1 + 4 are addressed here).
- `oideachais/STATUS.md` § 1, § 2 — the BAML × dlt × Dagster matrix and per-nation × per-cycle coverage matrix that this change updates.
- `baml_src/{primary,junior_cycle,author_archive}.baml` — the BAML schemas this change wires up.
- `oideachais/dlt_sources/ireland/{aistear,senior_cycle,leaving_cert}.py` — the existing dlt source pattern that the new `primary.py` and `junior_cycle.py` mirror.
- `oideachais/dagster_defs/assets/ie/education/curriculum_dlt_assets.py` — the existing 70+ @dlt_assets pattern that the new `ireland_curriculum_assets.py` extends.
