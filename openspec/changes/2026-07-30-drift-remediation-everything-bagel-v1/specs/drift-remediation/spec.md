...
> **The cross-cutting hygiene contract that catches the 4 drift categories the `repo-hygiene-agent-routing-and-sync-wiring-v1` change initially missed. Establishes the 4 `boundary` rules + the `restore_on_truncate` invariant + the `direct_python_path` mise-task convention.**
...
- `openspec/changes/archive/2026-07-30-2026-07-29-repo-hygiene-agent-routing-and-sync-wiring-v1/`
  (the Wave 9 commit that this change extends)
- `openspec/changes/2026-08-15-baml-sync-loop-v1/` (the sibling change that
  accidentally truncated `sync_assets.py` in commit `91b85c1c1`)
- `openspec/specs/knowledge-sync-loop/spec.md` (the parent spec whose "Daily sync_health cron"
  requirement is restored by Section A)
- `openspec/specs/centralized-model-registry/spec.md` (the parent spec whose
  audit requirement is extended by Section C)
- `scripts/registry_audit.py` (the audit-pattern whose gap is closed by Section C)
- `scripts/sync/ast_walk.py` (the Layer 6 AST scan that validates Section B)
