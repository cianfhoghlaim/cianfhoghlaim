...
The `author-archive-pipeline` spec is the canonical ingest path for the
160 British-Isles official_media sources, the 226 Gemini Deep Research
PDFs, and the 1,938 University of Galway coursework files. It is the
v1 replacement for the Instagram-export-first `official-media-pipeline`
change (archived 2026-06-18).

## Purpose

- Run a 4-stage ingest (pre-research → bulk scrape → condense → UI
  identification) on every official_media source
- Record the source provenance in a way the marimo dashboard can render
  (URL → sitemap → recommended strategy → backend used → bytes in/out
  → credit cost)
- Condense raw markdown to a 1-2 KB `CondensedPage` with key facts and
  entities (the user said: "we want to know what data we have and how
  it was sourced")
- Identify any UI element on a source page and store its bounding box
  for later game rendering (the user said: "for later game aspects")
- Use the 20,000-credit Firecrawl budget to maximum effect (95% cost
  saving vs naive full-scrape)
