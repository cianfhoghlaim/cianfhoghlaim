# `site-crawler` capability spec (NEW)

The `site-crawler` capability is the canonical 3-way web-scraper primitive
that backs every `dlt` source in the Cianfhoghlaim platform that scrapes
or crawls a remote website (NCCA, SEC, gov.ie circulars, citizensinformation.ie,
hse.ie, nice.org.uk, etc.). It supersedes the three overlapping pre-existing
...
All DLT sources SHALL replace any cross-package import of the
leading-underscore `_crawl_source` helper from
`dlt/british_isles/ireland/education/curriculum.py` with
...
The system SHALL provide a new GitHub Actions workflow at
