...
1. **`baml_src/` duplication**: 13 files at root + 20 files in `oideachais/baml_src/` + 5 in `tuatha/baml_src/` + 9 in `croilar/baml_src/` + 6 in `crypteolas/baml_src/` + a duplicate `croilar/baml/` directory (byte-for-byte identical to `croilar/baml_src/`) — three sets of duplicate BAML files need merging.
2. **`dg.toml` staleness**: the root `dg.toml` uses the deprecated `[[workspace.projects]]` syntax, references a now-moved `tuatha/apps/crypteolas_demo/` path, and is missing a per-sruth `dg.toml` for `oideachais` (the largest sruth). One pre-sruth reference names `meaisin_heartbeat` instead of `meaisinfhoghlaim`.
...
| **oideachais** | `oideachais/` | `sruth/oideachais/` |
| **meaisinfhoghlaim** | `meaisinfhoghlaim/` | `sruth/meaisinfhoghlaim/` |
| **tuatha** | `tuatha/` | `sruth/tuatha/` |
| **crypteolas** | `crypteolas/` | `sruth/crypteolas/` |
| **croilar** | `croilar/` | `sruth/croilar/` |
...
- **`croilar/baml/` is DELETED** (9 byte-for-byte duplicates of `croilar/baml_src/`).
...
- Update stale root `dg.toml` entry: `path = "tuatha/apps/crypteolas_demo"` → `path = "sruth/crypteolas/apps/crypteolas_demo"` (the `apps/crypteolas_demo/` directory now lives under the moved `sruth/crypteolas/`).
...
| `refactor: move tuatha, crypteolas, croilar to sruth/` | tuatha + crypteolas + croilar | 3 sruth in one commit (Q4 acknowledged); deletes `croilar/baml/`; updates `tuatha/apps/crypteolas_demo` → `sruth/crypteolas/apps/crypteolas_demo` |
...
- `infrastructure/stacks/oideachais/compose.yaml` — bind-mount paths `../../../../oideachais/...` → `../../../../sruth/oideachais/...`; `working_dir` and `dockerfile` paths
- `infrastructure/stacks/agent-os/compose.yaml` — `context: ../../../oideachais` → `../../../sruth/oideachais`; `context: ../../../tuatha/crypteolas` → `../../../sruth/crypteolas`
- `infrastructure/stacks/frontend/compose.yaml` — `context: ../../../../oideachais/web/apps/{web,api}` → `../../../../sruth/oideachais/web/apps/{web,api}`; `tuatha-ui`, `croilar-web`, `croilar-portal` paths
- `infrastructure/stacks/frontend/scripts/dev-start.sh` — `cd "$ROOT/tuatha"` → `cd "$ROOT/sruth/tuatha"`; `cd "$ROOT/croilar/apps/web"` → `cd "$ROOT/sruth/croilar/apps/web"`
...
- Dagster workspace loads 5 code-locations instead of 6 (the 3 separate paths for `tuatha`, `tuatha/crypteolas`, `tuatha/apps/crypteolas_demo` collapse into `sruth/tuatha` + `sruth/crypteolas` + `sruth/crypteolas/apps/crypteolas_demo`)
- All 9 croilar BAML files resolve correctly via `croilar/baml_src/` (the duplicate `croilar/baml/` is deleted; only `croilar/baml_src/` remains)
- New `sruth/meaisinfhoghlaim/baml_src/` directory is the canonical home for 14 BAML files (1 from root + 5 from oideachais + 4 in gaois/ subdir + 4 new schemas found in oideachais/baml_src per Q5 audit)

### Documentation change

- Root `README.md` lines 55, 69, 137–138, 287–303: subagent roster corrected to match `opencode.json` (5 sruth specialists: `oideachais/infrastructure/meaisinfhoghlaim/croilar/tuatha`, NOT 5 generic roles)
...
- 6 commits land on `origin/q3-2026-oideachais-consolidation` in order (codeolas → oideachais → meaisinfhoghlaim → tuatha/crypteolas/croilar → root baml_src deletion → README/skill updates)
...
- Root `dg.toml` references `sruth/crypteolas/apps/crypteolas_demo` (NOT the stale `tuatha/apps/crypteolas_demo`)
- Root `dg.toml` references `meaisinfhoghlaim` (NOT `meaisin_heartbeat`)
- `ls croilar/baml/` returns "No such file or directory" (duplicate deleted)
...
- `.agents/skills/monorepo/SKILL.md` — already says "Every `sruth/` frontend (`oideachais/web`)"
- `openspec/specs/oideachais-pipeline/spec.md` — references `oideachais/data_platform/dagster_defs/` (will be updated to `sruth/oideachais/`)
