...
- [ ] A.2 — Confirm `oideachais/baml_src/` has 20+ .baml files + `gaois/` subdir + `_archive/` subdir; record sha256s.
- [ ] A.3 — Confirm `tuatha/baml_src/`, `croilar/baml_src/`, `crypteolas/baml_src/`, `tuatha/crypteolas/` (no baml_src expected there).
- [ ] A.4 — Confirm `croilar/baml/` is EXACT duplicate of `croilar/baml_src/` (`diff -r croilar/baml croilar/baml_src` returns nothing).
- [ ] A.5 — Confirm 3 BAML conflicts: root `clients.baml`, `curriculum_extraction.baml`, `official_media.baml` have parallel copies in `oideachais/baml_src/`.
...
- [ ] A.10 — Confirm `oideachais/dg.toml` is MISSING (will CREATE).
...
- [ ] A.14 — Confirm `tuatha/crypteolas/` exists at `tuatha/crypteolas/` AND that root `crypteolas/` exists (Q3 acknowledged both will move separately).
...
- [ ] B.3 — Update `pyproject.toml` root `[tool.uv.workspace] members`: remove `tuatha/codeolas`, add `sruth/codeolas`
...
- [ ] B.6 — Update `infrastructure/stacks/croilar/Dockerfile.dagster` if it references `codeolas/` (it does — 2 `COPY` lines).
- [ ] B.7 — Update `croilar/pyproject.toml` if it depends on `codeolas` (it does — change the path).
...
## Phase C — Move `oideachais/` to `sruth/oideachais/` (commit 2 — HIGHEST RISK)
...
- [ ] C.6 — Update `package.json` workspaces: `oideachais/web` → `sruth/oideachais/web`, `oideachais/mcp/filesystem` → `sruth/oideachais/mcp/filesystem`.
- [ ] C.7 — Update `infrastructure/stacks/oideachais/compose.yaml`: 7 bind-mount paths from `../../../../oideachais/...` → `../../../../sruth/oideachais/...`.
- [ ] C.8 — Update `infrastructure/stacks/agent-os/compose.yaml`: `context: ../../../oideachais` → `../../../sruth/oideachais`.
- [ ] C.9 — Update `infrastructure/stacks/frontend/compose.yaml`: `context: ../../../../oideachais/web/apps/{web,api}` → `../../../../sruth/oideachais/web/apps/{web,api}`.
- [ ] C.10 — Update `infrastructure/stacks/frontend/scripts/dev-start.sh`: `cd "$ROOT/oideachais/web"` → `cd "$ROOT/sruth/oideachais/web"`.
...
- [ ] C.19 — Update `sruth/oideachais/AGENTS.md` (was `oideachais/AGENTS.md`).
- [ ] C.20 — Update `sruth/oideachais/README.md` (was `oideachais/README.md`).
- [ ] C.21 — Update `sruth/oideachais/STATUS.md` — only the header examples/paths change.
- [ ] C.22 — Update `openspec/specs/oideachais-*/spec.md` (12 specs) — replace `oideachais/X` with `sruth/oideachais/X` in the prose examples.
...
## Phase D — Move `meaisinfhoghlaim/` to `sruth/meaisinfhoghlaim/` (commit 3)
...
- [ ] D.6 — Update `sruth/meaisinfhoghlaim/dg.toml` (was `meaisinfhoghlaim/dg.toml`):
...
- [ ] D.12 — Update `sruth/meaisinfhoghlaim/AGENTS.md` (was `meaisinfhoghlaim/AGENTS.md`).
- [ ] D.13 — Update `sruth/meaisinfhoghlaim/README.md` (was `meaisinfhoghlaim/README.md`).
...
## Phase E — Move `tuatha/`, `crypteolas/`, `croilar/` to `sruth/` (commit 4)

- [ ] E.1 — `git mv tuatha sruth/tuatha`.
- [ ] E.2 — `git mv crypteolas sruth/crypteolas`.
- [ ] E.3 — `git mv croilar sruth/croilar`.
- [ ] E.4 — **Update stale dg.toml path**: root `dg.toml` entry `path = "tuatha/apps/crypteolas_demo"` → `path = "sruth/crypteolas/apps/crypteolas_demo"`.
- [ ] E.5 — Update root `dg.toml` to use `[[workspace.locations]]` (modernize).
- [ ] E.6 — Remove `tuatha/codeolas` from `[tool.uv.workspace] members` (already moved in Phase B; but verify no stale ref).
- [ ] E.7 — Remove `tuatha/crypteolas` from `[tool.uv.workspace] members`.
- [ ] E.8 — Remove `tuatha/apps/crypteolas_demo` from `[tool.uv.workspace] members`.
- [ ] E.9 — Add `sruth/tuatha`, `sruth/crypteolas`, `sruth/crypteolas/apps/crypteolas_demo`, `sruth/croilar` to `[tool.uv.workspace] members`.
- [ ] E.10 — Update `package.json` workspaces: `tuatha/ui` → `sruth/tuatha/ui`, `tuatha/apps/crypteolas_demo` → `sruth/crypteolas/apps/crypteolas_demo`, `croilar/apps/{web,portal}` → `sruth/croilar/apps/{web,portal}`, `croilar/hono-api` → `sruth/croilar/hono-api`.
- [ ] E.11 — Update `turbo.json` `croilar:export:wasm` output: `croilar/apps/portal/public/wasm/**` → `sruth/croilar/apps/portal/public/wasm/**`.
- [ ] E.12 — Update `mise.toml` `cd tuatha` → `cd sruth/tuatha`, `cd croilar` → `cd sruth/croilar`, `cd crypteolas` → `cd sruth/crypteolas`.
- [ ] E.13 — **Delete `croilar/baml/`** (9 byte-for-byte duplicates of `sruth/croilar/baml_src/`): `git rm -r sruth/croilar/baml`.
...
- [ ] E.23 — Update `infrastructure/stacks/agent-os/compose.yaml`: `context: ../../../tuatha/crypteolas` → `../../../sruth/crypteolas`.
- [ ] E.24 — Update `infrastructure/stacks/frontend/compose.yaml`: `tuatha-ui` context path.
- [ ] E.25 — Update `infrastructure/stacks/croilar/compose.yaml` (5 self-contained paths but the 5 compose-yaml files in `infrastructure/stacks/croilar/` may reference `../../croilar/...`; verify).
- [ ] E.26 — Update `infrastructure/stacks/tuatha/compose.dev.yaml` (3 paths).
- [ ] E.27 — Update `infrastructure/stacks/frontend/scripts/dev-start.sh`: `cd "$ROOT/tuatha"` → `cd "$ROOT/sruth/tuatha"`; `cd "$ROOT/croilar/apps/web"` → `cd "$ROOT/sruth/croilar/apps/web"`.
- [ ] E.28 — Update `sruth/tuatha/AGENTS.md`, `sruth/croilar/AGENTS.md` (was `tuatha/AGENTS.md`, `croilar/AGENTS.md`).
- [ ] E.29 — Update `sruth/tuatha/README.md`, `sruth/croilar/README.md`, `sruth/crypteolas/README.md`.
- [ ] E.30 — Update 4 tuatha/croilar/crypteolas skills: `.agents/skills/{tuatha-platform,tuatha-mmo,tuatha-mcp-server-tools,tuatha-achievement-ledger,croilar-stream-registry,pent-elemental-cosmology,british-isles-formative-assessment}/SKILL.md` — path examples.
...
- [ ] G.3 — Update root `README.md` lines 55, 69, 137–138: replace 5 generic subagent names (`explorer/data-engineer/ai-engineer/frontend-dev/devops-architect`) with 5 sruth specialists from `opencode.json` (`oideachais/infrastructure/meaisinfhoghlaim/croilar/tuatha`).
...
## Total: 6 commits across 8 phases (A pre + B codeolas + C oideachais + D meaisinfhoghlaim + E tuatha/crypteolas/croilar + F root baml_src + G docs + H push)
...
- `ls croilar/baml/` → "No such file or directory"
- `ls sruth/` → codeolas/ crypteolas/ croilar/ meaisinfhoghlaim/ oideachais/ tuatha/ (6 dirs)
