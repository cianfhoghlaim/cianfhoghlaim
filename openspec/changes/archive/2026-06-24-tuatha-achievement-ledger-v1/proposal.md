...
`sruth/tuatha/sruth/crypteolas/achievements/` skill-tree badge ledger.
The spec was written and archived, but the implementation
was never created. The 4 prerequisites for this change:

1. The canonical `sruth/oideachais/agents/adk/tools/tuatha_*.py`
   ...
   ready to consume.
2. The `sruth/tuatha/agents/mcp_server/server.py` has a broken
   import at lines 23-33 (`from ..tools.curriculum_search
   import ...` fails because `sruth/tuatha/agents/tools/` does
   not exist).
...
1. **Achievement-ledger implementation** — the 6 new files
   at `sruth/tuatha/sruth/crypteolas/achievements/`
   (`__init__.py`, `schema.py`, `storage.py`, `ledger.py`,
   `cli.py`) plus the `sruth/tuatha/sruth/crypteolas/__init__.py` package
...
2. **MCP server shim fix** — the 3 shim files at
   `sruth/tuatha/agents/tools/{__init__,curriculum_search,mythology_query}.py`
   that preserve the historical import pattern while
   pointing at the canonical home in `sruth/oideachais/`.
3. **3 new skills** —
...
- `sruth/tuatha/sruth/crypteolas/__init__.py` (new)
- `sruth/tuatha/sruth/crypteolas/achievements/__init__.py` (new)
- `sruth/tuatha/sruth/crypteolas/achievements/schema.py` (new)
- `sruth/tuatha/sruth/crypteolas/achievements/storage.py` (new)
- `sruth/tuatha/sruth/crypteolas/achievements/ledger.py` (new)
- `sruth/tuatha/sruth/crypteolas/achievements/cli.py` (new)
- `sruth/tuatha/agents/tools/__init__.py` (new)
- `sruth/tuatha/agents/tools/curriculum_search.py` (new)
- `sruth/tuatha/agents/tools/mythology_query.py` (new)
...
- `sruth/tuatha/AGENTS.md` (updated — added 3 new skill rows to
  the priority quick reference)
- `sruth/tuatha/README.md` (updated — new achievement-ledger
