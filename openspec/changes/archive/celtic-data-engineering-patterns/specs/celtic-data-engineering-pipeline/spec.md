...
> `sruth/oideachais/` quadrant, and adds the marimo statistical-analysis surface
> under `sruth/meaisinfhoghlaim/marimo/`.

## ADDED Requirements

### Requirement: dbt-duckdb project for celtic data
The system SHALL provide a dbt project at `sruth/oideachais/dbt_project/` that
...
- **WHEN** `dbt build --project-dir sruth/oideachais/dbt_project --target dev` is run
...
- **AND** the output database SHALL be at
  `dashboard/sources/sruth/oideachais/oideachais.duckdb`
...
- **WHEN** `dbt build --project-dir sruth/oideachais/dbt_project --target prod` is run
...
The system SHALL provide `sruth/oideachais/dagster_defs/dbt_translator.py` with
...
`sruth/meaisinfhoghlaim/marimo/` that read from `oideachais` via MotherDuck and
expose reactive descriptive + time-series analysis.

#### Scenario: Descriptive notebook renders
- **GIVEN** `sruth/meaisinfhoghlaim/marimo/01_leabharlann_descriptive.py` exists
- **WHEN** `marimo edit sruth/meaisinfhoghlaim/marimo/01_leabharlann_descriptive.py` is run
...
- **GIVEN** `sruth/meaisinfhoghlaim/marimo/02_dpre_lag_analysis.py` exists
...
The system SHALL include `sruth/meaisinfhoghlaim/marimo/` as a first-class
sub-package of the meaisinfhoghlaim quadrant, exposed as a `[marimo]`
optional extra in `sruth/meaisinfhoghlaim/pyproject.toml`.
