...
The `oideachais/` data platform today is **deep on Ireland education** and **shallow everywhere else**. The `dlt_sources/` tree carries 30+ Irish sources plus 4 UK nations' education sources and 3 crown‑dependency sources, but:

- There are **no DLT sources** for `medicine` (HSE, Medical Council, GMC‑UK, NHS England/Scotland/Wales), `law` (irishstatutebook.ie, legislation.gov.uk, workplacerelations.ie, courts.ie, ACAS), or `statistics` beyond the four UK census endpoints already documented in `oideachais/firecrawl_configs/scraping_config.yaml`.
- The existing `oideachais/sources.md` lists 5 Irish education sources + 1 medicine row + 1 law row (and the law row is the wrong shape — `medicalcouncil.ie`, `gmc-uk.org`, `hse.ie` are listed under "medicine" but the value `https://www.gov.ie/en/department-of-health/` is the Department of Health, not the GMC).
...
- The firecrawl + browserbase MCP servers in `opencode.json` and the `sruth_browser` stack at `infrastructure/browser/sruth_browser/` are wired but **not** exercised by any production asset. The only consumer is the legacy `oideachais/dagster_defs/assets/ireland/firecrawl_assets.py` which writes a JSON dump to a hard‑coded `/app/storage/data/scrapes/` path and is bypassed by the production assets.
- The Dagster × DLT integration has **no automated test coverage**. The four `oideachais/test_crawl*.py` files are ad‑hoc smoke scripts run from a venv, not in CI, and they break whenever the local `stedding/ingest_queue/` cache is empty.
- The Leaving Cert reader (`oideachais/api/ducklake_reader.py`) opens 7 separate DLT datasets and globs 7 S3 prefixes; there is no shared "one big DB with schemas" model. Per the user's decision (this conversation) we converge on **`md:oideachais` with `oideachais.{domain}.{nation}` schemas**.
...
1. **Canonical `oideachais/sources.yaml`** — single source of truth listing every DLT source across the four domains (`education`, `medicine`, `law`, `statistics`) and the eight nations (`ie, ni, en, sct, wls, iom, jey, ggy`). Pydantic‑validated by `SourceFactory.from_yaml()`.
2. **`SourceFactory`** in `oideachais/dlt_utils/source_factory.py` — 7‑method contract that turns a YAML ID into a DLT source / `@dlt_assets` decorator / Dagster `@asset` / LanceDB table name / Cognee dataset / marimo notebook path / pytest path.
...
6. **16 pytests** in `oideachais/tests/`, `tuatha/tests/`, `croilar/tests/`, `tests/sources/` — exercise the existing DLT/Dagster asset graph with `USE_LOCAL_SCRAPES=true` and a temp DuckLake; no live network, no new features.
7. **Existing source re‑organisation** — `oideachais/dlt_sources/ireland/*` and `oideachais/dlt_sources/uk/{england,scotland,wales,northern_ireland}/*` and `oideachais/dlt_sources/crown_dependencies/*` are re‑export shimmed to the new `oideachais/dlt_sources/domains/education/{nation}/*` package layout. Same behaviour, new address.
...
10. **`oideachais/site_analysis/`** package that drives **firecrawl + browserbase MCP** to produce a `SiteAnalysis` BAML record (software fingerprint, layout fingerprint, page description, screenshot path) for every public source, written to `oideachais.site_analysis` in DuckLake, embedded in LanceDB, cognified in Cognee.
...
- `oideachais/sources.yaml` (new)
- `oideachais/dlt_utils/source_factory.py` (new)
- `oideachais/dlt_utils/{firecrawl_source.py, destinations.py, batching.py, mixins.py, safety.py}` (extended; no new behaviour)
- `oideachais/dlt_sources/domains/education/{ie,ni,en,sct,wls,iom,jey,ggy}/*` (new package layout; existing sources moved)
- `oideachais/dlt_sources/domains/medicine/{ie,ni,en,sct,wls}/*` (new)
- `oideachais/dlt_sources/domains/law/{ie,ni,en,sct,wls}/*` (new, statutory only)
- `oideachais/dlt_sources/{ireland,uk,crown_dependencies}/*` (re‑export shims)
- `oideachais/dagster_defs/definitions.py` (asset key aliases; SourceFactory integration)
- `oideachais/dagster_defs/assets/ie/education/*` (moved from `assets/ireland/`)
- `oideachais/dagster_defs/assets/ie/education/leaving_cert/*` (moved)
- `oideachais/dagster_defs/assets/{ni,en,sct,wls,iom,jey,ggy}/education/*` (new asset directories)
- `oideachais/dagster_defs/assets/{domain}/{nation}/*` for medicine + law (new)
- `oideachais/dagster_defs/assets/site_analysis/*` (new)
- `oideachais/api/ducklake_reader.py` (refactored to single attach)
- `oideachais/site_analysis/` (new package)
- `oideachais/baml_src/site_analysis.baml` (new)
- `oideachais/cocoindex_flows/site_analysis_embedding.py` (new)
- `oideachais/cognee_integration/site_analysis_cognify.py` (new)
- `oideachais/notebooks/dashboards/{education,medicine,law,site_analysis}/*` (new)
- `oideachais/tests/**` (new pytest tree)
- `tuatha/tests/test_definitions_loads.py` (new)
- `croilar/tests/dlt_assets/test_spotify_soundcloud_labels.py` (new)
- `tests/sources/test_cross_namespace.py` (new)
- `mise.toml`, `oideachais/pyproject.toml`, `croilar/pyproject.toml`, `tuatha/pyproject.toml`, `oideachais/Dockerfile.dagster`, `infrastructure/stacks/engineering/oideachais/Dockerfile.dagster` (toolchain bump)
...
### Hosting / infra
- No new Docker Compose stacks. Existing `infrastructure/stacks/engineering/dagster/`, `…/marimo/`, `…/oideachais/`, `…/crawl4ai/`, and `infrastructure/stacks/storage/lakehouse/` already host the required services.
