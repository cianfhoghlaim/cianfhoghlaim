...
## Purpose
The identity-aware AI Gateway contract that fronts the bunchloch self-hosted VLM fleet
through Pangolin. Implements the overlapping Public+Private resource pattern on a shared
FQDN so the same URL serves both humans (via the Pangolin.app/CLI tunnel) and CI agents
(via virtual API keys), with per-role budget caps and session-log capture.
