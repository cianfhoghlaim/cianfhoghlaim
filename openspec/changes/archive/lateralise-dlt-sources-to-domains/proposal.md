...
|---|---|---|
| **Legacy:** `dlt_sources/uk/{england,scotland,wales,northern_ireland}/` + `dlt_sources/ireland/` + `dlt_sources/crown_dependencies/` | `sruth/oideachais/AGENTS.md` "Quick routing" table (line 44-45) | NO — the contract is `{nation}.{domain}.{entity}`, not `{region}/{nation}` |
...
### Phase 1: Update `sruth/oideachais/AGENTS.md` "Quick routing" table
The table at line 42-58 currently points new contributors to:
- `sruth/oideachais/dlt_sources/ireland/` (22 files)
- `sruth/oideachais/dlt_sources/uk/{england,scotland,wales,northern_ireland}/` (4 directories)
- `sruth/oideachais/dlt_sources/crown_dependencies/{channel_islands,isle_of_man}.py`
- `sruth/oideachais/dlt_sources/author_archive/` (4 dlt sources)

After the change, the table points to:
- `sruth/oideachais/dlt_sources/domains/education/{nation}/{source}.py` (where `{nation}` is one of `ie`, `en`, `sct`, `wls`, `ni`, `iom`, `jey`, `ggy`)
- `sruth/oideachais/dlt_sources/domains/education/{nation}/` is the canonical home for ALL education domain sources, regardless of which `region` (UK / IE / CD) they originate from
...
```python
# sruth/oideachais/dlt_sources/uk/england/__init__.py
...
- Update `sruth/oideachais/dagster_defs/assets/uk_education_assets.py`
  (12 imports)
- Update `sruth/oideachais/dagster_defs/assets/ie/education/*.py`
  (5 imports)
- Update `sruth/oideachais/dagster_defs/assets/wire_unwired_dlt_sources.py`
  (11 imports)
- Update `sruth/oideachais/dagster_defs/assets/author_archive_assets.py`
  (any dlt_sources imports)
- Update `sruth/oideachais/dagster_defs/definitions.py` (any dlt_sources imports)
- Update `sruth/oideachais/dagster_defs/asset_checks.py`
- Update `sruth/oideachais/dagster_defs/factories.py` (if it imports dlt_sources)
- Update `sruth/oideachais/dagster_defs/resources.py` (if it imports dlt_sources)
- Update `sruth/oideachais/cognee_integration/cross_stage_cognify.py`
  (if it imports dlt_sources)
- Update `sruth/oideachais/cognee_integration/author_archive_cognify.py`
  (if it imports dlt_sources)

### Phase 5: Update `sruth/oideachais/STATUS.md`
...
- **MODIFIED:** `sruth/oideachais/AGENTS.md` "Quick routing" table
- **MODIFIED:** `sruth/oideachais/STATUS.md` (new "Layout migration" section)
- **NEW:** `sruth/oideachais/REFACTORING.md` entry (deprecation timeline)

### Affected specs
- MODIFIED `oideachais-pipeline` — the rule that all dlt sources
  for the education domain MUST live in
  `sruth/oideachais/dlt_sources/domains/education/{nation}/`. The legacy
...
3. Update `sruth/oideachais/AGENTS.md` "Quick routing" table
