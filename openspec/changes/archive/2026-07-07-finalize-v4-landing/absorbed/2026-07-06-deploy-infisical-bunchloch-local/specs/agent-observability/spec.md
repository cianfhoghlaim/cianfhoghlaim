...
The system SHALL require every `secrets.env` file under `infrastructure/stacks/`
to use the canonical `infisical://dev-baile/<service>/<key>` URI format
compatible with the Locket sidecar at runtime. Jinja template syntax
(`{{ infisical:///... }}`) SHALL NOT be used.
...
#### Scenario: paddleocr + unstract secrets.env are Locket-compatible

- **GIVEN** both `bonneagar/stacks/paddleocr/secrets.env` and
  `bonneagar/stacks/unstract/secrets.env`
- **WHEN** each file is grepped for `{{ infisical:///`
- **THEN** zero matches SHALL be found
- **AND** after Change 2 archives the `unstract/secrets.env` SHALL declare at
  minimum 20 canonical `infisical://dev-baile/unstract/<key>` entries
  covering the full upstream 15-service env surface