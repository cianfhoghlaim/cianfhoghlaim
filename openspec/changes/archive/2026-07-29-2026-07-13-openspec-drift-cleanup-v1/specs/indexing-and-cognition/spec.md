...
This delta modifies existing requirements in the `indexing-and-cognition` capability (renamed `sruth.cianfhoghlaim.cocoindex_flows.X` → `cianfhoghlaim.cocoindex_flows.X` + renamed `sruth.meaisinfhoghlaim.agents` → `meaisinfhoghlaim.agents`) and adds one new requirement to codify the v4 namespace convention.
...
1. **CocoIndex CLI invocations** in scenarios SHALL use the v4 form: `uv run cocoindex update cianfhoghlaim.cocoindex_flows.<flow>:<symbol>` (NOT `uv run cocoindex update sruth.cianfhoghlaim.cocoindex_flows.<flow>:<symbol>`). The `sruth.cianfhoghlaim.*` namespace no longer exists post-v4.
...
- **THEN** the invocation SHALL use the v4 form `uv run cocoindex update cianfhoghlaim.cocoindex_flows.<flow>:<symbol>`
...
- **THEN** it SHALL invoke `uv run cocoindex update cianfhoghlaim.cocoindex_flows.codebase_indexing:CodebaseIndex`
- **AND** it SHALL write the `codebase_chunks` LanceDB table to the `codebase` asset group
- **AND** the legacy `sruth.cianfhoghlaim.cocoindex_flows.codebase_indexing:CodebaseIndex` invocation SHALL fail with `ModuleNotFoundError`
