...
- **142 bare `cianfhoghlaim.*` references** across the 5 same specs
...
`cianfhoghlaim-pipeline` + `meaisinfhoghlaim-platform` remain in
...
| Old | New | Count | Note |
|:--|:--|--:|:--|
| `sruth.cianfhoghlaim.X` | `cianfhoghlaim.X` | ~50 | canonical-positive refs renamed; negative-test `NOT import from sruth.X` refs preserved |
...
| `from sruth.*` (broad regex) | (preserved) | 1 | KEEP — broad-pattern regex check in `cianfhoghlaim-pipeline` spec line 1533 |

Net: 67 → 6 sruth.* refs (91% reduction). The remaining 6 are
historical/negative-test references that should NOT be renamed.

### 2. `cianfhoghlaim.dlt_sources.X` → `cianfhoghlaim.dlt.X` rename (7 refs)

Per the v4 layout, `dlt_sources/` was renamed to `dlt/` and
reorganised into `dlt/{british_isles,language,filesystem,...}/`.

### 3. `cianfhoghlaim.dagster_defs.X` → `cianfhoghlaim.orchestration.defs.X` rename (9 refs)
...
### 4. `cianfhoghlaim.dagster_assets` → `cianfhoghlaim.orchestration.defs.assets` rename (1 ref)
...
`cianfhoghlaim-pipeline/spec.md:1535` originally listed both
`sruth.cianfhoghlaim.*` and bare `cianfhoghlaim.*` as forbidden
namespaces. After the v4 rename, the two collapsed into a single
`cianfhoghlaim.*` listing (duplicate). The line was rewritten to
...
`cianfhoghlaim.*` remain forbidden).

## What does NOT change

- **142 bare `cianfhoghlaim.*` refs** — the bare-`cianfhoghlaim.*` namespace is the **legitimate post-v4 quadrant-namespace shorthand** used throughout the openspec text to refer to Python modules under `cianfhoghlaim/cianfhoghlaim/<X>/`. Per the spec text convention (line 1533: "the system SHALL have zero `from sruth.*` or `from cianfhoghlaim.*` imports"), the **Python imports** use `from cianfhoghlaim.X import Y`, but the **spec text** uses `cianfhoghlaim.X` as a logical quadrant shorthand. We do NOT rename these.
...
- **Pre-existing validation errors** in `cianfhoghlaim-pipeline` (1 Requirement outside main section) + `meaisinfhoghlaim-platform` (3 Requirements outside main section) — these are pre-existing on HEAD `54c21dd52` and are tracked separately.
...
|:--|--:|--:|:--|
| `cianfhoghlaim-pipeline` | ~16 | ~18 | canonical-positive `sruth.X` refs renamed; `dagster_defs` → `orchestration.defs`; `dlt_sources` → `dlt` |
...
| `cianfhoghlaim-leabharlann` | 0 | 0 | (no renames needed — only historical "formerly `sruth.X`" refs which were preserved) |
...
- [x] `cianfhoghlaim.dlt_sources` + `cianfhoghlaim.dagster_defs` + `cianfhoghlaim.dagster_assets` stale-subpath refs reduced from 17 to 0
...
1. **Pre-existing validation errors in `cianfhoghlaim-pipeline` (1)** + **`meaisinfhoghlaim-platform` (3)** — Requirements outside the main `## Requirements` section. Tracked separately.
...
6. **The 142 bare `cianfhoghlaim.*` refs in non-renameable contexts** (DB schemas, CLI invocations, agent paths) — these are the legitimate post-v4 quadrant-namespace shorthand.