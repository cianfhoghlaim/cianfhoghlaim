# Centralize Cross-Cutting Docs

> **The anti-drift contract that prevents the AGENTS.md numbers, the per-spec AGENTS.md files, and the priority-quick-reference blocks from going stale. Inherits the `centralized-model-registry` audit pattern (script as a CI gate, non-zero exit on violation, JSON + Markdown report).**
...
- `scripts/lint_drift_docs.py` — the lint (Phase 2 deliverable)
- `scripts/sync/spec_agents.py` — the per-spec AGENTS.md generator
- `scripts/registry_audit.py` — the pattern template (centralized-model-registry audit)
- `.github/workflows/lint-drift-docs.yaml` — the CI gate (Phase 5 deliverable)
- `.forgejo/workflows/lint-drift-docs.yaml` — the Forgejo mirror
- `openspec/specs/repo-hygiene-agent-routing/spec.md` — the per-spec AGENTS.md convention
- `openspec/specs/centralized-model-registry/spec.md` — the audit pattern this spec inherits
