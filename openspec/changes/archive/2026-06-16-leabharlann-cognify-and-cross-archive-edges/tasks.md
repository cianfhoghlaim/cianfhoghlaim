...
- [ ] Add `sruth/oideachais/cognee_integration/leabharlann_cognify.py` with the
...
- [ ] Add a `sruth/oideachais/tests/test_leabharlann_cognify.py` test (graceful
      if `cognee` is missing).

## Phase 2 — Cross-archive edges

- [ ] Add `sruth/oideachais/cognify_rules/leabharlann_cross_archive.py` with 3
      rule classes: `ArxivIdMatch`, `ModuleTitleMatch`, `UrlMatch`.
- [ ] Add `sruth/oideachais/api/cross_archive_graph.py` FastAPI route
      `GET /cross-archive-graph/{query}` that runs a FalkorDB query.
- [ ] Wire `cross_archive_edges` Dagster asset to call the rules and
      populate FalkorDB via the `sruth/oideachais/graph/falkordb_client.py`.
- [ ] Add a `sruth/oideachais/tests/test_leabharlann_cross_archive.py` test
      (graceful if `falkordb` is missing).

## Phase 3 — Cron sensor

- [ ] Add `sruth/oideachais/dagster_defs/sensors/cognee_cron_sensor.py` with a
      daily cron that fires the 4 cognify + cross-archive assets.
- [ ] Register the sensor in `sruth/oideachais/dagster_defs/sensors/__init__.py`.
