...
The `sruth/tuatha/pyproject.toml` `[tool.hatch.build.targets.wheel] packages` SHALL include
...
- **WHEN** `sruth/tuatha/pyproject.toml` is read
...
The `sruth/tuatha/dg.toml` SHALL register every Dagster code-location hosted in
...
- **WHEN** `dagster dev -m dagster_assets.definitions` is started from `sruth/tuatha/`
...
- **WHEN** both `sruth/tuatha/baml_src/clients.baml` and `sruth/tuatha/sruth/crypteolas/baml_src/clients.baml`
  are present
- **THEN** `sruth/tuatha/baml_src/clients.baml` SHALL be renamed to `tuatha_clients.baml`
  to avoid the collision, and one combined `sruth/tuatha/baml_client/` SHALL be
  generated.

### Requirement: Followup issue filed
A follow-up GitHub/Forgejo issue SHALL be filed for the pre-existing broken
`sru th.shared.*` imports in `sruth/tuatha/dlt_sources/geospatial/` and
`sruth/tuatha/storage/serial_executor.py`. This change SHALL NOT fix those imports.
...
- **THEN** an issue titled "Followup: simplify sruth.shared.* abstraction in
  sruth/tuatha/dlt_sources/geospatial/ and sruth/tuatha/storage/" exists with the full
  plan inline.
