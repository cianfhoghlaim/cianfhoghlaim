# Spec delta: `bonneagar-tuatha-iac-stack`

This delta is part of the openspec change
`2026-07-29-tuatha-onboarding-and-wiring-v1`. It adds 5 requirements to the
canonical IaC contract that the `bonneagar/stacks/tuatha/` directory must
satisfy, mirroring the contract that `bonneagar/stacks/pocket-id/` already
satisfies.
...
These 5 requirements together implement the IaC contract operators have
learned from Pocket ID. Shipping Tuatha on the same pattern eliminates
the "two patterns per platform" knowledge tax that slows onboarding and
makes incident response slower.
