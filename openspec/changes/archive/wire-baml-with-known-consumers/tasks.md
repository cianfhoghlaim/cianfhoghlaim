...
- [ ] Create `sruth/oideachais/baml_src/early_childhood.baml` with:
...
- [ ] Verify: `grep -n "ExtractAistearFramework" sruth/oideachais/baml_src/early_childhood.baml` shows 1 hit

## Phase 2: Wire aistear.py

- [ ] In `sruth/oideachais/dlt_sources/ireland/aistear.py`:
...
- [ ] Create `sruth/oideachais/dagster_defs/assets/ie/education/aistear_dlt_assets.py`:
...
- [ ] Read `sruth/oideachais/dagster_defs/assets/ie/education/__init__.py` to find the registration point
...
- [ ] In `sruth/oideachais/dlt_sources/ireland/__init__.py`:
...
- [ ] `grep -rn "ExtractAistearFramework" sruth/oideachais/` shows 2 hits (the BAML declaration + the aistear.py call)
- [ ] `grep -rn "PENDING_BAML" sruth/oideachais/dlt_sources/ireland/aistear.py` returns 0 hits (placeholder replaced)
