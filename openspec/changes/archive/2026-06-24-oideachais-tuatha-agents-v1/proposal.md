...
`oideachais/agents/adk/`". The 4 specialists + the root agent move
...
- 11 files move from `tuatha/agents/` to `oideachais/agents/adk/`:
...
- 5 thin wrapper files at `tuatha/agents/adk/*.py` re-export from
  `oideachais.agents.adk.*` (12-30 lines each, vs. 161-191 lines
  in the v0).
- `tuatha/agents/orchestrator.py` keeps working without changes
  (the thin wrappers preserve `from .adk.root_agent import
  celtic_tutor_agent, ...`).
- `oideachais/agents/adk/__init__.py` re-exports the 5 new modules.

## What Changes

### 1. `oideachais/agents/adk/celtic_tutor_agent.py` (NEW — moved from `tuatha/agents/adk/celtic_tutor.py`)
...
### 2. `oideachais/agents/adk/mythology_narrator_agent.py` (NEW — moved from `tuatha/agents/adk/mythology_narrator.py`)
...
### 3. `oideachais/agents/adk/quest_guide_agent.py` (NEW — moved from `tuatha/agents/adk/quest_guide.py`)
...
### 4. `oideachais/agents/adk/research_assistant_agent.py` (NEW — moved from `tuatha/agents/adk/research_assistant.py`)

The canonical LlmAgent construction for the research assistant.
Uses 3 tools (`research_curriculum`, `research_mythology`,
`compare_languages`).

### 5. `oideachais/agents/adk/tuatha_root_agent.py` (NEW — moved from `tuatha/agents/adk/root_agent.py`)
...
### 6. `oideachais/agents/adk/tools/tuatha_*.py` (NEW — moved from `tuatha/agents/tools/`)
...
### 7. `oideachais/agents/adk/tuatha_config.py` (NEW — moved from `tuatha/agents/config.py`)
...
### 8. `tuatha/agents/adk/*.py` (MODIFIED — thin wrappers)
...
- Affected code:
  - 5 new files in `oideachais/agents/adk/`
  - 5 new files in `oideachais/agents/adk/tools/`
  - 1 new file in `oideachais/agents/adk/tuatha_config.py`
  - 5 thin wrappers in `tuatha/agents/adk/`
  - 1 thin `tuatha/agents/adk/__init__.py`
  - 1 modified `oideachais/agents/adk/__init__.py`
