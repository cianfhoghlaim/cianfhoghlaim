...
- [ ] 5. Create `sruth/oideachais/dlt_sources/ireland/primary.py` — 4 dlt resources, hash-based incremental.
- [ ] 6. Create `sruth/oideachais/dlt_sources/ireland/junior_cycle.py` — 3 dlt resources, hash-based incremental.
- [ ] 7. Update `sruth/oideachais/dlt_sources/ireland/__init__.py` to re-export the new `primary_source()` and `junior_cycle_source()` factories.
- [ ] 8. Add 2 Dagster assets to `sruth/oideachais/dagster_defs/assets/ie/education/curriculum_dlt_assets.py` (or new file): `ireland_primary_raw` + `ireland_junior_cycle_raw` with their partition definitions.
- [ ] 9. Register the 2 new assets in `sruth/oideachais/dagster_defs/definitions.py`.

## Phase 2 — Wire `b.ExtractZoteroMetadata` into `zotero.py` (week 1-2)

- [ ] 10. Add a 4th resource `arxiv_papers_baml` to `sruth/oideachais/dlt_sources/author_archive/zotero.py` that invokes `b.ExtractZoteroMetadata(pdf_text, file_name, arxiv_id)` for each arXiv paper. Graceful degradation when the BAML client is not generated.
...
- [ ] 12. Create `sruth/oideachais/dagster_defs/assets/leabharlann_demo_assets.py` with the `oideachais_cocoindex_leabharlann_full_stack_demo` asset.
...
- [ ] 15. Create `sruth/oideachais/notebooks/leabharlann_full_stack_demo.py` Marimo notebook with the 5-step pipeline UI.
- [ ] 16. Register the new Marimo app in `sruth/oideachais/marimo/__init__.py` (or equivalent).

## Phase 4 — Tests (week 2-3)

- [ ] 17. Add `sruth/oideachais/tests/test_ireland_primary_jc_pipeline.py` with tests for:
...
- [ ] 18. Add `sruth/oideachais/tests/test_leabharlann_full_stack_demo.py` with tests for:
  - The demo asset declares its sample PDF paths.
  - The asset checks fire correctly.
  - The Marimo notebook file is parseable.
- [ ] 19. Run `uv run pytest sruth/oideachais/tests/test_ireland_primary_jc_pipeline.py sruth/oideachais/tests/test_leabharlann_full_stack_demo.py -q`; all tests pass.
