...
### Requirement: Pipeline-level Python classes live in `dagster_defs/assets/` or `sruth/meaisinfhoghlaim/`
The oideachais quadrant SHALL NOT contain a top-level `sruth/oideachais/pipelines/`
...
- `sruth/meaisinfhoghlaim/<subpackage>/` (the model-layer quadrant, for
...
- **AND** if it is library code (no Dagster wiring), it MUST be
  added to the appropriate `sruth/meaisinfhoghlaim/<subpackage>/` module
  per the model-layer quadrant layout in `sruth/meaisinfhoghlaim/AGENTS.md`
- **AND** it MUST NOT be added to a top-level `sruth/oideachais/pipelines/`
