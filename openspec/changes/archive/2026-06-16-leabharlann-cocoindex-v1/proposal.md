...
Three forces are converging on the `sruth/oideachais/` data platform:
...
3. **CocoIndex v0 is dead.** The venv has `cocoindex==1.0.9` (v1 API). All 8 modules in `sruth/oideachais/cocoindex_flows/` use the removed v0 DSL (`@cocoindex.flow_def`, `FlowBuilder`, `DataScope`, `cocoindex.sources.DuckDB`, `cocoindex.targets.lancedb`, `cocoindex.functions.SplitRecursively`, `cocoindex.functions.SentenceTransformerEmbed`). The `.agents/skills/cocoindex/SKILL.md` and the canonical `docs/cocoindex/` examples are all v1. The v0 code is broken at import time.

## What Changes

### 1. CocoIndex v0 → v1 migration (`sruth/oideachais/cocoindex_flows/`)
...
### 2. New dlt sources (`sruth/oideachais/dlt_sources/author_archive/`)
...
- **Add** `_epub_extractor.py` — `ebooklib`-based chapter-by-chapter text extraction. The library is added to `sruth/oideachais/pyproject.toml` as an optional dep with graceful degradation.
...
### 4. Dagster asset group (`sruth/oideachais/dagster_defs/assets/leabharlann_assets.py`)
...
### 5. Sensor (`sruth/oideachais/dagster_defs/sensors/leabharlann_sensors.py`)
...
### 6. Tests (`sruth/oideachais/tests/test_leabharlann_pipeline.py`)
...
### 7. `sruth/oideachais/pyproject.toml`
...
|:--|:--|:--|
| `sruth/oideachais/cocoindex_flows/` modules | 8 (all v0, broken on import) | 11 (all v1, working) |
| CocoIndex App count | 0 (no `@coco.App` anywhere) | 11 (one per module) |
| `sruth/oideachais/dlt_sources/author_archive/` modules | 7 | 11 (+ `leabharlann_books`, `zotero`, `takeout_v1`, `_epub_extractor`, `previews`) |
...
- `sruth/croilar/cocoindex_flows/{cv_embedding,artwork_embedding}.py` v0→v1 migration — same package family but separate `dg.toml` code location. Becomes a follow-up change.
...
- Lakehouse destination: `sruth/oideachais/dlt_utils/destinations.py:118`
- Existing author-archive dlt source: `sruth/oideachais/dlt_sources/author_archive/{_scanner,gemini_deep_research,university_of_galway,google_takeout}.py`
