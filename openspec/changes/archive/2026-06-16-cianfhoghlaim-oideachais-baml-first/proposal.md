...
The current `sruth/oideachais/` project has a mature **data layer** (Dagster + DLT + DuckLake + LanceDB + BAML schemas for exam papers and marking schemes) but the **front-end and agent surface are misaligned with the project's bilingual, multi-stage scope**:

1. **Brand drift.** The interactive web app is branded as **"Awen Hub"** (`sruth/oideachais/web/apps/web/src/routes/index.tsx:5`, `routeTree.tsx:20`), while the canonical project name across the rest of the monorepo, the published PyPI package, and the `openspec/project.md` is **"Cianfhoghlaim Oideachais"** (`infrastructure/stacks/infrastructure/forgejo/docs/pypi-usage.md:116`, `readme2.md:1`). The MMO/Game theme from `sruth/tuatha/` is leaking into the oideachais UI copy (`docs/web/frontend/agentic-platform.md:45,61`).
...
### 1. Rebrand
- **Replace** "Awen Hub" with "Cianfhoghlaim Oideachais" in 4 source files: `sruth/oideachais/web/apps/web/src/routes/index.tsx:5`, `sruth/oideachais/web/apps/web/src/routeTree.tsx:20`, `sruth/oideachais/web/apps/web/src/components/Header.tsx`, and `readme2.md`.
- **Rename** `AwenChat.tsx` → `OideachasChat.tsx`. Drop the x402/Anam token copy. The MMORPG framing lives in `sruth/tuatha/`, not in `sruth/oideachais/`.
- **Demote** `docs/web/frontend/agentic-platform.md` and `docs/sruth/tuatha/Agentic Education Platform Development.md` to use the Cianfhoghlaim Oideachais brand.
- **Keep** the `sruth/oideachais/` monorepo directory name (decision: do not rename the dir; the PyPI package is already `cianfhoghlaim-oideachais`).
...
- **Migrate** `sruth/oideachais/web/apps/web` from Vite SPA → real TanStack Start with `createStartHandler`, `getRouter`, `StartClient`, and file-based routes in `apps/web/src/routes/`.
...
- **Add** `sruth/oideachais/web/convex/schema.ts` with 5 tables: `subject_sessions`, `practice_attempts`, `annotations`, `classmate_shares`, `extraction_budget`.
...
### 6. Agno stage teams
- **Add** `sruth/oideachais/data_platform/agents/agno/stage_teams/{aistear,primary,junior_cycle,senior_cycle,tertiary}_team.py` — 5 Agno `Team` instances with stage-specific sub-agents.
...
| Monorepo directory | `sruth/oideachais/` (unchanged) | `sruth/oideachais/` (unchanged) |

## Out of scope

- Renaming the `sruth/oideachais/` monorepo directory to `cianfhoghlaim-sruth/oideachais/` (decision: keep the dir name; only the brand is refronted).
- Renaming `códeolas/` back to `códeolas/` (the PyPI package is already `cianfhoghlaim-oideachais`).
- Porting the `sruth/tuatha/` MMO documentation onto the oideachais web app (the `sruth/tuatha/` theme is shelved).
