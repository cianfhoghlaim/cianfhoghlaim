...
This spec covers the per-module ingest + BAML extraction of the
1,938 University of Galway coursework files in
`leabharlann/ollscoil_na_gaillimhe/` and the 39 personal records in
`cian_mac_an_déisigh_uí_liatháin/`.

## Purpose

- Run the DLT filesystem scan + BAML extraction for each of the 4 UoG
  modules (mata, software, irish, education) and the personal records
- Exclude the `identity/` subdir of personal records by default
- Return a uniform `UoGModuleExtraction` record shape so the marimo
  dashboard can render all 5 modules in a single table
- Surface the user's own intellectual output alongside the
  official_media sources (the user said: "we want to know what
  data we have and how it was sourced")
...
- `baml_src/author_archive.baml` — `ExtractUoGMathModule`,
