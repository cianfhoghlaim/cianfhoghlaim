...
`cocoindex_flows/british_isles/<full>/`. The `<full>` placeholder SHALL
...
- **AND** `orchestration/defs/1_ingestion/british_isles/scotland/education/defs.yaml`
  SHALL exist
- **AND** `cocoindex_flows/british_isles/scotland/education_embedding.py`
  SHALL exist
- **AND** `baml_src/british_isles/sct/`,
  `dlt/british_isles/sct/`,
  `orchestration/defs/1_ingestion/british_isles/sct/`, and
  `cocoindex_flows/british_isles/sct_education_embedding.py` SHALL NOT
  exist
...
`cocoindex_flows/british_isles/_cross/education_embedding.py`. The
legacy path `cocoindex_flows/biep_parity/` SHALL remain as a separate
conformance-test directory (it is not the BIEP v1 App itself).

#### Scenario: BIEP cross-jurisdiction app is in _cross/

- **WHEN** the directory consolidation change is materialised
- **THEN** `cocoindex_flows/british_isles/_cross/education_embedding.py`
  SHALL exist
- **AND** `cocoindex_flows/biep_parity/` SHALL still exist with the 7
