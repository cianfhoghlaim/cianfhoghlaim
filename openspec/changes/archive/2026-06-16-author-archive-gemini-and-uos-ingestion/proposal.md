...
Two rich personal archives sit on the workstation and are not yet wired into the existing `sruth/oideachais/` data platform:
...
The existing oideachais data platform has all the building blocks: `bunchloch_source` for filesystem ingestion, `local_documents.py` for hash-based incremental + extraction, `research_embedding.py` for CocoIndex embeddings, `dagster_defs/assets/research_assets.py` for the Dagster asset group, and `sruth/oideachais/ocr/` for the handwritten content pipeline. **What is missing is the new `author_archive` namespace that ties them together for these three specific archive types.**

## What Changes

### 1. New dlt source package

- **Add** `sruth/oideachais/dlt_sources/author_archive/` with:
  - `_scanner.py` — refactor of the bunchloch scanner into a generic, path-parameterised, multi-account module. Re-uses the existing `FileHashTracker` from `sruth/oideachais/dlt_sources/ireland/local_documents.py:420` (hoisted into the new package as `oideachais.dlt_sources.author_archive._scanner.FileHashTracker`; the old import path continues to work via a re-export shim).
...
- All write to DuckLake via `sruth/oideachais/dlt_utils/destinations.py:118` (`get_dlt_destination()`). Partition columns: `account`, `domain`. `primary_key=["file_hash"]`, `write_disposition="merge"`.
...
- **Add** `sruth/oideachais/cocoindex_flows/author_archive_embedding.py` mirroring `sruth/oideachais/cocoindex_flows/research_embedding.py:125`:
...
- **Export** the new symbols from `sruth/oideachais/cocoindex_flows/__init__.py`.

### 4. New Dagster asset group

- **Add** `sruth/oideachais/dagster_defs/assets/author_archive_assets.py` with 7 `@asset`s, group `author_archive_ingestion`:
  ...
  - `author_archive_handwriting_ocr` (depends on the three raw assets; runs the OCR chain from `sruth/oideachais/ocr/author_archive_ocr.py`).
...
- **Add** `sruth/oideachais/dagster_defs/sensors/author_archive_sensors.py` — directory-watcher sensor that emits `RunRequest`s on mtime change.
- **Register** the new assets in `sruth/oideachais/dagster_defs/definitions.py`.

### 5. New OCR module

- **Add** `sruth/oideachais/ocr/author_archive_ocr.py` with `AuthorArchiveOCRConfig`, `AuthorArchiveOCRRunner` and `run_ocr_for_page(path, page_index) -> dict` that selects the back-end (Pylaia for Irish HTR, TrOCR for Latin handwriting, VLM for equations) based on the file's path grammar. Re-uses `sruth/oideachais/ocr/pylaia_comparison.py` and `sruth/oideachais/ocr/irish_processing.py`.
...
- **Add** `sruth/oideachais/tests/test_author_archive_pipeline.py` (pytest) that:
...
|:--|:--|:--|
| `sruth/oideachais/dlt_sources/author_archive/` | (absent) | 5 modules (`__init__`, `_scanner`, `_takeout_paths`, `_citation_extractor`, `university_of_galway`, `gemini_deep_research`, `google_takeout`) |
| `baml_src/author_archive.baml` | (absent) | 4 classes, 3 functions, 5 enums |
| `sruth/oideachais/cocoindex_flows/author_archive_embedding.py` | (absent) | 4 collectors, 1 query handler, IVF_HNSW + FTS indexes |
| `sruth/oideachais/dagster_defs/assets/author_archive_assets.py` | (absent) | 7 assets, 3 DynamicPartitions |
| `sruth/oideachais/dagster_defs/sensors/author_archive_sensors.py` | (absent) | 1 FsEvent-based sensor |
| `sruth/oideachais/ocr/author_archive_ocr.py` | (absent) | Pylaia + TrOCR + VLM back-ends |
...
- Pattern precedents:
  - `sruth/oideachais/dlt_sources/bunchloch/filesystem_source.py:1` (filesystem source)
  - `sruth/oideachais/dlt_sources/ireland/local_documents.py:1` (extraction)
  - `sruth/oideachais/cocoindex_flows/research_embedding.py:125` (embedding flow)
  - `sruth/oideachais/dagster_defs/assets/research_assets.py:1` (asset group)
  - `sruth/oideachais/dagster_defs/assets/embedding_assets.py:1` (embedding assets)
  - `sruth/oideachais/dlt_utils/destinations.py:118` (DuckLake factory)
  - `sruth/oideachais/ocr/pylaia_comparison.py:1` (OCR back-end)
  - `sruth/oideachais/agents/baml_integration.py:1` (BAML adapter)
