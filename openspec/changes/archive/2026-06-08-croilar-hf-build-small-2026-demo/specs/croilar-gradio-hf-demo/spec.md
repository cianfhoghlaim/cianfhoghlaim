# `croilar-gradio-hf-demo` spec delta — NEW for HuggingFace Build Small 2026
...
- **AND** the BAML function `ExtractLeavingCertSyllabus` from `sruth/oideachais/data_platform/baml_src/leaving_cert_syllabus_extraction.baml` SHALL be the data source
...
- **THEN** a 7-row cognate table SHALL render with `teach` (Irish), `taigh` (Scottish Gaelic), `tŷ` (Welsh), `thie` (Manx), `chi` (Cornish), `ti` (Breton — marked "in progress"), sourced from `sruth/oideachais/samplaí/cognates.yaml`
...
- **THEN** 12 BAML agents (one per Celtic nation × per subject band: STEM, languages, arts, social) SHALL vote on the answer via the BAML `CompareCurricula` function from `sruth/tuatha/baml_src/celtic_curriculum.baml:188-214`
...
- **AND** the BAML `GenerateNPCDialogue` from `sruth/tuatha/baml_src/mythology_extraction.baml:189-219` SHALL drive the level-gated dialogue trees
...
- **AND** correct answers SHALL grant passage to the Isle of Man zone and mint a (local Anvil) Anam SBT via the CuchulainnNFT.sol 5-element system from `sruth/crypteolas/apps/crypteolas_demo/anam-contracts/src/CuchulainnNFT.sol`
...
- **AND** each tab's headline SHALL cite the corresponding element's symbol + School-subject mapping per `docs/bunchloch/sruth/tuatha/learn-to-earn-model.md:224-233`:
...
- **AND** the winner's output SHALL flow into the BAML `ExtractPastPaper` from `sruth/oideachais/data_platform/baml_src/leaving_cert_past_paper_extraction.baml:30-54` to produce typed, searchable markdown
- **AND** the typed output SHALL be stored in the Space 1 ingestion queue for the An Scrúdú heatmap

## Cross-references

- `openspec/specs/curriculum-ingestion/spec.md` — existing NCCA/SEC document processing capability (extended by this change)
- `openspec/specs/assessment-extraction/spec.md` — existing BAML ExtractMarkingScheme/ExtractPastPaper capability (extended)
- `openspec/specs/oideachais-pipeline/spec.md` — existing oideachais pipeline capability (extended)
- `openspec/specs/knowledge-graph/spec.md` — existing Cognee + Graphiti capability (extended)
- `openspec/specs/semantic-search/spec.md` — existing LanceDB vector search capability (extended)
- `openspec/specs/bilingual-content/spec.md` — existing EN+GA content capability (extended)
- `openspec/specs/agent-frameworks/spec.md` — existing Agno / ADK / CopilotKit agent capability (extended)
- `openspec/specs/observability/spec.md` — existing observability capability (extended via Langfuse)
- `openspec/specs/data-pipeline/spec.md` — existing DLT + Dagster pipeline capability (extended)
- `openspec/specs/frontend-frameworks/spec.md` — existing TanStack + CopilotKit capability (extended)
- `openspec/specs/curriculum-ingestion/spec.md` — existing capability (extended)
- Related skills at `.agents/skills/oideachas-pipeline/SKILL.md`, `.agents/skills/dagster/SKILL.md`, `.agents/skills/dlt/SKILL.md`, `.agents/skills/cognee/SKILL.md`, `.agents/skills/ccc/SKILL.md`
