# `official-media-pipeline` capability spec (delta)

> The Instagram-export-to-government-source enrichment pipeline. A new
> `official_media` domain in the oideachais quadrant that ingests the
> JSON bundle Instagram ships, filters out the noise via a curated
> allowlist + BAML fallback, resolves the canonical official source
> (Wikipedia + Companies House + CRO + Mastodon + Bluesky) for each
> surviving profile, and writes the enriched records to the lakehouse
> so every existing `oideachais` consumer (Dagster, marimo, TanStack,
> Cognee) can render them.
