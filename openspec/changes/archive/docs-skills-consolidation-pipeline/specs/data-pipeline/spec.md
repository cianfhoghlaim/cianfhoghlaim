...
- **GIVEN** the `sruth/oideachais/dagster_defs/assets/docs_skills_assets.py` module
...
| `docs_skills_chunk_and_tag` | `@asset` | `cocoindex update sruth/oideachais/cocoindex_flows/docs_skills_consolidation.py` (batch) |
...
| `codebase_chunk_and_embed` | `@asset` | `cocoindex update sruth/oideachais/cocoindex_flows/codebase_indexing.py` (batch) |
| `codebase_live` | `@asset` (sensor-launched) | `cocoindex update -L` for the codebase index |

- **AND** all six assets SHALL be added to `sruth/oideachais/dagster_defs/definitions.py`'s `combined_assets` list
