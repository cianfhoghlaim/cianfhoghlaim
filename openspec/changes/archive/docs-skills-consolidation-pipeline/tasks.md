...
- [ ] 2.1 Create `sruth/oideachais/cocoindex_flows/docs_skills_consolidation.py` with:
...
- [ ] 2.2 Re-export the new App in `sruth/oideachais/cocoindex_flows/__init__.py`.

## 3. Dagster wrapper

- [ ] 3.1 Create `sruth/oideachais/dagster_defs/assets/docs_skills_assets.py` with 6 assets:
...
- [ ] 3.2 Register the 6 assets in `sruth/oideachais/dagster_defs/definitions.py` (in the `combined_assets` list).
...
- [ ] 4.1 Create `sruth/oideachais/cocoindex_flows/codebase_indexing.py` with:
...
- [ ] 4.2 Re-export the new App in `sruth/oideachais/cocoindex_flows/__init__.py`.
- [ ] 4.3 Add `sruth/oideachais/cocoindex_flows/cli.py` (or extend existing) with a `search_codebase(query: str, k: int = 10)` helper that loads the embedder, encodes the query, and runs a LanceDB ANN query against `codebase_chunks`.
...
- [ ] 6.1 In `.agents/skills/ccc/SKILL.md`, prepend a deprecation banner: "v1 app at `sruth/oideachais/cocoindex_flows/codebase_indexing.py` is the new path; this CLI is in maintenance mode until 2026-07-15."
...
- [ ] 7.3 `uv run cocoindex update sruth/oideachais/cocoindex_flows/docs_skills_consolidation.py` — materialises ≥ 1 row per category.
- [ ] 7.4 `uv run cocoindex update sruth/oideachais/cocoindex_flows/codebase_indexing.py` — materialises ≥ 100 rows on first run.
