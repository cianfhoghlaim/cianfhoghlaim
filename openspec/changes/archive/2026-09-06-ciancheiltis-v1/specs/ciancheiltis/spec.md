...
The system MUST organise the ciancheiltis work as six staged phases,
one per language pair, in this order:
...
Each phase MUST inherit the source-ground-truth pattern of the
previous phase (the en-cy Phase 1 ships first because it has the
largest statutory footprint).
...
The system MUST classify every ingested bilingual document by exactly
one of ten themes:
...
Every DLT source MUST declare its `theme` (one of T1–T10) and the
schema column for the classification is `theme_code`.
...
└── _shared/                      # language_detector, opaque_url_scanner, gov_wales_waf_bypass, bilingual_page_validator
```
...
lingua-py heuristic on the first 5 KB of the body) and MUST NOT trust
any `language` metadata tag. This is required because
`legislation.gov.uk/uksi/2007/1484/made` (UK Statutory Instrument
2007 No. 1484 — *The Citizenship Oath and Pledge (Welsh Language)
Order 2007*) ships with `metadata["language"] = "eng"` while the body
is predominantly Welsh.
...
- **THEN** the language detector MUST classify the body as
  ...
  `Cadarnhad teyrngarwch`, and `Adduned` markers in the second-half
  of the page
...
slugs** that hide their language pair (e.g.
`/uksi/2007/1484/made`, `/wsi/2007/2044/made`) and MUST seed these
into the per-phase discovery checklist. The
`dlt_sources/ciancheiltis/_shared/opaque_url_scanner.py` module is
the single canonical implementation.
...
`dlt_sources/ciancheiltis/_shared/gov_wales_waf_bypass.py` module is
the single canonical implementation.

#### Scenario: gov.wales WAF fallback engages

- **WHEN** the en-cy Phase 1 source requests a `gov.wales/...` page
  and receives 403 or a CAPTCHA challenge
...
(`https://www.clarin.ac.uk/resource-families/celtic-languages/`) as
a ground-truth bilingual corpus provider, alongside the Cadhan Aonair
...
These MUST land under `dlt_sources/ciancheiltis/clarin_uk/` with one
`@dlt.source` per major resource family. They are not licensed
CC-BY or public-domain in every case — license MUST be modelled as a
first-class schema column and ingestion MUST be gated on license
allowance.
...
- **AND** each row MUST carry `language_pair`, `license`,
  `corpus_size_mb`, `has_bilingual_text`, `download_url` columns
...
MUST be declared in
`baml_src/british_isles/_shared/ciancheiltis.baml` (shared) and
...
- **WHEN** a user calls
  `ExtractCiancheiltisBilingualPage(page, language="cy")`
- **THEN** the BAML compiler MUST route to the `ciancheiltisCyExtract`
  client
- **AND** the client MUST use `gemma-4-26B-A4B` model

### Requirement: CocoIndex v1 R1-R4 conformance

Each phase's CocoIndex App MUST conform to R1-R4 (the canonical
contract in the `oideachais-cocoindex-v1` skill):
- **R1**: MUST import `from ._lifespan import shared_lifespan`
- **R2**: MUST use `BAAI/bge-m3` embedder (1024-d, multilingual incl.
  CY/GA/GD/GV)
- **R3**: MUST wrap every flow as `@coco.fn(memo=True, deps=...)`
- **R4**: MUST mount each LanceDB table via `mount_table_target(...)`
  with `conformance_required=True`

#### Scenario: All 6 CocoIndex Apps pass R1-R4

- **WHEN** the `cocoindex_v1_conformance` App runs against the 6
  phase Apps
...
Each phase MUST have a complete 5-layer Dagster asset graph following
the `CelticIngestionComponent` + `CelticMaterialsComponent` +
...
Asset checks (one per phase) MUST gate ≥ 0.70 RAGAS bilingual-pair
score and ≥ 500 bilingual pairs seeded.
...
- **AND** it MUST block unless ≥ 500 bilingual `(en, cy)`
  paragraph-level pairs are present in the lakehouse

### Requirement: MotherDuck Dive + Flight per phase

Each phase MUST ship one MotherDuck Dive
(`ciancheiltis_<phase>_dive.py`) and one MotherDuck Flight
(`ciancheiltis_<phase>_flight`). The Dive MUST show, per theme
(T1-T10): row count, language-pair coverage percentage,
metadata-language-match rate.

#### Scenario: en-cy Dive renders theme coverage

- **WHEN** the user opens `ciancheiltis_en_cy_dive`
- **THEN** the Dive MUST show a 10-row theme table with row counts
...
The system MUST integrate with the existing BIEP and
celtic-language-pipeline surfaces via the shared
`bilingual_concept_registry.py`. Newly discovered bilingual pairs
MUST land in
...
The cross-linguistic edge table from
`meaisinfhoghlaim/alignment/schema.py` (`BilingualTopicEdge`, key
`(concept_id, language_pair)`) MUST be extended with a new dimension
`source = "ciancheiltis"`.

#### Scenario: en-cy pairs land in the bilingual concept registry

- **WHEN** the en-cy Phase 1 BAML extraction produces 100 bilingual
  pairs
...
(`from cianfhoghlaim.dlt_sources...`) inside ciancheiltis source
files — all imports MUST be relative to the ciancheiltis subtree or
via the shared `dlt_sources/common/` helpers.
...
#### Scenario: A new en-cy source uses relative imports

- **WHEN** a developer adds an en-cy DLT source
- **THEN** the source MUST import via
  `from dlt_sources.ciancheiltis._shared.language_detector import detect_languages`
- **AND** MUST NOT import via
  `from cianfhoghlaim.dlt_sources.ciancheiltis...`
