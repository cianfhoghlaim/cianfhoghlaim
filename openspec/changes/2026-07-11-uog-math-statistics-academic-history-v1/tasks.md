...
- [ ] `cianfhoghlaim/baml/education/university/mathematics_statistics_extraction.baml` (NEW)
- [ ] Extend `cianfhoghlaim/baml/processing/author_archive.baml` with a math-aware wrapper
- [ ] Run `baml-cli check` — must exit 0

### 2.3 Validation helpers

- [ ] `cianfhoghlaim/baml/education/university/math_validation.py` (NEW)
...
- [ ] `cianfhoghlaim/orchestration/defs/1_ingestion/filesystem/uog_math_coursework/defs.yaml`
- [ ] `cianfhoghlaim/orchestration/defs/2_materials/baml_extraction/uog_math_coursework/defs.yaml`
- [ ] `cianfhoghlaim/orchestration/defs/2_materials/academic_history_validation/defs.yaml`
- [ ] `cianfhoghlaim/orchestration/defs/3_model_lifecycle/cocoindex_v1/academic_history_flow/defs.yaml`
- [ ] `cianfhoghlaim/orchestration/defs/4_asset_generation/marimo_dashboards/uog_math_coursework/defs.yaml`

### 2.5 CocoIndex v1 App

- [ ] `cianfhoghlaim/cocoindex/academic_history_flow.py` (R1–R4 conforming)

## Phase 3 — Notebooks + agent + generic template

### 3.1 Marimo notebooks (`cianfhoghlaim/notebooks/14_academic_history/`)
...
- [ ] Register the new group in `cianfhoghlaim/notebooks/cli.py::GROUPS`
- [ ] Verify all 8 notebooks parse with `marimo run --headless`

### 3.2 Academic-history agent

- [ ] `cianfhoghlaim/agents/routing_keywords.py` — add 13th bucket
...
- [ ] `cianfhoghlaim/agents/meaisinfhoghlaim/educational/academic_history_manifest.py`
  (Pydantic model)
- [ ] `openspec/changes/2026-07-11-uog-math-statistics-academic-history-v1/templates/academic_history_manifest.example.yaml`

### 3.4 Tests

- [ ] `cianfhoghlaim/tests/_oideachais/test_academic_history_pipeline.py`
...
- [ ] `uv run pytest cianfhoghlaim/tests/_oideachais/test_academic_history_pipeline.py`
