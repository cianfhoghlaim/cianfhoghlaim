...
notebook stubs already live at `notebooks/leaving_cert/`
...
1. **Enhance the 5 existing stub notebooks** at
   `notebooks/leaving_cert/{chemistry,computer_science,gaeilge,geography,mathematics}.py`
...
- Do NOT modify the existing `motherduck/`
...
- `motherduck/flights/lc_pdf_sync_flight.py`
  exists + AST-parses.
- `motherduck/flights/config.yaml` has the
