...
**Migration**: The legacy `personas/<id>.ts` files are migrated to `croilar/config/sources.yaml` `streams:` entries. The `getPersona("aleyum")` API is replaced by `get_stream("music")`. The `definePersona({...})` helper is removed entirely; the Stream registry is YAML-driven and validated by Pydantic.
...
**Migration**: The registry is replaced by `croilar/_shared/streams.py::get_stream` / `list_streams`. The 3 personas (`aleyum`, `cianfhoghlaim`, `carlcashman`) collapse to 2 streams (`music`, `teaching`) plus the new `cv` and `research` streams. `carlcashman` is removed with no replacement.
...
**Migration**: New streams are added by appending to `croilar/config/sources.yaml`. Validation is `openspec validate --strict` over the spec deltas, then `bun run turbo typecheck lint test` over the resulting code.
