# Change: pipeline-gaeilge-v1 (BIEP v3 Ireland LC Gaeilge Pipeline)

## Why

The Ireland Leaving Certificate (LC) Gaeilge curriculum is the
BIEP v3 M1 milestone cohort 5/12. Currently the BIEP v3
...
sisters the Gaeilge pipeline — same template, parameterised for
Gaeilge.

## What changes

- **Gaeilge CocoIndex v1 App** (NEW module
  ...
  `leaving_certificate/gaeilge/<lang>/`, embeds via the canonical
  BAAI/bge-m3 1024-d embedder, writes to `cianhoghlaim.ireland.leaving_cycle.gaeilge.{level}_{language}_chunks`.

- **Gaeilge per-subject pytest** (NEW file
  `tests/biep_parity_lc/test_gaeilge.py`): 4 tests that exercise
  the full load → embed → store → query pipeline against a 3-row
  Gaeilge fixture (HL/OL/FL snippets).

- **openspec delta to `british-isles-education-pipeline-v3/spec.md`**:
  the new "Per-subject Gaeilge pipeline (BIEP v3 M1 cohort
  5/12)" Requirement.
...
- The Dagster asset wiring for Gaeilge — tracked separately by
