...
at `tuatha/crypteolas/achievements/` (formerly called
`tuatha/crypteolas/` as the "crypto data platform"). The
ledger holds **skill-tree badges**, NOT a financial token.

Per the user's plan (Phase 6 of the 6-phase refactor):
"crypto = educational achievements (not finance)". The
ledger metadata includes:
