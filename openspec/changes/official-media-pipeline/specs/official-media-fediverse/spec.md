# `official-media-fediverse` capability spec (delta)

> A pure Python library for resolving Mastodon and Bluesky handles to
> their canonical URL, verification status, and follower count. The
> library has no Dagster dependency and can be reused by the
