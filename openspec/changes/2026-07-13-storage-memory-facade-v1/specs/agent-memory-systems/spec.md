...
(at `cianfhoghlaim/storage/memf.py`) in a CI-runnable pytest
...
`cianfhoghlaim/agents/tuatha/wiring.py` module's
...
- **GIVEN** `cianfhoghlaim/tests/test_memory_backend_smoke.py`
- **WHEN** `uv run pytest cianfhoghlaim/tests/test_memory_backend_smoke.py`
...
- **WHEN** `grep -n "graphiti_client\|falkordb_client\|memgraph_client"
  cianfhoghlaim/agents/tuatha/<slug>_agent.py` runs for each of
  the 8 agents
- **THEN** the output SHALL be empty (0 matches per agent)
- **AND** each agent module SHALL import at least one symbol
  from `cianfhoghlaim/agents/tuatha/wiring.py` (the canonical
