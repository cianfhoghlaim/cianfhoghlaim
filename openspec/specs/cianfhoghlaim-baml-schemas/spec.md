...
`cianfhoghlaim-baml-schemas` is a capability of the Cianfhoghlaim platform.
...
- `senior_cycle.baml` — Senior Cycle subjects (covered by `cianfhoghlaim-pipeline` spec)
...
on the *website* side (per the `cianfhoghlaim-university-deep-extraction`
spec).
