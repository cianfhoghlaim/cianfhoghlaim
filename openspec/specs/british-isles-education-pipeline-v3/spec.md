...
### Requirement: M11 — Tertiary Modules (Personal Archive)

The system SHALL lift
...
DLT source (`uog_personal_archive`), a BAML schema with the typed
artefact / assignment / question / topic / reading-item / code-cell /
CA-mark / transcript-row classes, 9 DuckLake tables under
`cianfhoghlaim.education.ie.personal_archive_*` +
`cianfhoghlaim.education.ie.student_transcripts`, 4 CocoIndex v1
Apps (`UoGPersonalArchiveArtefactsApp` + `…QuestionsApp` +
`…TopicsApp` + `…LectureNotesApp`), 10 typed Cognee edges
(`Artefact-DESCRIBES-Module`, `Artefact-CONTAINS-Question`,
`Question-ANSWERED_BY-Response`,
`Response-GRADED_AS-TranscriptGrade`, `Module-COVERS-Topic`,
`Topic-RELATED_TO-Topic`, `Topic-FOUND_IN-LectureArtefact`,
`Artefact-PROVIDED_BY-Lecturer`, `CodeCell-DEMONSTRATES-Topic`,
`ReadingItem-CITED_IN-LectureArtefact`), 6 Dagster assets
(`uog_personal_archive_stage0_audit` → `…stage1_collect` →
`…baml_extract` → `…typed_join` → `…embed_lance` → `…duckdb_sink`),
an 8-tab Marimo notebook at `notebooks/15_personal_archive.py`,
Convex actions (`chatOverMyArchive` + 4 queries/actions),
CopilotKit panel (`AskMyArchive.tsx`), Genie tile
(`genie/personal_archive_browser.ts`), ADK agent
(`personal_archive_module_assistant` with 10 tools), tests
(`tests/personal_archive/`), Grafana dashboard
(`observability/dashboards/personal_archive.json`), and thesis
figures (`orchestration/defs/uog_personal_archive_figures.py`).

Per the openspec change `2026-08-23-uog-personal-archive-tertiary-modules-v1/`.

#### Scenario: M11 acceptance gate

- **WHEN** the operator runs `dagster asset materialize --select uog_personal_archive_duckdb_sink`
- **THEN** the 6-asset chain materialises (Stage 0 audit → Stage 1
  collect → BAML extract → typed join → embed Lance → DuckLake sink)
- **AND** the 4 CocoIndex v1 Apps (`UoGPersonalArchive{Artefacts,
  Questions,Topics,LectureNotes}App`) are wired and degrade gracefully
  when CocoIndex is unavailable
- **AND** the 10 typed Cognee edges are emitted via
  `scripts/graph_storage/cognify/rules/personal_archive_typed_edges.py`
- **AND** `notebooks/15_personal_archive.py` renders the canonical 8-tab
  BIEP pattern (Health / Filters / Materials / URL Health / Heatmap /
  Recent / Lance Search / SQL Console) with a CS4423 worked-example
  sidebar
- **AND** `orchestration/defs/uog_personal_archive_figures.py` writes
  6 PDFs to `figures/thesis/`
