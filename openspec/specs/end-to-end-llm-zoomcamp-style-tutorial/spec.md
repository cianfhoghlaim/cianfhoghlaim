# Spec: `end-to-end-llm-zoomcamp-style-tutorial`

## Purpose

The Cianfhoghlaim `notebooks/` folder currently holds 50+ marimo notebooks split across 13 numbered directories, organised by **stage** (`01_dev_env` / `02_vision_models` / `03_leaving_cert` / ...) rather than by **tutorial flow**. External readers hit dead-ends because no cell is sequenced — there's no one path from "what is this" to "deploy it".

This spec defines an **8-step end-to-end tutorial** at the top of `notebooks/` (mirroring the `DataTalksClub/llm-zoomcamp/cohorts/2026/workshops/dlt` workshop's `01-overview → 07-where-to-go` numbering pattern), with reactive marimo cells that chain forward. The 50+ existing stage-specific notebooks are preserved under `notebooks/legacy/` so the existing analysis work is not lost — the legacy notebooks are reachable from each tutorial step but not in the primary flow.

## Capability summary

| Step | Notebook | What it does |
|:--|:--|:--|
| 1 | `01_overview_setup.py` | Welcome + architecture diagram + `nb_utils.connect_*()` tour + MotherDuck status |
| 2 | `02_ingest_filesystem.py` | DLT filesystem source → DuckDB → read back via `cursor.df()` |
| 3 | `03_extract_baml.py` | 1 live PDF → `b.ExtractCurriculumSyllabus` → inspect the structured result |
| 4 | `04_embed_cocoindex.py` | CocoIndex v1 update status + LanceDB table preview (the 6 LC subjects) |
| 5 | `05_query_ibis_duckdb.py` | DuckLake tables + ibis query + native `mo.sql(engine=md:oideachais, ...)` cells |
| 6 | `06_serve_marimo_app.py` | `marimo_pair` demo + `marimo run` deployment |
| 7 | `07_deploy_dlthub.py` | `dlthub local run jobs.government_circulars_job --dry-run` + dashboard serve |
| 8 | `08_where_to_go.py` | Cross-archive navigation + BIEP-v2 hooks |

Each step is one marimo notebook. Editing a cell causes dependent cells to re-run reactively. The CLI `cianfhoghlaim-marimo {list,edit,run,dashboard} <name>` works for both the tutorial and the legacy/.

## Non-goals

- The tutorial is **not a replacement** for the 50+ legacy notebooks — those remain as the working surfaces for the BIEP/CRIO/LMMO/Official-Media/etc. domains.
- The tutorial does NOT introduce new ingestion logic. It exercises existing dlt sources + BAML extractions + CocoIndex flows.
- The tutorial does NOT rewire the `dlthub-platform-integration` deployment manifest.
