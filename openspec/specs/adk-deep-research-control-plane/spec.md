# ADK Deep Research Control Plane Capability
...
pipelines, and makes **Gemini Deep Research API** the default
`RESEARCH` backend with feature parity to Firecrawl / Crawl4AI.

The control plane exposes 5 agents (`root_agent` + 4 specialists) +
3 pipeline-trigger agents (`dlt_trigger_agent` +
`cocoindex_index_agent` + `baml_extract_agent`) + 1 backend
(`GoogleDeepResearchBackend`) under a single `minimax`-routed ADK
runtime.
