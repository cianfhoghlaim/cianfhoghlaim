# retro-game-design-catalogue Specification

## Purpose

Catalogue the **design patterns** (not the literal assets) of retro
educational games (Number Munchers, Oregon Trail, Where in the World is
Carmen Sandiego?, and the Irish/British Isles edutainment titles) so the
Cianfhoghlaim Educational MMO designers can riff on them through the
existing `celtic-asset-generation` subject_assets + language_assets +
exporters pipelines. The catalogue is generated by:

1. Playing each retro game via deterministic headless libretro agents
   on the `libretro-retroarch` stack and saving screenshots through
   `ludusavi`-managed save states.
2. Segmenting sprites + UI overlays through `SAM3` (served by the
   `sam3-server` stack) — and optionally `SAM-3D-Objects`
   (`sam3d-objects-server`) for 3D-aware segmentation.
3. Extracting typed design patterns through the canonical Bolmo /
   Molmo2 / Qwen3-VL VLM backbone + a new
   `ExtractGameDesignPattern` BAML function + a new
   `retro_design_pattern_embedding` CocoIndex v1 flow (reusing the
   shared `_lifespan.py`).
4. Generating subject-conditioned 2D + 3D educational game assets via
   Flux / Z-Image-Turbo / Qwen-Image / FIBO (2D, via InvokeAI) and
   TRELLIS.2-4B + SAM-3D-Objects (3D, staged to Garage S3).
...
NCCA learning outcomes + bilingual EN + GA syllabus content.
