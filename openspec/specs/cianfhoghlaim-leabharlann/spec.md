...
`cianfhoghlaim-leabharlann` is a capability of the Cianfhoghlaim platform.
...
- **THEN** the dlt source module is `cianfhoghlaim.pipelines.ingest.leabharlann.{books,zotero,takeout_v1,university_of_galway,gemini_deep_research,google_takeout}` (formerly `cianfhoghlaim.dlt.leabharlann.*`)
- **AND** the CocoIndex embedding flow is `cianfhoghlaim.embeddings.leabharlann` (formerly `cianfhoghlaim.cocoindex_flows.leabharlann_embedding`)
...
the `cianfhoghlaim-university-deep-extraction` pipeline:
...
- [`openspec/specs/cianfhoghlaim-baml-schemas/spec.md`](cianfhoghlaim-baml-schemas/spec.md) (the upstream BAML extraction)
- [`openspec/specs/cianfhoghlaim-cognify-knowledge-graph/spec.md`](cianfhoghlaim-cognify-knowledge-graph/spec.md) (the downstream cognify + edges)
- [`openspec/specs/cianfhoghlaim-marimo-dashboards/spec.md`](cianfhoghlaim-marimo-dashboards/spec.md) (the full-stack demo dashboard)
