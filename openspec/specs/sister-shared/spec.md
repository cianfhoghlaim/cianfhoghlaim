...
Codify the cross-repo sister-shared contract that governs the surface
all 5 sister repos (cianchosaint + ciandlithe + ciancheiltis + tuatha +
gemini-hackathon) inherit from cianfhoghlaim. The 5 Requirements
encode: (1) the 4-tier ModelProviderRouter fallback shape, (2) the
JurisdictionPipelineBase wholesale-copy convention, (3) the
cocoindex_flows/_shared wholesale-copy convention, (4) the canonical
openspec workflow, (5) the 6-file GOLD_STANDARD for every
bonneagar/stacks/ entry. Without this spec each sister fork
re-implements the platform from scratch, producing 5x drift.
