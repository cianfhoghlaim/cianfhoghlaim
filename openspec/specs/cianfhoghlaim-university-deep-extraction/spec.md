# cianfhoghlaim-university-deep-extraction Specification

## Purpose

`cianfhoghlaim-university-deep-extraction` is a capability of the
...
model at `cianfhoghlaim/pipelines/ingest/_cianfhoghlaim_dlt_sources/_university_deep_factory.py`
...
- **GIVEN** a developer appends the following to `cianfhoghlaim/sources/_cianfhoghlaim_sources.yaml`:
  ```yaml
...
`cianfhoghlaim/core/baml/_cianfhoghlaim_src/university_extraction.baml`:
...
client (per the `cianfhoghlaim-baml-schemas` spec). The BAML classes SHALL
...
`cianfhoghlaim/assets/_cianfhoghlaim_dagster_defs/assets/university_deep_extraction/uog_assets.py`:
...
(`cianfhoghlaim.education.ie.university_courses` / `university_modules` /
...
- **AND** persist the result to `cianfhoghlaim.education.ie.university_research_sitemap` (LanceDB)
...
- **AND** persist the resulting `ModuleDescriptor` records to `cianfhoghlaim.education.ie.university_modules` (DuckLake)
...
Both Apps SHALL use the canonical v1 pattern (per `cianfhoghlaim-cocoindex-v1-migration` spec): `@coco.lifespan` + `@coco.fn` + `lancedb.mount_table_target` + `SentenceTransformerEmbedder("BAAI/bge-m3")`. Both SHALL respect the 100-batch minimum + `HNSW-DROP-THRESHOLD=50` rule.
...
`cianfhoghlaim/notebooks/_cianfhoghlaim/university_courses.py` with 4 tabs:
