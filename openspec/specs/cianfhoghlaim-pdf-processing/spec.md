...
`cianfhoghlaim-pdf-processing` is the 7th canonical capability of the
...
- `cianfhoghlaim/ocr/evaluation/compare.py` — the OCR evaluation
  ...
  × corpora.
- `cianfhoghlaim/dlt/sources/cianfhoghlaim/curriculumonline_pdfs.py` +
...
- `spaces/cianfhoghlaim-pdf-review/` — the Gradio HF Space for human
  review of topic-validation flags.

This spec is the **PDF-specific extension** of:

- `cianfhoghlaim-pipeline` (the general DLT + Dagster pipeline)
- `cianfhoghlaim-baml-schemas` (the BAML extraction clients)
- `meaisinfhoghlaim-ocr-htr` (the 22-entry VISION_MODELS registry)
- `cianfhoghlaim-marimo-dashboards` (the marimo visualisation layer)
- `cianfhoghlaim-cocoindex-v1-migration` (the CocoIndex v1 embedding
  flow)
- `cianfhoghlaim-semantic-search` (the LanceDB HNSW search layer)
...
- **Outputs:** `page_text` (per-page) + `page_image` (rendered as
  PNG, 200 DPI)
- **Sink:**
  `motherduck://cianfhoghlaim.pdf_processing.{subject}.{year}.{paper}.ocr_pages`
...
- **Outputs:** `page_diagrams` (list of `{bbox, type, caption}` per
  page)
- **Sink:**
  `motherduck://cianfhoghlaim.pdf_processing.{subject}.{year}.{paper}.diagrams`
...
- **Sink:**
  `motherduck://cianfhoghlaim.pdf_processing.{subject}.{year}.{paper}.extracted`
...
- **Sink:**
  `motherduck://cianfhoghlaim.pdf_processing.{subject}.{year}.{paper}.validated`
...
- **Outputs:** `chunks` table (chunk_id, doc_id, chunk_type, text,
  embedding)
- **Sink:** `lancedb://cianfhoghlaim.pdf_processing_chunks` (IVF_HNSW
  + FTS index)
...
- **Outputs:**
  `motherduck://cianfhoghlaim.assets.official_documents.{syllabus|past_papers|marking_schemes}.{subject}.{year}.{paper}`
  + `cognee://cianfhoghlaim.pdf_processing` dataset
...
- **AND** Stage 6 writes to
  `ducklake://cianfhoghlaim.assets.official_documents.syllabus.mathematics.2026`
  + embeds in LanceDB
...
- **AND** Stage 6 writes to
  `ducklake://cianfhoghlaim.assets.official_documents.past_papers.irish.2024.paper-1`
...
- **AND** Stage 5 chunks into 18 marking-point chunks + 18 formula
  chunks = 36 chunks
- **AND** Stage 6 writes to
  `ducklake://cianfhoghlaim.assets.official_documents.marking_schemes.mathematics.2024.paper-1`
  + Graphiti episode:
...
`spaces/cianfhoghlaim-pdf-review/` (HF Space, deployable via the
...
- `cianfhoghlaim-pipeline/spec.md` — the parent DLT + Dagster pipeline
- `cianfhoghlaim-baml-schemas/spec.md` — the BAML extraction clients
- `cianfhoghlaim-marimo-dashboards/spec.md` — the marimo visualisation
  layer
- `cianfhoghlaim-cocoindex-v1-migration/spec.md` — the CocoIndex v1
  embedding flow
- `cianfhoghlaim-semantic-search/spec.md` — the LanceDB HNSW search
  layer
