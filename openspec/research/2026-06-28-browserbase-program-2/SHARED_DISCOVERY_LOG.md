...
## Agent 24 — UK sites — top 3 findings per site (12 lines total)
1. **gov.uk** — Fastly Varnish HIT (x-cache: HIT, age 124s), nginx + Rails Publishing Service, sitemapindex at `/sitemap.xml` (225 B → nested), LUX speedcurve JS but no headless cap; robots.txt allows default `*` with `AhrefsBot` throttled 10s. DLT source `en/education/national_curriculum.py` already has 7 hard-coded URLs + 3 exam boards.
2. **gov.uk — drift** — 1-day-old sitemap `lastmod 2026-06-28 02:56:49 GMT`. CSP `default-src 'self'; base-uri 'none'` enforces strict, no `Crawl-delay:` for default. Recommended: iterate `/sitemap.xml.gz` per-section, use `User-Agent: Googlebot/2.1` to skip LUX speedcurve.
3. **gov.uk — refactor** — split exam-board URLs (AQA/Pearson/OCR) from `_national_curriculum_helpers.py:14-30` into a separate dlt source; add `If-Modified-Since` revalidation using sitemap `lastmod`.
4. **education.gov.scot** — **ACTIVE OUTAGE**: `/`, `/sitemap.xml`, `/robots.txt` all return **HTTP 500** via Azure Application Gateway (`ApplicationGatewayAffinityCORS` cookie). CfE dlt source `sct/education/curriculum_for_excellence.py:1-40` will hang on retries — needs health-check precondition + SQA (`sqa.org.uk`) fallback for National 5 / Higher / Advanced Higher.
5. **education.gov.scot — refactor** — wire Gaelic Medium explicitly (`foghlam tron Ghàidhlig` URL is hard-coded); add `gaidhlig.scot` peer; cache by `lastmod` since CfE pages rarely change.
6. **education.gov.scot — drift from S05** — Phase 3 S05 report is now stale (assumed site up). Current state: full Azure App Gateway 500. Open issue / retry in 24h.
7. **gov.wales** — **DRIFT: now CloudFront + AWS WAF with CAPTCHA** (`x-amzn-waf-action: captcha`, HTTP 405) — was Cloudflare in Phase 3 S06. Blocks `/sitemap.xml` + `/robots.txt` + `/`. SAN list: `gov.wales` + `llyw.cymru` (Welsh mirror). DLT source `wls/education/curriculum_for_wales.py` does NOT crawl gov.wales — it crawls **`hwb.gov.wales`** (Thinqi 7.6.4 LMS, unaffected by WAF).
8. **gov.wales / Hwb** — Hwb top-level: Curriculum for Wales · Curriculum 2008 · Literacy · Professional learning · School improvement · Resources · News · Events · Keeping safe online · Minecraft Education · Siarter Iaith. 6 AoLEs match `AOLE_AREAS` in `_curriculum_for_wales_helpers.py:14-21`. Refactor: rename `_crawl_hwb_curriculum` → `_crawl_hwb` (avoid gov.wales confusion); wire `llyw.cymru` for CY content; add `cbac.co.uk` as peer to `wjec.co.uk`.
9. **education-ni.gov.uk** — Drupal 10 on Platform.sh UK-1 (origin `nigov.main-bvxea6i-dnvkwx4xjhiza.uk-1.platformsh.site`), Azure App Gateway front. Sitemap is **3-page paginated** (`?page=1,2,3`, 267 B sitemapindex, lastmod 2026-04-19). robots.txt throttles `GPTBot/ClaudeBot/BingPreview/PerplexityBot` to 5s; blocks `/*.pdf` direct access; AI crawlers must use 5s delay.
10. **education-ni.gov.uk — refactor** — current `_crawl_ni_curriculum` likely only fetches sitemap `?page=1`; add `for page in range(1, 4)` loop. Respect 5s Crawl-delay. Add CCEA direct crawl (`ccea.org.uk` is the actual NI curriculum body; DENI site only covers policy).
11. **education-ni.gov.uk — topics** — 10 top-level topics live: Curriculum and learning · Children and Young People Issues · Pupils and parents · Teaching staff · Non-teaching staff · Schools and infrastructure · Support and development · Statistics and research · TransformED (major education reform) · Good Relations and Social Change · Corporate Governance.
12. **Cross-UK synthesis** — All 4 sites are **fully cloud-native** (no on-prem): gov.uk=Fastly, education.gov.scot=Azure App Gateway, gov.wales=CloudFront+WAF, education-ni.gov.uk=Azure App Gateway+Platform.sh. **2 of 4 sites cannot be crawled today**: education.gov.scot (500), gov.wales (WAF CAPTCHA). Best-first-ingest UK order: **education-ni.gov.uk** (Drupal, sitemap paginated, AI-crawler-aware) > **gov.uk** (cached, sitemapindex) > **Hwb** (gov.wales peer, Thinqi LMS) > **education.gov.scot** (skip until 500 resolves, use SQA).

## Agent 21 — HuggingFace — top 3 findings (3 lines)
