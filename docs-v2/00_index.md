---
title: docs-v2 — Consolidated Documentation
status: living-document
generated: 2026-06-13
---

# docs-v2 — Consolidated Documentation Index

**Regenerated from `docs/` via ccc + Cognee. No files in `docs/` are deleted.**

## Domain routing

| Domain | Path | File count |
|:--|:--|--:|
| 01-platform-architecture | `01-platform-architecture/` | 15 |
| 02-data-platform | `02-data-platform/` | 7 |
| 03-agents | `03-agents/` | 2 |
| 04-ai-ml | `04-ai-ml/` | 19 |
| 05-web | `05-web/` | 13 |
| 06-infrastructure | `06-infrastructure/` | 14 |
| 07-standards | `07-standards/` | 10 |
| 08-misc | `08-misc/` | 138 |
| 09-cognee | `09-cognee/` | 1 |
| 10-loose-files | `10-loose-files/` | 34 |
| 11-scripts | `11-scripts/` | 1919 |
| 12-configs | `12-configs/` | 715 |
| 13-images | `13-images/` | 505 |
