...
change. Reads from `cianfhoghlaim.celtic.canuint.*` via the local DuckDB
destination.
"""
...
def build_canuint_dialect_dive() -> str:
    ...
    return """
    CREATE OR REPLACE VIEW md:cianfhoghlaim.dives.canuint_dialect AS
    SELECT
        word_id,
        recording_id,
        location_id,
        ...
        duration_ms,
        language
    FROM md:cianfhoghlaim.celtic.canuint.word_alignments
...
CANUINT_KPI_QUERIES = {
    "province_breakdown": """
        ...
        FROM md:cianfhoghlaim.dives.canuint_dialect
        GROUP BY province
