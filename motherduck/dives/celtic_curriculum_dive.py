...
Added 2026-07-17. Reads from `cianfhoghlaim.celtic.curriculum.*` via
...
def build_celtic_curriculum_dive() -> str:
    ...
    return """
    CREATE OR REPLACE VIEW md:cianfhoghlaim.dives.celtic_curriculum AS
    SELECT
        language,
        nation_code,
        education_level,
        ...
        content_text
    FROM (
        SELECT 'irish' AS language, * FROM md:cianfhoghlaim.celtic.curriculum.irish
        UNION ALL SELECT 'scottish_gaelic', * FROM md:cianfhoghlaim.celtic.curriculum.scottish_gaelic
        UNION ALL SELECT 'welsh', * FROM md:cianfhoghlaim.celtic.curriculum.welsh
        UNION ALL SELECT 'breton', * FROM md:cianfhoghlaim.celtic.curriculum.breton
        UNION ALL SELECT 'manx', * FROM md:cianfhoghlaim.celtic.curriculum.manx
        UNION ALL SELECT 'cornish', * FROM md:cianfhoghlaim.celtic.curriculum.cornish
    );
    """
...
CELTIC_CURRICULUM_KPI_QUERIES = {
    "per_language_coverage": """
        ...
        FROM md:cianfhoghlaim.dives.celtic_curriculum
        GROUP BY language
