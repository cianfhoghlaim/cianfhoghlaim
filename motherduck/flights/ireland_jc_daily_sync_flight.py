"""
ireland_jc_daily_sync_flight — BIEP v3 MotherDuck Flight (M2).

Per the 2026-08-13-biep-v3-systematic-download-ireland-england-v1 change.

Daily Python job that:
1. Runs `mise run biep:v3:m2` to re-extract the 88 Ireland JC cohorts
   (36 specs + 16 short courses + 36 CBAs).
2. Runs the 88 per-cohort CocoIndex v1 Apps to re-embed the
...
logger = logging.getLogger("ireland_jc_daily_sync_flight")


def run_m2_pipeline() -> dict:
    """Run the M2 Ireland JC pipeline via `mise run biep:v3:m2`."""
...
def replicate_ireland_jc_to_lance() -> dict:
    """Replicate the 88 cohort tables (36 specs + 16 short + 36 CBAs) to LanceDB."""
    ...
    result = subprocess.run(
        ["python3", str(script_path), "--jurisdiction", "ireland", "--stage", "junior_cycle"],
        capture_output=True,
        text=True,
        timeout=1800,
    )
...
def main() -> int:
    """Run the canonical Ireland JC daily sync flight. Exit 0 on success."""
    lakehouse_uri = os.environ.get("LAKEHOUSE_DUCKDB", "md:cianfhoghlaim")
    logger.info("Starting Ireland JC daily sync flight (cron yearly 1st September 00:00 UTC)...")

    # Step 1: run the M2 pipeline
    pipeline_status = run_m2_pipeline()
    if not pipeline_status["ok"]:
        logger.error("M2 pipeline failed: %s", pipeline_status["stderr_tail"])
        write_status(lakehouse_uri, pipeline_status)
        return 1

    # Step 2: replicate to LanceDB
    lance_status = replicate_ireland_jc_to_lance()
    ...
    # Step 3: write status
    write_status(lakehouse_uri, {"ok": True, "exit_code": 0, "stdout_tail": "all 88 JC cohorts re-extracted + re-embedded", "stderr_tail": ""})
    logger.info("Ireland JC daily sync flight complete.")
    return 0
