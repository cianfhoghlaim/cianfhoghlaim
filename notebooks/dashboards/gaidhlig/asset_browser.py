"""marimo notebook: gaidhlig/asset_browser — the Scottish Gaelic asset browser.
...
def _intro() -> None:
    ...
    mo.md(
        """
        # Gàidhlig (Scottish Gaelic) Asset Browser (Plan 7 of the 2026-10 convergence saga)

        Browse the Scottish Gaelic asset table (`media.image_gen_chunks_gaidhlig`)
        populated by `GenerateScottishGaelicAsset`.

        ## Reference
        - `baml_src/british_isles/_cross/asset_generation.baml`
        - `cocoindex_flows/media/gaidhlig/asset_index.py`
...
def _browser(subject, role) -> None:
    ...
    for s in ["mathematics", "applied_mathematics", "chemistry", "geography", "history"]:
        for r in ["default", "bilingual"]:
            gd_assets.append({
                ...
                "subject": s,
                "language": "gd",
                "language_label": "Gàidhlig",
                "role": r,
                "title": f"Cruthachadh {s.title()} ({r.title()})",
    ...
    mo.md(
        f"""
        ## Filtered results ({len(filtered)} gaidhlig assets)
...
def _summary() -> None:
    ...
    mo.md(
        ...
        - **Language**: Scottish Gaelic (Gàidhlig / gd)
        - **Subjects**: 8
        - **Roles**: 5
        - **Table**: `media.image_gen_chunks_gaidhlig`
        - **BAML function**: `GenerateScottishGaelicAsset(prompt, subject, role)`
