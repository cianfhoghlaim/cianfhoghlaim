"""marimo notebook: brezhoneg/asset_browser — the Breton (brezhoneg) asset browser.
...
def _intro() -> None:
    ...
    mo.md(
        """
        # Brezhoneg (Breton) Asset Browser (Plan 7 of the 2026-10 convergence saga)

        Browse the Breton asset table (`media.image_gen_chunks_brezhoneg`)
        populated by `GenerateBretonAsset`.

        ## Reference
        - `baml_src/british_isles/_cross/asset_generation.baml`
        - `cocoindex_flows/media/brezhoneg/asset_index.py`
...
def _browser(subject, role) -> None:
    ...
    for s in ["mathematics", "applied_mathematics", "chemistry", "geography", "history"]:
        for r in ["default", "bilingual"]:
            br_assets.append({
                ...
                "subject": s,
                "language": "br",
                "language_label": "Brezhoneg",
                "role": r,
                "title": f"Krouiñ {s.title()} ({r.title()})",
    ...
    mo.md(
        f"""
        ## Filtered results ({len(filtered)} brezhoneg assets)
...
def _summary() -> None:
    ...
    mo.md(
        ...
        - **Language**: Breton (Brezhoneg / br)
        - **Subjects**: 8
        - **Roles**: 5
        - **Table**: `media.image_gen_chunks_brezhoneg`
        - **BAML function**: `GenerateBretonAsset(prompt, subject, role)`
