...
def _():
    ...
    mo.md(
        r"""
        # Applied Mathematics (Leaving Certificate (LC))

        > **LC-APM-LO** — the canonical BIEP per-subject analysis.
        ...
        **Applied Mathematics**:
        ...
        | Display name | Applied Mathematics |
        | NCCA code | `LC-APM-LO` |
...
def _(duckdb, ibis, pd):
    ...
    # (consumed from leaving_certificate/lc/applied_mathematics/)
    import os
...
async def _():
    ...
    try:
        ...
        baml_result = {"stub": True, "subject": "applied_mathematics", "stage": "lc"}
    return baml_result,
...
def _():
    ...
    # (consumed from ireland_lc_applied_mathematics_untiered_en_embedding)
...
def _(alt, mo, pd):
    ...
    chart = (
        ...
        .properties(title="BIEP per-subject pipeline status (applied_mathematics)", width=400, height=200)
    )
...
if __name__ == "__main__":
    ...
    def _cli_main(argv=None):
        parser = cli_argparser_biep("notebooks_lc_applied_mathematics.py")
        args = parser.parse_args(argv)
        import subprocess, json
        subprocess.run(
            ["dagster", "asset", "materialize", "--asset-key", "sf_filesystem_leaving_cert_applied_mathematics"],
            capture_output=True, text=True, timeout=60,
        )
        payload = {
            "notebook": "notebooks/lc/applied_mathematics.py",
            "milestone": args.milestone,
            "asset_check": args.asset_check,
            "jurisdiction": args.jurisdiction,
            "pdf_root": "/leaving_certificate/applied_mathematics",
