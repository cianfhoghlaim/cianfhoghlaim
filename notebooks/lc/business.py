...
def _():
    ...
    mo.md(
        r"""
        # Business (Leaving Certificate (LC))

        > **LC-BUS-LO** — the canonical BIEP per-subject analysis.
...
def _(duckdb, ibis, pd):
    ...
    # (consumed from leaving_certificate/lc/business/)
    import os
...
async def _():
    ...
    try:
        ...
        baml_result = {"stub": True, "subject": "business", "stage": "lc"}
    return baml_result,
...
def _():
    ...
    # (consumed from ireland_lc_business_untiered_en_embedding)
...
def _(alt, mo, pd):
    ...
    chart = (
        ...
        .properties(title="BIEP per-subject pipeline status (business)", width=400, height=200)
    )
...
if __name__ == "__main__":
    ...
    def _cli_main(argv=None):
        parser = cli_argparser_biep("notebooks_lc_business.py")
        args = parser.parse_args(argv)
        import subprocess, json
        subprocess.run(
            ["dagster", "asset", "materialize", "--asset-key", "sf_filesystem_leaving_cert_business"],
            capture_output=True, text=True, timeout=60,
        )
        payload = {
            "notebook": "notebooks/lc/business.py",
            "milestone": args.milestone,
            "asset_check": args.asset_check,
            "jurisdiction": args.jurisdiction,
            "pdf_root": "/leaving_certificate/business",
