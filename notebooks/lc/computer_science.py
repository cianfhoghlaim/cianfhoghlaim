...
def _():
    ...
    mo.md(
        r"""
        # Computer Science (Leaving Certificate (LC))

        > **LC-COMP-LO** — the canonical BIEP per-subject analysis.
...
def _(duckdb, ibis, pd):
    ...
    # (consumed from leaving_certificate/lc/computer_science/)
    import os
...
async def _():
    ...
    try:
        ...
        baml_result = {"stub": True, "subject": "computer_science", "stage": "lc"}
    return baml_result,
...
def _():
    ...
    # (consumed from ireland_lc_computer_science_untiered_en_embedding)
...
def _(alt, mo, pd):
    ...
    chart = (
        ...
        .properties(title="BIEP per-subject pipeline status (computer_science)", width=400, height=200)
    )
...
if __name__ == "__main__":
    ...
    def _cli_main(argv=None):
        parser = cli_argparser_biep("notebooks_lc_computer_science.py")
        args = parser.parse_args(argv)
        import subprocess, json
        subprocess.run(
            ["dagster", "asset", "materialize", "--asset-key", "sf_filesystem_leaving_cert_computer_science"],
            capture_output=True, text=True, timeout=60,
        )
        payload = {
            "notebook": "notebooks/lc/computer_science.py",
            "milestone": args.milestone,
            "asset_check": args.asset_check,
            "jurisdiction": args.jurisdiction,
            "pdf_root": "/leaving_certificate/computer_science",
