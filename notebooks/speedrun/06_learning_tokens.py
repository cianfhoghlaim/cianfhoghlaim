...
def _(
    ...
    class LearningTokenProtocol:
        ...
        def redeem_tokens(
            ...
            option = next(
                (o for o in self.redemption_options if o.option_id == option_id),
                None
            )
...
def _(AchievementLevel, CurriculumStage, SubjectArea, protocol):
    ...
    print("-" * 60)
    print(f"{'Level':<12} {'Primary Jr':<12} {'Primary Sr':<12} {'Junior Cyc':<12} {'Senior Cyc':<12}")
    print("-" * 60)

    for level in AchievementLevel:
        row = f"{level.name:<12}"
        for stage in [CurriculumStage.PRIMARY_JUNIOR, CurriculumStage.PRIMARY_SENIOR,
                      CurriculumStage.JUNIOR_CYCLE, CurriculumStage.SENIOR_CYCLE]:
