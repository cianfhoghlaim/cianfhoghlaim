...
def _():
    ...
    mo.md(
        r"""
        # {SUBJECT_DISPLAY_NAME} ({STAGE_DISPLAY_NAME})

        > **{NCCA_CODE}** — the canonical BIEP per-subject analysis.
...
def _(duckdb, ibis, pd):
    ...
    # (consumed from leaving_certificate/{STAGE}/{SUBJECT}/)
    pdf_root = f"leaving_certificate/{STAGE}/{SUBJECT}"
...
async def _():
    ...
    try:
        ...
        baml_result = {"stub": True, "subject": "{SUBJECT}", "stage": "{STAGE}"}
    return baml_result,
...
def _(alt, mo, pd):
    ...
    chart = (
        ...
        .properties(title="BIEP per-subject pipeline status ({SUBJECT})", width=400, height=200)
    )
