...
# ///
"""
Sources Load — federate every source-registry YAML under the
v4-consolidated `cianfhoghlaim/` tree into a single DuckDB file.
Read-only: the upstream YAMLs are never modified.
...
v4 path conventions (post-2026-06-28 consolidation):
- DLT registry     → cianfhoghlaim.dlt.british_isles.ireland.education.*
- BAML source      → cianfhoghlaim.baml_src.education.subjects.*
- CocoIndex flow   → cianfhoghlaim.cocoindex_flows.leabharlann_embedding.*
- Dagster          → cianfhoghlaim.orchestration.definitions
- Lakehouse        → md:oideachais (MotherDuck + DuckLake)
"""
import marimo
...
def _():
    ...
    while not (REPO_ROOT / "cianfhoghlaim").exists() and REPO_ROOT != REPO_ROOT.parent:
    ...
    mo.md(
        ...
        Federate the source-registry YAMLs under the v4 `cianfhoghlaim/`
        tree into one DuckDB at `registry.duckdb` (next to the notebook).
...
def _(REPO_ROOT, mo):
    ...
    roots_select = mo.ui.multiselect(
        options=[
            ...
            "cianfhoghlaim/baml_src/education",
...
def _(REPO_ROOT, Path, mo, pl, roots_select):
    ...
    def infer_family(path: Path) -> str:
        s = str(path)
        # v4-consolidated registry paths
        if "cianfhoghlaim/dlt/british_isles/ireland/education/sources.yaml" in s:
            return "biep_education_yaml"
        if "cianfhoghlaim/dlt/british_isles/ireland/education/" in s:
            return "biep_education_dlt"
        if "cianfhoghlaim/baml_src/education/" in s:
            return "biep_baml"
        if "cianfhoghlaim/cocoindex_flows/" in s:
            return "biep_cocoindex"
        if "cianfhoghlaim/orchestration/" in s:
            return "biep_orchestration"
        # v4 Irish/British-Isles support
        if "cianfhoghlaim/baml_src/celtic/" in s:
            return "celtic_baml"
        if "cianfhoghlaim/baml_src/processing/" in s:
            return "processing_baml"
        if "cianfhoghlaim/dlt/common/" in s:
            return "dlt_common"
        # legacy v3 fallback (still loadable)
        if "oideachais/sources.yaml" in s:
            return "legacy_oideachais_yaml"
        if "oideachais/firecrawl_configs/" in s:
            return "legacy_oideachais_firecrawl"
        if "oideachais/dbt_project/" in s:
            return "legacy_oideachais_dbt"
        if "oideachais/samplaí/" in s:
            return "legacy_oideachais_samplai"
        return "other"
