"""marimo notebook: 02_text_llm: 9-entry text_llm family (minimax-m3 + the 7-tier fallback) — K-12 teacher + student pipeline walkthrough.
...
9-entry text_llm family (minimax-m3 + the 7-tier fallback)
...
Run with:
    uv run marimo edit notebooks/_shared/k12/walkthroughs/02_text_llm.py
    uv run python notebooks/_shared/k12/walkthroughs/02_text_llm.py --cli
"""
...
def _intro() -> None:
    ...
    mo.md(
        ...
        This notebook walks through **9-entry text_llm family (minimax-m3 + the 7-tier fallback)** — what it is, how it works, where the docs live, and how to query the live DuckLake data.
...
def _phase_status() -> None:
    ...
    mo.md(
        ...
        Walkthrough `02_text_llm: 9-entry text_llm family (minimax-m3 + the 7-tier fallback)` — fetched at {datetime.utcnow().isoformat()}.
        """
    )
    return
