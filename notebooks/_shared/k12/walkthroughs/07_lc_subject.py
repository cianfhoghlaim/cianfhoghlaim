"""marimo notebook: 07_lc_subject: Leaving Certificate subject picker + past papers — K-12 teacher + student pipeline walkthrough.
...
Leaving Certificate subject picker + past papers

Reference surfaces:
dlt_sources/.../education/senior_cycle.py + 
cocoindex_flows/subjects/senior_cycle_embedding.py

Run with:
    uv run marimo edit notebooks/_shared/k12/walkthroughs/07_lc_subject.py
    uv run python notebooks/_shared/k12/walkthroughs/07_lc_subject.py --cli
"""
...
def _intro() -> None:
    ...
    mo.md(
        ...
        This notebook walks through **Leaving Certificate subject picker + past papers** — what it is, how it works, where the docs live, and how to query the live DuckLake data.
...
def _phase_status() -> None:
    ...
    mo.md(
        ...
        Walkthrough `07_lc_subject: Leaving Certificate subject picker + past papers` — fetched at {datetime.utcnow().isoformat()}.
        """
    )
    return
