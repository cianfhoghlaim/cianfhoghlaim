"""marimo notebook: 05_image_gen: 5-entry image_gen family (for SEN visual aids + classroom posters) — K-12 teacher + student pipeline walkthrough.
...
5-entry image_gen family (for SEN visual aids + classroom posters)

Reference surfaces:
meaisinfhoghlaim/models/model_registry.py

Run with:
    uv run marimo edit notebooks/_shared/k12/walkthroughs/05_image_gen.py
    uv run python notebooks/_shared/k12/walkthroughs/05_image_gen.py --cli
"""
...
def _intro() -> None:
    ...
    mo.md(
        ...
        This notebook walks through **5-entry image_gen family (for SEN visual aids + classroom posters)** — what it is, how it works, where the docs live, and how to query the live DuckLake data.
...
def _phase_status() -> None:
    ...
    mo.md(
        ...
        Walkthrough `05_image_gen: 5-entry image_gen family (for SEN visual aids + classroom posters)` — fetched at {datetime.utcnow().isoformat()}.
        """
    )
    return
