"""marimo notebook: 01_ingestion: L1 Ingestion — 9 K-12 DLT sources (aistear + primary + jc + sc + oide + pdst + teacher_pd + class_roster + teacher_workload) — K-12 teacher + student pipeline walkthrough.
...
L1 Ingestion — 9 K-12 DLT sources (aistear + primary + jc + sc + oide + pdst + teacher_pd + class_roster + teacher_workload)

Reference surfaces:
orchestration/defs/1_ingestion/primary_jc/{9 DLT asset subdirs}

Run with:
    uv run marimo edit notebooks/_shared/k12/walkthroughs/01_ingestion.py
    uv run python notebooks/_shared/k12/walkthroughs/01_ingestion.py --cli
"""
...
def _intro() -> None:
    ...
    mo.md(
        ...
        This notebook walks through **L1 Ingestion — 9 K-12 DLT sources (aistear + primary + jc + sc + oide + pdst + teacher_pd + class_roster + teacher_workload)** — what it is, how it works, where the docs live, and how to query the live DuckLake data.
...
def _phase_status() -> None:
    ...
    mo.md(
        ...
        Walkthrough `01_ingestion: L1 Ingestion — 9 K-12 DLT sources (aistear + primary + jc + sc + oide + pdst + teacher_pd + class_roster + teacher_workload)` — fetched at {datetime.utcnow().isoformat()}.
        """
    )
    return
