"""marimo notebook: 03_primary_jc_combined: Primary → Junior Cycle bridging pipeline — K-12 teacher + student pipeline walkthrough.
...
Primary → Junior Cycle bridging pipeline

Reference surfaces:
dlt_sources/.../education/primary_jc_combined.py + 
baml_src/.../stages/primary.baml + 
stages/junior_cycle.baml

Run with:
    uv run marimo edit notebooks/_shared/k12/walkthroughs/03_primary_jc_combined.py
    uv run python notebooks/_shared/k12/walkthroughs/03_primary_jc_combined.py --cli
"""
...
def _intro() -> None:
    ...
    mo.md(
        ...
        This notebook walks through **Primary → Junior Cycle bridging pipeline** — what it is, how it works, where the docs live, and how to query the live DuckLake data.
...
def _phase_status() -> None:
    ...
    mo.md(
        ...
        Walkthrough `03_primary_jc_combined: Primary → Junior Cycle bridging pipeline` — fetched at {datetime.utcnow().isoformat()}.
        """
    )
    return
