"""marimo notebook: 05_jc_cba: Junior Cycle Classroom-Based Assessment (CBA) tracker — K-12 teacher + student pipeline walkthrough.
...
Junior Cycle Classroom-Based Assessment (CBA) tracker

Reference surfaces:
baml_src/.../junior_cycle_extraction/cba_task_extraction.baml + 
agents/.../students_jc/cba_planner_agent.py

Run with:
    uv run marimo edit notebooks/_shared/k12/walkthroughs/05_jc_cba.py
    uv run python notebooks/_shared/k12/walkthroughs/05_jc_cba.py --cli
"""
...
def _intro() -> None:
    ...
    mo.md(
        ...
        This notebook walks through **Junior Cycle Classroom-Based Assessment (CBA) tracker** — what it is, how it works, where the docs live, and how to query the live DuckLake data.
...
def _phase_status() -> None:
    ...
    mo.md(
        ...
        Walkthrough `05_jc_cba: Junior Cycle Classroom-Based Assessment (CBA) tracker` — fetched at {datetime.utcnow().isoformat()}.
        """
    )
    return
