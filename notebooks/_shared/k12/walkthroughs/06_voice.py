"""marimo notebook: 06_voice: 5-entry voice family (whisper-large for SEN audio supports + wav2vec2-irish for Gaeltacht schools) — K-12 teacher + student pipeline walkthrough.
...
5-entry voice family (whisper-large for SEN audio supports + wav2vec2-irish for Gaeltacht schools)

Reference surfaces:
meaisinfhoghlaim/models/model_registry.py

Run with:
    uv run marimo edit notebooks/_shared/k12/walkthroughs/06_voice.py
    uv run python notebooks/_shared/k12/walkthroughs/06_voice.py --cli
"""
...
def _intro() -> None:
    ...
    mo.md(
        ...
        This notebook walks through **5-entry voice family (whisper-large for SEN audio supports + wav2vec2-irish for Gaeltacht schools)** — what it is, how it works, where the docs live, and how to query the live DuckLake data.
...
def _phase_status() -> None:
    ...
    mo.md(
        ...
        Walkthrough `06_voice: 5-entry voice family (whisper-large for SEN audio supports + wav2vec2-irish for Gaeltacht schools)` — fetched at {datetime.utcnow().isoformat()}.
        """
    )
    return
