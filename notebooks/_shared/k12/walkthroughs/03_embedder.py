"""marimo notebook: 03_embedder: 3-entry embedder family (bge-m3 for K-12 CocoIndex flows) — K-12 teacher + student pipeline walkthrough.
...
3-entry embedder family (bge-m3 for K-12 CocoIndex flows)
...
Run with:
    uv run marimo edit notebooks/_shared/k12/walkthroughs/03_embedder.py
    uv run python notebooks/_shared/k12/walkthroughs/03_embedder.py --cli
"""
...
def _intro() -> None:
    ...
    mo.md(
        ...
        This notebook walks through **3-entry embedder family (bge-m3 for K-12 CocoIndex flows)** — what it is, how it works, where the docs live, and how to query the live DuckLake data.
...
def _phase_status() -> None:
    ...
    mo.md(
        ...
        Walkthrough `03_embedder: 3-entry embedder family (bge-m3 for K-12 CocoIndex flows)` — fetched at {datetime.utcnow().isoformat()}.
        """
    )
    return
