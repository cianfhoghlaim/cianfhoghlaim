"""marimo notebook: 08_teacher_workload: Teacher timetable + class assignments + OIDE/PDST planning — K-12 teacher + student pipeline walkthrough.
...
Teacher timetable + class assignments + OIDE/PDST planning

Reference surfaces:
dlt_sources/.../education/teacher_workload.py + 
dlt_sources/.../education/teacher_pd.py + 
agents/.../teachers/lesson_planner_agent.py

Run with:
    uv run marimo edit notebooks/_shared/k12/walkthroughs/08_teacher_workload.py
    uv run python notebooks/_shared/k12/walkthroughs/08_teacher_workload.py --cli
"""
...
def _intro() -> None:
    ...
    mo.md(
        ...
        This notebook walks through **Teacher timetable + class assignments + OIDE/PDST planning** — what it is, how it works, where the docs live, and how to query the live DuckLake data.
...
def _phase_status() -> None:
    ...
    mo.md(
        ...
        Walkthrough `08_teacher_workload: Teacher timetable + class assignments + OIDE/PDST planning` — fetched at {datetime.utcnow().isoformat()}.
        """
    )
    return
