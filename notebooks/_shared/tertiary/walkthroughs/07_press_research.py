"""marimo notebook: 07_press_research: Press releases + research outputs pipelines — UoG tertiary pipeline walkthrough.
...
Press releases + research outputs pipelines

Reference surfaces:
dlt_sources/.../tertiary/uog/{press_releases,research_outputs}.py + 
2 CocoIndex flows + 
2 BAML files

Run with:
    uv run marimo edit notebooks/_shared/tertiary/walkthroughs/07_press_research.py
    uv run python notebooks/_shared/tertiary/walkthroughs/07_press_research.py --cli
"""
...
def _intro() -> None:
    ...
    mo.md(
        ...
        This notebook walks through **Press releases + research outputs pipelines** — what it is, how it works, where the docs live, and how to query the live DuckLake data.
...
def _phase_status() -> None:
    ...
    mo.md(
        ...
        Walkthrough `07_press_research: Press releases + research outputs pipelines` — fetched at {datetime.utcnow().isoformat()}.
        """
    )
    return
