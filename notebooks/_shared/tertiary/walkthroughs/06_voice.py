"""marimo notebook: 06_voice: 5-entry voice family (whisper-large + wav2vec2-irish + chatterbox + aba-tts) — UoG tertiary pipeline walkthrough.
...
5-entry voice family (whisper-large + wav2vec2-irish + chatterbox + aba-tts)

Reference surfaces:
meaisinfhoghlaim/models/model_registry.py

Run with:
    uv run marimo edit notebooks/_shared/tertiary/walkthroughs/06_voice.py
    uv run python notebooks/_shared/tertiary/walkthroughs/06_voice.py --cli
"""
...
def _intro() -> None:
    ...
    mo.md(
        ...
        This notebook walks through **5-entry voice family (whisper-large + wav2vec2-irish + chatterbox + aba-tts)** — what it is, how it works, where the docs live, and how to query the live DuckLake data.
...
def _phase_status() -> None:
    ...
    mo.md(
        ...
        Walkthrough `06_voice: 5-entry voice family (whisper-large + wav2vec2-irish + chatterbox + aba-tts)` — fetched at {datetime.utcnow().isoformat()}.
        """
    )
    return
