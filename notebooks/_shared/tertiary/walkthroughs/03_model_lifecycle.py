"""marimo notebook: 03_model_lifecycle: L3 Model Lifecycle — embedder + OCR ensemble + vision-language routing — UoG tertiary pipeline walkthrough.
...
L3 Model Lifecycle — embedder + OCR ensemble + vision-language routing

Reference surfaces:
orchestration/defs/3_model_lifecycle/tertiary/ + 
meaisinfhoghlaim/models/model_registry.py (the 7-family registry)

Run with:
    uv run marimo edit notebooks/_shared/tertiary/walkthroughs/03_model_lifecycle.py
    uv run python notebooks/_shared/tertiary/walkthroughs/03_model_lifecycle.py --cli
"""
...
def _intro() -> None:
    ...
    mo.md(
        ...
        This notebook walks through **L3 Model Lifecycle — embedder + OCR ensemble + vision-language routing** — what it is, how it works, where the docs live, and how to query the live DuckLake data.
...
def _phase_status() -> None:
    ...
    mo.md(
        ...
        Walkthrough `03_model_lifecycle: L3 Model Lifecycle — embedder + OCR ensemble + vision-language routing` — fetched at {datetime.utcnow().isoformat()}.
        """
    )
    return
