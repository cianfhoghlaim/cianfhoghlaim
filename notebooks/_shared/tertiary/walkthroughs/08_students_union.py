"""marimo notebook: 08_students_union: 5 SU agents + snake_case full-name module map — UoG tertiary pipeline walkthrough.
...
5 SU agents + snake_case full-name module map

Reference surfaces:
agents/meaisinfhoghlaim/educational/students_union/{root_agent,config,5 specialists,5 tools}.py

Run with:
    uv run marimo edit notebooks/_shared/tertiary/walkthroughs/08_students_union.py
    uv run python notebooks/_shared/tertiary/walkthroughs/08_students_union.py --cli
"""
...
def _intro() -> None:
    ...
    mo.md(
        ...
        This notebook walks through **5 SU agents + snake_case full-name module map** — what it is, how it works, where the docs live, and how to query the live DuckLake data.
...
def _phase_status() -> None:
    ...
    mo.md(
        ...
        Walkthrough `08_students_union: 5 SU agents + snake_case full-name module map` — fetched at {datetime.utcnow().isoformat()}.
        """
    )
    return
