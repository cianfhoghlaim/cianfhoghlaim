"""marimo notebook: 02_materials: L2 Materials — 8 BAML extractions + 6 RAGAS asset checks — UoG tertiary pipeline walkthrough.
...
L2 Materials — 8 BAML extractions + 6 RAGAS asset checks

Reference surfaces:
baml_src/british_isles/ireland/tertiary/{8 BAML files} + 
orchestration/defs/2_materials/tertiary_extraction/

Run with:
    uv run marimo edit notebooks/_shared/tertiary/walkthroughs/02_materials.py
    uv run python notebooks/_shared/tertiary/walkthroughs/02_materials.py --cli
"""
...
def _intro() -> None:
    ...
    mo.md(
        ...
        This notebook walks through **L2 Materials — 8 BAML extractions + 6 RAGAS asset checks** — what it is, how it works, where the docs live, and how to query the live DuckLake data.
...
def _phase_status() -> None:
    ...
    mo.md(
        ...
        Walkthrough `02_materials: L2 Materials — 8 BAML extractions + 6 RAGAS asset checks` — fetched at {datetime.utcnow().isoformat()}.
        """
    )
    return
