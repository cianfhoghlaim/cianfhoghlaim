"""marimo notebook: 01_modules_pipeline: 4-tier College → School → Programme → Module pipeline — UoG tertiary pipeline walkthrough.
...
4-tier College → School → Programme → Module pipeline

Reference surfaces:
dlt_sources/british_isles/ireland/tertiary/uog/{colleges,schools,programmes,modules}.py + 
uog_to_tertiary.py bridge

Run with:
    uv run marimo edit notebooks/_shared/tertiary/walkthroughs/01_modules_pipeline.py
    uv run python notebooks/_shared/tertiary/walkthroughs/01_modules_pipeline.py --cli
"""
...
def _intro() -> None:
    ...
    mo.md(
        ...
        This notebook walks through **4-tier College → School → Programme → Module pipeline** — what it is, how it works, where the docs live, and how to query the live DuckLake data.
...
def _phase_status() -> None:
    ...
    mo.md(
        ...
        Walkthrough `01_modules_pipeline: 4-tier College → School → Programme → Module pipeline` — fetched at {datetime.utcnow().isoformat()}.
        """
    )
    return
