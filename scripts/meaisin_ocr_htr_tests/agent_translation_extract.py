"""meaisinfhoghlaim Agent entrypoint script for translation.

Per the meaisinfhoghlaim v5 umbrella spec + the 2026-08-13-biep-v3-systematic-download-ireland-england-v1 change.

Runs the canonical agent framework pipeline for the translation agent.
...
logger = logging.getLogger("agent_translation_extract")
...
def main() -> int:
    """Run the 3 steps for the translation agent. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("Agent entrypoint: translation")
    logger.info("=" * 60)

    steps = [
        ("1. translation agent", _run_agent),
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("translation entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("translation agent entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
