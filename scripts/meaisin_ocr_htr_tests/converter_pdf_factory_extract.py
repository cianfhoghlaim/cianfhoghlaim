"""meaisinfhoghlaim Document Converter entrypoint script for pdf_factory.

Per the meaisinfhoghlaim v5 umbrella spec + the 2026-08-13-biep-v3-systematic-download-ireland-england-v1 change.

Runs the canonical document converter pipeline for the pdf_factory converter.
The converter is registered in the meaisinfhoghlaim document factory
at `meaisinfhoghlaim.document_factory`.

Usage:
    uv run python scripts/meaisin_ocr_htr_tests/converter_pdf_factory_extract.py
"""
...
logger = logging.getLogger("converter_pdf_factory_extract")


def _run_converter() -> bool:
    """Run the pdf_factory converter on a sample PDF."""
    logger.info("Running pdf_factory converter on sample PDF...")
    try:
        from meaisinfhoghlaim.document_factory import CONVERTERS
        converter = CONVERTERS.get("pdf_factory")
        if converter is None:
            logger.warning("pdf_factory not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the pdf_factory converter. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("Document converter entrypoint: pdf_factory")
    logger.info("=" * 60)
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("pdf_factory entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("pdf_factory converter entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
