...
logger = logging.getLogger("agent_geospatial_extract")


def _run_agent() -> bool:
    """Run the geospatial agent."""
    logger.info("Running geospatial agent...")
    try:
        from agents.meaisinfhoghlaim.registry import AGENTS

        agent = AGENTS.get("geospatial")
        if agent is None:
            logger.warning("geospatial not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the geospatial agent. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("Agent entrypoint: geospatial")
    logger.info("=" * 60)

    steps = [
        ("1. geospatial agent", _run_agent),
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("geospatial entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("geospatial agent entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
