"""meaisinfhoghlaim Agent entrypoint script for curriculum_comparison.

Per the meaisinfhoghlaim v5 umbrella spec + the 2026-08-13-biep-v3-systematic-download-ireland-england-v1 change.

Runs the canonical agent framework pipeline for the curriculum_comparison agent.
The agent is registered in the meaisinfhoghlaim 12-agent framework
at `agents/meaisinfhoghlaim/registry.py`.

Usage:
    uv run python scripts/meaisin_ocr_htr_tests/agent_curriculum_comparison_extract.py
"""
...
logger = logging.getLogger("agent_curriculum_comparison_extract")


def _run_agent() -> bool:
    """Run the curriculum_comparison agent."""
    logger.info("Running curriculum_comparison agent...")
    try:
        from agents.meaisinfhoghlaim.registry import AGENTS

        agent = AGENTS.get("curriculum_comparison")
        if agent is None:
            logger.warning("curriculum_comparison not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the curriculum_comparison agent. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("Agent entrypoint: curriculum_comparison")
    logger.info("=" * 60)

    steps = [
        ("1. curriculum_comparison agent", _run_agent),
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("curriculum_comparison entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("curriculum_comparison agent entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
