...
logger = logging.getLogger("agent_root_extract")


def _run_agent() -> bool:
    """Run the root agent."""
    logger.info("Running root agent...")
    try:
        from agents.meaisinfhoghlaim.registry import AGENTS

        agent = AGENTS.get("root")
        if agent is None:
            logger.warning("root not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the root agent. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("Agent entrypoint: root")
    logger.info("=" * 60)

    steps = [
        ("1. root agent", _run_agent),
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("root entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("root agent entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
