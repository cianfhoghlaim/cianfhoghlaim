"""OCR/VLM model entrypoint script for paddleocr-vl-1.6.

Per the meaisinfhoghlaim v5 umbrella spec + the 2026-08-13-biep-v3-systematic-download-ireland-england-v1 change.

Runs the canonical 4-path OCR ensemble for the paddleocr-vl-1.6 OCR/VLM model.
The 4 paths are:
- Path 1 (BAML): Docling-serve -> text -> BAML paddleocr-vl-1.6
...
Usage:
    uv run python scripts/meaisin_ocr_htr_tests/ocr_model_paddleocr_vl_1.6_extract.py
...
logger = logging.getLogger("ocr_model_paddleocr_vl_1.6_extract")


def _run_4_path_ensemble() -> bool:
    """Run the 4-path OCR ensemble for the paddleocr-vl-1.6 model."""
    logger.info("Running 4-path OCR ensemble for paddleocr-vl-1.6...")
    try:
        ...
        model = VISION_MODELS.get("paddleocr-vl-1.6")
        if model is None or not model.available:
            logger.warning("paddleocr-vl-1.6 not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the paddleocr-vl-1.6 OCR/VLM model. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("OCR/VLM model entrypoint: paddleocr-vl-1.6")
    logger.info("=" * 60)
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("paddleocr-vl-1.6 entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("paddleocr-vl-1.6 OCR/VLM entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
