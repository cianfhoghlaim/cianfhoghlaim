"""OCR/VLM model entrypoint script for deepseek-ocr-2.

Per the meaisinfhoghlaim v5 umbrella spec + the 2026-08-13-biep-v3-systematic-download-ireland-england-v1 change.

Runs the canonical 4-path OCR ensemble for the deepseek-ocr-2 OCR/VLM model.
The 4 paths are:
...
Usage:
    uv run python scripts/meaisin_ocr_htr_tests/ocr_model_deepseek_ocr_2_extract.py
...
logger = logging.getLogger("ocr_model_deepseek_ocr_2_extract")


def _run_4_path_ensemble() -> bool:
    """Run the 4-path OCR ensemble for the deepseek-ocr-2 model."""
    logger.info("Running 4-path OCR ensemble for deepseek-ocr-2...")
    try:
        ...
        model = VISION_MODELS.get("deepseek-ocr-2")
        if model is None or not model.available:
            logger.warning("deepseek-ocr-2 not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the deepseek-ocr-2 OCR/VLM model. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("OCR/VLM model entrypoint: deepseek-ocr-2")
    logger.info("=" * 60)
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("deepseek-ocr-2 entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("deepseek-ocr-2 OCR/VLM entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
