...
logger = logging.getLogger("converter_docling_extract")


def _run_converter() -> bool:
    """Run the docling converter on a sample PDF."""
    logger.info("Running docling converter on sample PDF...")
    try:
        from meaisinfhoghlaim.document_factory import CONVERTERS
        converter = CONVERTERS.get("docling")
...
def main() -> int:
    """Run the 3 steps for the docling converter. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("Document converter entrypoint: docling")
    logger.info("=" * 60)
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("docling entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("docling converter entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
