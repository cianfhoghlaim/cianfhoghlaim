...
logger = logging.getLogger("agent_corpus_extract")


def _run_agent() -> bool:
    """Run the corpus agent."""
    logger.info("Running corpus agent...")
    try:
        from agents.meaisinfhoghlaim.registry import AGENTS

        agent = AGENTS.get("corpus")
        if agent is None:
            logger.warning("corpus not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the corpus agent. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("Agent entrypoint: corpus")
    logger.info("=" * 60)

    steps = [
        ("1. corpus agent", _run_agent),
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("corpus entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("corpus agent entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
