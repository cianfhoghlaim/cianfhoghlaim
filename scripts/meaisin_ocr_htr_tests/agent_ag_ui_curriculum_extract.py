...
logger = logging.getLogger("agent_ag_ui_curriculum_extract")


def _run_agent() -> bool:
    """Run the ag_ui_curriculum agent."""
    logger.info("Running ag_ui_curriculum agent...")
    try:
        from agents.meaisinfhoghlaim.registry import AGENTS

        agent = AGENTS.get("ag_ui_curriculum")
        if agent is None:
            logger.warning("ag_ui_curriculum not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the ag_ui_curriculum agent. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("Agent entrypoint: ag_ui_curriculum")
    logger.info("=" * 60)

    steps = [
        ("1. ag_ui_curriculum agent", _run_agent),
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("ag_ui_curriculum entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("ag_ui_curriculum agent entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
