...
logger = logging.getLogger("agent_site_analysis_extract")


def _run_agent() -> bool:
    """Run the site_analysis agent."""
    logger.info("Running site_analysis agent...")
    try:
        from agents.meaisinfhoghlaim.registry import AGENTS

        agent = AGENTS.get("site_analysis")
        if agent is None:
            logger.warning("site_analysis not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the site_analysis agent. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("Agent entrypoint: site_analysis")
    logger.info("=" * 60)

    steps = [
        ("1. site_analysis agent", _run_agent),
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("site_analysis entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("site_analysis agent entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
