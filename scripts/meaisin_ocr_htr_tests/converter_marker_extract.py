...
logger = logging.getLogger("converter_marker_extract")


def _run_converter() -> bool:
    """Run the marker converter on a sample PDF."""
    logger.info("Running marker converter on sample PDF...")
    try:
        from meaisinfhoghlaim.document_factory import CONVERTERS
        converter = CONVERTERS.get("marker")
        if converter is None:
            logger.warning("marker not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the marker converter. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("Document converter entrypoint: marker")
    logger.info("=" * 60)
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("marker entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("marker converter entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
