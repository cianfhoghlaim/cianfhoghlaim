"""OCR/VLM model entrypoint script for docling-serve.

Per the meaisinfhoghlaim v5 umbrella spec + the 2026-08-13-biep-v3-systematic-download-ireland-england-v1 change.

Runs the canonical 4-path OCR ensemble for the docling-serve OCR/VLM model.
The 4 paths are:
- Path 1 (BAML): Docling-serve -> text -> BAML docling-serve
...
Usage:
    uv run python scripts/meaisin_ocr_htr_tests/ocr_model_docling_serve_extract.py
...
logger = logging.getLogger("ocr_model_docling_serve_extract")


def _run_4_path_ensemble() -> bool:
    """Run the 4-path OCR ensemble for the docling-serve model."""
    logger.info("Running 4-path OCR ensemble for docling-serve...")
    try:
        ...
        model = VISION_MODELS.get("docling-serve")
        if model is None or not model.available:
            logger.warning("docling-serve not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the docling-serve OCR/VLM model. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("OCR/VLM model entrypoint: docling-serve")
    logger.info("=" * 60)
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("docling-serve entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("docling-serve OCR/VLM entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
