"""meaisinfhoghlaim Agent entrypoint script for bunchloch_research.

Per the meaisinfhoghlaim v5 umbrella spec + the 2026-08-13-biep-v3-systematic-download-ireland-england-v1 change.

Runs the canonical agent framework pipeline for the bunchloch_research agent.
The agent is registered in the meaisinfhoghlaim 12-agent framework
at `agents/meaisinfhoghlaim/registry.py`.

Usage:
    uv run python scripts/meaisin_ocr_htr_tests/agent_bunchloch_research_extract.py
"""
...
logger = logging.getLogger("agent_bunchloch_research_extract")


def _run_agent() -> bool:
    """Run the bunchloch_research agent."""
    logger.info("Running bunchloch_research agent...")
    try:
        from agents.meaisinfhoghlaim.registry import AGENTS
        agent = AGENTS.get("bunchloch_research")
        if agent is None:
            logger.warning("bunchloch_research not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the bunchloch_research agent. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("Agent entrypoint: bunchloch_research")
    logger.info("=" * 60)

    steps = [
        ("1. bunchloch_research agent", _run_agent),
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("bunchloch_research entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("bunchloch_research agent entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
