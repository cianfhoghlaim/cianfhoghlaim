"""OCR/VLM model entrypoint script for glm-4.6v-flash.

Per the meaisinfhoghlaim v5 umbrella spec + the 2026-08-13-biep-v3-systematic-download-ireland-england-v1 change.

Runs the canonical 4-path OCR ensemble for the glm-4.6v-flash OCR/VLM model.
The 4 paths are:
- Path 1 (BAML): Docling-serve -> text -> BAML glm-4.6v-flash
...
Usage:
    uv run python scripts/meaisin_ocr_htr_tests/ocr_model_glm_4.6v_flash_extract.py
...
logger = logging.getLogger("ocr_model_glm_4.6v_flash_extract")


def _run_4_path_ensemble() -> bool:
    """Run the 4-path OCR ensemble for the glm-4.6v-flash model."""
    logger.info("Running 4-path OCR ensemble for glm-4.6v-flash...")
    try:
        ...
        model = VISION_MODELS.get("glm-4.6v-flash")
        if model is None or not model.available:
            logger.warning("glm-4.6v-flash not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the glm-4.6v-flash OCR/VLM model. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("OCR/VLM model entrypoint: glm-4.6v-flash")
    logger.info("=" * 60)
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("glm-4.6v-flash entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("glm-4.6v-flash OCR/VLM entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
