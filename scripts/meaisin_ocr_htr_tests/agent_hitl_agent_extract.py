...
logger = logging.getLogger("agent_hitl_agent_extract")


def _run_agent() -> bool:
    """Run the hitl_agent agent."""
    logger.info("Running hitl_agent agent...")
    try:
        from agents.meaisinfhoghlaim.registry import AGENTS
        agent = AGENTS.get("hitl_agent")
        if agent is None:
            logger.warning("hitl_agent not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the hitl_agent agent. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("Agent entrypoint: hitl_agent")
    logger.info("=" * 60)

    steps = [
        ("1. hitl_agent agent", _run_agent),
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("hitl_agent entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("hitl_agent agent entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
