...
logger = logging.getLogger("agent_curriculum_extract")


def _run_agent() -> bool:
    """Run the curriculum agent."""
    logger.info("Running curriculum agent...")
    try:
        from agents.meaisinfhoghlaim.registry import AGENTS

        agent = AGENTS.get("curriculum")
        if agent is None:
            logger.warning("curriculum not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the curriculum agent. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("Agent entrypoint: curriculum")
    logger.info("=" * 60)

    steps = [
        ("1. curriculum agent", _run_agent),
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("curriculum entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("curriculum agent entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
