"""OCR/VLM model entrypoint script for qwen3.6-27b-mtp.

Per the meaisinfhoghlaim v5 umbrella spec + the 2026-08-13-biep-v3-systematic-download-ireland-england-v1 change.

Runs the canonical 4-path OCR ensemble for the qwen3.6-27b-mtp OCR/VLM model.
The 4 paths are:
- Path 1 (BAML): Docling-serve -> text -> BAML qwen3.6-27b-mtp
...
Usage:
    uv run python scripts/meaisin_ocr_htr_tests/ocr_model_qwen3.6_27b_mtp_extract.py
...
logger = logging.getLogger("ocr_model_qwen3.6_27b_mtp_extract")


def _run_4_path_ensemble() -> bool:
    """Run the 4-path OCR ensemble for the qwen3.6-27b-mtp model."""
    logger.info("Running 4-path OCR ensemble for qwen3.6-27b-mtp...")
    try:
        ...
        model = VISION_MODELS.get("qwen3.6-27b-mtp")
        if model is None or not model.available:
            logger.warning("qwen3.6-27b-mtp not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the qwen3.6-27b-mtp OCR/VLM model. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("OCR/VLM model entrypoint: qwen3.6-27b-mtp")
    logger.info("=" * 60)
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("qwen3.6-27b-mtp entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("qwen3.6-27b-mtp OCR/VLM entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
