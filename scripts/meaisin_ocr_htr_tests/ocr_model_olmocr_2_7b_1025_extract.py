"""OCR/VLM model entrypoint script for olmocr-2-7b-1025.

Per the meaisinfhoghlaim v5 umbrella spec + the 2026-08-13-biep-v3-systematic-download-ireland-england-v1 change.

Runs the canonical 4-path OCR ensemble for the olmocr-2-7b-1025 OCR/VLM model.
The 4 paths are:
- Path 1 (BAML): Docling-serve -> text -> BAML olmocr-2-7b-1025
...
Usage:
    uv run python scripts/meaisin_ocr_htr_tests/ocr_model_olmocr_2_7b_1025_extract.py
...
logger = logging.getLogger("ocr_model_olmocr_2_7b_1025_extract")


def _run_4_path_ensemble() -> bool:
    """Run the 4-path OCR ensemble for the olmocr-2-7b-1025 model."""
    logger.info("Running 4-path OCR ensemble for olmocr-2-7b-1025...")
    try:
        ...
        model = VISION_MODELS.get("olmocr-2-7b-1025")
        if model is None or not model.available:
            logger.warning("olmocr-2-7b-1025 not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the olmocr-2-7b-1025 OCR/VLM model. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("OCR/VLM model entrypoint: olmocr-2-7b-1025")
    logger.info("=" * 60)
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("olmocr-2-7b-1025 entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("olmocr-2-7b-1025 OCR/VLM entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
