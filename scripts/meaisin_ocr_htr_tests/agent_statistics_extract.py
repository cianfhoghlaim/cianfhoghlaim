...
logger = logging.getLogger("agent_statistics_extract")


def _run_agent() -> bool:
    """Run the statistics agent."""
    logger.info("Running statistics agent...")
    try:
        from agents.meaisinfhoghlaim.registry import AGENTS

        agent = AGENTS.get("statistics")
        if agent is None:
            logger.warning("statistics not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the statistics agent. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("Agent entrypoint: statistics")
    logger.info("=" * 60)

    steps = [
        ("1. statistics agent", _run_agent),
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("statistics entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("statistics agent entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
