"""OCR/VLM model entrypoint script for llama-3.2-vision-11b.

Per the meaisinfhoghlaim v5 umbrella spec + the 2026-08-13-biep-v3-systematic-download-ireland-england-v1 change.

Runs the canonical 4-path OCR ensemble for the llama-3.2-vision-11b OCR/VLM model.
The 4 paths are:
- Path 1 (BAML): Docling-serve -> text -> BAML llama-3.2-vision-11b
...
Usage:
    uv run python scripts/meaisin_ocr_htr_tests/ocr_model_llama_3.2_vision_11b_extract.py
...
logger = logging.getLogger("ocr_model_llama_3.2_vision_11b_extract")


def _run_4_path_ensemble() -> bool:
    """Run the 4-path OCR ensemble for the llama-3.2-vision-11b model."""
    logger.info("Running 4-path OCR ensemble for llama-3.2-vision-11b...")
    try:
        ...
        model = VISION_MODELS.get("llama-3.2-vision-11b")
        if model is None or not model.available:
            logger.warning("llama-3.2-vision-11b not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the llama-3.2-vision-11b OCR/VLM model. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("OCR/VLM model entrypoint: llama-3.2-vision-11b")
    logger.info("=" * 60)
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("llama-3.2-vision-11b entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("llama-3.2-vision-11b OCR/VLM entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
