"""OCR/VLM model entrypoint script for uccix-mistral-24b.

Per the meaisinfhoghlaim v5 umbrella spec + the 2026-08-13-biep-v3-systematic-download-ireland-england-v1 change.

Runs the canonical 4-path OCR ensemble for the uccix-mistral-24b OCR/VLM model.
The 4 paths are:
- Path 1 (BAML): Docling-serve -> text -> BAML uccix-mistral-24b
...
Usage:
    uv run python scripts/meaisin_ocr_htr_tests/ocr_model_uccix_mistral_24b_extract.py
...
logger = logging.getLogger("ocr_model_uccix_mistral_24b_extract")


def _run_4_path_ensemble() -> bool:
    """Run the 4-path OCR ensemble for the uccix-mistral-24b model."""
    logger.info("Running 4-path OCR ensemble for uccix-mistral-24b...")
    try:
        ...
        model = VISION_MODELS.get("uccix-mistral-24b")
        if model is None or not model.available:
            logger.warning("uccix-mistral-24b not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the uccix-mistral-24b OCR/VLM model. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("OCR/VLM model entrypoint: uccix-mistral-24b")
    logger.info("=" * 60)
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("uccix-mistral-24b entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("uccix-mistral-24b OCR/VLM entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
