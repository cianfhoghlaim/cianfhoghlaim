"""OCR/VLM model entrypoint script for unstract-api.

Per the meaisinfhoghlaim v5 umbrella spec + the 2026-08-13-biep-v3-systematic-download-ireland-england-v1 change.

Runs the canonical 4-path OCR ensemble for the unstract-api OCR/VLM model.
The 4 paths are:
- Path 1 (BAML): Docling-serve -> text -> BAML unstract-api
...
Usage:
    uv run python scripts/meaisin_ocr_htr_tests/ocr_model_unstract_api_extract.py
...
logger = logging.getLogger("ocr_model_unstract_api_extract")


def _run_4_path_ensemble() -> bool:
    """Run the 4-path OCR ensemble for the unstract-api model."""
    logger.info("Running 4-path OCR ensemble for unstract-api...")
    try:
        ...
        model = VISION_MODELS.get("unstract-api")
        if model is None or not model.available:
            logger.warning("unstract-api not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the unstract-api OCR/VLM model. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("OCR/VLM model entrypoint: unstract-api")
    logger.info("=" * 60)
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("unstract-api entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("unstract-api OCR/VLM entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
