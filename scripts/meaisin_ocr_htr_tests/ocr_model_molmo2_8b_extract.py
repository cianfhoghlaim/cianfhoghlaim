"""OCR/VLM model entrypoint script for molmo2-8b.

Per the meaisinfhoghlaim v5 umbrella spec + the 2026-08-13-biep-v3-systematic-download-ireland-england-v1 change.

Runs the canonical 4-path OCR ensemble for the molmo2-8b OCR/VLM model.
The 4 paths are:
- Path 1 (BAML): Docling-serve -> text -> BAML molmo2-8b
...
Usage:
    uv run python scripts/meaisin_ocr_htr_tests/ocr_model_molmo2_8b_extract.py
...
logger = logging.getLogger("ocr_model_molmo2_8b_extract")


def _run_4_path_ensemble() -> bool:
    """Run the 4-path OCR ensemble for the molmo2-8b model."""
    logger.info("Running 4-path OCR ensemble for molmo2-8b...")
    try:
        ...
        model = VISION_MODELS.get("molmo2-8b")
        if model is None or not model.available:
            logger.warning("molmo2-8b not available; skipping")
            return True
...
def main() -> int:
    """Run the 3 steps for the molmo2-8b OCR/VLM model. Exit 0 on success."""
    logger.info("=" * 60)
    logger.info("OCR/VLM model entrypoint: molmo2-8b")
    logger.info("=" * 60)
    ...
    for name, step in steps:
        ...
        if not step():
            logger.error("molmo2-8b entrypoint failed at step %s.", name)
            return 1

    logger.info("=" * 60)
    logger.info("molmo2-8b OCR/VLM entrypoint complete. All 3 steps passed.")
    logger.info("=" * 60)
    return 0
