#!/usr/bin/env bash
# Layer 2: append the 22nd CCC concept guide + reindex
# Per the 2026-08-15-baml-sync-loop-v1 change.
set -uo pipefail
mkdir -p stedding/sync-reports
REPORT="stedding/sync-reports/baml-ccc-$(date +%Y-%m-%d).md"
{
  echo "# BAML CCC Sync Report — $(date -u +%Y-%m-%dT%H:%M:%SZ)"
  echo ""
  echo "## 22nd Concept Guide"
  echo '```'
  python3 scripts/sync_openspec_to_ccc.py 2>&1 | head -3 || echo "(no concept guide script)"
  echo '```'
  echo ""
  echo "(the canonical 'baml-function-search' guide lives at .cocoindex_code/guides.yml)"
  echo ""
