...
def test_examinations_source(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            pipeline.run(source.with_resources("all_exam_materials"))
            elapsed = (datetime.now() - start_time).total_seconds()
...
def test_full_cycle_source(
    ...
    if cycle == "senior_cycle":
        from dlt_sources.british_isles.ireland.education.subjects.senior_cycle import (
        ...
        from dlt_sources.british_isles.ireland.education.subjects.junior_cycle import (
    ...
    results = []
    for _i, subject_slug in enumerate(list(subjects.keys())[:limit_subjects]):
