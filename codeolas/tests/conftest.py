...
def cianfhoghlaim_path():
    ...
    Honours the ``CODEOLAS_REPO_PATH`` environment variable and falls back
    to the current working directory when the variable is not set, so the
    tests do not assume a specific machine layout.
    """
    return Path(os.environ.get("CODEOLAS_REPO_PATH", os.getcwd()))
