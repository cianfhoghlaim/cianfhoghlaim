...
from pipelines.defs._partitions import hourly_partitions
from pipelines.defs._helpers import get_duckdb_path
...
def coingecko_assets(context: AssetExecutionContext) -> Output[dict[str, Any]]:
    ...
    from pipelines.sources.coingecko import coingecko_prices
...
def defillama_assets(context: AssetExecutionContext) -> Output[dict[str, Any]]:
    ...
    from pipelines.sources.defillama import defillama_yields, defillama_protocol_tvl
...
def binance_funding_assets(context: AssetExecutionContext) -> Output[dict[str, Any]]:
    ...
    from pipelines.sources.binance import binance_funding_rates
...
def aave_subgraph_assets(context: AssetExecutionContext) -> Output[dict[str, Any]]:
    ...
    from pipelines.sources.subgraphs import aave_reserves, aave_user_positions
...
def pendle_subgraph_assets(context: AssetExecutionContext) -> Output[dict[str, Any]]:
    ...
    from pipelines.sources.subgraphs import pendle_markets, pendle_swaps
