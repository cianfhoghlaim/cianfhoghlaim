/**
 * useStudyPlan — Phase 1 hook that fetches the canonical Phase 1
 * study-plan response shape from the Hono `/api/copilotkit/lc/<subject>/get_study_plan`
 * endpoint. Returns `{ data, loading, error, request }` so the
 * `<StudyPlanCard>` A2UI surface can render.
 *
 * Per the 2026-09-01-cianfhoghlaim-nua-end-to-end-showcase-v1 change
 * (Phase 1, §3.4 of tasks.md).
 */

import * as React from "react";

import type { StudyPlanCardData } from "../components/study-plan/StudyPlanCard";
