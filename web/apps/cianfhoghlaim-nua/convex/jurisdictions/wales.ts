/**
 * Wales jurisdiction subject-spec table.
 *
 * Per the 2026-09-XX-orchestration-integration-v1 change (Phase 11 §3).
 * The WLSubjectSpec row from `b.ExtractWalesSubjectSpec` is materialised
 * here by the canonical Python orchestrator at
...
export const wales_subject_specs = defineTable({
  jurisdiction: v.literal("wales"),
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    const id = await ctx.db.insert("wales_subject_specs", {
