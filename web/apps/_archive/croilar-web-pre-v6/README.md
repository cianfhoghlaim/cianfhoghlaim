...
> **reference implementation** for the rest of `cianfhoghlaim`.
...
| 2 | **RESOLVED 2026-06-26 (Round 11 Phase 12 / croilar audit Phase 1).** `sruth/croilar/dlt_utils/destinations.py` (126 lines) was a defensive shim with an 88-line local fallback that duplicated pre-Phase-2.3 DuckLake logic (only triggered when `oideachais` was not on sys.path). The precondition (croilar packaging fix from issue #17 / commit `e9e0fc7d2`) is now in place: `croilar/__init__.py` exists + pyproject `packages = ["."]` + post-install `croilar/scripts/fix-pth.sh` ensures `oideachais` is on sys.path. Deleted the local fallback; rewrote `destinations.py` to a 13-line canonical-only shim: `from sruth.oideachais.dlt_utils.destinations import with_namespace; with_namespace("croilar").re_export_into(globals())`. Updated `sruth/croilar/dlt_utils/__init__.py` to re-export the canonical 4-name surface (`NAMESPACE`, `create_pipeline`, `get_dlt_destination`, `get_duckdb_fallback_destination`) instead of the old fallback's surface (`DuckLakeConfig`, `get_duckdb_fallback`, etc.). The pre-existing test failures documented in Known issues #3 (3 broken tests in `tests/test_smoke.py` + `tests/dlt_assets/test_spotify_soundcloud_labels.py`) are unchanged — those predate Phase 12 and are out of scope. | Round 11 Phase 12 (`croilar-audit-phase-1-delete-dlt-utils-fallback`) | **closed** |
...
| 4 | **RESOLVED 2026-06-26 (Round 11 Phase 13 / croilar audit Phase 2).** `sruth/croilar/pipelines/shared/destinations.py` (126 lines) was a drift duplicate of the canonical `sruth/oideachais/dlt_utils/destinations.py` (it had its own 4-function destination factory — `create_duckdb_destination`, `create_ducklake_destination`, `create_lancedb_destination`, `get_destination_for_env`). `sruth/croilar/pipelines/shared/ducklake.py` (352 lines) was a dead `DuckLakeCatalog` class with zero production callers. Deleted both; rewrote `pipelines/shared/__init__.py` to re-export only `R2Client` (the only production-relevant symbol — used by `pipelines/soundcloud/downloader.py:18`). Patched `tests/test_smoke.py:133-137` (`test_pipelines_shared_exports`) to assert only `R2Client`. After this change `pipelines/shared/` contains only `__init__.py` (1 line) + `r2_client.py` (187 lines). | Round 11 Phase 13 (`croilar-audit-phase-2-delete-pipelines-shared-drift`) | **closed** |
| 5 | **RESOLVED 2026-06-26 (Round 11 Phase 14 / croilar audit Phase 3).** `sruth/croilar/_shared/` had 4 dead subdirs left over from the pre-Croílár Aleyum agent framework: `observability/` (481 lines — `AleyumTracer` shadow of `sruth/oideachais/observability/unified_tracer.py`), `agents/` (372 lines — `AgentRouter` shadow of `sruth/meaisinfhoghlaim/agents/`), `mcp/` (419 lines — `MCPGateway` shadow of `sruth/oideachais/mcp/filesystem/`), `embeddings/` (124 lines — `EmbeddingBatcher` shadow of `sruth/codeolas/core/embeddings.py`). Each had zero production callers — the parent `_shared/__init__.py` even had them as commented-out imports (lines 42-44): `# from .mcp import MCPGateway` etc. Deleted all 4 subdirs (1396 lines total); trimmed the parent `__init__.py` docstring (removed the "embeddings, MCP gateway, agent orchestration, and observability" clause) + removed the 3 commented-out sibling import lines. After this change `_shared/` contains only `__init__.py` + `streams.py` + `config/` + `database/` (the kept production-relevant modules). | Round 11 Phase 14 (`croilar-audit-phase-3-delete-dead-shared-subdirs`) | **closed** |
...
new subproject in `cianfhoghlaim` (e.g. a *consulting* persona for
...
`cianfhoghlaim` is a polyglot monorepo with four large subprojects
...
If a future `cianfhoghlaim` subproject (`oideachais-portal`,
...
deliberately *not* the only project in `cianfhoghlaim`; it is
