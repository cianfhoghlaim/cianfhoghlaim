// /en/subjects/english — English per-subject interactive landing page.
//
...
// This is the production landing page for the English subject,
// rendered alongside (and superseding for interactive use) the flat
// `routes/en/subjects/english.tsx` file.
//
...
export const Route = createFileRoute("/en/subjects/english/")({
  component: EnglishPerSubjectLanding,
});

function EnglishPerSubjectLanding() {
  if (!isBIEPSubject("english")) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <nav
        aria-label="English interactive sub-routes"
        className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 px-6"
      >
        <SubRouteCard
          to="/en/subjects/english/syllabus"
          title="Syllabus"
          blurb="NCCA syllabus topics + learning outcomes"
        />
        <SubRouteCard
          to="/en/subjects/english/exam-papers"
          title="Past exam papers"
          blurb="All English past exam papers tagged by topic + paper + year"
        />
        <SubRouteCard
          to="/en/subjects/english/marking-schemes"
          title="Marking schemes"
          blurb="PCLM (Partial Credit, Logical Marking) patterns"
        />
        <SubRouteCard
          to="/en/subjects/english/study-plan"
          title="Study plan"
          blurb="Generate a per-subject study plan via the BAML backend"
        />
      </nav>
      <div className="px-6 text-sm text-slate-500">
        <Link to="/ga/subjects/bearla">Gaeilge: Béarla →</Link>
      </div>
    </div>
  );
}
