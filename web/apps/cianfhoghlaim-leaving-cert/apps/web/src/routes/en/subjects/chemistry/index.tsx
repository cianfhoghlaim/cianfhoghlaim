// /en/subjects/chemistry — Chemistry per-subject interactive landing page.
//
...
// This is the production landing page for the Chemistry subject,
// rendered alongside (and superseding for interactive use) the flat
// `routes/en/subjects/chemistry.tsx` file.
//
...
export const Route = createFileRoute("/en/subjects/chemistry/")({
  component: ChemistryPerSubjectLanding,
});

function ChemistryPerSubjectLanding() {
  if (!isBIEPSubject("chemistry")) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <nav
        aria-label="Chemistry interactive sub-routes"
        className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 px-6"
      >
        <SubRouteCard
          to="/en/subjects/chemistry/syllabus"
          title="Syllabus"
          blurb="NCCA syllabus topics + learning outcomes"
        />
        <SubRouteCard
          to="/en/subjects/chemistry/exam-papers"
          title="Past exam papers"
          blurb="All Chemistry past exam papers tagged by topic + paper + year"
        />
        <SubRouteCard
          to="/en/subjects/chemistry/marking-schemes"
          title="Marking schemes"
          blurb="PCLM (Partial Credit, Logical Marking) patterns"
        />
        <SubRouteCard
          to="/en/subjects/chemistry/study-plan"
          title="Study plan"
          blurb="Generate a per-subject study plan via the BAML backend"
        />
      </nav>
      <div className="px-6 text-sm text-slate-500">
        <Link to="/ga/subjects/ceimic">Gaeilge: Ceimic →</Link>
      </div>
    </div>
  );
}
