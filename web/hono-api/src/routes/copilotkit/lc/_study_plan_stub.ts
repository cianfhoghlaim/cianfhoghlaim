 ...
 * (Phase 1, §3.2 of tasks.md). The shared TypeScript stub for the
 * 4 Phase 1 subjects' `POST /get_study_plan` Hono routes. Returns
 * the canonical response shape that the Python planner at
 * `agents/adk/subjects/lc/planner.py::generate_study_plan(...)`
 * produces, marked with a `phase` field so callers know this is the
 * Phase 1 stub. Replaced by a real HTTP bridge to the Python
 * planner service when the planner is deployed as a sidecar.
 */

export interface StudyPlanStubParams {
  user_id?: string;
  trace_id?: string;
  lo_codes?: string[];
  target_date?: string;
...
export interface StudyPlanWeekStub {
  week_number: number;
...
export function studyPlanStubResponse(
  ...
  const weeks_plan: StudyPlanWeekStub[] = Array.from({ length: duration }, (_, i) => {
    const week = i + 1;
    return {
      week_number: week,
      theme: {
