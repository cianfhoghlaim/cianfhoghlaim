...
Pocket ID OIDC setup on arm1-oci (the bons-iac's `sync:resources`
command needs a Pangolin session cookie via Pocket ID OIDC, which isn't
configured yet).
...
- **Scopes**: bons-iac can read `dev-baile/dev/{infisical,openclaw,hermes,
...
| Bons-iac identity | local bons-iac | production bons-iac |
| Network | Direct from host + containers via `host.docker.internal` | Via Pangolin private resource on arm1-oci |
| Locket compatibility | ✅ WORKS with `bons-locker-shim:0.2.0` (avoids the snake_case/camelCase bug) | ❌ BROKEN — locket v0.17.3 uses snake_case; Infisical v0.161+ requires camelCase |
...
3. The 4 locket-shims (using the bons-locker-shim:0.2.0) work correctly
...
2. The bons-iac is upgraded to mint fresh API keys against the OCI vault
...
- Bons-iac credential: `~/.locket/infisical_secret` (mode 0600)
- Bons-iac shim: `bonneagar/scripts/bons-locket-shim.py` (v0.2.0)
- Bons-iac image: `bons-locker-shim:infisical-0.2.0` (local Docker image)
