...
const CONFIG = {
  ...
  sshPublicKeyPath: process.env.SSH_PUBLIC_KEY_PATH ?? path.join(process.env.HOME ?? "/root", ".ssh", "ansible.pub"),
  // OCI authentication
  ociProfile: process.env.OCI_CLI_PROFILE ?? "bunchloch",
  ociConfigFile: process.env.OCI_CLI_CONFIG_FILE ?? path.join(process.env.HOME ?? "/root", ".oci", "config"),
  // Infisical
...
// =============================================================================
// INFISICAL HELPERS (uses iac/clients/infisical-rest.ts — direct REST)
// =============================================================================
...
function getInfisicalSecret(): string {
  ...
  const tokenPath = path.join(
    __dirname, "..", "..", "pangolin", "pangolin-core", "config", "secrets", "infisical_secret",
  );
...
/**
 * Load Cloudflare credentials from Infisical (direct REST, no SDK)
 */
async function loadCloudflareCredentials(): Promise<{ apiToken: string; zoneId: string } | null> {
  ...
  if (!CONFIG.infisicalClientId || !CONFIG.infisicalClientSecret) {
    console.log("Infisical credentials not set, DNS records will not be created");
    return null;
  }
  ...
  try {
    ...
    if (apiTokenSecret && zoneIdSecret) {
      console.log(`Loaded Cloudflare credentials from Infisical (zone: ${zoneIdSecret.value})`);
...
async function saveToInfisical(outputs: Record<string, { value: unknown }>) {
  if (!CONFIG.infisicalClientId || !CONFIG.infisicalClientSecret) {
    console.log("Infisical credentials not set, skipping saveToInfisical");
    return;
  }
