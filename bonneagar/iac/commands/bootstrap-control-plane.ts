...
export async function bootstrapControlPlane(opts?: BootstrapControlPlaneOpts): Promise<{ ts: string; target: string; results: Record<string, unknown> }> {
  ...
  try {
    ...
    try {
      ...
      for (const stack of ["infisical", "pangolin", "pocket-id", "tinyauth", "komodo"]) {
        try {
          execSync(`cd /Users/cianmacandeisigh/dev/cianfhoghlaim/bonneagar/stacks/${stack} && docker compose up -d 2>&1 | tail -5`, { stdio: "inherit" });
        } catch {
  ...
  try {
    execSync("cd /Users/cianmacandeisigh/dev/cianfhoghlaim/bonneagar && bun run iac:health 2>&1 | tail -20", { stdio: "inherit" });
