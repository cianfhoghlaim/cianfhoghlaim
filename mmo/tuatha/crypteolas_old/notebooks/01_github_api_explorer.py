...
3. Using custom dlt marimo widgets
...
def _(mo):
    ...
    1. Set up your GitHub token in `.dlt/secrets.toml`:
    ```toml
    [sources.github_api]
    ...
    3. (Optional) Issue dlthub license for transformations:
    ```bash
...
@app.cell
def _(mo):
    owner_input = mo.ui.text(value="dlt-hub", label="Owner")
    repo_input = mo.ui.text(value="dlt", label="Repository")
...
def _(mo, owner_input, repo_input, resources_select, run_button):
    ...
    if run_button.value:
        ...
        import os
        access_token = os.environ.get("GITHUB_ACCESS_TOKEN", dlt.secrets.get("sources.github_api.access_token", ""))
...
def _(load_info, mo):
    if load_info:
        ...
        **Loaded tables:** {', '.join([t['name'] for pkg in load_info.load_packages for t in pkg.get('tables', [])])}
        """)
    return
...
def _(mo):
    ...
    dlt provides custom widgets for exploring pipelines and data:
    """)
    return
...
async def _(mo):
    try:
        ...
        mo.md("*dlt marimo widgets not available. Update dlt to latest version.*")
    return
...
def _(mo):
    ...
    - **Knowledge Graph**: Run `03_knowledge_graph.py` to build and query knowledge graphs with Memgraph
    - **Unified Dashboard**: Run `04_unified_dashboard.py` for a combined view of all data sources
