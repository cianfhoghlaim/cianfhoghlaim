...
description: Router for the data engineering pipeline documentation. The single source of truth for the BAML × DLT × Dagster × CocoIndex matrix is `sruth/cianfhoghlaim/STATUS.md`; the refactor backlog is `sruth/cianfhoghlaim/REFACTORING.md`; the per-area READMEs (`sruth/cianfhoghlaim/README.md`, `sruth/cianfhoghlaim/api/README.md`, etc.) document specific subsystems. Use when you need to add a new BAML extraction, wire a new DLT source, add a new Dagster asset, check the per-source status, or find the right README for a question. Triggers: 'BAML × dlt × Dagster matrix', 'STATUS.md', 'REFACTORING.md', 'pipeline status', 'data engineering doc', 'extraction schema', 'where is the lakehouse README', 'pipeline architecture doc', 'celtic-asset-generation'.
---
...
|:--|:--|:--|
| `sruth/cianfhoghlaim/STATUS.md` | The single source of truth for the BAML × dlt × Dagster × CocoIndex matrix. Every source has a status (DONE / TODO / BLOCKED) and a notes column. | "What's the status of source X?" "Which BAML extractions are wired?" "Which dlt sources have Dagster asset wrappers?" |
| `sruth/cianfhoghlaim/REFACTORING.md` | The refactor backlog. Every proposed change has a status, an owner, and a deadline. | "What's on the roadmap?" "Who owns refactor Y?" "What's blocked on what?" |
| `sruth/cianfhoghlaim/README.md` | The quadrant-level overview. Architecture layers, data contracts, the 4 storage layers, the 7 Quick navigation table. | "Where do I add a new DLT source?" "What are the data contracts?" "How does the lakehouse work?" |
| `sruth/cianfhoghlaim/api/README.md` (and per-area READMEs) | Per-subsystem documentation. | "How do I add a new FastAPI route?" "How does the Dagster webserver start?" |
...
1. **BAML extraction** — `sruth/cianfhoghlaim/baml_src/`
2. **CocoIndex v1 embedding** — `sruth/cianfhoghlaim/cocoindex_flows/`
3. **Cognee cognify** — `sruth/cianfhoghlaim/cognee_integration/`
4. **Graphiti temporal memory** — `sruth/cianfhoghlaim/memory/`
5. **LanceDB vector search** — `sruth/cianfhoghlaim/cocoindex_flows/leabharlann_embedding.py`
...
- `kcg-leabharlann-pipeline/SKILL.md` — the KCG-leabharlann
  variant
- `cianfhoghlaim-storage/SKILL.md` — the storage mental model
...
- `sruth/cianfhoghlaim/STATUS.md` — the matrix
- `sruth/cianfhoghlaim/REFACTORING.md` — the backlog
- `sruth/cianfhoghlaim/README.md` — the quadrant overview
