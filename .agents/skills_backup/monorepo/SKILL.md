...
Only 3 TypeScript workspaces (all in `sruth/oideachais/` and
...
```bash
mkdir -p sruth/oideachais/<new-member>
cd sruth/oideachais/<new-member>
bun init
# Add to root package.json:
#   "workspaces": ["sruth/oideachais/<new-member>"]
bun install
```
...
#   [tool.uv.workspace]
#   members = ["sruth/oideachais/<new-member>"]
uv sync
```
...
CLI**. Every `sruth/` frontend (`sruth/oideachais/web`,
...
frontends (e.g. `sruth/oideachais/web` + `sruth/croilar/portal` +
