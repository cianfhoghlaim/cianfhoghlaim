...
// package.json
{
  "workspaces": [
    ...
    "sruth/cianfhoghlaim/mcp/filesystem",
    "sruth/tuatha/ui"
  ]
}
```

Only 3 TypeScript workspaces (all in `sruth/cianfhoghlaim/` and
`sruth/tuatha/` quadrants).
...
```bash
mkdir -p sruth/cianfhoghlaim/<new-member>
cd sruth/cianfhoghlaim/<new-member>
bun init
# Add to root package.json:
#   "workspaces": ["sruth/cianfhoghlaim/<new-member>"]
bun install
```
...
#   [tool.uv.workspace]
#   members = ["sruth/cianfhoghlaim/<new-member>"]
uv sync
```
...
CLI**. Every `sruth/` frontend (`sruth/cianfhoghlaim/web`,
...
frontends (e.g. `sruth/cianfhoghlaim/web` + `sruth/croilar/portal` +
