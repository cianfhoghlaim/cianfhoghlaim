...
The Cianfhoghlaim project uses Convex as the primary real-time backend for `sruth/cianfhoghlaim/` (curriculum data, student progress), `tuath/` (game state, quest tracking), and `sruth/crypteolas/` (real-time DeFi data). The `better-auth-convex` integration shows how to combine Convex's reactivity with BetterAuth's session management — the exact pattern used across all sruth/ frontends. The generative AI canvas example (SolidJS + Cloudflare Workers + Convex) demonstrates the multi-service architecture used in `tuath/`. The image generation system guide informs the AI agent orchestration patterns used for educational content generation.
