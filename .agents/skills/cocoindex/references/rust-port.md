...
is in `oideachais/cocoindex_flows/rust/` and at the upstream
...
- `oideachais/cocoindex_flows/rust/text_embedding/src/main.rs` —
  mirror of `oideachais/cocoindex_flows/leabharlann_embedding.py`
  (text embedding + LanceDB target)

## Running a Rust port

```bash
cd oideachais/cocoindex_flows/rust/text_embedding
