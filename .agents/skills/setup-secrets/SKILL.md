---
name: setup-secrets
description: "DEPRECATED — canonical replacement is `secrets-management` (Infisical + Locket + mise three-way contract). Manages dlthub secrets in *.secrets.toml. Use when configuring API keys, database passwords, or tokens via dlt.secrets. Do NOT use for pipeline creation or debugging."
...
> replacement is `secrets-management` at
> `.agents/skills/secrets-management/SKILL.md`. Use the canonical
> `secrets-management` skill (Infisical + Locket + mise three-way contract)
> for new work.
