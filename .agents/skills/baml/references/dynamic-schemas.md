...
- **Site analysis** — `oideachais/baml_src/site_analysis.baml`
  extracts site properties (CMS, captcha, robots.txt) where the
  exact set of properties varies by site
- **UI component extraction** — `oideachais/baml_src/ui_components.baml`
  can adapt to novel UI patterns (e.g. a new dashboard layout)
- **Portfolio extraction** — `oideachais/baml_src/portfolio_extraction.baml`
