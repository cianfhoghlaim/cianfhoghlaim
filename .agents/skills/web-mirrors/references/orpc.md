...
oRPC is used as the API layer across all Cianfhoghlaim TypeScript applications — the `turborepo-based` monorepo pattern in `orpc_query` directly matches the project's own monorepo structure. The `learn-orpc` example demonstrates OpenAPI auto-generation from oRPC procedures — the pattern used for the `oideachais/` API. The `orpc-multiservice-monorepo-playground` shows the multi-service architecture used for the `api-unified` example that combines MCP, oRPC, OpenAPI, and AI streaming. End-to-end type safety between TanStack Start frontends and Hono backends is powered by oRPC's contract-first design.
