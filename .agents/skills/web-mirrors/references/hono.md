...
Hono is used as the HTTP framework throughout the Cianfhoghlaim stack: API gateways on Cloudflare Workers, MCP server endpoints, and the `api-unified` example that combines MCP + oRPC + OpenAPI + AI streaming in a single Hono app. The `cloudflare-auth-worker` shows the auth pattern used for securing API routes. The `duckdb-api` example demonstrates querying analytical databases from edge functions — the pattern used for curriculum analytics in `oideachais/`. The `hn-summary` example shows AI-powered content summarization at the edge, directly informing the content generation pipelines.
