...
description: "The 5-layer pull-based sync architecture that keeps all 8 knowledge surfaces in the cianfhoghlaim repo in sync — openspec changes/specs, agent skills, MCP servers, code (CCC), docs (Cognee), Dagster, agent definitions, and the lakehouse. Use when the user asks 'how do I keep openspec in sync with the rest of the repo', 'how do I sync skills + MCP + CCC + Cognee', 'what's the knowledge graph of openspec changes', 'how do I grow the knowledge surface over time', or 'where is the sync health dashboard'. Per the 2026-08-15-knowledge-sync-loop-v1 change. Triggers: 'sync:paths', 'sync:ccc', 'sync:cognee', 'sync:skills', 'sync:mcp', 'sync:all', 'stedding/sync-reports/', 'deployment control panel', 'notebook 24'."
---
