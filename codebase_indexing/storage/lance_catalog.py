"""
Lance Catalog for Códeolas.

Provides LanceDB namespace management for code embeddings.
Manages embedding tables for code chunks with proper HNSW index handling.
...
Adapted from sruth/crypteolas/storage/lance_catalog.py
"""
...
class LanceCatalog:
    ...
    def add_code_chunks(
        ...
        if batch_size < self.config.min_batch_size:
            logger.warning(
                f"Batch size {batch_size} below minimum {self.config.min_batch_size}, "
                f"using minimum"
            )
