...
class HybridSearchEngine:
    ...
    async def _vector_search(
        ...
        if not content_types or any(t in ["mythology", "character", "story", "location"] for t in (content_types or [])):
            try:
    ...
    async def _graph_search(
        ...
        try:
            ...
            for r in graph_results:
                ...
                related = await self._get_related_entities(
                    r.fact, config.relationship_depth
                ) if config.include_relationships else []
    ...
    async def _hybrid_search(
        ...
        for rank, result in enumerate(graph_results):
            ...
            if key in fused_scores:
                ...
                merged.related_entities = list(set(
                    existing_result.related_entities + result.related_entities
                ))
