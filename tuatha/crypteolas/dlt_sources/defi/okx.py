    ...
    with _get_client() as client:
        for inst_id in inst_ids:
            try:
                ...
                if data.get("code") != "0":
                    import logging
                    logging.warning(f"OKX funding history API error for {inst_id}: {data.get('msg')}")
                    continue
                ...
                for rate in data.get("data", []):
                    ...
                    yield {
                        ...
                        "realized_rate": float(rate.get("realizedRate", 0)) if rate.get("realizedRate") else None,
            ...
            try:
                ...
                for rate in data.get("data", []):
                    ...
                    yield {
                        "inst_id": inst_id,
                        "funding_rate": float(rate.get("fundingRate", 0)),
                        "next_funding_rate": float(rate.get("nextFundingRate", 0)) if rate.get("nextFundingRate") else None,
                        "next_funding_time": datetime.fromtimestamp(
                            next_time / 1000, tz=timezone.utc
                        ).isoformat() if next_time else None,
                        "exchange": "okx",
                        "data_status": "success",
                        "fetched_at": datetime.now(timezone.utc).isoformat(),
                    }
