...
def _(mo):
    ...
    mo.vstack([
        ...
        mo.ui.table(df),
        mo.md("*Full database available in `sruth/tuath/dlt_sources/mythology/celtic_mythology.py`*"),
    ])
...
def _(creature_name, magic, mo, strength, tradition_select, type_select, wisdom):
    ...
    def generate_creature_svg(name: str, tradition: str, ctype: str,
                               str_val: int, wis_val: int, mag_val: int) -> str:
...
def _(mo):
    ...
    checklist = mo.ui.array([
        mo.ui.checkbox(label=task, value=done)
    ...
    mo.vstack([
        ...
        mo.md("**Next:** [Challenge 1: Language Staking](./01_language_staking.py)") if completed == total else None,
    ])
