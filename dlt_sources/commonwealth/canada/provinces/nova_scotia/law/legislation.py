"""DLT source for the Nova Scotia Legislation.
...
``stedding/ingest_queue/commonwealth/can/ns/law/<lang>/``.
"""
...
DOMAIN = "law"
SLUG = "legislation"
CANONICAL_URL = "https://nslegislature.ca/legc"
DEFAULT_LANGUAGE = "en"
...
def ns_legislation(language=None):
    """Yield Nova Scotia Legislation rows from the canonical cache."""
    import hashlib
    import json
    from datetime import UTC, datetime
    from pathlib import Path

    cache_dir = Path("stedding/ingest_queue/commonwealth/can/ns/law")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("ns_legislation_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="ns_legislation")
def ns_legislation_source(language=None):
    """DLT source for the Nova Scotia Legislation ingestion."""
    return ns_legislation(language=language)
