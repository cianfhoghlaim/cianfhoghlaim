"""DLT source for the Alberta Legislation.
...
``stedding/ingest_queue/commonwealth/can/ab/law/<lang>/``.
"""
...
DOMAIN = "law"
SLUG = "legislation"
CANONICAL_URL = "https://www.alberta.ca/laws-and-justice"
DEFAULT_LANGUAGE = "en"
...
def ab_legislation(language=None):
    """Yield Alberta Legislation rows from the canonical cache."""
    import hashlib
    import json
    from datetime import UTC, datetime
    from pathlib import Path

    cache_dir = Path("stedding/ingest_queue/commonwealth/can/ab/law")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("ab_legislation_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="ab_legislation")
def ab_legislation_source(language=None):
    """DLT source for the Alberta Legislation ingestion."""
    return ab_legislation(language=language)
