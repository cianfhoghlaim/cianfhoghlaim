"""DLT source for the Yukon Legislation.
...
``stedding/ingest_queue/commonwealth/can/yt/law/<lang>/``.
"""
...
DOMAIN = "law"
SLUG = "legislation"
CANONICAL_URL = "https://laws.yukon.ca"
DEFAULT_LANGUAGE = "en"
...
def yt_legislation(language=None):
    """Yield Yukon Legislation rows from the canonical cache."""
    import hashlib
    import json
    from datetime import UTC, datetime
    from pathlib import Path

    cache_dir = Path("stedding/ingest_queue/commonwealth/can/yt/law")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("yt_legislation_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="yt_legislation")
def yt_legislation_source(language=None):
    """DLT source for the Yukon Legislation ingestion."""
    return yt_legislation(language=language)
