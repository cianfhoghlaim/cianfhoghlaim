"""DLT source for the Manitoba Legislation.
...
``stedding/ingest_queue/commonwealth/can/mb/law/<lang>/``.
"""
...
DOMAIN = "law"
SLUG = "legislation"
CANONICAL_URL = "https://web2.gov.mb.ca/laws"
DEFAULT_LANGUAGE = "en"
...
def mb_legislation(language=None):
    """Yield Manitoba Legislation rows from the canonical cache."""
    import hashlib
    import json
    from datetime import UTC, datetime
    from pathlib import Path

    cache_dir = Path("stedding/ingest_queue/commonwealth/can/mb/law")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("mb_legislation_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="mb_legislation")
def mb_legislation_source(language=None):
    """DLT source for the Manitoba Legislation ingestion."""
    return mb_legislation(language=language)
