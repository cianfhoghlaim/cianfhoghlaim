"""DLT source for the Nunavut Legislation.
...
``stedding/ingest_queue/commonwealth/can/nu/law/<lang>/``.
"""
...
DOMAIN = "law"
SLUG = "legislation"
CANONICAL_URL = "https://www.gov.nu.ca/justice"
DEFAULT_LANGUAGE = "en"
...
def nu_legislation(language=None):
    """Yield Nunavut Legislation rows from the canonical cache."""
    import hashlib
    import json
    from datetime import UTC, datetime
    from pathlib import Path

    cache_dir = Path("stedding/ingest_queue/commonwealth/can/nu/law")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("nu_legislation_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="nu_legislation")
def nu_legislation_source(language=None):
    """DLT source for the Nunavut Legislation ingestion."""
    return nu_legislation(language=language)
