"""DLT source for the New Brunswick Legislation.
...
``stedding/ingest_queue/commonwealth/can/nb/law/<lang>/``.
"""
...
DOMAIN = "law"
SLUG = "legislation"
CANONICAL_URL = "https://laws.gnb.ca"
DEFAULT_LANGUAGE = "en"
...
def nb_legislation(language=None):
    """Yield New Brunswick Legislation rows from the canonical cache."""
    import hashlib
    import json
    from datetime import UTC, datetime
    from pathlib import Path

    cache_dir = Path("stedding/ingest_queue/commonwealth/can/nb/law")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("nb_legislation_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="nb_legislation")
def nb_legislation_source(language=None):
    """DLT source for the New Brunswick Legislation ingestion."""
    return nb_legislation(language=language)
