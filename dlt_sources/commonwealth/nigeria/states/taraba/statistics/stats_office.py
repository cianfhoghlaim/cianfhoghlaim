...
def nga_tar_stats_office(language=None):
    ...
    cache_dir = Path(
        "stedding/ingest_queue/commonwealth/nga/states/Taraba State/statistics/stats_office"
    )
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "nga_tar_stats_office_cache_parse_failed", path=str(json_path), error=str(exc)
                )
                continue
