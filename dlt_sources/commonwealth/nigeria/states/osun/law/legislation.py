...
``stedding/ingest_queue/commonwealth/nga/states/Osun State/law/legislation/<lang>/``.
"""
...
def nga_osn_legislation(language=None):
    """Yield state rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/commonwealth/nga/states/Osun State/law/legislation")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("nga_osn_legislation_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="nga_osn_legislation")
def nga_osn_legislation_source(language=None):
    """DLT source for the state ingestion."""
    return nga_osn_legislation(language=language)
