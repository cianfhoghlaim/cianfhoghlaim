"""DLT source for the Cross River state.
...
``stedding/ingest_queue/commonwealth/nga/states/Cross River State/education/ministry_of_education/<lang>/``.
"""
...
@dlt.resource(
    name="nga_crs_ministry_of_education",
...
def nga_crs_ministry_of_education(language=None):
    """Yield state rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/commonwealth/nga/states/Cross River State/education/ministry_of_education")
    languages = (language,) if language else ("en", "pcm")
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("nga_crs_ministry_of_education_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="nga_crs_ministry_of_education")
def nga_crs_ministry_of_education_source(language=None):
    """DLT source for the state ingestion."""
    return nga_crs_ministry_of_education(language=language)
...
__all__ = [
    ...
    "STATE_CODE",
    "STATE_NAME",
    "nga_crs_ministry_of_education",
    "nga_crs_ministry_of_education_source",
]
