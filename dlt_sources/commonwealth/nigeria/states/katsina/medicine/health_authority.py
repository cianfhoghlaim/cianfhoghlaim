...
``stedding/ingest_queue/commonwealth/nga/states/Katsina State/medicine/health_authority/<lang>/``.
"""
...
def nga_kat_health_authority(language=None):
    """Yield state rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/commonwealth/nga/states/Katsina State/medicine/health_authority")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("nga_kat_health_authority_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="nga_kat_health_authority")
def nga_kat_health_authority_source(language=None):
    """DLT source for the state ingestion."""
    return nga_kat_health_authority(language=language)
...
__all__ = [
    ...
    "STATE_CODE",
    "STATE_NAME",
    "nga_kat_health_authority",
    "nga_kat_health_authority_source",
]
