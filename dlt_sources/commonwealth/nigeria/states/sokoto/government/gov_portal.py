...
def nga_sok_gov_portal(language=None):
    ...
    cache_dir = Path(
        "stedding/ingest_queue/commonwealth/nga/states/Sokoto State/government/gov_portal"
    )
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "nga_sok_gov_portal_cache_parse_failed", path=str(json_path), error=str(exc)
                )
                continue
