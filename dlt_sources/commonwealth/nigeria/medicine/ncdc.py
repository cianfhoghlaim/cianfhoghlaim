"""DLT source for the Nigerian federal Nigeria Centre for Disease Control.
...
``stedding/ingest_queue/commonwealth/nga/medicine/ncdc/<lang>/``.
"""
...
FEDERAL_INSTITUTION = "ncdc"
CANONICAL_URL = "https://www.ncdc.gov.ng"
DEFAULT_LANGUAGE = "en"


@dlt.resource(
    name="nga_federal_ncdc",
...
def nga_federal_ncdc(language=None):
    """Yield Nigerian federal rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/commonwealth/nga/medicine/ncdc")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("nga_federal_ncdc_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="nga_federal_ncdc")
def nga_federal_ncdc_source(language=None):
    """DLT source for the Nigerian federal ingestion."""
    return nga_federal_ncdc(language=language)
