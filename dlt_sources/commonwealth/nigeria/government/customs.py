"""DLT source for the Nigerian federal Nigeria Customs Service.
...
``stedding/ingest_queue/commonwealth/nga/government/customs/<lang>/``.
"""
...
FEDERAL_INSTITUTION = "customs"
CANONICAL_URL = "https://www.customs.gov.ng"
DEFAULT_LANGUAGE = "en"


@dlt.resource(
    name="nga_federal_customs",
...
def nga_federal_customs(language=None):
    """Yield Nigerian federal rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/commonwealth/nga/government/customs")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("nga_federal_customs_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="nga_federal_customs")
def nga_federal_customs_source(language=None):
    """DLT source for the Nigerian federal ingestion."""
    return nga_federal_customs(language=language)
