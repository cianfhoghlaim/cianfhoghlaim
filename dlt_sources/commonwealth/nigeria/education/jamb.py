"""DLT source for the Nigerian federal Joint Admissions & Matriculation Board.
...
``stedding/ingest_queue/commonwealth/nga/education/jamb/<lang>/``.
"""
...
FEDERAL_INSTITUTION = "jamb"
CANONICAL_URL = "https://www.jamb.gov.ng"
DEFAULT_LANGUAGE = "en"


@dlt.resource(
    name="nga_federal_jamb",
...
def nga_federal_jamb(language=None):
    """Yield Nigerian federal rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/commonwealth/nga/education/jamb")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("nga_federal_jamb_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="nga_federal_jamb")
def nga_federal_jamb_source(language=None):
    """DLT source for the Nigerian federal ingestion."""
    return nga_federal_jamb(language=language)
