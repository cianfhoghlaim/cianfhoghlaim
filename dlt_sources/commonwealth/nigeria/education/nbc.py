"""DLT source for the Nigerian federal National Broadcasting Commission.
...
``stedding/ingest_queue/commonwealth/nga/education/nbc/<lang>/``.
"""
...
FEDERAL_INSTITUTION = "nbc"
CANONICAL_URL = "https://www.nbc.gov.ng"
DEFAULT_LANGUAGE = "en"


@dlt.resource(
    name="nga_federal_nbc",
...
def nga_federal_nbc(language=None):
    """Yield Nigerian federal rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/commonwealth/nga/education/nbc")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("nga_federal_nbc_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="nga_federal_nbc")
def nga_federal_nbc_source(language=None):
    """DLT source for the Nigerian federal ingestion."""
    return nga_federal_nbc(language=language)
