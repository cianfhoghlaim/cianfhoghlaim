            ...
            for split in splits:
                ...
                for conllu_file in treebank_path.glob(conllu_pattern):
                    for token in _parse_conllu_tokens(conllu_file, treebank_name):
                        ...
                        if deprel:
                            relation_counts[lang][deprel] = (
                                relation_counts[lang].get(deprel, 0) + 1
                            )
...
def _parse_token_line(fields: list[str]) -> dict | None:
...
def extract_mutation_features(token: dict) -> dict | None:
...
def extract_verb_features(token: dict) -> dict | None:
