"""Per-island DLT source for Government of Guernsey.

Phase 2 (BIEP parity) change: the previous
``dlt/british_isles/guernsey/education/channel_islands.py`` was a
...
ISLAND = "guernsey"
CANONICAL_URL = "https://www.gov.gg/education"


@dlt.resource(
    name="guernsey_education",
...
def guernsey_education(language=None):
    """Yield Government of Guernsey education rows from the canonical cache."""
    cache_dir = Path("stedding/site_scrape_samples/biep/crown/guernsey/education")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("guernsey_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
def guernsey_education_source(language=None):
    """DLT source for the Government of Guernsey ingestion."""
    return guernsey_education(language=language)
