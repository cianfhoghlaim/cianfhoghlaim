"""Geography DLT source — Cianfhoghlaim Oideachais."""
...
GEOG_CORPUS = CIANFHOGHLAIM_ROOT / "leaving_certificate" / "geography"
...
@dlt.source(name="geography_lc")
def geog_source(language: str | None = None, use_baml: bool = True) -> list[Any]:
    return [_geog_syllabus(language, use_baml), _geog_papers(language, use_baml), _geog_marking_schemes(language, use_baml)]


def _list_pdfs(language: str | None) -> list[Path]:
    out: list[Path] = []
    if not GEOG_CORPUS.exists():
        return out
    for lang in (["en", "ga"] if language is None else [language]):
        corpus_dir = GEOG_CORPUS / lang
        if corpus_dir.exists():
            out.extend(sorted(corpus_dir.glob("*.pdf")))
    return out


@dlt.resource(name="geog_syllabus", primary_key=["sha256"])
def _geog_syllabus(language: str | None, use_baml: bool) -> Iterator[dict[str, Any]]:
    from cianfhoghlaim.baml_client import b
    syllabus_pdfs = [p for p in _list_pdfs(language) if "syllabus" in p.name.lower() or "SCSEC" in p.name.upper()]
...
@dlt.resource(name="geog_past_papers", primary_key=["sha256"])
def _geog_papers(language: str | None, use_baml: bool) -> Iterator[dict[str, Any]]:
    from cianfhoghlaim.baml_client import b
...
@dlt.resource(name="geog_marking_schemes", primary_key=["sha256"])
def _geog_marking_schemes(language: str | None, use_baml: bool) -> Iterator[dict[str, Any]]:
    from cianfhoghlaim.baml_client import b
