"""Per-subject DLT source for England (computing_science, AQA).

Per-subject DLT source for the British Isles parity change
(`2026-07-12-british-isles-parity-pipeline-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/site_scrape_samples/biep/en/computing_science/<lang>/``.
...
@dlt.resource(
    name="{nation}_{subject}_syllabus".format(nation="en", subject=SUBJECT),
...
def en_computing_science(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield England computing_science syllabus rows from the canonical cache.
    ...
    cache_dir = Path("stedding/site_scrape_samples/biep/en/computing_science")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "en_computing_science_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="en_computing_science")
def en_computing_science_source(language: str | None = None):
    """DLT source for the England computing_science ingestion."""
    return en_computing_science(language=language)
...
__all__ = [
    ...
    "SUBJECT",
    "DEFAULT_LEVEL",
    "en_computing_science",
    "en_computing_science_source",
]
