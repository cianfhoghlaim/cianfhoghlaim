...
def en_mathematics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield England mathematics syllabus rows from the canonical cache.
    ...
    cache_dir = Path("stedding/site_scrape_samples/biep/en/mathematics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "en_mathematics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="en_mathematics")
def en_mathematics_source(language: str | None = None):
    """DLT source for the England mathematics ingestion."""
    return en_mathematics(language=language)
