...
from ._common import DUCKLAKE_NAMESPACE
...
# ─── Public namespace accessor (Wave 4 §4.1 verification) ──────────────────


def get_ducklake_namespace() -> str:
    """Return the canonical consolidated DuckLake namespace.

    Per Wave 4 §4.1 of the 2026-08-24 master refactor plan, the 6
    legacy namespaces (`ducklake_oideachais`, `ducklake_crypteolas`,
    `ducklake_croilar`, `ducklake_tuath`, `ducklake_meaisinfhoghlaim`,
    `ducklake_aleyum`) are all consolidated into the single canonical
    namespace `ducklake_cianfhoghlaim`. Every code path should ask
    this accessor for the namespace name rather than hard-coding it.

    Returns:
        Always ``"ducklake_cianfhoghlaim"``.

    Reference: master plan §3.5 (the canonical DuckLake layout) +
    Wave 4 §4.1 (namespace consolidation migration).
    """
    return DUCKLAKE_NAMESPACE


# Backwards-compatible alias: `get_ducklake_namespace()` is the new
# canonical surface; `get_namespace()` is preserved as a short alias
# for the existing callers that want a brief surface.
get_namespace = get_ducklake_namespace
...
# ─── Wave 4 §4.1 — Namespace consolidation ──────────────────────────────────
# The 6 legacy namespaces that pre-Wave-4 code paths still use. The
# namespace consolidation migration
# (`scripts/sync/consolidate_ducklake_namespaces.py`) reads each of
# these namespaces from the Postgres catalog and writes the data to
# the consolidated `ducklake_cianfhoghlaim` namespace. After the
# migration runs (per Wave 4 task 4.1), only `ducklake_cianfhoghlaim`
# remains.


LEGACY_DUCKLAKE_NAMESPACES: tuple[str, ...] = (
    "ducklake_oideachais",
    "ducklake_crypteolas",
    "ducklake_croilar",
    "ducklake_tuath",
...
"""The 6 legacy DuckLake namespace names consolidated in Wave 4 §4.1.

Each legacy namespace is migrated (via `ATTACH ... RENAME TO` or
table-level copy) into the canonical `ducklake_cianfhoghlaim`
namespace. After the migration, this set is empty and the user
explicitly drops each legacy Postgres `ducklake_*` database.
"""


# ─── Wave 4 §4.6 — Snapshot expiry policy ───────────────────────────────────
# Per-quadrant snapshot retention windows. The BIEP v3 hot path keeps
# 30 days of snapshots for time-travel; the UoG personal archive only
# keeps 7 days because of the student-data policy.


DEFAULT_BIEP_SNAPSHOT_RETENTION_DAYS: int = 30
"""Default snapshot retention for the BIEP v3 hot path (Wave 4 §4.6).

Per `ducklake.select/docs/stable/duckdb/maintenance/expire_snapshots`.
The BIEP v3 daily cron keeps 30 days for syllabus-version pinning
(`SELECT ... AT (TIMESTAMP => '...')`).
"""

DEFAULT_PERSONAL_ARCHIVE_SNAPSHOT_RETENTION_DAYS: int = 7
"""Default snapshot retention for the UoG personal-archive namespace.

Per the student-data policy (UoG NUI federation contract), snapshots
beyond 7 days must be expired for the personal-archive namespace.
"""

DEFAULT_MEDIA_INTEL_SNAPSHOT_RETENTION_DAYS: int = 7
"""Default snapshot retention for the media_intel pipelines.

The media_intel pipeline is high-churn + low-correctness-cost;
7 days is sufficient for the BAML backfill flow.
"""

SNAPSHOT_RETENTION_BY_QUADRANT: dict[str, int] = {
    "oideachais": DEFAULT_BIEP_SNAPSHOT_RETENTION_DAYS,
    "tuatha": DEFAULT_BIEP_SNAPSHOT_RETENTION_DAYS,
    "croilar": DEFAULT_BIEP_SNAPSHOT_RETENTION_DAYS,
    "agents": DEFAULT_MEDIA_INTEL_SNAPSHOT_RETENTION_DAYS,
    "media": DEFAULT_MEDIA_INTEL_SNAPSHOT_RETENTION_DAYS,
    "ducklake_cianfhoghlaim": DEFAULT_BIEP_SNAPSHOT_RETENTION_DAYS,
    "ducklake_uog_personal_archive": DEFAULT_PERSONAL_ARCHIVE_SNAPSHOT_RETENTION_DAYS,
}
"""Snapshot retention per quadrant (Wave 4 §4.6).

Used by the `orchestration/assets/ducklake_maintenance.py` Dagster
asset to call `ducklake_expire_snapshots()` with the right
`older_than` interval per quadrant.
"""


def expire_snapshots_for_quadrant(
    quadrant: str,
    *,
    ducklake: str = DUCKLAKE_NAME,
) -> str:
    """Return SQL to expire snapshots on the given quadrant.

    Thin wrapper over `ducklake_expire_snapshots_sql(...)` that
    resolves `quadrant` to one of the per-quadrant retention windows
    in `SNAPSHOT_RETENTION_BY_QUADRANT`. Falls back to
    `DEFAULT_BIEP_SNAPSHOT_RETENTION_DAYS` if the quadrant is unknown.

    Args:
        quadrant: One of the quadrant keys in
            `SNAPSHOT_RETENTION_BY_QUADRANT` (e.g. `"oideachais"`,
            `"media"`, `"ducklake_uog_personal_archive"`).
        ducklake: The DuckLake name. Default: `"cianfhoghlaim"`.

    Returns:
        The `CALL ducklake_expire_snapshots(...)` SQL string.
    """
    days = SNAPSHOT_RETENTION_BY_QUADRANT.get(
        quadrant,
        DEFAULT_BIEP_SNAPSHOT_RETENTION_DAYS,
    )
    return ducklake_expire_snapshots_sql(
        snapshot_retention_days=days,
        ducklake=ducklake,
    )


def expire_snapshots_all_quadrants(
    *,
    ducklake: str = DUCKLAKE_NAME,
) -> str:
    """Return a SQL batch string that expires snapshots for every quadrant.

    Concatenates the 5 per-quadrant `CALL ducklake_expire_snapshots(...)`
    statements into one multi-statement script that the
    `ducklake_maintenance` Dagster asset executes in a single
    transaction.
    ...
    Returns:
        A newline-joined string of `CALL ducklake_expire_snapshots(...)`
        statements, one per quadrant key in
    ...
    statements = [
        ...
        for days in sorted(set(SNAPSHOT_RETENTION_BY_QUADRANT.values()))
    ]
    return ";\n".join(statements) + ";"


# ─── Wave 4 §4.5 — Per-namespace encryption ─────────────────────────────────
# Per the UoG student-data policy + the DuckLake v1.0 spec
# `ducklake.select/docs/stable/specification/security`, set
# `encryption.key-id` per-namespace. The Postgres catalog stores the
# key-id metadata; DuckDB uses it to wrap data-file encryption keys.


ENCRYPTED_NAMESPACES: frozenset[str] = frozenset(
    {
        "ducklake_uog_personal_archive",
        "ducklake_uog_personal_archive_figures",
        # Future: per Wave 4 §4.5 the encrytion policy extends to any
        # namespace handling student-data. Keep this set explicit so
        # the migration script can audit it.
    }
)
"""The per-namespace encryption set (Wave 4 §4.5).

Each namespace in this set is configured with
`encryption.key-id = <uuid>` via `set_namespace_encryption_sql(...)`.
Read back via `SELECT encryption_key_id FROM
ducklake_namespace_info WHERE namespace = '...'`.
"""

DEFAULT_UOG_ENCRYPTION_KEY_ID: str = os.getenv(
    "CIANFHOGHLAIM_DUCKLAKE_UOG_ENCRYPTION_KEY_ID",
    "00000000-0000-0000-0000-000000000001",
)
"""The default encryption key-id for the UoG personal-archive namespace.

Override via `CIANFHOGHLAIM_DUCKLAKE_UOG_ENCRYPTION_KEY_ID` env var.
Production deployments should source this from Infisical
(`dev-baile/lakehouse-encryption/uog_personal_archive_key_id`).
"""


def set_namespace_encryption_sql(
    namespace: str,
    key_id: str = DEFAULT_UOG_ENCRYPTION_KEY_ID,
) -> str:
    """Return SQL to set `encryption.key-id` on a DuckLake namespace.

    Per `ducklake.select/docs/stable/specification/security` + the
    Wave 4 §4.5 recommendation, DuckLake namespaces that hold
    sensitive data (UoG student records) must be configured with
    `encryption.key-id`. The Postgres catalog row in
    `ducklake_namespace_info` stores the key-id; DuckDB reads it on
    attach and wraps the data-file encryption keys with the supplied
    KMS-managed data encryption key.

    Args:
        namespace: The DuckLake namespace name (e.g.
            `"ducklake_uog_personal_archive"`).
        key_id: The KMS-managed data encryption key id. Default:
            `DEFAULT_UOG_ENCRYPTION_KEY_ID` (the UoG default).

    Returns:
        The SQL `ALTER NAMESPACE ... SET (encryption_key_id = '...')`
        statement.

    Reference: Wave 4 §4.5 of the 2026-08-24 master refactor plan.
    """
    safe_namespace = namespace.replace("'", "''")
    safe_key_id = key_id.replace("'", "''")
    return (
        f"ALTER NAMESPACE {safe_namespace} "
        f"SET (encryption_key_id = '{safe_key_id}')"
    )


def namespace_encryption_info_sql(namespace: str = DUCKLAKE_NAMESPACE) -> str:
    """Return SQL to read the encryption key-id for a namespace.

    Wave 4 §4.5 verification: per the canonical DuckLake v1.0 spec,
    the Postgres catalog row `ducklake_namespace_info` exposes the
    encryption key-id. Run this query against the attached DuckLake
    to confirm the key-id is set.

    Args:
        namespace: The DuckLake namespace name. Default:
            `DUCKLAKE_NAMESPACE` (`"ducklake_cianfhoghlaim"`).

    Returns:
        A `SELECT encryption_key_id FROM ducklake_namespace_info
        WHERE namespace = '...'` query string.
    """
    safe_namespace = namespace.replace("'", "''")
    return (
        f"SELECT encryption_key_id FROM ducklake_namespace_info "
        f"WHERE namespace = '{safe_namespace}'"
    )


# ─── Wave 4 §4.7 — Iceberg REST catalog (Lakekeeper) ────────────────────────
# The Lakekeeper REST catalog (per `bonneagar/stacks/lakehouse`) lives
# at `:8181`. The Iceberg REST endpoint is the canonical cross-engine
# surface; the raw DuckLake ATTACH pattern is the Postgres-catalog
# surface. New data sources SHOULD prefer the Iceberg REST endpoint
# for cross-engine compatibility (Spark / Trino / PyIceberg).


DEFAULT_LAKEKEEPER_ENDPOINT: str = os.getenv(
    "CIANFHOGHLAIM_LAKEKEEPER_ENDPOINT",
    "http://lakekeeper:8181",
)
"""The canonical Lakekeeper Iceberg REST endpoint (Wave 4 §4.7).

Per `bonneagar/stacks/lakehouse/compose.yaml`, the Lakekeeper
container exposes `:8181` on the `lakehouse` Docker network
(`http://lakekeeper:8181`). Override via
`CIANFHOGHLAIM_LAKEKEEPER_ENDPOINT` for PlanetScale / managed
deployments.
"""


def get_iceberg_rest_endpoint(
    *,
    lakekeeper_endpoint: Optional[str] = None,
    warehouse: str = "lakehouse",
) -> str:
    """Return the Iceberg REST catalog endpoint URL for the given warehouse.

    Per `https://docs.lakekeeper.io/` the Iceberg REST catalog
    endpoint is `<lakekeeper>/catalog/v1/<warehouse>`. The warehouse
    name defaults to `lakehouse` (the canonical PlanetScale /
    local Postgres warehouse declared in `compose.yaml:215`).

    Args:
        lakekeeper_endpoint: Override the default
            `DEFAULT_LAKEKEEPER_ENDPOINT`. Useful for tests + the
            Wave 4 §4.7 verification path.
        warehouse: The Iceberg warehouse name. Default:
            `"lakehouse"`.

    Returns:
        The full REST URL (e.g. `http://lakekeeper:8181/catalog/v1/lakehouse`).
    """
    endpoint = (
        lakekeeper_endpoint
        or os.getenv("CIANFHOGHLAIM_LAKEKEEPER_ENDPOINT")
        or DEFAULT_LAKEKEEPER_ENDPOINT
    )
    endpoint = endpoint.rstrip("/")
    return f"{endpoint}/catalog/v1/{warehouse}"


def attach_as_iceberg_rest_sql(
    alias: str,
    *,
    lakekeeper_endpoint: Optional[str] = None,
    warehouse: str = "lakehouse",
    namespace: str = DUCKLAKE_NAMESPACE,
) -> str:
    """Return SQL to ATTACH DuckLake via the Iceberg REST catalog.

    Per `ducklake.select/docs/stable/iceberg/attach` + the Wave 4 §4.7
    recommendation, attach DuckLake via the Iceberg REST endpoint
    (the Lakekeeper `:8181` surface) for cross-engine compatibility
    with Spark / Trino / PyIceberg.

    Args:
        alias: The local DuckDB alias for the attached database.
        lakekeeper_endpoint: Override the default
            `DEFAULT_LAKEKEEPER_ENDPOINT`.
        warehouse: The Iceberg warehouse name. Default: `"lakehouse"`.
        namespace: The DuckLake namespace inside the warehouse.
            Default: `DUCKLAKE_NAMESPACE`.

    Returns:
        The `ATTACH 'iceberg_rest://<endpoint>' AS <alias>`
        statement.
    """
    endpoint = get_iceberg_rest_endpoint(
        lakekeeper_endpoint=lakekeeper_endpoint,
        warehouse=warehouse,
    )
    return f"ATTACH 'iceberg_rest://{endpoint}' AS {alias}"
...
__all__ = [
    ...
    # Canonical namespace accessor (Wave 4 §4.1)
    "DUCKLAKE_NAMESPACE",
    "get_ducklake_namespace",
    "get_namespace",
    # Default snapshot retention policies (Wave 4 §4.6)
    "DEFAULT_BIEP_SNAPSHOT_RETENTION_DAYS",
    "DEFAULT_PERSONAL_ARCHIVE_SNAPSHOT_RETENTION_DAYS",
    "DEFAULT_MEDIA_INTEL_SNAPSHOT_RETENTION_DAYS",
    "SNAPSHOT_RETENTION_BY_QUADRANT",
    # Per-namespace encryption (Wave 4 §4.5)
    "ENCRYPTED_NAMESPACES",
    "DEFAULT_UOG_ENCRYPTION_KEY_ID",
    # Iceberg REST catalog (Lakekeeper) — Wave 4 §4.7
    "DEFAULT_LAKEKEEPER_ENDPOINT",
    ...
    # Encryption SQL (Wave 4 §4.5)
    "set_namespace_encryption_sql",
    "namespace_encryption_info_sql",
    # Iceberg REST attach SQL (Wave 4 §4.7)
    ...
    # Wave 4 §4.6 — multi-quadrant snapshot expiry
    "expire_snapshots_for_quadrant",
    "expire_snapshots_all_quadrants",
    # Wave 4 §4.1 — namespace consolidation
    "LEGACY_DUCKLAKE_NAMESPACES",
]
