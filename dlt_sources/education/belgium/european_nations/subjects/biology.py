"""Per-subject DLT source for Belgium (biology).
...
``stedding/ingest_queue/european_nations/bel/education/subjects/biology/<lang>/``.
...
class BelgiumBiologyEducationSource(NationSource):
    """Belgium biology curriculum DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                ...
                "title": "Belgium biology curriculum (nl,fr,de)",
            },
        )
...
_NATION_SOURCE = BelgiumBiologyEducationSource()
...
def bel_biology(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Belgium biology rows from the canonical per-subject cache."""
    if not use_local_scrapes():
        logger.warning(
            "bel_biology_live_mode_not_implemented",
...
@dlt.source(name="bel_biology")
def bel_biology_source(language: str | None = None):
    """DLT source for the Belgium biology ingestion."""
    return bel_biology(language=language)


__all__ = [
    "BelgiumBiologyEducationSource",
    "bel_biology",
    "bel_biology_source",
]
