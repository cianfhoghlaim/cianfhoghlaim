"""Per-subject DLT source for Belgium (chemistry).
...
``stedding/ingest_queue/european_nations/bel/education/subjects/chemistry/<lang>/``.
...
class BelgiumChemistryEducationSource(NationSource):
    """Belgium chemistry curriculum DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                ...
                "title": "Belgium chemistry curriculum (nl,fr,de)",
            },
        )
...
_NATION_SOURCE = BelgiumChemistryEducationSource()
...
def bel_chemistry(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Belgium chemistry rows from the canonical per-subject cache."""
    if not use_local_scrapes():
        logger.warning(
            "bel_chemistry_live_mode_not_implemented",
...
@dlt.source(name="bel_chemistry")
def bel_chemistry_source(language: str | None = None):
    """DLT source for the Belgium chemistry ingestion."""
    return bel_chemistry(language=language)


__all__ = [
    "BelgiumChemistryEducationSource",
    "bel_chemistry",
    "bel_chemistry_source",
]
