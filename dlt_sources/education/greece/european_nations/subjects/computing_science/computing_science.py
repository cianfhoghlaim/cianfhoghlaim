"""Per-subject DLT source for Greece (Computing / Computer Science, YPETh).

Per-subject DLT source for the EU nations full-depth expansion
(`2026-07-13-eu-nations-full-depth-expansion-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/grc/education/subjects/computing_science/<lang>/``.
...
def grc_computing_science(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Greece Computing / Computer Science syllabus rows from the canonical cache.

    Honours ``USE_LOCAL_SCRAPES=true`` by reading cached Firecrawl
    snapshots under ``stedding/ingest_queue/european_nations/grc/education/subjects/computing_science/<lang>/``.
    """
    cache_dir = Path("stedding/ingest_queue/european_nations/grc/education/subjects/computing_science")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "grc_computing_science_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="grc_computing_science")
def grc_computing_science_source(language: str | None = None):
    """DLT source for the Greece Computing / Computer Science ingestion."""
    return grc_computing_science(language=language)
