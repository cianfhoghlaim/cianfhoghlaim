"""Per-subject DLT source for Greece (Native language and literature, YPETh).

Per-subject DLT source for the EU nations full-depth expansion
(`2026-07-13-eu-nations-full-depth-expansion-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/grc/education/subjects/language/<lang>/``.
...
def grc_language(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Greece Native language and literature syllabus rows from the canonical cache.

    Honours ``USE_LOCAL_SCRAPES=true`` by reading cached Firecrawl
    snapshots under ``stedding/ingest_queue/european_nations/grc/education/subjects/language/<lang>/``.
    """
    cache_dir = Path("stedding/ingest_queue/european_nations/grc/education/subjects/language")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "grc_language_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="grc_language")
def grc_language_source(language: str | None = None):
    """DLT source for the Greece Native language and literature ingestion."""
    return grc_language(language=language)
