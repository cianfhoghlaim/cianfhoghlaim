"""Per-subject DLT source for Norway (biology).
...
``stedding/ingest_queue/european_nations/nor/education/subjects/biology/<lang>/``.
...
class NorwayBiologyEducationSource(NationSource):
    """Norway biology curriculum DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                ...
                "title": "Norway biology curriculum (nb,nn,se)",
            },
        )
...
_NATION_SOURCE = NorwayBiologyEducationSource()
...
def nor_biology(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Norway biology rows from the canonical per-subject cache."""
    if not use_local_scrapes():
        logger.warning(
            "nor_biology_live_mode_not_implemented",
...
@dlt.source(name="nor_biology")
def nor_biology_source(language: str | None = None):
    """DLT source for the Norway biology ingestion."""
    return nor_biology(language=language)


__all__ = [
    "NorwayBiologyEducationSource",
    "nor_biology",
    "nor_biology_source",
]
