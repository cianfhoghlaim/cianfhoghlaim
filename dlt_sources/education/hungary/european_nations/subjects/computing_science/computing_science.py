"""Per-subject DLT source for Hungary (Computing / Computer Science, OH).

Per-subject DLT source for the EU nations full-depth expansion
(`2026-07-13-eu-nations-full-depth-expansion-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/hun/education/subjects/computing_science/<lang>/``.
...
COUNTRY_CODE = "hun"
CANONICAL_ROOT = "https://oktatas.hu"
SUPPORTED_LANGUAGES: tuple[str, ...] = ("hu",)
DEFAULT_LANGUAGE = "hu"
...
def hun_computing_science(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Hungary Computing / Computer Science syllabus rows from the canonical cache.

    Honours ``USE_LOCAL_SCRAPES=true`` by reading cached Firecrawl
    snapshots under ``stedding/ingest_queue/european_nations/hun/education/subjects/computing_science/<lang>/``.
    """
    cache_dir = Path("stedding/ingest_queue/european_nations/hun/education/subjects/computing_science")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "hun_computing_science_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="hun_computing_science")
def hun_computing_science_source(language: str | None = None):
    """DLT source for the Hungary Computing / Computer Science ingestion."""
    return hun_computing_science(language=language)
