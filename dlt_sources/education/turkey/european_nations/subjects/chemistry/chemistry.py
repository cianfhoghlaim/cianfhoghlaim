"""Per-subject DLT source for Turkey (Chemistry).
...
``stedding/ingest_queue/european_nations/tur/education/subjects/chemistry/<lang>/``.
...
class TurkeyChemistrySource(NationSource):
    """Turkey Chemistry DLT source."""
...
_NATION_SOURCE = TURChemistrySource()
...
def tur_chemistry(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Turkey Chemistry rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "tur_chemistry_live_mode_not_implemented",
...
@dlt.source(name="tur_chemistry")
def tur_chemistry_source(language: str | None = None):
    """DLT source for the Turkey Chemistry ingestion."""
    return tur_chemistry(language=language)
