"""Per-subject DLT source for the Poland {Physics} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/pol/education/subjects/physics/pl/sample.json``.
"""
...
def pol_physics(language=None) -> Iterator[dict[str, Any]]:
    """Yield Poland Physics rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/pol/education/subjects/physics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "pol_physics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="pol_physics")
def pol_physics_source(language=None):
    """DLT source for the Poland Physics ingestion."""
    return pol_physics(language=language)
