"""
DLT source for the Chemistry NCCA syllabus — BIEP v1 per-subject ingestion.
...
NCCA syllabus crawl source. This file is the Chemistry variant.

The canonical BIEP v1 pattern (per `9e97ca0ca`):
- `@dlt.resource(name="chemistry_syllabus", write_disposition="merge", primary_key=["url"])`
- Uses the `named_destinations` factory (the `ducklake_cianfhoghlaim` named
  destination)
- Honours `USE_LOCAL_SCRAPES=true` to read from
  `stedding/ingest_queue/ncca/chemistry/`
- Uses the `default` BAML client (minimax-m3 per `667635dfd`)

Usage:

    from dlt_sources.education.ireland.british_isles.education.ncca_chemistry import (
        ncca_chemistry_source,
    )

    os.environ["USE_LOCAL_SCRAPES"] = "true"
    pipeline.run(ncca_chemistry_source(language="en"))
...
INGEST_QUEUE = REPO_ROOT / "stedding" / "ingest_queue" / "ncca" / "chemistry"
...
def _iter_local_cache(language: str) -> Iterator[dict[str, Any]]:
    """Yield cached Chemistry pages from the local ingest queue."""
...
def _iter_live_crawl(language: str, max_pages: int) -> Iterator[dict[str, Any]]:
    """Crawl ncca.ie/en/senior-cycle/chemistry live via Firecrawl."""
    try:
        from firecrawl import FirecrawlApp
    except ImportError:
        yield {
            "url": "https://ncca.ie",
            "title": "Chemistry (NCCA — Firecrawl not installed)",
            "language": language,
            "subject": "chemistry",
            "source": "ncca",
            "crawled_at": _now_iso(),
    ...
    if not api_key:
        yield {
            "url": "https://ncca.ie",
            "title": "Chemistry (NCCA — FIRECRAWL_API_KEY missing)",
            "language": language,
            "subject": "chemistry",
            "source": "ncca",
            "crawled_at": _now_iso(),
    ...
    keyword = "ceimic" if language == "ga" else "chemistry"
    ...
    try:
        ...
        yield {
            "url": base,
            "title": f"Chemistry crawl failed: {exc}",
            "language": language,
            "subject": "chemistry",
            "source": "ncca",
            "crawled_at": _now_iso(),
...
def chemistry_syllabus_resource(language: str = "en") -> Iterator[dict[str, Any]]:
    """Yield one row per Chemistry NCCA page (EN or GA)."""
...
@dlt.source(name="ncca_chemistry_lc6")
def ncca_chemistry_source(language: str = "en") -> Any:
    """The canonical BIEP v1 Chemistry NCCA source."""
    return chemistry_syllabus_resource(language=language)


def ncca_chemistry_partitions() -> Any:
    """Return the canonical Dagster MultiPartitionsDefinition for the
    Chemistry NCCA crawl: (chemistry) x (en + ga) = 2 partitions.
    ...
    return MultiPartitionsDefinition({
        "subject": StaticPartitionsDefinition(["chemistry"]),
...
__all__ = [
    "INGEST_QUEUE",
    "LC_LANGUAGES",
    "chemistry_syllabus_resource",
    "ncca_chemistry_partitions",
    "ncca_chemistry_source",
]
...
__all__ = [
    ...
    "LC_LANGUAGES",
    "chemistry_syllabus_resource",
    "create_ncca_chemistry_pipeline",
    "ncca_chemistry_partitions",
    "ncca_chemistry_source",
]
