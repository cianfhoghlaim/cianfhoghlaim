"""DLT source for the Nigerian federal National Business & Technical Examinations Board.
...
``stedding/ingest_queue/commonwealth/nga/education/nabteb/<lang>/``.
"""
...
FEDERAL_INSTITUTION = "nabteb"
CANONICAL_URL = "https://www.nabteb.gov.ng"
DEFAULT_LANGUAGE = "en"


@dlt.resource(
    name="nga_federal_nabteb",
...
def nga_federal_nabteb(language=None):
    """Yield Nigerian federal rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/commonwealth/nga/education/nabteb")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning("nga_federal_nabteb_cache_parse_failed", path=str(json_path), error=str(exc))
                continue
...
@dlt.source(name="nga_federal_nabteb")
def nga_federal_nabteb_source(language=None):
    """DLT source for the Nigerian federal ingestion."""
    return nga_federal_nabteb(language=language)
