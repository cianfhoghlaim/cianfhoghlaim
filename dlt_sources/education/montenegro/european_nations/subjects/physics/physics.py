"""Per-subject DLT source for Montenegro (Physics).
...
``stedding/ingest_queue/european_nations/mne/education/subjects/physics/<lang>/``.
...
class MontenegroPhysicsSource(NationSource):
    """Montenegro Physics DLT source."""
...
_NATION_SOURCE = MNEPhysicsSource()
...
def mne_physics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Montenegro Physics rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "mne_physics_live_mode_not_implemented",
...
@dlt.source(name="mne_physics")
def mne_physics_source(language: str | None = None):
    """DLT source for the Montenegro Physics ingestion."""
    return mne_physics(language=language)
