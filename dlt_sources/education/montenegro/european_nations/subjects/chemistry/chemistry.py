"""Per-subject DLT source for Montenegro (Chemistry).
...
``stedding/ingest_queue/european_nations/mne/education/subjects/chemistry/<lang>/``.
...
class MontenegroChemistrySource(NationSource):
    """Montenegro Chemistry DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                ...
                "title": "Montenegro Chemistry (MNE)",
                "subject": SUBJECT,
...
_NATION_SOURCE = MNEChemistrySource()
...
def mne_chemistry(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Montenegro Chemistry rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "mne_chemistry_live_mode_not_implemented",
...
@dlt.source(name="mne_chemistry")
def mne_chemistry_source(language: str | None = None):
    """DLT source for the Montenegro Chemistry ingestion."""
    return mne_chemistry(language=language)
