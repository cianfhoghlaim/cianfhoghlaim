"""Per-subject DLT source for Romania (Native language and literature, MEN).

Per-subject DLT source for the EU nations full-depth expansion
(`2026-07-13-eu-nations-full-depth-expansion-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/rou/education/subjects/language/<lang>/``.
...
def rou_language(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Romania Native language and literature syllabus rows from the canonical cache.

    Honours ``USE_LOCAL_SCRAPES=true`` by reading cached Firecrawl
    snapshots under ``stedding/ingest_queue/european_nations/rou/education/subjects/language/<lang>/``.
    """
    cache_dir = Path("stedding/ingest_queue/european_nations/rou/education/subjects/language")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "rou_language_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="rou_language")
def rou_language_source(language: str | None = None):
    """DLT source for the Romania Native language and literature ingestion."""
    return rou_language(language=language)
