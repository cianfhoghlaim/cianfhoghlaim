"""
Northern Ireland / CCEA — scaffolded cross-nation DLT source.

Reads cached CCEA mathematics syllabus snapshots from
...
Firecrawl-crawl of the CCEA qualification finder
(``https://ccea.org.uk/``), the BAML
...
CCEA (``Council for the Curriculum, Examinations & Assessment``) is
the sole Northern Ireland awarding body (since 1994). The
language code ``ga`` is shared with the Irish Republic
(Gaeilge / Irish) — the CCEA Irish-medium syllabuses mirror the
...
def ccea_mathematics_syllabus(
    language: str = "en",
    subject: str = "mathematics",
) -> Iterator[dict[str, Any]]:
    """Yield 1 row per cached CCEA syllabus snapshot for ``subject`` in ``language``.
...
@dlt.source(name="ccea_syllabus")
def ccea_syllabus_source(
    language: str = "en",
    subject: str = "mathematics",
):
    """DLT source for the CCEA scaffolded syllabus read."""
    return ccea_mathematics_syllabus(language=language, subject=subject)
