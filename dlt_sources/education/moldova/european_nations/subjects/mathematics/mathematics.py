"""Per-subject DLT source for Moldova (Mathematics).
...
``stedding/ingest_queue/european_nations/mda/education/subjects/mathematics/<lang>/``.
...
class MoldovaMathematicsSource(NationSource):
    """Moldova Mathematics DLT source."""
...
_NATION_SOURCE = MDAMathematicsSource()
...
def mda_mathematics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Moldova Mathematics rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "mda_mathematics_live_mode_not_implemented",
...
@dlt.source(name="mda_mathematics")
def mda_mathematics_source(language: str | None = None):
    """DLT source for the Moldova Mathematics ingestion."""
    return mda_mathematics(language=language)
