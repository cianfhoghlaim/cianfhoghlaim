"""Per-subject DLT source for Moldova (Physics).
...
``stedding/ingest_queue/european_nations/mda/education/subjects/physics/<lang>/``.
...
class MoldovaPhysicsSource(NationSource):
    """Moldova Physics DLT source."""
...
_NATION_SOURCE = MDAPhysicsSource()
...
def mda_physics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Moldova Physics rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "mda_physics_live_mode_not_implemented",
...
@dlt.source(name="mda_physics")
def mda_physics_source(language: str | None = None):
    """DLT source for the Moldova Physics ingestion."""
    return mda_physics(language=language)
