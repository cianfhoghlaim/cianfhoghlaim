"""Per-subject DLT source for Moldova (Language).
...
``stedding/ingest_queue/european_nations/mda/education/subjects/language/<lang>/``.
...
class MoldovaLanguageSource(NationSource):
    """Moldova Language DLT source."""
...
_NATION_SOURCE = MDALanguageSource()
...
def mda_language(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Moldova Language rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "mda_language_live_mode_not_implemented",
...
@dlt.source(name="mda_language")
def mda_language_source(language: str | None = None):
    """DLT source for the Moldova Language ingestion."""
    return mda_language(language=language)
