"""Per-subject DLT source for Latvia (Native language and literature, VISC).

Per-subject DLT source for the EU nations full-depth expansion
(`2026-07-13-eu-nations-full-depth-expansion-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/lva/education/subjects/language/<lang>/``.
...
def lva_language(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Latvia Native language and literature syllabus rows from the canonical cache.

    Honours ``USE_LOCAL_SCRAPES=true`` by reading cached Firecrawl
    snapshots under ``stedding/ingest_queue/european_nations/lva/education/subjects/language/<lang>/``.
    """
    cache_dir = Path("stedding/ingest_queue/european_nations/lva/education/subjects/language")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "lva_language_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="lva_language")
def lva_language_source(language: str | None = None):
    """DLT source for the Latvia Native language and literature ingestion."""
    return lva_language(language=language)
