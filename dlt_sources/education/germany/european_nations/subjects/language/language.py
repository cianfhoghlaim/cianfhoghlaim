"""Per-subject DLT source for the Germany {Language} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/deu/education/subjects/language/de/sample.json``.
"""
...
def deu_language(language=None) -> Iterator[dict[str, Any]]:
    """Yield Germany Language rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/deu/education/subjects/language")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "deu_language_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="deu_language")
def deu_language_source(language=None):
    """DLT source for the Germany Language ingestion."""
    return deu_language(language=language)
