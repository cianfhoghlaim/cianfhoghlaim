"""Per-subject DLT source for the Italy {Language} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/ita/education/subjects/language/it/sample.json``.
"""
...
def ita_language(language=None) -> Iterator[dict[str, Any]]:
    """Yield Italy Language rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/ita/education/subjects/language")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "ita_language_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="ita_language")
def ita_language_source(language=None):
    """DLT source for the Italy Language ingestion."""
    return ita_language(language=language)
