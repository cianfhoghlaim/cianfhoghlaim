"""Per-subject DLT source for Croatia (Physics, NCVVO).

Per-subject DLT source for the EU nations full-depth expansion
(`2026-07-13-eu-nations-full-depth-expansion-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/hrv/education/subjects/physics/<lang>/``.
...
def hrv_physics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Croatia Physics syllabus rows from the canonical cache.

    Honours ``USE_LOCAL_SCRAPES=true`` by reading cached Firecrawl
    snapshots under ``stedding/ingest_queue/european_nations/hrv/education/subjects/physics/<lang>/``.
    """
    cache_dir = Path("stedding/ingest_queue/european_nations/hrv/education/subjects/physics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "hrv_physics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="hrv_physics")
def hrv_physics_source(language: str | None = None):
    """DLT source for the Croatia Physics ingestion."""
    return hrv_physics(language=language)
