"""Per-subject DLT source for Croatia (Biology, NCVVO).

Per-subject DLT source for the EU nations full-depth expansion
(`2026-07-13-eu-nations-full-depth-expansion-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/hrv/education/subjects/biology/<lang>/``.
...
def hrv_biology(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Croatia Biology syllabus rows from the canonical cache.

    Honours ``USE_LOCAL_SCRAPES=true`` by reading cached Firecrawl
    snapshots under ``stedding/ingest_queue/european_nations/hrv/education/subjects/biology/<lang>/``.
    """
    cache_dir = Path("stedding/ingest_queue/european_nations/hrv/education/subjects/biology")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "hrv_biology_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="hrv_biology")
def hrv_biology_source(language: str | None = None):
    """DLT source for the Croatia Biology ingestion."""
    return hrv_biology(language=language)
