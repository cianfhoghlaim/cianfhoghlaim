"""DLT source for Statistik Austria.

Crawls ``https://www.statistik.at`` and emits one row per ``(national_statistics_id, language)`` for
...
``dlt/european_nations/aut/statistics/national_statistics.py`` with
``source_id="european_nations.aut.statistics.national_statistics"`` and lands in
the DuckLake table ``oideachais.statistics.european_nations.aut``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/aut/statistics/<lang>/``.
...
class AustriaNationalStatisticsSource(NationSource):
    """Statistik Austria DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://www.statistik.at",
                "title": "Statistik Austria",
            },
        )


_NATION_SOURCE = AustriaNationalStatisticsSource()
...
def national_statistics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Statistik Austria rows from the canonical cache."""
...
def national_statistics_source(language: str | None = None):
    """DLT source for the Statistik Austria ingestion."""
    return national_statistics(language=language)


__all__ = [
    "AustriaNationalStatisticsSource",
    "national_statistics",
