"""Per-subject DLT source for Portugal (Mathematics, IAVE).

Per-subject DLT source for the EU nations full-depth expansion
(`2026-07-13-eu-nations-full-depth-expansion-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/prt/education/subjects/mathematics/<lang>/``.
...
def prt_mathematics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Portugal Mathematics syllabus rows from the canonical cache.

    Honours ``USE_LOCAL_SCRAPES=true`` by reading cached Firecrawl
    snapshots under ``stedding/ingest_queue/european_nations/prt/education/subjects/mathematics/<lang>/``.
    """
    cache_dir = Path("stedding/ingest_queue/european_nations/prt/education/subjects/mathematics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "prt_mathematics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="prt_mathematics")
def prt_mathematics_source(language: str | None = None):
    """DLT source for the Portugal Mathematics ingestion."""
    return prt_mathematics(language=language)
