"""DLT source for the Government Portal of Moldova.

Crawls ``https://gov.md`` and emits one row per ``(gov_portal_id, language)``
...
``dlt/european_nations/mda/government/government_portal.py`` with
``source_id="european_nations.mda.government.government_portal"`` and lands in
the DuckLake table ``oideachais.government.european_nations.mda``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/mda/government/<lang>/``.
...
class MoldovaGovernmentSource(NationSource):
    """Government Portal of Moldova DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://gov.md",
                "title": "Government Portal of Moldova",
            },
        )


_NATION_SOURCE = MoldovaGovernmentSource()
...
def government_portal(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Moldova Government Portal rows from the canonical cache."""
...
def government_portal_source(language: str | None = None):
    """DLT source for the Government Portal of Moldova ingestion."""
    return government_portal(language=language)


__all__ = [
    "MoldovaGovernmentSource",
    "government_portal",
