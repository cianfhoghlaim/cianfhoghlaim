"""Per-subject DLT source for Moldova (Biology).
...
``stedding/ingest_queue/european_nations/mda/education/subjects/biology/<lang>/``.
...
_NATION_SOURCE = MDABiologySource()
...
def mda_biology(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Moldova Biology rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "mda_biology_live_mode_not_implemented",
...
@dlt.source(name="mda_biology")
def mda_biology_source(language: str | None = None):
    """DLT source for the Moldova Biology ingestion."""
    return mda_biology(language=language)
