"""Per-subject DLT source for Slovenia (Native language and literature, RIC).

Per-subject DLT source for the EU nations full-depth expansion
(`2026-07-13-eu-nations-full-depth-expansion-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/svn/education/subjects/language/<lang>/``.
...
def svn_language(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Slovenia Native language and literature syllabus rows from the canonical cache.

    Honours ``USE_LOCAL_SCRAPES=true`` by reading cached Firecrawl
    snapshots under ``stedding/ingest_queue/european_nations/svn/education/subjects/language/<lang>/``.
    """
    cache_dir = Path("stedding/ingest_queue/european_nations/svn/education/subjects/language")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "svn_language_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="svn_language")
def svn_language_source(language: str | None = None):
    """DLT source for the Slovenia Native language and literature ingestion."""
    return svn_language(language=language)
