"""DLT source for Statbel.

Crawls ``https://statbel.fgov.be`` and emits one row per ``(national_statistics_id, language)`` for
...
``dlt/european_nations/bel/statistics/national_statistics.py`` with
``source_id="european_nations.bel.statistics.national_statistics"`` and lands in
the DuckLake table ``oideachais.statistics.european_nations.bel``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/bel/statistics/<lang>/``.
...
class BelgiumNationalStatisticsSource(NationSource):
    """Statbel DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://statbel.fgov.be",
                "title": "Statbel",
            },
        )


_NATION_SOURCE = BelgiumNationalStatisticsSource()
...
def national_statistics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Statbel rows from the canonical cache."""
...
def national_statistics_source(language: str | None = None):
    """DLT source for the Statbel ingestion."""
    return national_statistics(language=language)


__all__ = [
    "BelgiumNationalStatisticsSource",
    "national_statistics",
