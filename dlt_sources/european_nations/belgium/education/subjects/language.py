"""Per-subject DLT source for Belgium (language).
...
``stedding/ingest_queue/european_nations/bel/education/subjects/language/<lang>/``.
...
class BelgiumLanguageEducationSource(NationSource):
    """Belgium language curriculum DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                ...
                "title": "Belgium language curriculum (nl,fr,de)",
            },
        )
...
_NATION_SOURCE = BelgiumLanguageEducationSource()
...
def bel_language(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Belgium language rows from the canonical per-subject cache."""
    if not use_local_scrapes():
        logger.warning(
            "bel_language_live_mode_not_implemented",
...
@dlt.source(name="bel_language")
def bel_language_source(language: str | None = None):
    """DLT source for the Belgium language ingestion."""
    return bel_language(language=language)


__all__ = [
    "BelgiumLanguageEducationSource",
    "bel_language",
    "bel_language_source",
]
