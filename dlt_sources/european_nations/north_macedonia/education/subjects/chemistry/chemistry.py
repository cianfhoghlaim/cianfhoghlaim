"""Per-subject DLT source for North Macedonia (Chemistry).
...
``stedding/ingest_queue/european_nations/mkd/education/subjects/chemistry/<lang>/``.
...
class NorthMacedoniaChemistrySource(NationSource):
    """North Macedonia Chemistry DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                ...
                "title": "North Macedonia Chemistry (MKD)",
                "subject": SUBJECT,
...
_NATION_SOURCE = MKDChemistrySource()
...
def mkd_chemistry(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield North Macedonia Chemistry rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "mkd_chemistry_live_mode_not_implemented",
...
@dlt.source(name="mkd_chemistry")
def mkd_chemistry_source(language: str | None = None):
    """DLT source for the North Macedonia Chemistry ingestion."""
    return mkd_chemistry(language=language)
