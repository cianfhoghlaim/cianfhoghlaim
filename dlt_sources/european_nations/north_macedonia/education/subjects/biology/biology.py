"""Per-subject DLT source for North Macedonia (Biology).
...
``stedding/ingest_queue/european_nations/mkd/education/subjects/biology/<lang>/``.
...
class NorthMacedoniaBiologySource(NationSource):
    """North Macedonia Biology DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                ...
                "title": "North Macedonia Biology (MKD)",
                "subject": SUBJECT,
...
_NATION_SOURCE = MKDBiologySource()
...
def mkd_biology(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield North Macedonia Biology rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "mkd_biology_live_mode_not_implemented",
...
@dlt.source(name="mkd_biology")
def mkd_biology_source(language: str | None = None):
    """DLT source for the North Macedonia Biology ingestion."""
    return mkd_biology(language=language)
