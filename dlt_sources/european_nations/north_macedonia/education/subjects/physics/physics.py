"""Per-subject DLT source for North Macedonia (Physics).
...
``stedding/ingest_queue/european_nations/mkd/education/subjects/physics/<lang>/``.
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                ...
                "title": "North Macedonia Physics (MKD)",
                "subject": SUBJECT,
...
_NATION_SOURCE = MKDPhysicsSource()
...
def mkd_physics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield North Macedonia Physics rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "mkd_physics_live_mode_not_implemented",
...
@dlt.source(name="mkd_physics")
def mkd_physics_source(language: str | None = None):
    """DLT source for the North Macedonia Physics ingestion."""
    return mkd_physics(language=language)
