"""DLT source for Stjornarrad Islands.

Crawls ``https://www.stjornarradid.is`` and emits one row per ``(national_portal_id, language)`` for
...
``dlt/european_nations/isl/government/national_portal.py`` with
``source_id="european_nations.isl.government.national_portal"`` and lands in
the DuckLake table ``oideachais.government.european_nations.isl``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/isl/government/<lang>/``.
...
class IcelandNationalPortalSource(NationSource):
    """Stjornarrad Islands DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                ...
                "title": "Stjornarrad Islands",
            },
        )


_NATION_SOURCE = IcelandNationalPortalSource()
...
def national_portal(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Stjornarrad Islands rows from the canonical cache."""
...
def national_portal_source(language: str | None = None):
    """DLT source for the Stjornarrad Islands ingestion."""
    return national_portal(language=language)


__all__ = [
    "IcelandNationalPortalSource",
    "national_portal",
