"""Per-subject DLT source for Iceland (physics).
...
``stedding/ingest_queue/european_nations/isl/education/subjects/physics/<lang>/``.
...
class IcelandPhysicsEducationSource(NationSource):
    """Iceland physics curriculum DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                ...
                "title": "Iceland physics curriculum (is)",
            },
        )
...
_NATION_SOURCE = IcelandPhysicsEducationSource()
...
def isl_physics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Iceland physics rows from the canonical per-subject cache."""
    if not use_local_scrapes():
        logger.warning(
            "isl_physics_live_mode_not_implemented",
...
@dlt.source(name="isl_physics")
def isl_physics_source(language: str | None = None):
    """DLT source for the Iceland physics ingestion."""
    return isl_physics(language=language)


__all__ = [
    "IcelandPhysicsEducationSource",
    "isl_physics",
    "isl_physics_source",
]
