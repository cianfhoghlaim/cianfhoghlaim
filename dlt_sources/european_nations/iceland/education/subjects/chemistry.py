"""Per-subject DLT source for Iceland (chemistry).
...
``stedding/ingest_queue/european_nations/isl/education/subjects/chemistry/<lang>/``.
...
class IcelandChemistryEducationSource(NationSource):
    """Iceland chemistry curriculum DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                ...
                "title": "Iceland chemistry curriculum (is)",
            },
        )
...
_NATION_SOURCE = IcelandChemistryEducationSource()
...
def isl_chemistry(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Iceland chemistry rows from the canonical per-subject cache."""
    if not use_local_scrapes():
        logger.warning(
            "isl_chemistry_live_mode_not_implemented",
...
@dlt.source(name="isl_chemistry")
def isl_chemistry_source(language: str | None = None):
    """DLT source for the Iceland chemistry ingestion."""
    return isl_chemistry(language=language)


__all__ = [
    "IcelandChemistryEducationSource",
    "isl_chemistry",
    "isl_chemistry_source",
]
