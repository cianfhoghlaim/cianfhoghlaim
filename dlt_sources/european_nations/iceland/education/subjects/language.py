"""Per-subject DLT source for Iceland (language).
...
``stedding/ingest_queue/european_nations/isl/education/subjects/language/<lang>/``.
...
class IcelandLanguageEducationSource(NationSource):
    """Iceland language curriculum DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                ...
                "title": "Iceland language curriculum (is)",
            },
        )
...
_NATION_SOURCE = IcelandLanguageEducationSource()
...
def isl_language(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Iceland language rows from the canonical per-subject cache."""
    if not use_local_scrapes():
        logger.warning(
            "isl_language_live_mode_not_implemented",
...
@dlt.source(name="isl_language")
def isl_language_source(language: str | None = None):
    """DLT source for the Iceland language ingestion."""
    return isl_language(language=language)


__all__ = [
    "IcelandLanguageEducationSource",
    "isl_language",
    "isl_language_source",
]
