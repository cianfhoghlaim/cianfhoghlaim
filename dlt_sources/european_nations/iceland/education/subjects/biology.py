"""Per-subject DLT source for Iceland (biology).
...
``stedding/ingest_queue/european_nations/isl/education/subjects/biology/<lang>/``.
...
class IcelandBiologyEducationSource(NationSource):
    """Iceland biology curriculum DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                ...
                "title": "Iceland biology curriculum (is)",
            },
        )
...
_NATION_SOURCE = IcelandBiologyEducationSource()
...
def isl_biology(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Iceland biology rows from the canonical per-subject cache."""
    if not use_local_scrapes():
        logger.warning(
            "isl_biology_live_mode_not_implemented",
...
@dlt.source(name="isl_biology")
def isl_biology_source(language: str | None = None):
    """DLT source for the Iceland biology ingestion."""
    return isl_biology(language=language)


__all__ = [
    "IcelandBiologyEducationSource",
    "isl_biology",
    "isl_biology_source",
]
