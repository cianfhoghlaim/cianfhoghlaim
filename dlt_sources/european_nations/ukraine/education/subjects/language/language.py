"""Per-subject DLT source for the Ukraine {Language} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/ukr/education/subjects/language/uk/sample.json``.
"""
...
def ukr_language(language=None) -> Iterator[dict[str, Any]]:
    """Yield Ukraine Language rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/ukr/education/subjects/language")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "ukr_language_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="ukr_language")
def ukr_language_source(language=None):
    """DLT source for the Ukraine Language ingestion."""
    return ukr_language(language=language)
