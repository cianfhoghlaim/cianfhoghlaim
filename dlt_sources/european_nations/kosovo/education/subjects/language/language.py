"""Per-subject DLT source for Kosovo (Language).
...
``stedding/ingest_queue/european_nations/xkx/education/subjects/language/<lang>/``.
...
class KosovoLanguageSource(NationSource):
    """Kosovo Language DLT source."""
...
_NATION_SOURCE = XKXLanguageSource()
...
def xkx_language(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Kosovo Language rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "xkx_language_live_mode_not_implemented",
...
@dlt.source(name="xkx_language")
def xkx_language_source(language: str | None = None):
    """DLT source for the Kosovo Language ingestion."""
    return xkx_language(language=language)
