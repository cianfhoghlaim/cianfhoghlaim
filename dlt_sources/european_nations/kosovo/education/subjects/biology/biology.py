"""Per-subject DLT source for Kosovo (Biology).
...
``stedding/ingest_queue/european_nations/xkx/education/subjects/biology/<lang>/``.
...
class KosovoBiologySource(NationSource):
    """Kosovo Biology DLT source."""
...
_NATION_SOURCE = XKXBiologySource()
...
def xkx_biology(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Kosovo Biology rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "xkx_biology_live_mode_not_implemented",
...
@dlt.source(name="xkx_biology")
def xkx_biology_source(language: str | None = None):
    """DLT source for the Kosovo Biology ingestion."""
    return xkx_biology(language=language)
