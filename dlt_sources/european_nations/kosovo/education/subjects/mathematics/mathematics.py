"""Per-subject DLT source for Kosovo (Mathematics).
...
``stedding/ingest_queue/european_nations/xkx/education/subjects/mathematics/<lang>/``.
...
_NATION_SOURCE = XKXMathematicsSource()
...
def xkx_mathematics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Kosovo Mathematics rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "xkx_mathematics_live_mode_not_implemented",
...
@dlt.source(name="xkx_mathematics")
def xkx_mathematics_source(language: str | None = None):
    """DLT source for the Kosovo Mathematics ingestion."""
    return xkx_mathematics(language=language)
