"""Per-subject DLT source for Kosovo (Chemistry).
...
``stedding/ingest_queue/european_nations/xkx/education/subjects/chemistry/<lang>/``.
...
_NATION_SOURCE = XKXChemistrySource()
...
def xkx_chemistry(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Kosovo Chemistry rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "xkx_chemistry_live_mode_not_implemented",
...
@dlt.source(name="xkx_chemistry")
def xkx_chemistry_source(language: str | None = None):
    """DLT source for the Kosovo Chemistry ingestion."""
    return xkx_chemistry(language=language)
