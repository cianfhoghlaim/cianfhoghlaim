"""Per-subject DLT source for Kosovo (Computing Science).
...
``stedding/ingest_queue/european_nations/xkx/education/subjects/computing_science/<lang>/``.
...
class KosovoComputingScienceSource(NationSource):
    """Kosovo Computing Science DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://masht.rks-gov.net",
                "title": "Kosovo Computing Science (XKX)",
                "subject": SUBJECT,
...
_NATION_SOURCE = XKXComputingScienceSource()
...
def xkx_computing_science(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Kosovo Computing Science rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "xkx_computing_science_live_mode_not_implemented",
...
@dlt.source(name="xkx_computing_science")
def xkx_computing_science_source(language: str | None = None):
    """DLT source for the Kosovo Computing Science ingestion."""
    return xkx_computing_science(language=language)
