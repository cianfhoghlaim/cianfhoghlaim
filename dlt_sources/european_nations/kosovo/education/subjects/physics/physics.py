"""Per-subject DLT source for Kosovo (Physics).
...
``stedding/ingest_queue/european_nations/xkx/education/subjects/physics/<lang>/``.
...
class KosovoPhysicsSource(NationSource):
    """Kosovo Physics DLT source."""
...
_NATION_SOURCE = XKXPhysicsSource()
...
def xkx_physics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Kosovo Physics rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "xkx_physics_live_mode_not_implemented",
...
@dlt.source(name="xkx_physics")
def xkx_physics_source(language: str | None = None):
    """DLT source for the Kosovo Physics ingestion."""
    return xkx_physics(language=language)
