"""DLT source for the Statistics Office of Kosovo.

Crawls ``https://ask.rks-gov.net`` and emits one row per ``(stat_office_id, language)``
...
``dlt/european_nations/xkx/statistics/statistics_office.py`` with
``source_id="european_nations.xkx.statistics.statistics_office"`` and lands in
the DuckLake table ``oideachais.statistics.european_nations.xkx``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/xkx/statistics/<lang>/``.
...
class KosovoStatisticsSource(NationSource):
    """Statistics Office of Kosovo DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://ask.rks-gov.net",
                "title": "Statistics Office of Kosovo",
            },
        )


_NATION_SOURCE = KosovoStatisticsSource()
...
def statistics_office(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Kosovo Statistics Office rows from the canonical cache."""
...
def statistics_office_source(language: str | None = None):
    """DLT source for the Statistics Office of Kosovo ingestion."""
    return statistics_office(language=language)


__all__ = [
    "KosovoStatisticsSource",
    "statistics_office",
