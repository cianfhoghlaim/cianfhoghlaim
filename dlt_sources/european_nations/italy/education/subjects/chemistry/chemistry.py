"""Per-subject DLT source for the Italy {Chemistry} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/ita/education/subjects/chemistry/it/sample.json``.
"""
...
def ita_chemistry(language=None) -> Iterator[dict[str, Any]]:
    """Yield Italy Chemistry rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/ita/education/subjects/chemistry")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "ita_chemistry_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="ita_chemistry")
def ita_chemistry_source(language=None):
    """DLT source for the Italy Chemistry ingestion."""
    return ita_chemistry(language=language)
