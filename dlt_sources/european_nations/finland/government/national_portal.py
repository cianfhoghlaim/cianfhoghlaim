"""DLT source for Valtioneuvosto.

Crawls ``https://valtioneuvosto.fi`` and emits one row per ``(national_portal_id, language)`` for
...
``dlt/european_nations/fin/government/national_portal.py`` with
``source_id="european_nations.fin.government.national_portal"`` and lands in
the DuckLake table ``oideachais.government.european_nations.fin``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/fin/government/<lang>/``.
...
class FinlandNationalPortalSource(NationSource):
    """Valtioneuvosto DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://valtioneuvosto.fi",
                "title": "Valtioneuvosto",
            },
        )


_NATION_SOURCE = FinlandNationalPortalSource()
...
def national_portal(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Valtioneuvosto rows from the canonical cache."""
...
def national_portal_source(language: str | None = None):
    """DLT source for the Valtioneuvosto ingestion."""
    return national_portal(language=language)


__all__ = [
    "FinlandNationalPortalSource",
    "national_portal",
