"""DLT source for Tilastokeskus.

Crawls ``https://stat.fi`` and emits one row per ``(national_statistics_id, language)`` for
...
``dlt/european_nations/fin/statistics/national_statistics.py`` with
``source_id="european_nations.fin.statistics.national_statistics"`` and lands in
the DuckLake table ``oideachais.statistics.european_nations.fin``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/fin/statistics/<lang>/``.
...
class FinlandNationalStatisticsSource(NationSource):
    """Tilastokeskus DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://stat.fi",
                "title": "Tilastokeskus",
            },
        )


_NATION_SOURCE = FinlandNationalStatisticsSource()
...
def national_statistics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Tilastokeskus rows from the canonical cache."""
...
def national_statistics_source(language: str | None = None):
    """DLT source for the Tilastokeskus ingestion."""
    return national_statistics(language=language)


__all__ = [
    "FinlandNationalStatisticsSource",
    "national_statistics",
