"""Per-subject DLT source for Bosnia and Herzegovina (Chemistry).
...
``stedding/ingest_queue/european_nations/bih/education/subjects/chemistry/<lang>/``.
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                ...
                "title": "Bosnia and Herzegovina Chemistry (BIH)",
                "subject": SUBJECT,
...
_NATION_SOURCE = BIHChemistrySource()
...
def bih_chemistry(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Bosnia and Herzegovina Chemistry rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "bih_chemistry_live_mode_not_implemented",
...
@dlt.source(name="bih_chemistry")
def bih_chemistry_source(language: str | None = None):
    """DLT source for the Bosnia and Herzegovina Chemistry ingestion."""
    return bih_chemistry(language=language)
