"""Per-subject DLT source for Bosnia and Herzegovina (Mathematics).
...
``stedding/ingest_queue/european_nations/bih/education/subjects/mathematics/<lang>/``.
...
class BosniaAndHerzegovinaMathematicsSource(NationSource):
    """Bosnia and Herzegovina Mathematics DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                ...
                "title": "Bosnia and Herzegovina Mathematics (BIH)",
                "subject": SUBJECT,
...
_NATION_SOURCE = BIHMathematicsSource()
...
def bih_mathematics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Bosnia and Herzegovina Mathematics rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "bih_mathematics_live_mode_not_implemented",
...
@dlt.source(name="bih_mathematics")
def bih_mathematics_source(language: str | None = None):
    """DLT source for the Bosnia and Herzegovina Mathematics ingestion."""
    return bih_mathematics(language=language)
