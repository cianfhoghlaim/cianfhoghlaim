"""Per-subject DLT source for Norway (language).
...
``stedding/ingest_queue/european_nations/nor/education/subjects/language/<lang>/``.
...
class NorwayLanguageEducationSource(NationSource):
    """Norway language curriculum DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                ...
                "title": "Norway language curriculum (nb,nn,se)",
            },
        )
...
_NATION_SOURCE = NorwayLanguageEducationSource()
...
def nor_language(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Norway language rows from the canonical per-subject cache."""
    if not use_local_scrapes():
        logger.warning(
            "nor_language_live_mode_not_implemented",
...
@dlt.source(name="nor_language")
def nor_language_source(language: str | None = None):
    """DLT source for the Norway language ingestion."""
    return nor_language(language=language)


__all__ = [
    "NorwayLanguageEducationSource",
    "nor_language",
    "nor_language_source",
]
