"""Per-subject DLT source for Norway (physics).
...
``stedding/ingest_queue/european_nations/nor/education/subjects/physics/<lang>/``.
...
class NorwayPhysicsEducationSource(NationSource):
    """Norway physics curriculum DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                ...
                "title": "Norway physics curriculum (nb,nn,se)",
            },
        )
...
_NATION_SOURCE = NorwayPhysicsEducationSource()
...
def nor_physics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Norway physics rows from the canonical per-subject cache."""
    if not use_local_scrapes():
        logger.warning(
            "nor_physics_live_mode_not_implemented",
...
@dlt.source(name="nor_physics")
def nor_physics_source(language: str | None = None):
    """DLT source for the Norway physics ingestion."""
    return nor_physics(language=language)


__all__ = [
    "NorwayPhysicsEducationSource",
    "nor_physics",
    "nor_physics_source",
]
