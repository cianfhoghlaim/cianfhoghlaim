"""DLT source for Statistisk sentralbyra.

Crawls ``https://www.ssb.no`` and emits one row per ``(national_statistics_id, language)`` for
...
``dlt/european_nations/nor/statistics/national_statistics.py`` with
``source_id="european_nations.nor.statistics.national_statistics"`` and lands in
the DuckLake table ``oideachais.statistics.european_nations.nor``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/nor/statistics/<lang>/``.
...
class NorwayNationalStatisticsSource(NationSource):
    """Statistisk sentralbyra DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://www.ssb.no",
                "title": "Statistisk sentralbyra",
            },
        )


_NATION_SOURCE = NorwayNationalStatisticsSource()
...
def national_statistics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Statistisk sentralbyra rows from the canonical cache."""
...
def national_statistics_source(language: str | None = None):
    """DLT source for the Statistisk sentralbyra ingestion."""
    return national_statistics(language=language)


__all__ = [
    "NorwayNationalStatisticsSource",
    "national_statistics",
