"""DLT source for Lovdata.

Crawls ``https://lovdata.no`` and emits one row per ``(legislation_id, language)`` for
...
``dlt/european_nations/nor/law/legislation.py`` with
``source_id="european_nations.nor.law.legislation"`` and lands in
the DuckLake table ``oideachais.law.european_nations.nor``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/nor/law/<lang>/``.
...
class NorwayLegislationSource(NationSource):
    """Lovdata DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://lovdata.no",
                "title": "Lovdata",
            },
        )


_NATION_SOURCE = NorwayLegislationSource()
...
def legislation(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Lovdata rows from the canonical cache."""
...
def legislation_source(language: str | None = None):
    """DLT source for the Lovdata ingestion."""
    return legislation(language=language)


__all__ = [
    "NorwayLegislationSource",
    "legislation",
    "legislation_source",
]
