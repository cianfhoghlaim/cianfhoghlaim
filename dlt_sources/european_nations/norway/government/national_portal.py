"""DLT source for Regjeringen.

Crawls ``https://www.regjeringen.no`` and emits one row per ``(national_portal_id, language)`` for
...
``dlt/european_nations/nor/government/national_portal.py`` with
``source_id="european_nations.nor.government.national_portal"`` and lands in
the DuckLake table ``oideachais.government.european_nations.nor``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/nor/government/<lang>/``.
...
class NorwayNationalPortalSource(NationSource):
    """Regjeringen DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://www.regjeringen.no",
                "title": "Regjeringen",
            },
        )


_NATION_SOURCE = NorwayNationalPortalSource()
...
def national_portal(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Regjeringen rows from the canonical cache."""
...
def national_portal_source(language: str | None = None):
    """DLT source for the Regjeringen ingestion."""
    return national_portal(language=language)


__all__ = [
    "NorwayNationalPortalSource",
    "national_portal",
