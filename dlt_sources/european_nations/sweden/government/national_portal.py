"""DLT source for Regeringskansliet.

Crawls ``https://www.regeringen.se`` and emits one row per ``(national_portal_id, language)`` for
...
``dlt/european_nations/swe/government/national_portal.py`` with
``source_id="european_nations.swe.government.national_portal"`` and lands in
the DuckLake table ``oideachais.government.european_nations.swe``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/swe/government/<lang>/``.
...
class SwedenNationalPortalSource(NationSource):
    """Regeringskansliet DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://www.regeringen.se",
                "title": "Regeringskansliet",
            },
        )


_NATION_SOURCE = SwedenNationalPortalSource()
...
def national_portal(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Regeringskansliet rows from the canonical cache."""
...
def national_portal_source(language: str | None = None):
    """DLT source for the Regeringskansliet ingestion."""
    return national_portal(language=language)


__all__ = [
    "SwedenNationalPortalSource",
    "national_portal",
