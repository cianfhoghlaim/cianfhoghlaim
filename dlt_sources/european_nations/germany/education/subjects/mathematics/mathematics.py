"""Per-subject DLT source for the Germany {Mathematics} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/deu/education/subjects/mathematics/de/sample.json``.
"""
...
def deu_mathematics(language=None) -> Iterator[dict[str, Any]]:
    """Yield Germany Mathematics rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/deu/education/subjects/mathematics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "deu_mathematics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="deu_mathematics")
def deu_mathematics_source(language=None):
    """DLT source for the Germany Mathematics ingestion."""
    return deu_mathematics(language=language)
