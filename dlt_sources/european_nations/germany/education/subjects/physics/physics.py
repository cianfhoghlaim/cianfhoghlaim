"""Per-subject DLT source for the Germany {Physics} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/deu/education/subjects/physics/de/sample.json``.
"""
...
def deu_physics(language=None) -> Iterator[dict[str, Any]]:
    """Yield Germany Physics rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/deu/education/subjects/physics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "deu_physics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
            ...
            yield {{
                ...
                "document_type": "Physics_curriculum",
...
@dlt.source(name="deu_physics")
def deu_physics_source(language=None):
    """DLT source for the Germany Physics ingestion."""
    return deu_physics(language=language)
