"""Per-subject DLT source for Cyprus (Chemistry, KYSATS).

Per-subject DLT source for the EU nations full-depth expansion
(`2026-07-13-eu-nations-full-depth-expansion-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/cyp/education/subjects/chemistry/<lang>/``.
...
def cyp_chemistry(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Cyprus Chemistry syllabus rows from the canonical cache.

    Honours ``USE_LOCAL_SCRAPES=true`` by reading cached Firecrawl
    snapshots under ``stedding/ingest_queue/european_nations/cyp/education/subjects/chemistry/<lang>/``.
    """
    cache_dir = Path("stedding/ingest_queue/european_nations/cyp/education/subjects/chemistry")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "cyp_chemistry_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="cyp_chemistry")
def cyp_chemistry_source(language: str | None = None):
    """DLT source for the Cyprus Chemistry ingestion."""
    return cyp_chemistry(language=language)
