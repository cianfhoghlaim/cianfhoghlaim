"""Per-subject DLT source for Czechia (Mathematics, MŠMT).

Per-subject DLT source for the EU nations full-depth expansion
(`2026-07-13-eu-nations-full-depth-expansion-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/cze/education/subjects/mathematics/<lang>/``.
...
def cze_mathematics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Czechia Mathematics syllabus rows from the canonical cache.

    Honours ``USE_LOCAL_SCRAPES=true`` by reading cached Firecrawl
    snapshots under ``stedding/ingest_queue/european_nations/cze/education/subjects/mathematics/<lang>/``.
    """
    cache_dir = Path("stedding/ingest_queue/european_nations/cze/education/subjects/mathematics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "cze_mathematics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="cze_mathematics")
def cze_mathematics_source(language: str | None = None):
    """DLT source for the Czechia Mathematics ingestion."""
    return cze_mathematics(language=language)
