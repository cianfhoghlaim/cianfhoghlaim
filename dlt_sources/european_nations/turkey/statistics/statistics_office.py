"""DLT source for the Statistics Office of Turkey.

Crawls ``https://tuik.gov.tr`` and emits one row per ``(stat_office_id, language)``
...
``dlt/european_nations/tur/statistics/statistics_office.py`` with
``source_id="european_nations.tur.statistics.statistics_office"`` and lands in
the DuckLake table ``oideachais.statistics.european_nations.tur``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/tur/statistics/<lang>/``.
...
class TurkeyStatisticsSource(NationSource):
    """Statistics Office of Turkey DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://tuik.gov.tr",
                "title": "Statistics Office of Turkey",
            },
        )


_NATION_SOURCE = TurkeyStatisticsSource()
...
def statistics_office(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Turkey Statistics Office rows from the canonical cache."""
...
def statistics_office_source(language: str | None = None):
    """DLT source for the Statistics Office of Turkey ingestion."""
    return statistics_office(language=language)


__all__ = [
    "TurkeyStatisticsSource",
    "statistics_office",
