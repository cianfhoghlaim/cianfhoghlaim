"""Per-subject DLT source for Turkey (Biology).
...
``stedding/ingest_queue/european_nations/tur/education/subjects/biology/<lang>/``.
...
class TurkeyBiologySource(NationSource):
    """Turkey Biology DLT source."""
...
_NATION_SOURCE = TURBiologySource()
...
def tur_biology(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Turkey Biology rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "tur_biology_live_mode_not_implemented",
...
@dlt.source(name="tur_biology")
def tur_biology_source(language: str | None = None):
    """DLT source for the Turkey Biology ingestion."""
    return tur_biology(language=language)
