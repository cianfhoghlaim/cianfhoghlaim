"""Per-subject DLT source for Turkey (Computing Science).
...
``stedding/ingest_queue/european_nations/tur/education/subjects/computing_science/<lang>/``.
...
class TurkeyComputingScienceSource(NationSource):
    """Turkey Computing Science DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                ...
                "title": "Turkey Computing Science (TUR)",
                "subject": SUBJECT,
...
_NATION_SOURCE = TURComputingScienceSource()
...
def tur_computing_science(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Turkey Computing Science rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "tur_computing_science_live_mode_not_implemented",
...
@dlt.source(name="tur_computing_science")
def tur_computing_science_source(language: str | None = None):
    """DLT source for the Turkey Computing Science ingestion."""
    return tur_computing_science(language=language)
