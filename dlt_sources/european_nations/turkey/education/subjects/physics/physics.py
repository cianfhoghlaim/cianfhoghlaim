"""Per-subject DLT source for Turkey (Physics).
...
``stedding/ingest_queue/european_nations/tur/education/subjects/physics/<lang>/``.
...
class TurkeyPhysicsSource(NationSource):
    """Turkey Physics DLT source."""
...
_NATION_SOURCE = TURPhysicsSource()
...
def tur_physics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Turkey Physics rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "tur_physics_live_mode_not_implemented",
...
@dlt.source(name="tur_physics")
def tur_physics_source(language: str | None = None):
    """DLT source for the Turkey Physics ingestion."""
    return tur_physics(language=language)
