"""Per-subject DLT source for Turkey (Language).
...
``stedding/ingest_queue/european_nations/tur/education/subjects/language/<lang>/``.
...
_NATION_SOURCE = TURLanguageSource()
...
def tur_language(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Turkey Language rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "tur_language_live_mode_not_implemented",
...
@dlt.source(name="tur_language")
def tur_language_source(language: str | None = None):
    """DLT source for the Turkey Language ingestion."""
    return tur_language(language=language)
