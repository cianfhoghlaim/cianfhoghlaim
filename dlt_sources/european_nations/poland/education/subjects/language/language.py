"""Per-subject DLT source for the Poland {Language} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/pol/education/subjects/language/pl/sample.json``.
"""
...
def pol_language(language=None) -> Iterator[dict[str, Any]]:
    """Yield Poland Language rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/pol/education/subjects/language")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "pol_language_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="pol_language")
def pol_language_source(language=None):
    """DLT source for the Poland Language ingestion."""
    return pol_language(language=language)
