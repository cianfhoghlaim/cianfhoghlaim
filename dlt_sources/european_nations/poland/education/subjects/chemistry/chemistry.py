"""Per-subject DLT source for the Poland {Chemistry} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/pol/education/subjects/chemistry/pl/sample.json``.
"""
...
def pol_chemistry(language=None) -> Iterator[dict[str, Any]]:
    """Yield Poland Chemistry rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/pol/education/subjects/chemistry")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "pol_chemistry_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="pol_chemistry")
def pol_chemistry_source(language=None):
    """DLT source for the Poland Chemistry ingestion."""
    return pol_chemistry(language=language)
