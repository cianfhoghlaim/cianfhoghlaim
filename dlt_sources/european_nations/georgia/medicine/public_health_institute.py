"""DLT source for the Public Health Institute of Georgia.

Crawls ``https://moh.gov.ge`` and emits one row per ``(phi_id, language)``
...
``dlt/european_nations/geo/medicine/public_health_institute.py`` with
``source_id="european_nations.geo.medicine.public_health_institute"`` and lands in
the DuckLake table ``oideachais.medicine.european_nations.geo``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/geo/medicine/<lang>/``.
...
class GeorgiaMedicineSource(NationSource):
    """Public Health Institute of Georgia DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://moh.gov.ge",
                "title": "Public Health Institute of Georgia",
            },
        )


_NATION_SOURCE = GeorgiaMedicineSource()
...
def public_health_institute(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Georgia Public Health Institute rows from the canonical cache."""
...
def public_health_institute_source(language: str | None = None):
    """DLT source for the Public Health Institute of Georgia ingestion."""
    return public_health_institute(language=language)


__all__ = [
    "GeorgiaMedicineSource",
