"""DLT source for the Statistics Office of Georgia.

Crawls ``https://geostat.ge`` and emits one row per ``(stat_office_id, language)``
...
``dlt/european_nations/geo/statistics/statistics_office.py`` with
``source_id="european_nations.geo.statistics.statistics_office"`` and lands in
the DuckLake table ``oideachais.statistics.european_nations.geo``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/geo/statistics/<lang>/``.
...
class GeorgiaStatisticsSource(NationSource):
    """Statistics Office of Georgia DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://geostat.ge",
                "title": "Statistics Office of Georgia",
            },
        )


_NATION_SOURCE = GeorgiaStatisticsSource()
...
def statistics_office(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Georgia Statistics Office rows from the canonical cache."""
...
def statistics_office_source(language: str | None = None):
    """DLT source for the Statistics Office of Georgia ingestion."""
    return statistics_office(language=language)


__all__ = [
    "GeorgiaStatisticsSource",
    "statistics_office",
