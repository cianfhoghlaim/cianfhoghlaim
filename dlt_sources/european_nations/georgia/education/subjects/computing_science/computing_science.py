"""Per-subject DLT source for Georgia (Computing Science).
...
``stedding/ingest_queue/european_nations/geo/education/subjects/computing_science/<lang>/``.
...
class GeorgiaComputingScienceSource(NationSource):
    """Georgia Computing Science DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://mes.gov.ge",
                "title": "Georgia Computing Science (GEO)",
                "subject": SUBJECT,
...
_NATION_SOURCE = GEOComputingScienceSource()
...
def geo_computing_science(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Georgia Computing Science rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "geo_computing_science_live_mode_not_implemented",
...
@dlt.source(name="geo_computing_science")
def geo_computing_science_source(language: str | None = None):
    """DLT source for the Georgia Computing Science ingestion."""
    return geo_computing_science(language=language)
