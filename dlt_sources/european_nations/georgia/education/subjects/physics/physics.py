"""Per-subject DLT source for Georgia (Physics).
...
``stedding/ingest_queue/european_nations/geo/education/subjects/physics/<lang>/``.
...
class GeorgiaPhysicsSource(NationSource):
    """Georgia Physics DLT source."""
...
_NATION_SOURCE = GEOPhysicsSource()
...
def geo_physics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Georgia Physics rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "geo_physics_live_mode_not_implemented",
...
@dlt.source(name="geo_physics")
def geo_physics_source(language: str | None = None):
    """DLT source for the Georgia Physics ingestion."""
    return geo_physics(language=language)
