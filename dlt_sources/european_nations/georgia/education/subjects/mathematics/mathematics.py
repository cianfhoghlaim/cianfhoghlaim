"""Per-subject DLT source for Georgia (Mathematics).
...
``stedding/ingest_queue/european_nations/geo/education/subjects/mathematics/<lang>/``.
...
_NATION_SOURCE = GEOMathematicsSource()
...
def geo_mathematics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Georgia Mathematics rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "geo_mathematics_live_mode_not_implemented",
...
@dlt.source(name="geo_mathematics")
def geo_mathematics_source(language: str | None = None):
    """DLT source for the Georgia Mathematics ingestion."""
    return geo_mathematics(language=language)
