"""Per-subject DLT source for Georgia (Chemistry).
...
``stedding/ingest_queue/european_nations/geo/education/subjects/chemistry/<lang>/``.
...
_NATION_SOURCE = GEOChemistrySource()
...
def geo_chemistry(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Georgia Chemistry rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "geo_chemistry_live_mode_not_implemented",
...
@dlt.source(name="geo_chemistry")
def geo_chemistry_source(language: str | None = None):
    """DLT source for the Georgia Chemistry ingestion."""
    return geo_chemistry(language=language)
