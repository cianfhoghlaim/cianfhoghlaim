"""Per-subject DLT source for Georgia (Biology).
...
``stedding/ingest_queue/european_nations/geo/education/subjects/biology/<lang>/``.
...
_NATION_SOURCE = GEOBiologySource()
...
def geo_biology(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Georgia Biology rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "geo_biology_live_mode_not_implemented",
...
@dlt.source(name="geo_biology")
def geo_biology_source(language: str | None = None):
    """DLT source for the Georgia Biology ingestion."""
    return geo_biology(language=language)
