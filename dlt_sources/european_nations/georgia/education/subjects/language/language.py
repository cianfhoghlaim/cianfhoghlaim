"""Per-subject DLT source for Georgia (Language).
...
``stedding/ingest_queue/european_nations/geo/education/subjects/language/<lang>/``.
...
_NATION_SOURCE = GEOLanguageSource()
...
def geo_language(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Georgia Language rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "geo_language_live_mode_not_implemented",
...
@dlt.source(name="geo_language")
def geo_language_source(language: str | None = None):
    """DLT source for the Georgia Language ingestion."""
    return geo_language(language=language)
