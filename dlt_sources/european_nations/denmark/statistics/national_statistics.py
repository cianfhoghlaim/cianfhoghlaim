"""DLT source for Danmarks Statistik.

Crawls ``https://www.dst.dk`` and emits one row per ``(national_statistics_id, language)`` for
...
``dlt/european_nations/dnk/statistics/national_statistics.py`` with
``source_id="european_nations.dnk.statistics.national_statistics"`` and lands in
the DuckLake table ``oideachais.statistics.european_nations.dnk``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/dnk/statistics/<lang>/``.
...
class DenmarkNationalStatisticsSource(NationSource):
    """Danmarks Statistik DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://www.dst.dk",
                "title": "Danmarks Statistik",
            },
        )


_NATION_SOURCE = DenmarkNationalStatisticsSource()
...
def national_statistics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Danmarks Statistik rows from the canonical cache."""
...
def national_statistics_source(language: str | None = None):
    """DLT source for the Danmarks Statistik ingestion."""
    return national_statistics(language=language)


__all__ = [
    "DenmarkNationalStatisticsSource",
    "national_statistics",
