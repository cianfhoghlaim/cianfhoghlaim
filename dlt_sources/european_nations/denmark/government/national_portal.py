"""DLT source for Statsministeriet.

Crawls ``https://www.regeringen.dk`` and emits one row per ``(national_portal_id, language)`` for
...
``dlt/european_nations/dnk/government/national_portal.py`` with
``source_id="european_nations.dnk.government.national_portal"`` and lands in
the DuckLake table ``oideachais.government.european_nations.dnk``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/dnk/government/<lang>/``.
...
class DenmarkNationalPortalSource(NationSource):
    """Statsministeriet DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://www.regeringen.dk",
                "title": "Statsministeriet",
            },
        )


_NATION_SOURCE = DenmarkNationalPortalSource()
...
def national_portal(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Statsministeriet rows from the canonical cache."""
...
def national_portal_source(language: str | None = None):
    """DLT source for the Statsministeriet ingestion."""
    return national_portal(language=language)


__all__ = [
    "DenmarkNationalPortalSource",
    "national_portal",
