"""DLT source for Amt fuer Statistik.

Crawls ``https://www.llv.li`` and emits one row per ``(national_statistics_id, language)`` for
...
``dlt/european_nations/lie/statistics/national_statistics.py`` with
``source_id="european_nations.lie.statistics.national_statistics"`` and lands in
the DuckLake table ``oideachais.statistics.european_nations.lie``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/lie/statistics/<lang>/``.
...
class LiechtensteinNationalStatisticsSource(NationSource):
    """Amt fuer Statistik DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                ...
                "title": "Amt fuer Statistik",
            },
        )


_NATION_SOURCE = LiechtensteinNationalStatisticsSource()
...
def national_statistics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Amt fuer Statistik rows from the canonical cache."""
...
def national_statistics_source(language: str | None = None):
    """DLT source for the Amt fuer Statistik ingestion."""
    return national_statistics(language=language)


__all__ = [
    "LiechtensteinNationalStatisticsSource",
    "national_statistics",
