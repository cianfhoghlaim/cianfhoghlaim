"""Per-subject DLT source for Montenegro (Computing Science).
...
``stedding/ingest_queue/european_nations/mne/education/subjects/computing_science/<lang>/``.
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                ...
                "title": "Montenegro Computing Science (MNE)",
                "subject": SUBJECT,
...
_NATION_SOURCE = MNEComputingScienceSource()
...
def mne_computing_science(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Montenegro Computing Science rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "mne_computing_science_live_mode_not_implemented",
...
@dlt.source(name="mne_computing_science")
def mne_computing_science_source(language: str | None = None):
    """DLT source for the Montenegro Computing Science ingestion."""
    return mne_computing_science(language=language)
