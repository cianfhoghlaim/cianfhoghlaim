"""Per-subject DLT source for Montenegro (Language).
...
``stedding/ingest_queue/european_nations/mne/education/subjects/language/<lang>/``.
...
class MontenegroLanguageSource(NationSource):
    """Montenegro Language DLT source."""
...
_NATION_SOURCE = MNELanguageSource()
...
def mne_language(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Montenegro Language rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "mne_language_live_mode_not_implemented",
...
@dlt.source(name="mne_language")
def mne_language_source(language: str | None = None):
    """DLT source for the Montenegro Language ingestion."""
    return mne_language(language=language)
