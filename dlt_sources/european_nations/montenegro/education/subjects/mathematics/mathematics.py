"""Per-subject DLT source for Montenegro (Mathematics).
...
``stedding/ingest_queue/european_nations/mne/education/subjects/mathematics/<lang>/``.
...
class MontenegroMathematicsSource(NationSource):
    """Montenegro Mathematics DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                ...
                "title": "Montenegro Mathematics (MNE)",
                "subject": SUBJECT,
...
_NATION_SOURCE = MNEMathematicsSource()
...
def mne_mathematics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Montenegro Mathematics rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "mne_mathematics_live_mode_not_implemented",
...
@dlt.source(name="mne_mathematics")
def mne_mathematics_source(language: str | None = None):
    """DLT source for the Montenegro Mathematics ingestion."""
    return mne_mathematics(language=language)
