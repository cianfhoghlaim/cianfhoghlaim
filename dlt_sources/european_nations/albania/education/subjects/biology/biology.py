"""Per-subject DLT source for Albania (Biology).
...
``stedding/ingest_queue/european_nations/alb/education/subjects/biology/<lang>/``.
...
class AlbaniaBiologySource(NationSource):
    """Albania Biology DLT source."""
...
_NATION_SOURCE = ALBBiologySource()
...
def alb_biology(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Albania Biology rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "alb_biology_live_mode_not_implemented",
...
@dlt.source(name="alb_biology")
def alb_biology_source(language: str | None = None):
    """DLT source for the Albania Biology ingestion."""
    return alb_biology(language=language)
