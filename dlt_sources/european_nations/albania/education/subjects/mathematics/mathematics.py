"""Per-subject DLT source for Albania (Mathematics).
...
``stedding/ingest_queue/european_nations/alb/education/subjects/mathematics/<lang>/``.
...
class AlbaniaMathematicsSource(NationSource):
    """Albania Mathematics DLT source."""
...
_NATION_SOURCE = ALBMathematicsSource()
...
def alb_mathematics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Albania Mathematics rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "alb_mathematics_live_mode_not_implemented",
...
@dlt.source(name="alb_mathematics")
def alb_mathematics_source(language: str | None = None):
    """DLT source for the Albania Mathematics ingestion."""
    return alb_mathematics(language=language)
