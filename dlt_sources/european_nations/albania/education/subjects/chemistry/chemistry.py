"""Per-subject DLT source for Albania (Chemistry).
...
``stedding/ingest_queue/european_nations/alb/education/subjects/chemistry/<lang>/``.
...
class AlbaniaChemistrySource(NationSource):
    """Albania Chemistry DLT source."""
...
_NATION_SOURCE = ALBChemistrySource()
...
def alb_chemistry(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Albania Chemistry rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "alb_chemistry_live_mode_not_implemented",
...
@dlt.source(name="alb_chemistry")
def alb_chemistry_source(language: str | None = None):
    """DLT source for the Albania Chemistry ingestion."""
    return alb_chemistry(language=language)
