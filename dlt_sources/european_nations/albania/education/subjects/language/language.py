"""Per-subject DLT source for Albania (Language).
...
``stedding/ingest_queue/european_nations/alb/education/subjects/language/<lang>/``.
...
class AlbaniaLanguageSource(NationSource):
    """Albania Language DLT source."""
...
_NATION_SOURCE = ALBLanguageSource()
...
def alb_language(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Albania Language rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "alb_language_live_mode_not_implemented",
...
@dlt.source(name="alb_language")
def alb_language_source(language: str | None = None):
    """DLT source for the Albania Language ingestion."""
    return alb_language(language=language)
