"""Per-subject DLT source for Slovakia (Computing / Computer Science, NÚCEM).

Per-subject DLT source for the EU nations full-depth expansion
(`2026-07-13-eu-nations-full-depth-expansion-v1`).

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/svk/education/subjects/computing_science/<lang>/``.
...
COUNTRY_CODE = "svk"
CANONICAL_ROOT = "https://www.minedu.sk"
SUPPORTED_LANGUAGES: tuple[str, ...] = ("sk",)
DEFAULT_LANGUAGE = "sk"
...
def svk_computing_science(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Slovakia Computing / Computer Science syllabus rows from the canonical cache.

    Honours ``USE_LOCAL_SCRAPES=true`` by reading cached Firecrawl
    snapshots under ``stedding/ingest_queue/european_nations/svk/education/subjects/computing_science/<lang>/``.
    """
    cache_dir = Path("stedding/ingest_queue/european_nations/svk/education/subjects/computing_science")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "svk_computing_science_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="svk_computing_science")
def svk_computing_science_source(language: str | None = None):
    """DLT source for the Slovakia Computing / Computer Science ingestion."""
    return svk_computing_science(language=language)
