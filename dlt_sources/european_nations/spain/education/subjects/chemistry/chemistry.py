"""Per-subject DLT source for the Spain {Chemistry} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/esp/education/subjects/chemistry/es/sample.json``.
"""
...
def esp_chemistry(language=None) -> Iterator[dict[str, Any]]:
    """Yield Spain Chemistry rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/esp/education/subjects/chemistry")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "esp_chemistry_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="esp_chemistry")
def esp_chemistry_source(language=None):
    """DLT source for the Spain Chemistry ingestion."""
    return esp_chemistry(language=language)
