"""Per-subject DLT source for Serbia (Mathematics).
...
``stedding/ingest_queue/european_nations/srb/education/subjects/mathematics/<lang>/``.
...
class SerbiaMathematicsSource(NationSource):
    """Serbia Mathematics DLT source."""
...
_NATION_SOURCE = SRBMathematicsSource()
...
def srb_mathematics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Serbia Mathematics rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "srb_mathematics_live_mode_not_implemented",
...
@dlt.source(name="srb_mathematics")
def srb_mathematics_source(language: str | None = None):
    """DLT source for the Serbia Mathematics ingestion."""
    return srb_mathematics(language=language)
