"""Per-subject DLT source for Serbia (Language).
...
``stedding/ingest_queue/european_nations/srb/education/subjects/language/<lang>/``.
...
class SerbiaLanguageSource(NationSource):
    """Serbia Language DLT source."""
...
_NATION_SOURCE = SRBLanguageSource()
...
def srb_language(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Serbia Language rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "srb_language_live_mode_not_implemented",
...
@dlt.source(name="srb_language")
def srb_language_source(language: str | None = None):
    """DLT source for the Serbia Language ingestion."""
    return srb_language(language=language)
