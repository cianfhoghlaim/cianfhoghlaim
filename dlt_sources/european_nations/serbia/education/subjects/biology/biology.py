"""Per-subject DLT source for Serbia (Biology).
...
``stedding/ingest_queue/european_nations/srb/education/subjects/biology/<lang>/``.
...
class SerbiaBiologySource(NationSource):
    """Serbia Biology DLT source."""
...
_NATION_SOURCE = SRBBiologySource()
...
def srb_biology(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Serbia Biology rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "srb_biology_live_mode_not_implemented",
...
@dlt.source(name="srb_biology")
def srb_biology_source(language: str | None = None):
    """DLT source for the Serbia Biology ingestion."""
    return srb_biology(language=language)
