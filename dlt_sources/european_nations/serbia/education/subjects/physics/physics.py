"""Per-subject DLT source for Serbia (Physics).
...
``stedding/ingest_queue/european_nations/srb/education/subjects/physics/<lang>/``.
...
class SerbiaPhysicsSource(NationSource):
    """Serbia Physics DLT source."""
...
_NATION_SOURCE = SRBPhysicsSource()
...
def srb_physics(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Serbia Physics rows from the canonical cache."""
    if not use_local_scrapes():
        logger.warning(
            "srb_physics_live_mode_not_implemented",
...
@dlt.source(name="srb_physics")
def srb_physics_source(language: str | None = None):
    """DLT source for the Serbia Physics ingestion."""
    return srb_physics(language=language)
