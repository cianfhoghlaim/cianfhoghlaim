"""DLT source for the Statistics Office of Serbia.

Crawls ``https://stat.gov.rs`` and emits one row per ``(stat_office_id, language)``
...
``dlt/european_nations/srb/statistics/statistics_office.py`` with
``source_id="european_nations.srb.statistics.statistics_office"`` and lands in
the DuckLake table ``oideachais.statistics.european_nations.srb``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/srb/statistics/<lang>/``.
...
class SerbiaStatisticsSource(NationSource):
    """Statistics Office of Serbia DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://stat.gov.rs",
                "title": "Statistics Office of Serbia",
            },
        )


_NATION_SOURCE = SerbiaStatisticsSource()
...
def statistics_office(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Serbia Statistics Office rows from the canonical cache."""
...
def statistics_office_source(language: str | None = None):
    """DLT source for the Statistics Office of Serbia ingestion."""
    return statistics_office(language=language)


__all__ = [
    "SerbiaStatisticsSource",
    "statistics_office",
