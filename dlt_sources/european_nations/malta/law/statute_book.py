"""DLT source for the Laws of Malta (laws.mt) (law, Malta).

Crawls ``https://laws.mt`` and emits one row per ``(statute_book_id, language)`` for
...
``dlt/european_nations/mlt/law/statute_book.py`` with
``source_id="european_nations.mlt.law.statute_book"`` and lands in
the DuckLake table ``oideachais.law.european_nations.mlt``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/mlt/law/<lang>/``.
...
class MaltaLawSource(NationSource):
    """Laws of Malta (laws.mt) DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://laws.mt",
                "title": "Laws of Malta (laws.mt)",
            },
        )


_NATION_SOURCE = MaltaLawSource()
...
def statute_book(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Malta law rows from the canonical cache."""
...
def statute_book_source(language: str | None = None):
    """DLT source for the Laws of Malta (laws.mt) ingestion."""
