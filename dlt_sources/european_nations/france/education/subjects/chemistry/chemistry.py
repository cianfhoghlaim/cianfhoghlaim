"""Per-subject DLT source for the France {Chemistry} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/fra/education/subjects/chemistry/fr/sample.json``.
"""
...
def fra_chemistry(language=None) -> Iterator[dict[str, Any]]:
    """Yield France Chemistry rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/fra/education/subjects/chemistry")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "fra_chemistry_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="fra_chemistry")
def fra_chemistry_source(language=None):
    """DLT source for the France Chemistry ingestion."""
    return fra_chemistry(language=language)
