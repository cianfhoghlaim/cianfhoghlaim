"""Per-subject DLT source for the France {Biology} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/fra/education/subjects/biology/fr/sample.json``.
"""
...
def fra_biology(language=None) -> Iterator[dict[str, Any]]:
    """Yield France Biology rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/fra/education/subjects/biology")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "fra_biology_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="fra_biology")
def fra_biology_source(language=None):
    """DLT source for the France Biology ingestion."""
    return fra_biology(language=language)
