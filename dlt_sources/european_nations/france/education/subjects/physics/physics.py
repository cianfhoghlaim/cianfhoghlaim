"""Per-subject DLT source for the France {Physics} curriculum.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/fra/education/subjects/physics/fr/sample.json``.
"""
...
def fra_physics(language=None) -> Iterator[dict[str, Any]]:
    """Yield France Physics rows from the canonical cache."""
    cache_dir = Path("stedding/ingest_queue/european_nations/fra/education/subjects/physics")
    ...
    for lang in languages:
        ...
        for json_path in sorted(lang_dir.glob("*.json")):
            try:
                ...
                logger.warning(
                    "fra_physics_cache_parse_failed",
                    path=str(json_path),
                    error=str(exc),
                )
                continue
...
@dlt.source(name="fra_physics")
def fra_physics_source(language=None):
    """DLT source for the France Physics ingestion."""
    return fra_physics(language=language)
