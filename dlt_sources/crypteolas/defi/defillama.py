            ...
            for chain, chain_data in chain_tvls.items():
                ...
                for entry in tvl_history:
                    yield {
                        "protocol_slug": slug,
                        "chain": chain,
                        "date": datetime.fromtimestamp(entry.get("date", 0), tz=timezone.utc).date().isoformat(),
        ...
        try:
            ...
            for stablecoin in data.get("peggedAssets", []):
                yield {
                    ...
                    "circulating_prev_day": stablecoin.get("circulatingPrevDay", {}).get("peggedUSD"),
                    "circulating_prev_week": stablecoin.get("circulatingPrevWeek", {}).get("peggedUSD"),
                    "circulating_prev_month": stablecoin.get("circulatingPrevMonth", {}).get("peggedUSD"),
