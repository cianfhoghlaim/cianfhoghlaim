"""DLT source for the Public Health Institute of Albania.

Crawls ``https://shendetesia.gov.al`` and emits one row per ``(phi_id, language)``
...
``dlt/european_nations/alb/medicine/public_health_institute.py`` with
``source_id="european_nations.alb.medicine.public_health_institute"`` and lands in
the DuckLake table ``oideachais.medicine.european_nations.alb``.

Honours ``USE_LOCAL_SCRAPES=true`` by reading from
``stedding/ingest_queue/european_nations/alb/medicine/<lang>/``.
...
class AlbaniaMedicineSource(NationSource):
    """Public Health Institute of Albania DLT source."""
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            extra_metadata={
                "canonical_root": "https://shendetesia.gov.al",
                "title": "Public Health Institute of Albania",
            },
        )


_NATION_SOURCE = AlbaniaMedicineSource()
...
def public_health_institute(language: str | None = None) -> Iterator[dict[str, Any]]:
    """Yield Albania Public Health Institute rows from the canonical cache."""
...
def public_health_institute_source(language: str | None = None):
    """DLT source for the Public Health Institute of Albania ingestion."""
    return public_health_institute(language=language)


__all__ = [
    "AlbaniaMedicineSource",
