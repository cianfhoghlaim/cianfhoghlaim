...
def test_msc_ai_source_with_no_credentials_skips_cleanly(monkeypatch):
    """The M.Sc. AI convenience source also short-circuits in fixture mode."""
    from dlt_sources.british_isles.ireland.education.university.exam_papers import (
        msc_ai_source,
    )
    ...
    assert all(r.get("status") == "skipped_fixture" for r in rows)
