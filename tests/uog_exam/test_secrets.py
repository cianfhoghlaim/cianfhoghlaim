...
def test_env_backend_wins_when_nothing_else_configured(monkeypatch):
    ...
    THEN env returns the value (and Infisical's HTTP layer is not contacted)."""
    monkeypatch.setenv("OOG_STUDENT_PASSWORD", "real-password")
    r = SecretsResolver()

    with patch("urllib.request.urlopen") as spy:
...
def test_infisical_wins_when_configured(monkeypatch):
    ...
    fake_payload = b"real-infisical-password"
...
def test_onepassword_cli_is_not_invoked(monkeypatch):
    ...
    WHEN get() is called
    THEN `op` is NOT invoked (no subprocess check_output)."""
